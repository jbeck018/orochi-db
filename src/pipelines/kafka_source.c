 ...
 */
void *
kafka_source_init(KafkaSourceConfig *config)
{
...
#ifdef HAVE_LIBRDKAFKA
    {
        ...
        {
            if (rd_kafka_conf_set(conf, "group.id", config->consumer_group,
                                  errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK)
            {
        ...
        if (err != RD_KAFKA_RESP_ERR_NO_ERROR)
        {
            snprintf(consumer->last_error, sizeof(consumer->last_error),
                     "Failed to subscribe to topic %s: %s",
 ...
 */
int
kafka_source_poll(void *consumer_ptr, char **messages, int max_messages, int timeout_ms)
{
 ...
 */
bool
kafka_source_commit(void *consumer_ptr, int64 offset)
{
    KafkaConsumer *consumer = (KafkaConsumer *)consumer_ptr;
...
#ifdef HAVE_LIBRDKAFKA
    {
        ...
        elog(DEBUG1, "Committed Kafka offset %ld for partition %d",
             offset, consumer->current_partition);
    }
#else
    consumer->committed_offset = offset;
    consumer->messages_committed++;
 ...
 */
int64
kafka_source_get_offset(void *consumer_ptr)
{
    KafkaConsumer *consumer = (KafkaConsumer *)consumer_ptr;
 ...
 */
bool
kafka_source_seek(void *consumer_ptr, int64 offset)
{
    KafkaConsumer *consumer = (KafkaConsumer *)consumer_ptr;
 ...
 */
void
kafka_source_close(void *consumer_ptr)
{
    KafkaConsumer *consumer = (KafkaConsumer *)consumer_ptr;
 ...
 */
int
kafka_process_batch(Pipeline *pipeline, KafkaConsumer *consumer, int batch_size)
{
    char **messages;
    int num_messages;
    int processed = 0;
    ...
    for (i = 0; i < num_messages; i++)
    {
        ...
        switch (pipeline->format)
        {
            ...
            case PIPELINE_FORMAT_CSV:
                {
                    List *records = parse_csv_records(messages[i], strlen(messages[i]),
                                                      ',', true);
                    if (records != NIL)
                    {
                        int64 inserted = pipeline_insert_batch(pipeline, records, true);
 ...
 */
List *
parse_json_records(const char *data, int64 size)
{
    List *records = NIL;
    text *json_text;
    Datum json_datum;
    ...
    if (data[0] == '[')
    {
        ...
        if (ret == SPI_OK_SELECT && SPI_processed > 0)
        {
            ...
            {
                char *record_str = SPI_getvalue(SPI_tuptable->vals[i],
 ...
 */
List *
parse_csv_records(const char *data, int64 size, char delimiter, bool has_header)
{
    List *records = NIL;
    List *headers = NIL;
            ...
            if (line_len > 0)
            {
                ...
                else
                {
                    ...
                    while (token != NULL)
                    {
                        ...
                        if (header_cell != NULL)
                        {
                            appendStringInfo(&json, "\"%s\": ",
                                             (char *)lfirst(header_cell));
 ...
 */
List *
parse_line_protocol(const char *data, int64 size)
{
    List *records = NIL;
    const char *line_start = data;
    const char *p = data;
