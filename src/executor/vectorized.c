...
static void filter_int64_scalar(VectorBatch *batch, int32 column_index,
                                VectorizedCompareOp op, int64 value);
static void filter_float64_scalar(VectorBatch *batch, int32 column_index,
...
VectorBatch *
vector_batch_create(int32 capacity, int32 column_count)
{
    VectorBatch *batch;
    ...
    batch->selection = vectorized_palloc_aligned(capacity * sizeof(uint32),
...
VectorColumn *
vector_batch_add_column(VectorBatch *batch, int32 column_index,
...
VectorizedDataType
vectorized_type_from_oid(Oid type_oid)
{
    switch (type_oid)
    {
        case INT2OID:
 ...
 */
static void
filter_int64_avx2(VectorBatch *batch, int32 column_index,
                  VectorizedCompareOp op, int64 value)
{
    ...
    for (i = 0; i + VECTORIZED_INT64_LANES <= batch->count; i += VECTORIZED_INT64_LANES)
    {
        ...
        switch (op)
        {
            ...
            case VECTORIZED_CMP_GE:
                ...
                cmp_result = _mm256_cmpgt_epi64(val_vec, data_vec);
                cmp_result = _mm256_xor_si256(cmp_result,
 ...
 */
static void
filter_float64_avx2(VectorBatch *batch, int32 column_index,
                    VectorizedCompareOp op, double value)
{
    VectorColumn *column = batch->columns[column_index];
 ...
 */
static int64
sum_int64_avx2(VectorBatch *batch, int32 column_index)
{
    VectorColumn *column = batch->columns[column_index];
    int64 *data = (int64 *)column->data;
 ...
 */
static double
sum_float64_avx2(VectorBatch *batch, int32 column_index)
{
    VectorColumn *column = batch->columns[column_index];
    double *data = (double *)column->data;
...
static void
filter_int64_scalar(VectorBatch *batch, int32 column_index,
                    VectorizedCompareOp op, int64 value)
{
    VectorColumn *column = batch->columns[column_index];
    int64 *data = (int64 *)column->data;
...
static void
filter_float64_scalar(VectorBatch *batch, int32 column_index,
                      VectorizedCompareOp op, double value)
{
    VectorColumn *column = batch->columns[column_index];
    double *data = (double *)column->data;
...
static int64
sum_int64_scalar(VectorBatch *batch, int32 column_index)
{
    VectorColumn *column = batch->columns[column_index];
    int64 *data = (int64 *)column->data;
    int64 sum = 0;
...
static double
sum_float64_scalar(VectorBatch *batch, int32 column_index)
{
    VectorColumn *column = batch->columns[column_index];
    double *data = (double *)column->data;
    double sum = 0.0;
...
void
vectorized_filter_int64(VectorBatch *batch, int32 column_index,
                        VectorizedCompareOp op, int64 value)
{
    if (batch == NULL || column_index < 0 || column_index >= batch->column_count)
        ereport(ERROR,
...
void
vectorized_filter_float64(VectorBatch *batch, int32 column_index,
                          VectorizedCompareOp op, double value)
{
    if (batch == NULL || column_index < 0 || column_index >= batch->column_count)
        ereport(ERROR,
...
void
vectorized_filter_int32(VectorBatch *batch, int32 column_index,
...
void
vectorized_filter_float32(VectorBatch *batch, int32 column_index,
...
void
vectorized_filter_generic(VectorBatch *batch, VectorizedFilterState *filter)
{
    switch (filter->data_type)
    {
        case VECTORIZED_TYPE_INT64:
        case VECTORIZED_TYPE_TIMESTAMP:
            vectorized_filter_int64(batch, filter->column_index,
                                    filter->compare_op,
        ...
        case VECTORIZED_TYPE_INT32:
        case VECTORIZED_TYPE_DATE:
            vectorized_filter_int32(batch, filter->column_index,
                                    filter->compare_op,
                                    DatumGetInt32(filter->const_value));
            break;

        case VECTORIZED_TYPE_FLOAT64:
            vectorized_filter_float64(batch, filter->column_index,
                                      filter->compare_op,
                                      DatumGetFloat8(filter->const_value));
            break;

        case VECTORIZED_TYPE_FLOAT32:
            vectorized_filter_float32(batch, filter->column_index,
                                      filter->compare_op,
...
void
vectorized_selection_and(VectorBatch *batch, uint32 *other_selection,
                         int32 other_count)
{
...
void
vectorized_selection_or(VectorBatch *batch, uint32 *other_selection,
                        int32 other_count)
{
    uint32 *new_selection;
    int32 new_count = 0;
...
int64
vectorized_sum_int64(VectorBatch *batch, int32 column_index)
{
    if (batch == NULL || column_index < 0 || column_index >= batch->column_count)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
double
vectorized_sum_float64(VectorBatch *batch, int32 column_index)
{
    if (batch == NULL || column_index < 0 || column_index >= batch->column_count)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
int64
vectorized_sum_int32(VectorBatch *batch, int32 column_index)
{
    VectorColumn *column;
    int32 *data;
    int64 sum = 0;
...
double
vectorized_sum_float32(VectorBatch *batch, int32 column_index)
{
    VectorColumn *column;
    float *data;
    double sum = 0.0;
...
int64
vectorized_count_column(VectorBatch *batch, int32 column_index)
{
    VectorColumn *column;
    int64 count = 0;
    int32 i;
...
int64
vectorized_min_int64(VectorBatch *batch, int32 column_index, bool *found)
{
    VectorColumn *column;
    int64 *data;
...
int64
vectorized_max_int64(VectorBatch *batch, int32 column_index, bool *found)
{
    VectorColumn *column;
    int64 *data;
...
double
vectorized_min_float64(VectorBatch *batch, int32 column_index, bool *found)
{
    VectorColumn *column;
    double *data;
...
double
vectorized_max_float64(VectorBatch *batch, int32 column_index, bool *found)
{
    VectorColumn *column;
    double *data;
...
void
vectorized_agg_update(VectorizedAggState *state, VectorBatch *batch)
{
    bool found;
...
Datum
vectorized_agg_finalize(VectorizedAggState *state, bool *is_null)
{
    *is_null = false;
...
int
vectorized_batch_to_slots(VectorBatch *batch, TupleTableSlot **slots,
                          TupleDesc tupdesc, int max_slots)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        for (j = 0; j < batch->column_count && j < tupdesc->natts; j++)
        {
            ...
            /* Check for null */
            if (column->has_nulls &&
                (column->nulls[row_idx / 64] & (1ULL << (row_idx % 64))))
            {
                slot->tts_isnull[j] = true;
                slot->tts_values[j] = (Datum) 0;
                continue;
...
const char *
vectorized_type_name(VectorizedDataType type)
{
    switch (type)
    {
        case VECTORIZED_TYPE_INT32:     return "int32";
        case VECTORIZED_TYPE_INT64:     return "int64";
        case VECTORIZED_TYPE_FLOAT32:   return "float32";
        case VECTORIZED_TYPE_FLOAT64:   return "float64";
        case VECTORIZED_TYPE_BOOL:      return "bool";
        case VECTORIZED_TYPE_STRING:    return "string";
        case VECTORIZED_TYPE_TIMESTAMP: return "timestamp";
        case VECTORIZED_TYPE_DATE:      return "date";
        default:                        return "unknown";
    }
}

const char *
vectorized_compare_op_name(VectorizedCompareOp op)
{
    switch (op)
    {
        case VECTORIZED_CMP_EQ:         return "=";
        case VECTORIZED_CMP_NE:         return "<>";
        case VECTORIZED_CMP_LT:         return "<";
        case VECTORIZED_CMP_LE:         return "<=";
        case VECTORIZED_CMP_GT:         return ">";
        case VECTORIZED_CMP_GE:         return ">=";
        case VECTORIZED_CMP_IS_NULL:    return "IS NULL";
        case VECTORIZED_CMP_IS_NOT_NULL: return "IS NOT NULL";
        default:                        return "?";
    }
}

const char *
vectorized_agg_type_name(VectorizedAggType type)
{
    switch (type)
    {
        case VECTORIZED_AGG_COUNT:  return "COUNT";
        case VECTORIZED_AGG_SUM:    return "SUM";
        case VECTORIZED_AGG_AVG:    return "AVG";
        case VECTORIZED_AGG_MIN:    return "MIN";
        case VECTORIZED_AGG_MAX:    return "MAX";
        default:                    return "?";
    }
}

bool
vectorized_cpu_supports_avx2(void)
{
#ifdef __AVX2__
    return true;
#else
...
void
vectorized_batch_debug_print(VectorBatch *batch)
{
    if (batch == NULL)
    {
    ...
    elog(DEBUG1, "VectorBatch: capacity=%d count=%d columns=%d",
         batch->capacity, batch->count, batch->column_count);
    elog(DEBUG1, "  selection: has=%s count=%d",
         batch->has_selection ? "true" : "false",
