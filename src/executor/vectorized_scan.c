...
static void load_batch_from_chunk_group(VectorizedScanState *state,
                                        VectorBatch *batch,
                                        ColumnarChunkGroup *chunk_group);
static VectorizedCompareOp pg_op_to_vectorized(Oid opno);
static bool can_pushdown_predicate(Node *node, int *column_index,
                               ...
                               Bitmapset *columns_needed)
{
    ...
    state->current_batch = vector_batch_create(VECTORIZED_BATCH_SIZE,
...
void
vectorized_scan_add_filter(VectorizedScanState *state, int32 column_index,
                           VectorizedCompareOp op, Datum value,
                           VectorizedDataType data_type)
{
    ...
    {
        state->filters = repalloc(state->filters,
                                  (state->filter_count + 1) * sizeof(VectorizedFilterState *));
    }
...
void
vectorized_columnar_scan_end(VectorizedScanState *state)
{
    int i;
 ...
 */
static void
load_batch_from_chunk_group(VectorizedScanState *state, VectorBatch *batch,
                            ColumnarChunkGroup *chunk_group)
{
    int32 row_count;
    int i;
 ...
 */
VectorBatch *
vectorized_columnar_scan(VectorizedScanState *state)
{
    ...
    while (rows_loaded < VECTORIZED_BATCH_SIZE)
    {
            ...
            columnar->current_row_in_chunk >= columnar->current_chunk_group->row_count)
        {
            ...
            if (columnar->current_stripe == NULL ||
                columnar->current_chunk_group_index >= columnar->current_stripe->chunk_group_count)
            {
                ...
                columnar->current_stripe = list_nth(columnar->stripe_list,
                                                    columnar->current_stripe_index);
            ...
            if (columnar->skip_list != NULL && state->predicates != NIL)
            {
                ...
                for (int i = 0; i < columnar->skip_list->entry_count; i++)
                {
                    ...
                    if (entry->stripe_id == stripe_id &&
                        entry->chunk_group_index == cg_idx)
                    {
            ...
            /* This triggers decompression and populates column_buffers */
            {
                ...
                column_chunks = orochi_catalog_get_stripe_columns(
                ...
                columnar->current_chunk_group->row_count =
                    columnar->current_stripe->row_count;
        ...
        int64 rows_available = columnar->current_chunk_group->row_count -
                               columnar->current_row_in_chunk;
        int32 rows_to_load = (int32) Min(rows_available,
                                         VECTORIZED_BATCH_SIZE - rows_loaded);
 ...
 */
static VectorizedCompareOp
pg_op_to_vectorized(Oid opno)
{
    switch (opno)
    {
        /* Integer equality */
 ...
 */
static bool
can_pushdown_predicate(Node *node, int *column_index, VectorizedCompareOp *op,
 ...
 */
void
vectorized_apply_predicate(VectorizedScanState *state, VectorBatch *batch)
{
    int i;
            ...
            if (can_pushdown_predicate(pred, &column_index, &op, &value, &data_type))
            {
                ...
                if (batch_col >= 0)
                {
                    switch (data_type)
                    {
                        case VECTORIZED_TYPE_INT64:
                        case VECTORIZED_TYPE_TIMESTAMP:
                            vectorized_filter_int64(batch, batch_col, op,
                                                    DatumGetInt64(value));
                            break;

                        case VECTORIZED_TYPE_INT32:
                        case VECTORIZED_TYPE_DATE:
                            vectorized_filter_int32(batch, batch_col, op,
                                                    DatumGetInt32(value));
                            break;

                        case VECTORIZED_TYPE_FLOAT64:
                            vectorized_filter_float64(batch, batch_col, op,
                                                      DatumGetFloat8(value));
                            break;

                        case VECTORIZED_TYPE_FLOAT32:
                            vectorized_filter_float32(batch, batch_col, op,
...
#ifdef __AVX2__
static void
load_int64_column_avx2(int64 *dest, const int64 *src, int32 count,
 ...
 */
static void
load_float64_column_avx2(double *dest, const double *src, int32 count,
                         const uint8 *null_bitmap, uint64 *dest_nulls)
{
    int32 i;
 ...
 */
static void
load_int32_column_avx2(int32 *dest, const int32 *src, int32 count,
                       const uint8 *null_bitmap, uint64 *dest_nulls)
{
    int32 i;
 ...
 */
static void
load_column_scalar(void *dest, const void *src, int32 count, int32 elem_size,
                   const uint8 *null_bitmap, uint64 *dest_nulls)
{
 ...
 */
void
vectorized_load_column_data(VectorColumn *column, const void *src_data,
                            int32 row_count, const uint8 *null_bitmap)
{
    ...
    switch (column->data_type)
    {
        case VECTORIZED_TYPE_INT64:
        case VECTORIZED_TYPE_TIMESTAMP:
            load_int64_column_avx2((int64 *)column->data,
        ...
        case VECTORIZED_TYPE_FLOAT64:
            load_float64_column_avx2((double *)column->data,
        ...
        case VECTORIZED_TYPE_INT32:
        case VECTORIZED_TYPE_DATE:
            load_int32_column_avx2((int32 *)column->data,
                                   (const int32 *)src_data,
                                   row_count,
                                   null_bitmap,
                                   column->nulls);
            break;
        ...
        default:
            load_column_scalar(column->data, src_data, row_count,
                               column->type_len > 0 ? column->type_len : sizeof(Datum),
