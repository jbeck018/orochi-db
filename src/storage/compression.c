...
zigzag_encode(int64 n)
{
    return (uint64)((n << 1) ^ (n >> 63));
}

static inline int64
zigzag_decode(uint64 n)
{
 ...
 */
static inline int
encode_varint(uint64 value, uint8 *output)
{
    int bytes = 0;
...
static inline int
decode_varint(const uint8 *input, uint64 *value)
{
    uint64 result = 0;
    int shift = 0;
    int bytes = 0;
                 ...
                 char *output, int64 max_output_size)
{
    ...
    #define WRITE_BITS(val, nbits) do { \
        ...
        while (_bits_remaining > 0) { \
            ...
            current_byte |= ((_value >> shift) & ((1 << bits_to_write) - 1)) << (bits_in_byte - bits_to_write); \
    ...
    for (i = 0; i < count; i++)
    {
        ...
        else
        {
            ...
            if (i > 0 && leading >= prev_leading && trailing >= prev_trailing)
            {
                ...
                uint64 significant = (xor_result >> prev_trailing) & ((1ULL << prev_meaningful) - 1);
            ...
            {
                /* New leading/trailing: write 11 + 5 bits leading + 6 bits length + bits */
                   ...
                   char *output, int64 expected_size)
{
    ...
    if (input_size >= (int64)sizeof(int64))
    {
        ...
        if (stored_count == count)
        {
            ...
            #define READ_BITS(nbits, result) do { \
...
dict_hash(const char *str, int len)
{
    uint32 hash = 5381;
    int i;
