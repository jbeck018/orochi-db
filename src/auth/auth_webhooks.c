...
static char *compute_hmac_signature(const char *payload, const char *secret,
                                     TimestampTz timestamp);
static bool deliver_webhook(AuthWebhookEndpoint *endpoint,
 ...
 */
static char *
compute_hmac_signature(const char *payload, const char *secret, TimestampTz timestamp)
{
    ...
    unix_timestamp = (timestamp / USECS_PER_SEC) +
              ...
              hmac_result, &hmac_len))
    {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("failed to compute HMAC signature")));
    }
 ...
 */
bool
orochi_auth_verify_webhook_signature(const char *payload,
                                      ...
                                      int64 timestamp)
{
    ...
    pg_timestamp = (timestamp - ((POSTGRES_EPOCH_JDATE - UNIX_EPOCH_JDATE) * SECS_PER_DAY))
                   * USECS_PER_SEC;
 ...
 */
AuthWebhookPayload *
orochi_auth_create_webhook_event(AuthWebhookEventType event_type,
                                  OrochiAuthUser *user,
                                  OrochiAuthUser *old_user)
{
    AuthWebhookPayload *payload;
    ...
    if (user)
    {
        ...
        payload->record = DatumGetJsonbP(DirectFunctionCall1(jsonb_in,
                                          CStringGetDatum(record_json.data)));
    ...
    if (old_user && event_type == AUTH_WEBHOOK_USER_UPDATED)
    {
        snprintf(uuid_str, sizeof(uuid_str),
                 "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
                 old_user->id.data[0], old_user->id.data[1], old_user->id.data[2], old_user->id.data[3],
                 old_user->id.data[4], old_user->id.data[5], old_user->id.data[6], old_user->id.data[7],
                 old_user->id.data[8], old_user->id.data[9], old_user->id.data[10], old_user->id.data[11],
                 old_user->id.data[12], old_user->id.data[13], old_user->id.data[14], old_user->id.data[15]);
        ...
        payload->old_record = DatumGetJsonbP(DirectFunctionCall1(jsonb_in,
                                              CStringGetDatum(old_record_json.data)));
 ...
 */
char *
orochi_auth_serialize_webhook_payload(AuthWebhookPayload *payload)
{
    StringInfoData json;
    ...
    snprintf(user_uuid_str, sizeof(user_uuid_str),
             "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
             payload->user_id.data[0], payload->user_id.data[1],
             payload->user_id.data[2], payload->user_id.data[3],
             payload->user_id.data[4], payload->user_id.data[5],
             payload->user_id.data[6], payload->user_id.data[7],
             payload->user_id.data[8], payload->user_id.data[9],
             payload->user_id.data[10], payload->user_id.data[11],
             payload->user_id.data[12], payload->user_id.data[13],
             payload->user_id.data[14], payload->user_id.data[15]);

    initStringInfo(&json);
    appendStringInfoChar(&json, '{');

    /* Event metadata */
    appendStringInfo(&json, "\"type\": \"%s\"",
                     webhook_event_type_name(payload->event_type));
    appendStringInfo(&json, ", \"event_id\": \"%s\"", event_uuid_str);
    appendStringInfo(&json, ", \"user_id\": \"%s\"", user_uuid_str);
    appendStringInfo(&json, ", \"timestamp\": \"%s\"",
                     timestamptz_to_str(payload->timestamp));

    /* Schema and table */
    appendStringInfo(&json, ", \"schema\": \"%s\"",
                     payload->schema ? payload->schema : "auth");
    appendStringInfo(&json, ", \"table\": \"%s\"",
                     payload->table ? payload->table : "users");

    /* Record data */
    if (payload->record)
    {
        initStringInfo(&record_str);
        (void) JsonbToCString(&record_str, &payload->record->root,
                              VARSIZE(payload->record));
 ...
 */
static bool
deliver_webhook(AuthWebhookEndpoint *endpoint,
                 AuthWebhookPayload *payload,
                 AuthWebhookDelivery *delivery)
{
#ifdef HAVE_LIBCURL
    ...
    signature = compute_hmac_signature(payload_json, endpoint->secret,
                                        payload->timestamp);
 ...
 */
bool
orochi_auth_deliver_webhook_event(AuthWebhookPayload *payload)
{
    ...
    appendStringInfoString(&query,
        "SELECT endpoint_id, url, secret, timeout_ms, max_retries, "
        "user_created, user_updated, user_deleted, user_signed_in, user_signed_out "
    ...
    for (i = 0; i < SPI_processed; i++)
    {
        ...
        if (isnull) endpoint.timeout_ms = AUTH_WEBHOOK_TIMEOUT_MS;
        endpoint.max_retries = DatumGetInt32(SPI_getbinval(tuple, tupdesc, 5, &isnull));
        if (isnull) endpoint.max_retries = AUTH_WEBHOOK_MAX_RETRIES;

        /* Check event filter */
        endpoint.events[AUTH_WEBHOOK_USER_CREATED] = DatumGetBool(SPI_getbinval(tuple, tupdesc, 6, &isnull));
        endpoint.events[AUTH_WEBHOOK_USER_UPDATED] = DatumGetBool(SPI_getbinval(tuple, tupdesc, 7, &isnull));
        endpoint.events[AUTH_WEBHOOK_USER_DELETED] = DatumGetBool(SPI_getbinval(tuple, tupdesc, 8, &isnull));
        endpoint.events[AUTH_WEBHOOK_USER_SIGNED_IN] = DatumGetBool(SPI_getbinval(tuple, tupdesc, 9, &isnull));
        endpoint.events[AUTH_WEBHOOK_USER_SIGNED_OUT] = DatumGetBool(SPI_getbinval(tuple, tupdesc, 10, &isnull));
 ...
 */
static void
log_delivery_attempt(AuthWebhookDelivery *delivery)
{
    StringInfoData query;
    char uuid_str[37];
    ...
    snprintf(uuid_str, sizeof(uuid_str),
             "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
             delivery->event_id.data[0], delivery->event_id.data[1],
             delivery->event_id.data[2], delivery->event_id.data[3],
             delivery->event_id.data[4], delivery->event_id.data[5],
             delivery->event_id.data[6], delivery->event_id.data[7],
             delivery->event_id.data[8], delivery->event_id.data[9],
             delivery->event_id.data[10], delivery->event_id.data[11],
             delivery->event_id.data[12], delivery->event_id.data[13],
             delivery->event_id.data[14], delivery->event_id.data[15]);
 ...
 */
int64
orochi_auth_create_webhook_endpoint(const char *url,
                                     const char *secret,
                                     bool *events,
                                     int timeout_ms,
                                     int max_retries)
{
    ...
    if (strlen(url) > AUTH_WEBHOOK_MAX_URL_LENGTH)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("webhook URL too long (max %d characters)",
                        AUTH_WEBHOOK_MAX_URL_LENGTH)));
    ...
    {
        bool isnull;
        endpoint_id = DatumGetInt64(SPI_getbinval(SPI_tuptable->vals[0],
 ...
 */
bool
orochi_auth_delete_webhook_endpoint(int64 endpoint_id)
{
    StringInfoData query;
    int ret;
    bool deleted = false;
    ...
    initStringInfo(&query);
    appendStringInfo(&query,
        "DELETE FROM auth.webhook_endpoints WHERE endpoint_id = %ld",
        endpoint_id);
 ...
 */
bool
orochi_auth_enable_webhook_endpoint(int64 endpoint_id, bool enabled)
{
    StringInfoData query;
    int ret;
    bool updated = false;
 ...
 */
void
orochi_auth_emit_user_created(OrochiAuthUser *user)
{
    AuthWebhookPayload *payload;
 ...
 */
void
orochi_auth_emit_user_updated(OrochiAuthUser *user, OrochiAuthUser *old_user)
{
    AuthWebhookPayload *payload;
 ...
 */
void
orochi_auth_emit_user_deleted(OrochiAuthUser *user)
{
    AuthWebhookPayload *payload;
 ...
 */
void
orochi_auth_emit_user_signed_in(OrochiAuthUser *user)
{
    AuthWebhookPayload *payload;
 ...
 */
void
orochi_auth_emit_user_signed_out(OrochiAuthUser *user)
{
    AuthWebhookPayload *payload;
...
static const char *
webhook_event_type_name(AuthWebhookEventType type)
{
    switch (type)
    {
...
static AuthWebhookEventType
parse_webhook_event_type(const char *name)
{
...
Datum
orochi_auth_create_webhook_endpoint_sql(PG_FUNCTION_ARGS)
{
    ...
    int64 endpoint_id = orochi_auth_create_webhook_endpoint(url, secret, NULL,
...
Datum
orochi_auth_delete_webhook_endpoint_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_auth_verify_webhook_signature_sql(PG_FUNCTION_ARGS)
{
    ...
    bool valid = orochi_auth_verify_webhook_signature(payload, signature,
                                                       secret, timestamp);
...
Datum
orochi_auth_list_webhook_endpoints_sql(PG_FUNCTION_ARGS)
{
