...
extern pg_uuid_t endpoint_create(
    pg_uuid_t cluster_id,
    pg_uuid_t branch_id,
    const char *name,
    EndpointPoolerMode pooler_mode,
...
extern bool endpoint_update_status(
    pg_uuid_t endpoint_id,
...
/* Token generation */
extern EndpointTokenResult *endpoint_generate_token(
    pg_uuid_t endpoint_id,
    pg_uuid_t user_id,
    const char *db_role,
    BranchPermissionLevel permission,
...
extern char *endpoint_generate_connection_string(
    pg_uuid_t credential_id,
...
/* Token validation */
extern EndpointAuthResult *endpoint_validate_token(
    const char *token,
    const char *endpoint_host,
    const char *client_ip);

extern EndpointAuthResult *endpoint_validate_password(
    const char *username,
    const char *password,
    pg_uuid_t endpoint_id);

extern bool endpoint_validate_jwt_signature(
    const char *token,
...
extern OrochiEndpointCredential *endpoint_get_credential_by_username(
    pg_uuid_t endpoint_id,
    const char *username);

extern bool endpoint_create_password_credential(
    pg_uuid_t endpoint_id,
    const char *username,
    const char *password,
    const char *db_role,
...
extern bool endpoint_revoke_credential(
    pg_uuid_t credential_id,
    pg_uuid_t revoked_by);

extern bool endpoint_rotate_token(
    pg_uuid_t credential_id,
...
/* Session context */
extern EndpointSessionContext *endpoint_create_session_context(
    EndpointJWTClaims *claims);

extern char **endpoint_get_startup_commands(
    EndpointSessionContext *context,
    int *command_count);

extern bool endpoint_set_session_variables(
    EndpointSessionContext *context);

/* PgCat integration */
extern char *endpoint_generate_pgcat_config(
    pg_uuid_t endpoint_id);

extern bool endpoint_update_pgcat_routing(
    pg_uuid_t endpoint_id);
