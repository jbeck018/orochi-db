...
static BranchPermissionLevel resolve_inherited_permission(
    pg_uuid_t user_id, pg_uuid_t branch_id, int depth);
static bool check_ip_restriction(
    OrochiBranchPermission *perm, const char *ip_address);
static bool check_time_restriction(
    OrochiBranchPermission *perm, TimestampTz check_time);
 ...
 */
void
branch_access_init(void)
{
    HASHCTL hash_ctl;

    if (BranchAccessContext == NULL)
    {
        BranchAccessContext = AllocSetContextCreate(TopMemoryContext,
                                                     "BranchAccessContext",
    ...
    BranchPermissionCache = hash_create("BranchPermissionCache",
                                         BRANCH_PERM_CACHE_SIZE,
                                         &hash_ctl,
 ...
 */
Size
branch_access_shmem_size(void)
{
    return sizeof(BranchAccessSharedState);
}
...
branch_get_permission(pg_uuid_t user_id, pg_uuid_t branch_id)
{
 ...
 */
BranchPermissionLevel
branch_get_effective_permission(pg_uuid_t user_id,
                                pg_uuid_t branch_id,
                                const char *ip_address)
{
    ...
    appendStringInfo(&cache_key, "%s:%s",
    ...
    {
        cache_entry = hash_search(BranchPermissionCache,
                                  cache_key.data,
                                  HASH_FIND,
                                  &found);
    ...
    {
        char *level_str = SPI_getvalue(SPI_tuptable->vals[0],
    ...
    {
        permission = resolve_inherited_permission(user_id,
        ...
        if (permission != BRANCH_PERM_NONE)
        {
            ...
            if (branch->protection_level == BRANCH_PROTECTION_READ_ONLY &&
                (permission == BRANCH_PERM_ADMIN || permission == BRANCH_PERM_WRITE))
            {
    ...
    appendStringInfo(&query,
        ...
        "LIMIT 1",
        uuid_to_string(branch->cluster_id),
        uuid_to_string(user_id));
...
resolve_inherited_permission(pg_uuid_t user_id, pg_uuid_t branch_id, int depth)
{
    ...
    /* Check parent branch */
    branch = branch_get_info(branch_id);
    if (branch != NULL && branch->inherit_permissions && branch->parent_branch_id != NULL)
    {
        permission = resolve_inherited_permission(user_id,
                                                  *branch->parent_branch_id,
                                                  depth + 1);
 ...
 */
BranchAccessContext *
branch_create_access_context(pg_uuid_t user_id, pg_uuid_t branch_id)
{
    MemoryContext old_context;
    ...
    if (branch != NULL)
    {
        ctx->is_owner = (memcmp(&branch->owner_user_id, &user_id,
                                sizeof(pg_uuid_t)) == 0);
        branch_free_info(branch);
    }
 ...
 */
bool
branch_check_permission(pg_uuid_t user_id,
                        pg_uuid_t branch_id,
 ...
 */
bool
branch_grant_permission(pg_uuid_t branch_id,
                        ...
                        TimestampTz valid_until)
{
    ...
    appendStringInfo(&query,
        ...
        "ON CONFLICT (branch_id, user_id) WHERE team_id IS NULL AND role_id IS NULL "
        "DO UPDATE SET permission_level = EXCLUDED.permission_level, "
        "granted_by = EXCLUDED.granted_by, granted_at = NOW(), "
        "revoked_at = NULL%s",
        valid_until != 0 ? ", valid_until" : "",
        uuid_to_string(branch_id),
        uuid_to_string(grantee_user_id),
        branch_permission_level_name(level),
        uuid_to_string(granted_by),
        valid_until != 0 ? ", to_timestamp($1)" : "",
 ...
 */
bool
branch_grant_team_permission(pg_uuid_t branch_id,
                             pg_uuid_t team_id,
                             BranchPermissionLevel level,
                             pg_uuid_t granted_by)
{
    ...
    appendStringInfo(&query,
        ...
        "VALUES ('%s', '%s', '%s', '%s', NOW()) "
        "ON CONFLICT (branch_id, team_id) WHERE user_id IS NULL AND role_id IS NULL "
        "DO UPDATE SET permission_level = EXCLUDED.permission_level, "
        "granted_by = EXCLUDED.granted_by, granted_at = NOW(), revoked_at = NULL",
        uuid_to_string(branch_id),
        uuid_to_string(team_id),
        branch_permission_level_name(level),
        uuid_to_string(granted_by));
 ...
 */
bool
branch_revoke_permission(pg_uuid_t branch_id,
                         pg_uuid_t user_id,
 ...
 */
bool
branch_revoke_all_permissions(pg_uuid_t branch_id, pg_uuid_t revoked_by)
{
    StringInfoData query;
    int ret;
 ...
 */
OrochiBranchInfo *
branch_get_info(pg_uuid_t branch_id)
{
    MemoryContext old_context;
    ...
    initStringInfo(&query);
    appendStringInfo(&query,
        "SELECT branch_id, cluster_id, parent_branch_id, name, slug, description, "
 ...
 */
OrochiBranchInfo *
branch_get_by_slug(pg_uuid_t cluster_id, const char *slug)
{
    MemoryContext old_context;
    OrochiBranchInfo *info = NULL;
    StringInfoData query;
 ...
 */
bool
branch_update_status(pg_uuid_t branch_id, BranchStatus new_status)
{
    StringInfoData query;
    int ret;
 ...
 */
bool
branch_update_protection(pg_uuid_t branch_id, BranchProtectionLevel level)
{
    StringInfoData query;
    int ret;
    ...
    initStringInfo(&query);
    appendStringInfo(&query,
        "UPDATE platform.branches SET protection_level = '%s', updated_at = NOW() "
        "WHERE branch_id = '%s'",
 ...
 */
bool
branch_validate_cross_branch_access(pg_uuid_t user_id,
 ...
 */
void
branch_invalidate_permission_cache(pg_uuid_t branch_id)
{
    ...
    snprintf(branch_str, sizeof(branch_str), ":%s",
             uuid_to_string(branch_id));
 ...
 */
void
branch_invalidate_user_cache(pg_uuid_t user_id)
{
    HASH_SEQ_STATUS status;
    BranchPermissionCacheEntry *entry;
    char user_str[64];
    ...
    snprintf(user_str, sizeof(user_str), "%s:",
             uuid_to_string(user_id));
 ...
 */
void
branch_clear_permission_cache(void)
{
    HASH_SEQ_STATUS status;
    BranchPermissionCacheEntry *entry;
 ...
 */
const char *
branch_permission_level_name(BranchPermissionLevel level)
{
    switch (level)
    {
 ...
 */
const char *
branch_status_name(BranchStatus status)
{
    switch (status)
    {
 ...
 */
const char *
branch_protection_level_name(BranchProtectionLevel level)
{
    switch (level)
    {
 ...
 */
const char *
branch_cow_state_name(BranchCowState state)
{
    switch (state)
    {
 ...
 */
BranchPermissionLevel
branch_parse_permission_level(const char *str)
{
    if (str == NULL)
        return BRANCH_PERM_NONE;
 ...
 */
BranchStatus
branch_parse_status(const char *str)
{
    if (str == NULL)
...
branch_parse_protection_level(const char *str)
{
    if (str == NULL)
 ...
 */
void
branch_free_info(OrochiBranchInfo *info)
{
    if (info == NULL)
        return;
 ...
 */
void
branch_free_permission(OrochiBranchPermission *perm)
{
    if (perm == NULL)
        return;
 ...
 */
void
branch_free_access_context(BranchAccessContext *ctx)
{
    if (ctx == NULL)
        return;
 ...
 */
Datum
branch_check_access_sql(PG_FUNCTION_ARGS)
{
    ...
    required = branch_parse_permission_level(text_to_cstring(required_level_text));
 ...
 */
Datum
branch_grant_access_sql(PG_FUNCTION_ARGS)
{
    ...
    success = branch_grant_permission(*branch_id, *grantee_id, level, *granted_by, 0);
 ...
 */
Datum
branch_revoke_access_sql(PG_FUNCTION_ARGS)
{
    pg_uuid_t *branch_id = PG_GETARG_UUID_P(0);
 ...
 */
Datum
branch_validate_operation_sql(PG_FUNCTION_ARGS)
{
    pg_uuid_t *user_id = PG_GETARG_UUID_P(0);
