 ...
 */
BernoulliSampleState *
bernoulli_sample_create(double probability, uint64 seed)
{
 ...
 */
bool
bernoulli_sample_row(BernoulliSampleState *state)
{
    state->rows_seen++;
 ...
 */
void
bernoulli_sample_stats(BernoulliSampleState *state,
                       int64 *rows_seen,
                       int64 *rows_selected,
                       double *actual_rate)
{
 ...
 */
ReservoirSampleState *
reservoir_sample_create(int32 reservoir_size, int32 num_columns,
                        Oid *column_types)
{
    ...
    for (int i = 0; i < num_columns; i++)
    {
        ...
        get_typlenbyval(column_types[i],
                        &state->column_typlen[i],
                        &state->column_typbyval[i]);
    }
 ...
 */
void
reservoir_sample_add(ReservoirSampleState *state,
                     Datum *values, bool *nulls)
{
    ...
    for (int i = 0; i < state->num_columns; i++)
    {
        ...
        {
            /* Copy the datum */
            state->values_array[idx] = datumCopy(values[i],
                                                  state->column_typbyval[i],
                                                  state->column_typlen[i]);
        }
        else
 ...
 */
int32
reservoir_sample_count(ReservoirSampleState *state)
{
 ...
 */
bool
reservoir_sample_get(ReservoirSampleState *state, int32 index,
                     Datum *values, bool *nulls)
{
 ...
 */
void
reservoir_sample_free(ReservoirSampleState *state)
{
 ...
 */
SystemSampleState *
system_sample_create(double probability)
{
 ...
 */
bool
system_sample_block(SystemSampleState *state)
{
    state->blocks_seen++;
 ...
 */
void
system_sample_row_returned(SystemSampleState *state)
{
    state->rows_returned++;
}
 ...
 */
Datum
sampling_bernoulli(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
sampling_set_seed(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
sampling_random(PG_FUNCTION_ARGS)
{
 ...
 */
Datum
reservoir_sample_trans(PG_FUNCTION_ARGS)
{
    ...
    reservoir_size = PG_NARGS() > 2 && !PG_ARGISNULL(2) ?
                     PG_GETARG_INT32(2) : SAMPLING_DEFAULT_RESERVOIR_SIZE;
 ...
 */
Datum
reservoir_sample_final(PG_FUNCTION_ARGS)
{
    ReservoirAggState *state;
    ArrayType *result;
    Datum *elems;
    ...
    result = construct_array(elems, state->num_items, FLOAT8OID,
 ...
 */
Datum
reservoir_sample_combine(PG_FUNCTION_ARGS)
{
    MemoryContext agg_context;
    MemoryContext old_context;
