 ...
 */
Datum
vector_from_array(PG_FUNCTION_ARGS)
{
 ...
 */
float
vector_l2_dist(Vector *a, Vector *b)
{
    if (a->dim != b->dim)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
 ...
 */
float
vector_cosine_dist(Vector *a, Vector *b)
{
 ...
 */
float
vector_inner_prod(Vector *a, Vector *b)
{
    if (a->dim != b->dim)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
 ...
 */
void
vector_normalize_inplace(Vector *v)
{
 ...
 */
static float
vector_l2_dist_scalar(const float *a, const float *b, int dim)
{
    float sum = 0.0f;
    int i;
 ...
 */
static float
vector_inner_prod_scalar(const float *a, const float *b, int dim)
{
    float sum = 0.0f;
    int i;
 ...
 */
static float
vector_l2_norm_scalar(const float *a, int dim)
{
    float sum = 0.0f;
    int i;
...
float
vector_l2_dist_avx2(const float *a, const float *b, int dim)
{
...
float
vector_inner_prod_avx2(const float *a, const float *b, int dim)
{
    __m256 sum = _mm256_setzero_ps();
    int i;
...
float
vector_l2_norm_avx2(const float *a, int dim)
{
    __m256 sum = _mm256_setzero_ps();
    int i;
 ...
 */
float
vector_l2_dist_simd(const float *a, const float *b, int dim)
{
#ifdef __AVX2__
...
float
vector_inner_prod_simd(const float *a, const float *b, int dim)
{
#ifdef __AVX2__
...
void
vector_batch_l2_distances(Vector *query, Vector **vectors, int count,
                          float *distances)
{
    int i;
 ...
 */
void
vector_knn(Vector *query, Vector **vectors, int count, int k,
           int *indices, float *distances, const char *distance_type)
{
    int i, j;
...
void
orochi_create_vector_index(Oid table_oid, const char *column_name,
                           ...
                           const char *distance_type)
{
    ...
    elog(NOTICE, "Created %s vector index on %s.%s",
         index_type, get_rel_name(table_oid), column_name);
