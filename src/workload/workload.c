 ...
 */
Size
orochi_workload_shmem_size(void)
{
    Size size = 0;
 ...
 */
void
orochi_workload_shmem_startup(void)
{
    bool found;
    ...
    WorkloadState = (WorkloadSharedState *)
        ShmemInitStruct("Orochi Workload State",
                        sizeof(WorkloadSharedState),
                        &found);
...
orochi_workload_init(void)
{
    ...
    /* Define GUCs */
    DefineCustomBoolVariable("orochi.workload_enabled",
                             "Enable workload management",
                             NULL,
                             &orochi_workload_enabled,
                             true,
                             PGC_SUSET,
                             0,
                             NULL, NULL, NULL);
    ...
    DefineCustomIntVariable("orochi.default_pool_concurrency",
                            ...
                            NULL,
                            &orochi_default_pool_concurrency,
                            0, 0, 10000,
                            PGC_SUSET,
                            0,
                            NULL, NULL, NULL);
    ...
    DefineCustomIntVariable("orochi.default_query_timeout_ms",
                            ...
                            NULL,
                            &orochi_default_query_timeout_ms,
                            30000, 0, 3600000,
                            PGC_SUSET,
                            0,
                            NULL, NULL, NULL);

    DefineCustomBoolVariable("orochi.workload_log_rejections",
                             "Log rejected queries",
                             NULL,
                             &orochi_workload_log_rejections,
                             true,
                             PGC_SUSET,
                             0,
                             NULL, NULL, NULL);
 ...
 */
int64
orochi_create_resource_pool(const char *pool_name,
 ...
 */
bool
orochi_alter_resource_pool(int64 pool_id,
                           ResourceLimits *new_limits,
                           int priority_weight)
{
    ...
    elog(DEBUG1, "Altered resource pool \"%s\" (id=%ld)",
 ...
 */
bool
orochi_drop_resource_pool(int64 pool_id, bool if_exists)
{
    ResourcePool *pool = NULL;
    bool found = false;
    ...
    elog(DEBUG1, "Dropped resource pool \"%s\" (id=%ld)",
         pool->pool_name, pool_id);
 ...
 */
ResourcePool *
orochi_get_resource_pool(int64 pool_id)
{
 ...
 */
ResourcePool *
orochi_get_resource_pool_by_name(const char *pool_name)
{
    ResourcePool *result = NULL;
 ...
 */
List *
orochi_list_resource_pools(void)
{
    List *pools = NIL;
 ...
 */
bool
orochi_enable_resource_pool(int64 pool_id)
{
    bool found = false;
 ...
 */
bool
orochi_disable_resource_pool(int64 pool_id)
{
    bool found = false;
 ...
 */
static WorkloadClass *
find_free_class_slot(void)
{
 ...
 */
int64
orochi_create_workload_class(const char *class_name,
                             int64 pool_id,
                             ...
                             ResourceLimits *limits)
{
    ...
    elog(DEBUG1, "Created workload class \"%s\" (id=%ld) in pool %ld",
         class_name, class_id, pool_id);
 ...
 */
bool
orochi_alter_workload_class(int64 class_id,
                            int64 pool_id,
 ...
 */
bool
orochi_drop_workload_class(int64 class_id, bool if_exists)
{
    WorkloadClass *wclass = NULL;
    bool found = false;
    ...
    elog(DEBUG1, "Dropped workload class \"%s\" (id=%ld)",
         wclass->class_name, class_id);
 ...
 */
WorkloadClass *
orochi_get_workload_class(int64 class_id)
{
 ...
 */
WorkloadClass *
orochi_get_workload_class_by_name(const char *class_name)
{
    WorkloadClass *result = NULL;
 ...
 */
List *
orochi_list_workload_classes(void)
{
    List *classes = NIL;
 ...
 */
static bool
check_pool_resources(ResourcePool *pool, int64 memory_requested)
{
    ...
    {
        int64 available = (pool->limits.memory_mb * 1024 * 1024) -
 ...
 */
bool
orochi_admit_query(int backend_pid, int64 pool_id,
                   QueryPriority priority,
                   int64 estimated_cost,
                   int64 memory_requested)
{
    ResourcePool *pool = NULL;
    bool admitted = false;
    ...
    if (pool->state != POOL_STATE_ACTIVE)
    {
        ...
        {
            elog(LOG, "Query rejected: pool \"%s\" is not active",
                 pool->pool_name);
        }
        return false;
    }
    ...
    if (check_pool_resources(pool, memory_requested))
    {
        ...
        elog(DEBUG2, "Admitted query for backend %d in pool \"%s\" "
             "(active=%d, memory=%ld)",
    ...
    else
    {
        ...
        if (orochi_workload_log_rejections)
        {
            elog(LOG, "Query for backend %d queued: pool \"%s\" at capacity "
                 "(active=%d/%d)",
                 backend_pid, pool->pool_name, pool->active_queries,
 ...
 */
bool
orochi_queue_query(int backend_pid, int64 pool_id,
                   QueryPriority priority,
                   int64 estimated_cost)
{
    QueryEntry *entry;
        ...
        pool->queued_queries >= pool->limits.max_queue_size)
    {
        ...
        if (orochi_workload_log_rejections)
        {
            elog(LOG, "Query rejected: pool \"%s\" queue full (%d/%d)",
                 pool->pool_name, pool->queued_queries,
                 pool->limits.max_queue_size);
        }
        return false;
    }
 ...
 */
void
orochi_release_query(int backend_pid)
{
    if (!orochi_workload_enabled)
        return;
 ...
 */
static void
remove_from_queue(int backend_pid)
{
    for (int i = 0; i < WORKLOAD_QUEUE_SIZE; i++)
    {
 ...
 */
bool
orochi_dequeue_next_query(int64 pool_id, int *backend_pid)
{
 ...
 */
int
orochi_get_queue_length(int64 pool_id)
{
    int count = 0;
 ...
 */
bool
orochi_cancel_queued_query(int backend_pid)
{
    bool found = false;
 ...
 */
const char *
orochi_priority_name(QueryPriority priority)
{
    switch (priority)
    {
 ...
 */
QueryPriority
orochi_parse_priority(const char *name)
{
 ...
 */
const char *
orochi_pool_state_name(ResourcePoolState state)
{
    switch (state)
    {
 ...
 */
ResourceLimits *
orochi_create_default_limits(void)
{
 ...
 */
void
orochi_copy_limits(ResourceLimits *dst, ResourceLimits *src)
{
 ...
 */
bool
orochi_validate_limits(ResourceLimits *limits)
{
    if (limits == NULL)
        return true;
 ...
 */
void
orochi_log_pool_stats(int64 pool_id)
{
    ResourcePool *pool = orochi_get_resource_pool(pool_id);

    if (pool != NULL)
    {
        elog(LOG, "Pool \"%s\" (id=%ld): active=%d, queued=%d, "
             "cpu_time=%ld us, memory=%ld bytes, "
             "executed=%ld, rejected=%ld",
             pool->pool_name, pool->pool_id,
             pool->active_queries, pool->queued_queries,
             pool->usage.cpu_time_us, pool->usage.memory_bytes,
             pool->usage.queries_executed, pool->usage.queries_rejected);
        pfree(pool);
    }
}
 ...
 */
void
orochi_log_workload_status(void)
{
    LWLockAcquire(WorkloadState->main_lock, LW_SHARED);

    elog(LOG, "Workload status: pools=%d, classes=%d, "
...
Datum
orochi_create_resource_pool_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_alter_resource_pool_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_drop_resource_pool_sql(PG_FUNCTION_ARGS)
{
    text *pool_name_text = PG_GETARG_TEXT_PP(0);
    char *pool_name = text_to_cstring(pool_name_text);
...
Datum
orochi_assign_workload_class_sql(PG_FUNCTION_ARGS)
{
...
bool
orochi_assign_workload_class_by_name(int backend_pid, const char *class_name)
{
    ...
    {
        ereport(WARNING,
                (errmsg("workload class \"%s\" not found", class_name)));
        return false;
    }
...
ResourceUsage *
orochi_get_pool_usage(int64 pool_id)
{
    ResourcePool *pool = orochi_get_resource_pool(pool_id);
...
ResourcePoolState
orochi_parse_pool_state(const char *name)
{
