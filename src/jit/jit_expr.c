...
int64
jit_convert_to_int64(Datum value, VectorizedDataType from_type)
{
    switch (from_type)
    {
...
int32
jit_convert_to_int32(Datum value, VectorizedDataType from_type)
{
    switch (from_type)
    {
        case VECTORIZED_TYPE_INT64:
...
double
jit_convert_to_float64(Datum value, VectorizedDataType from_type)
{
    switch (from_type)
    {
        case VECTORIZED_TYPE_INT64:
...
float
jit_convert_to_float32(Datum value, VectorizedDataType from_type)
{
    switch (from_type)
    {
        case VECTORIZED_TYPE_INT64:
...
bool
jit_convert_to_bool(Datum value, VectorizedDataType from_type)
{
    switch (from_type)
    {
        case VECTORIZED_TYPE_INT64:
...
bool
jit_expr_is_const(JitExprNode *expr)
{
    return expr != NULL && expr->type == JIT_EXPR_CONST;
}

bool
jit_expr_is_column(JitExprNode *expr)
{
    return expr != NULL && expr->type == JIT_EXPR_COLUMN;
}

bool
jit_expr_is_simple_comparison(JitExprNode *expr)
{
    if (expr == NULL)
        return false;
...
void
jit_extract_column_refs(JitExprNode *expr, int32 *columns, int32 *count,
                        int32 max_columns)
{
    int i;
...
JitExprNode *
jit_optimize_expr(JitContext *ctx, JitExprNode *expr)
{
        ...
        jit_expr_is_const(expr->left) && jit_expr_is_const(expr->right))
    {
        ...
        switch (expr->type)
        {
            ...
            case JIT_EXPR_BOOL_AND:
                {
                    bool left_val = jit_convert_to_bool(expr->left->const_value,
                                                        expr->left->result_type);
    ...
    if (expr->type == JIT_EXPR_BOOL_AND)
    {
        ...
        {
            bool val = jit_convert_to_bool(expr->left->const_value,
                                           expr->left->result_type);
            if (!val)
...
static int64
jit_eval_int64_internal(VectorBatch *batch, JitExprNode *expr,
                        int32 row_idx, bool *is_null)
{
    ...
    *is_null = false;
    ...
    switch (expr->type)
    {
        ...
        case JIT_EXPR_ARITH_ADD:
            ...
            right_val = jit_eval_int64_internal(batch, expr->right, row_idx, &right_null);
        ...
        case JIT_EXPR_CAST:
            {
                ...
                switch (expr->left->result_type)
                {
                    ...
                    case VECTORIZED_TYPE_INT32:
                    case VECTORIZED_TYPE_DATE:
                        return (int64)jit_eval_int32_internal(batch, expr->left, row_idx, is_null);
                    case VECTORIZED_TYPE_FLOAT64:
                        return (int64)jit_eval_float64_internal(batch, expr->left, row_idx, is_null);
                    case VECTORIZED_TYPE_FLOAT32:
                        return (int64)jit_eval_float32_internal(batch, expr->left, row_idx, is_null);
                    case VECTORIZED_TYPE_BOOL:
                        return jit_eval_bool_internal(batch, expr->left, row_idx, is_null) ? 1 : 0;
                    default:
                        return 0;
                }
            }
...
static int32
jit_eval_int32_internal(VectorBatch *batch, JitExprNode *expr,
                        int32 row_idx, bool *is_null)
{
    ...
    *is_null = false;
    ...
    switch (expr->type)
    {
        ...
        case JIT_EXPR_ARITH_ADD:
            ...
            right_val = jit_eval_int32_internal(batch, expr->right, row_idx, &right_null);
            *is_null = left_null || right_null;
            return left_val + right_val;
...
static double
jit_eval_float64_internal(VectorBatch *batch, JitExprNode *expr,
                          int32 row_idx, bool *is_null)
{
    ...
    *is_null = false;
    ...
    switch (expr->type)
    {
        ...
        case JIT_EXPR_ARITH_ADD:
            left_val = jit_eval_float64_internal(batch, expr->left, row_idx, &left_null);
            right_val = jit_eval_float64_internal(batch, expr->right, row_idx, &right_null);
            *is_null = left_null || right_null;
            return left_val + right_val;
        ...
        case JIT_EXPR_CAST:
            {
                switch (expr->left->result_type)
                {
                    case VECTORIZED_TYPE_INT64:
                    case VECTORIZED_TYPE_TIMESTAMP:
                        return (double)jit_eval_int64_internal(batch, expr->left, row_idx, is_null);
                    case VECTORIZED_TYPE_INT32:
                    case VECTORIZED_TYPE_DATE:
                        return (double)jit_eval_int32_internal(batch, expr->left, row_idx, is_null);
                    case VECTORIZED_TYPE_FLOAT64:
                        return jit_eval_float64_internal(batch, expr->left, row_idx, is_null);
                    case VECTORIZED_TYPE_FLOAT32:
                        return (double)jit_eval_float32_internal(batch, expr->left, row_idx, is_null);
                    default:
                        return 0.0;
                }
            }
...
static float
jit_eval_float32_internal(VectorBatch *batch, JitExprNode *expr,
                          int32 row_idx, bool *is_null)
{
    *is_null = false;
    ...
    switch (expr->type)
    {
        ...
        case JIT_EXPR_CAST:
            return (float)jit_eval_float64_internal(batch, expr->left, row_idx, is_null);
...
static bool
jit_eval_bool_internal(VectorBatch *batch, JitExprNode *expr,
                       int32 row_idx, bool *is_null)
{
    ...
    *is_null = false;
    ...
    switch (expr->type)
    {
        ...
        case JIT_EXPR_CMP_GE:
            {
                ...
                {
                    double left_f = jit_eval_float64_internal(batch, expr->left, row_idx, &left_null);
                    double right_f = jit_eval_float64_internal(batch, expr->right, row_idx, &right_null);
                    *is_null = left_null || right_null;
                    if (*is_null)
                        return false;
                ...
                {
                    int64 left_i = jit_eval_int64_internal(batch, expr->left, row_idx, &left_null);
                    int64 right_i = jit_eval_int64_internal(batch, expr->right, row_idx, &right_null);
                    *is_null = left_null || right_null;
                    if (*is_null)
                        return false;
        ...
        case JIT_EXPR_BOOL_AND:
            ...
            right_val = jit_eval_bool_internal(batch, expr->right, row_idx, &right_null);
            *is_null = right_null;
            return right_val;
        ...
        case JIT_EXPR_BETWEEN:
            {
                ...
                double val = jit_eval_float64_internal(batch, expr->children[0], row_idx, &left_null);
                double low = jit_eval_float64_internal(batch, expr->children[1], row_idx, &right_null);
                bool high_null;
                double high = jit_eval_float64_internal(batch, expr->children[2], row_idx, &high_null);
...
static int64
jit_wrapper_eval_int64(const void *data, int32 row_idx,
...
static int32
jit_wrapper_eval_int32(const void *data, int32 row_idx,
                       const uint64 *nulls, bool *is_null)
{
    VectorBatch *batch = (VectorBatch *)data;
...
static double
jit_wrapper_eval_float64(const void *data, int32 row_idx,
                         const uint64 *nulls, bool *is_null)
{
    VectorBatch *batch = (VectorBatch *)data;
...
static float
jit_wrapper_eval_float32(const void *data, int32 row_idx,
                         const uint64 *nulls, bool *is_null)
{
    VectorBatch *batch = (VectorBatch *)data;
...
static bool
jit_wrapper_eval_bool(const void *data, int32 row_idx,
                      const uint64 *nulls, bool *is_null)
{
    VectorBatch *batch = (VectorBatch *)data;
...
static void
jit_batch_filter_internal(VectorBatch *batch, JitExprNode *expr, bool *out_mask)
{
    int32 i;
    bool is_null;
...
JitEvalInt64Fn
jit_generate_eval_int64(JitContext *ctx, JitExprNode *expr)
{
...
JitEvalInt32Fn
jit_generate_eval_int32(JitContext *ctx, JitExprNode *expr)
{
    if (ctx == NULL || expr == NULL)
        return NULL;
...
JitEvalFloat64Fn
jit_generate_eval_float64(JitContext *ctx, JitExprNode *expr)
{
    if (ctx == NULL || expr == NULL)
        return NULL;
...
JitEvalFloat32Fn
jit_generate_eval_float32(JitContext *ctx, JitExprNode *expr)
{
    if (ctx == NULL || expr == NULL)
        return NULL;
...
JitEvalBoolFn
jit_generate_eval_bool(JitContext *ctx, JitExprNode *expr)
{
    if (ctx == NULL || expr == NULL)
        return NULL;
...
JitFilterExprFn
jit_generate_batch_filter(JitContext *ctx, JitExprNode *expr)
{
    if (ctx == NULL || expr == NULL)
        return NULL;
...
static int64
jit_sum_int64_internal(VectorBatch *batch, int32 column_index)
{
    return vectorized_sum_int64(batch, column_index);
}

static double
jit_sum_float64_internal(VectorBatch *batch, int32 column_index)
{
    return vectorized_sum_float64(batch, column_index);
}

JitSumInt64Fn
jit_generate_sum_int64(JitContext *ctx, int32 column_index)
{
    if (ctx == NULL)
        return NULL;
...
JitSumFloat64Fn
jit_generate_sum_float64(JitContext *ctx, int32 column_index)
{
    if (ctx == NULL)
        return NULL;
