...
/* Custom scan methods */
CustomScanMethods orochi_scan_methods = {"OrochiScan",
...
static PlannedStmt *create_distributed_plan_stmt(Query *parse,
    ...
    if (rte->rtekind == RTE_RELATION) {
      ...
      if (info != NULL && info->is_distributed && info->shard_count > 1) {
        ...
        if (parse->commandType == CMD_SELECT) {
          ...
          view_oid =
              get_relname_relid(view_name, get_rel_namespace(rte->relid));
...
PlannedStmt *orochi_planner_hook(Query *parse, const char *query_string,
  ...
  if (prev_planner_hook)
    result = prev_planner_hook(rewritten_query, query_string, cursorOptions,
                               boundParams);
  else
    result = standard_planner(rewritten_query, query_string, cursorOptions,
                              boundParams);
 ...
 */
static bool var_is_distribution_column(Var *var, Oid table_oid,
 ...
 * column
 */
static void analyze_qual_node(Node *node, Oid table_oid,
                              const char *dist_column,
  ...
  if (IsA(node, OpExpr)) {
    ...
    if (list_length(opexpr->args) == 2) {
      ...
      if (var_node != NULL && const_node != NULL) {
        ...
        if (is_equality &&
            var_is_distribution_column(var_node, table_oid, dist_column)) {
          Datum value;
          Oid type_oid;
    ...
    if (boolexpr->boolop == AND_EXPR) {
      ...
      foreach (lc, boolexpr->args) {
        analyze_qual_node((Node *)lfirst(lc), table_oid, dist_column,
                          restriction);
...
ShardRestriction *orochi_analyze_quals_for_pruning(Node *quals, Oid table_oid) {
  ...
  analyze_qual_node(quals, table_oid, table_info->distribution_column,
                    restriction);
...
List *orochi_analyze_predicates(Query *parse, Oid table_oid) {
  ...
  if (parse->jointree && parse->jointree->quals) {
    restriction =
        orochi_analyze_quals_for_pruning(parse->jointree->quals, table_oid);
    if (restriction != NULL)
...
List *orochi_prune_shards(Oid table_oid, List *predicates) {
  ...
  foreach (plc, predicates) {
    ...
    if (restriction->is_equality) {
      ...
      if (OidIsValid(type_oid)) {
        ...
        target_shard_index =
            orochi_get_shard_index(hash_value, table_info->shard_count);
        found_equality = true;
  ...
  if (found_equality && target_shard_index >= 0) {
    ...
    if (list_length(pruned_shards) == 0) {
      elog(WARNING, "Shard pruning: target shard %d not found",
           target_shard_index);
      return all_shards;
    }
...
Node *orochi_create_scan_state(CustomScan *cscan) {
  ...
  state =
      (OrochiScanState *)newNode(sizeof(OrochiScanState), T_CustomScanState);
  state->css.methods = &orochi_scan_methods;

  /* Extract distributed plan from custom_private */
  if (cscan->custom_private != NIL)
    state->distributed_plan =
        (DistributedPlan *)linitial(cscan->custom_private);
...
void orochi_begin_custom_scan(CustomScanState *node, EState *estate,
                              int eflags) {
  ...
  foreach (lc, dplan->fragment_queries) {
    ...
    if (conn == NULL) {
      elog(WARNING, "Failed to connect to node %d for shard %ld",
           fragment->node_id, fragment->shard_id);
      continue;
    }
...
void orochi_explain_custom_scan(CustomScanState *node, List *ancestors,
                                ExplainState *es) {
  OrochiScanState *state = (OrochiScanState *)node;
  DistributedPlan *dplan = state->distributed_plan;
...
bool orochi_can_join_locally(Oid table1_oid, Oid table2_oid,
    ...
    foreach (lc, join_conditions) {
      ...
      if (IsA(condition, OpExpr)) {
        ...
        if (list_length(op->args) == 2) {
          ...
          if (IsA(left, Var)) {
            Var *var = (Var *)left;
            if (var_is_distribution_column(var, table1_oid,
                                           info1->distribution_column))
              left_is_dist1 = true;
            if (var_is_distribution_column(var, table2_oid,
                                           info2->distribution_column))
              left_is_dist2 = true;
          }
           ...
           */
          if ((left_is_dist1 && right_is_dist2) ||
              (left_is_dist2 && right_is_dist1))
            return true;
        }
      }
    }
  ...
  foreach (lc, parse->targetList) {
    ...
    if (IsA(tle->expr, Aggref)) {
      ...
      } else if (agg->aggfnoid == 2100 || agg->aggfnoid == 2101) /* avg */
      {
        ...
        appendStringInfo(&sql, "SUM(col%d), COUNT(col%d)", tle->resno,
                         tle->resno);
      } else {
    ...
    if (IsA(tle->expr, Aggref)) {
      ...
      } else if (agg->aggfnoid == 2100 || agg->aggfnoid == 2101) /* avg */
      {
        appendStringInfoString(
            &sql, "SUM(partial_sum)::float / NULLIF(SUM(partial_count), 0)");
      } else {
