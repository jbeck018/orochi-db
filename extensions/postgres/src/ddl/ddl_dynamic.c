...
List *ddl_extract_query_sources(const char *query)
{
    ...
    while (*p) {
        ...
        while (*p && !isspace(*p) && *p != ',' && *p != '(' && strncasecmp(p, "AS", 2) != 0 &&
               len < NAMEDATALEN - 1) {
            table_name[len++] = *p++;
        }
        table_name[len] = '\0';
...
DynamicTable *ddl_parse_create_dynamic_table(const char *sql)
{
    ...
    while (*p) {
        ...
        if (strncasecmp(p, "INITIALIZE", 10) == 0) {
            ...
            } else if (strncasecmp(p, "OFF", 3) == 0 || strncasecmp(p, "FALSE", 5) == 0) {
                dt->initialize = false;
...
bool ddl_validate_dynamic_table(DynamicTable *dt, char **error_msg)
{
    ...
    if (strlen(dt->table_name) > DYNAMIC_TABLE_MAX_NAME_LENGTH) {
        *error_msg =
            psprintf("table name exceeds maximum length of %d", DYNAMIC_TABLE_MAX_NAME_LENGTH);
        return false;
    }
    ...
    if (strlen(dt->query_text) > DYNAMIC_TABLE_MAX_QUERY_LENGTH) {
        *error_msg = psprintf("query exceeds maximum length of %d", DYNAMIC_TABLE_MAX_QUERY_LENGTH);
        return false;
    }
    ...
    /* Check minimum lag */
    int64 lag_seconds = dt->target_lag->time / USECS_PER_SEC + dt->target_lag->day * 86400;
    if (lag_seconds < DYNAMIC_TABLE_MIN_LAG_SECONDS) {
        *error_msg =
            psprintf("TARGET_LAG must be at least %d seconds", DYNAMIC_TABLE_MIN_LAG_SECONDS);
        return false;
    }
...
void ddl_catalog_update_dynamic_table(DynamicTable *dt)
{
    ...
    appendStringInfo(
        ...
        dt->last_refresh_at ? psprintf("'%s'", timestamptz_to_str(dt->last_refresh_at)) : "NULL",
        dt->next_refresh_at ? psprintf("'%s'", timestamptz_to_str(dt->next_refresh_at)) : "NULL",
        dt->total_refreshes, dt->successful_refreshes, dt->failed_refreshes, dt->dynamic_table_id);
 ...
 * Finish a refresh
 */
void ddl_catalog_finish_refresh(int64 refresh_id, bool success, const char *error_msg)
{
    StringInfoData query;
 ...
 */
int64 orochi_create_dynamic_table(const char *name, const char *query, Interval *target_lag,
                                  DynamicTableRefreshMode mode, bool initialize)
{
    DynamicTable *dt;
    char *error_msg;
    ...
    elog(LOG, "created dynamic table '%s' with ID %ld", dt->table_name, dt->dynamic_table_id);
...
int64 orochi_refresh_dynamic_table(int64 dynamic_table_id)
{
    ...
    elog(LOG, "started dynamic table %ld (%s) refresh %ld", dynamic_table_id, dt->table_name,
         refresh_id);

    /* Update state to refreshing */
    initStringInfo(&update_query);
    appendStringInfo(&update_query, "UPDATE orochi.%s SET state = %d WHERE dynamic_table_id = %ld",
                     OROCHI_DYNAMIC_TABLES_TABLE, (int)DYNAMIC_STATE_REFRESHING, dynamic_table_id);
    ...
    PG_TRY();
    {
        ...
        if (dt->refresh_mode == DYNAMIC_REFRESH_FULL) {
            ...
            appendStringInfo(&refresh_query,
                             "TRUNCATE TABLE %s.%s; "
                             "INSERT INTO %s.%s %s",
                             dt->table_schema, dt->table_name, dt->table_schema, dt->table_name,
                             dt->query_text);
        } else {
        ...
        if (ret < 0) {
            ...
            elog(LOG, "dynamic table %ld refresh %ld completed, %lu rows", dynamic_table_id,
                 refresh_id, (unsigned long)SPI_processed);
        }
    ...
    if (success) {
        appendStringInfo(&update_query,
                         ...
                         "WHERE dynamic_table_id = %ld",
                         OROCHI_DYNAMIC_TABLES_TABLE, (int)DYNAMIC_STATE_ACTIVE, dynamic_table_id);
    } else {
        appendStringInfo(&update_query,
                         "UPDATE orochi.%s SET "
                         "state = %d, "
                         ...
                         "WHERE dynamic_table_id = %ld",
                         OROCHI_DYNAMIC_TABLES_TABLE, (int)DYNAMIC_STATE_FAILED, dynamic_table_id);
    }
...
Interval *orochi_get_current_lag(int64 dynamic_table_id)
{
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        bool isnull;
        Datum lag_datum = SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull);
        if (!isnull) {
...
{
    int64 a_total =
        a->time + a->day * 86400 * USECS_PER_SEC + a->month * 30 * 86400 * USECS_PER_SEC;
    int64 b_total =
        b->time + b->day * 86400 * USECS_PER_SEC + b->month * 30 * 86400 * USECS_PER_SEC;
...
{
    int64 total_seconds =
        interval->time / USECS_PER_SEC + interval->day * 86400 + interval->month * 30 * 86400;
...
Datum orochi_create_dynamic_table_sql(PG_FUNCTION_ARGS)
{
    ...
    dt_id = orochi_create_dynamic_table(name, query, target_lag, DYNAMIC_REFRESH_AUTO, true);
...
Datum orochi_dynamic_table_status_sql(PG_FUNCTION_ARGS)
{
    ...
    appendStringInfo(&query, "SELECT state FROM orochi.%s WHERE dynamic_table_id = %ld",
