 ...
 */
static DynamicTableFreshness *
dynamic_freshness_alloc(void)
{
 ...
 */
static Interval *
parse_interval_string(const char *str)
{
    Interval *interval;
    int       value;
    char      unit[32];
 ...
 */
Interval *
ddl_parse_target_lag(const char *lag_str)
{
...
ddl_parse_refresh_mode(const char *mode_str)
{
 ...
 */
List *
ddl_extract_query_sources(const char *query)
{
    List       *sources = NIL;
    const char *p = query;
    const char *from_pos;
    ...
    while (*p)
    {
        ...
        if (strncasecmp(p, "WHERE", 5) == 0 ||
            ...
            strncasecmp(p, "UNION", 5) == 0)
            break;

        /* Skip JOIN keywords */
        if (strncasecmp(p, "JOIN", 4) == 0 ||
            strncasecmp(p, "LEFT", 4) == 0 ||
            strncasecmp(p, "RIGHT", 5) == 0 ||
            strncasecmp(p, "INNER", 5) == 0 ||
            strncasecmp(p, "OUTER", 5) == 0 ||
            strncasecmp(p, "CROSS", 5) == 0 ||
 ...
 */
DynamicTable *
ddl_parse_create_dynamic_table(const char *sql)
{
    DynamicTable *dt;
    const char   *p;
    size_t        len;
    ...
    if (dt->target_lag == NULL)
    {
        ...
        dt->target_lag->time = DYNAMIC_TABLE_DEFAULT_LAG_HOURS * 3600 * USECS_PER_SEC;
    }
 ...
 */
bool
ddl_validate_dynamic_table(DynamicTable *dt, char **error_msg)
{
    if (dt == NULL)
    {
    ...
    /* Check minimum lag */
    int64 lag_seconds = dt->target_lag->time / USECS_PER_SEC +
                        dt->target_lag->day * 86400;
 ...
 */
void
ddl_catalog_store_dynamic_table(DynamicTable *dt)
{
    ...
    appendStringInfo(&query,
        ...
        dt->table_name,
        dt->table_schema ? dt->table_schema : "public",
        dt->query_text,
        (int) dt->refresh_mode,
        (int) dt->schedule_mode,
        (dt->target_lag->time / USECS_PER_SEC + dt->target_lag->day * 86400),
        dt->initialize ? "TRUE" : "FALSE",
        (int) dt->state);
    ...
    for (int i = 0; i < dt->num_sources; i++)
    {
        ...
        appendStringInfo(&query,
            ...
            "VALUES (%ld, '%s', '%s', %s)",
            OROCHI_DYNAMIC_SOURCES_TABLE,
            dt->dynamic_table_id,
            dt->sources[i].source_schema,
            dt->sources[i].source_name,
 ...
 */
void
ddl_catalog_update_dynamic_table(DynamicTable *dt)
{
    StringInfoData query;
    ...
    appendStringInfo(&query,
        ...
        (int) dt->state,
        dt->last_refresh_at ?
            psprintf("'%s'", timestamptz_to_str(dt->last_refresh_at)) : "NULL",
        dt->next_refresh_at ?
            psprintf("'%s'", timestamptz_to_str(dt->next_refresh_at)) : "NULL",
        dt->total_refreshes,
 ...
 */
void
ddl_catalog_delete_dynamic_table(int64 dynamic_table_id)
{
    StringInfoData query;
    ...
    appendStringInfo(&query,
        "DELETE FROM orochi.%s WHERE dynamic_table_id = %ld",
 ...
 */
int64
ddl_catalog_create_refresh(int64 dynamic_table_id)
{
    StringInfoData query;
    int64          refresh_id = 0;
    int            ret;
    ...
    appendStringInfo(&query,
        ...
        OROCHI_DYNAMIC_REFRESHES_TABLE,
        dynamic_table_id);
 ...
 */
void
ddl_catalog_finish_refresh(int64 refresh_id, bool success, const char *error_msg)
{
    StringInfoData query;
 ...
 */
int64
orochi_create_dynamic_table(const char *name, const char *query,
                            Interval *target_lag,
                            ...
                            bool initialize)
{
    ...
    elog(LOG, "created dynamic table '%s' with ID %ld",
         dt->table_name, dt->dynamic_table_id);
 ...
 */
bool
orochi_drop_dynamic_table(int64 dynamic_table_id, bool if_exists)
{
 ...
 */
bool
orochi_suspend_dynamic_table(int64 dynamic_table_id)
{
 ...
 */
bool
orochi_resume_dynamic_table(int64 dynamic_table_id)
{
 ...
 */
bool
dynamic_table_needs_refresh(DynamicTable *dt)
{
 ...
 */
TimestampTz
dynamic_table_next_refresh(DynamicTable *dt)
{
    if (dt->target_lag == NULL)
        return 0;
...
const char *
dynamic_table_state_name(DynamicTableState state)
{
    switch (state)
    {
        case DYNAMIC_STATE_ACTIVE:       return "ACTIVE";
        case DYNAMIC_STATE_SUSPENDED:    return "SUSPENDED";
        case DYNAMIC_STATE_FAILED:       return "FAILED";
        case DYNAMIC_STATE_INITIALIZING: return "INITIALIZING";
        case DYNAMIC_STATE_REFRESHING:   return "REFRESHING";
        default:                         return "UNKNOWN";
    }
}

DynamicTableState
dynamic_table_parse_state(const char *name)
{
    if (strcasecmp(name, "ACTIVE") == 0)       return DYNAMIC_STATE_ACTIVE;
    if (strcasecmp(name, "SUSPENDED") == 0)    return DYNAMIC_STATE_SUSPENDED;
    if (strcasecmp(name, "FAILED") == 0)       return DYNAMIC_STATE_FAILED;
    if (strcasecmp(name, "INITIALIZING") == 0) return DYNAMIC_STATE_INITIALIZING;
    if (strcasecmp(name, "REFRESHING") == 0)   return DYNAMIC_STATE_REFRESHING;
    return DYNAMIC_STATE_ACTIVE;
}

const char *
dynamic_refresh_mode_name(DynamicTableRefreshMode mode)
{
    switch (mode)
    {
        case DYNAMIC_REFRESH_AUTO:        return "AUTO";
        case DYNAMIC_REFRESH_FULL:        return "FULL";
        case DYNAMIC_REFRESH_INCREMENTAL: return "INCREMENTAL";
        default:                          return "UNKNOWN";
    }
}

DynamicTableRefreshMode
dynamic_refresh_parse_mode(const char *name)
{
    if (strcasecmp(name, "AUTO") == 0)        return DYNAMIC_REFRESH_AUTO;
    if (strcasecmp(name, "FULL") == 0)        return DYNAMIC_REFRESH_FULL;
    if (strcasecmp(name, "INCREMENTAL") == 0) return DYNAMIC_REFRESH_INCREMENTAL;
    return DYNAMIC_REFRESH_AUTO;
}

const char *
dynamic_schedule_mode_name(DynamicSchedulingMode mode)
{
    switch (mode)
    {
        case DYNAMIC_SCHEDULE_TARGET_LAG: return "TARGET_LAG";
        case DYNAMIC_SCHEDULE_DOWNSTREAM: return "DOWNSTREAM";
        default:                          return "UNKNOWN";
    }
}

bool
interval_is_less_than(Interval *a, Interval *b)
{
...
char *
interval_to_string(Interval *interval)
{
    int64 total_seconds = interval->time / USECS_PER_SEC +
                          interval->day * 86400 +
...
void
dynamic_freshness_free(DynamicTableFreshness *freshness)
{
    if (freshness == NULL)
        return;
...
void
dynamic_refresh_stats_free(DynamicRefreshStats *stats)
{
    if (stats == NULL)
        return;
...
void
dynamic_table_free(DynamicTable *dt)
{
    if (dt == NULL)
        return;
...
Datum
orochi_create_dynamic_table_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_drop_dynamic_table_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_refresh_dynamic_table_sql(PG_FUNCTION_ARGS)
{
    int64 dt_id = PG_GETARG_INT64(0);
    int64 refresh_id;
...
Datum
orochi_suspend_dynamic_table_sql(PG_FUNCTION_ARGS)
{
    int64 dt_id = PG_GETARG_INT64(0);
...
Datum
orochi_resume_dynamic_table_sql(PG_FUNCTION_ARGS)
{
    int64 dt_id = PG_GETARG_INT64(0);
