...
bool ddl_validate_workflow(Workflow *workflow, char **error_msg) {
  ...
  if (workflow->num_steps > WORKFLOW_MAX_STEPS) {
    *error_msg =
        psprintf("workflow exceeds maximum of %d steps", WORKFLOW_MAX_STEPS);
    return false;
  }
...
void ddl_catalog_store_workflow(Workflow *workflow) {
  ...
  appendStringInfo(
      ...
      workflow->auto_resume ? "TRUE" : "FALSE", workflow->max_concurrent,
      (int)workflow->state);
...
int64 ddl_catalog_create_run(int64 workflow_id) {
  ...
  appendStringInfo(&query,
                   ...
                   "VALUES (%ld, %d, NOW()) "
                   "RETURNING run_id",
                   OROCHI_WORKFLOW_RUNS_TABLE, workflow_id,
 ...
 */
void ddl_catalog_finish_run(int64 run_id, WorkflowState state,
...
int64 orochi_create_workflow(const char *name, const char *definition) {
  ...
  elog(LOG, "created workflow '%s' with ID %ld", workflow->workflow_name,
       workflow->workflow_id);
  ...
  if (wf != NULL) {
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
      ...
      for (uint64 i = 0; i < SPI_processed; i++) {
        ...
        Datum step_id_datum =
            SPI_getbinval(step_tuple, step_tupdesc, 1, &isnull);
        if (!isnull)
          step->step_id = DatumGetInt32(step_id_datum);

        Datum step_name_datum =
            SPI_getbinval(step_tuple, step_tupdesc, 2, &isnull);
        if (!isnull)
          step->step_name = pstrdup(TextDatumGetCString(step_name_datum));

        Datum step_type_datum =
            SPI_getbinval(step_tuple, step_tupdesc, 3, &isnull);
        if (!isnull)
          step->step_type = (WorkflowStepType)DatumGetInt32(step_type_datum);

        Datum timeout_datum =
            SPI_getbinval(step_tuple, step_tupdesc, 4, &isnull);
        if (!isnull)
        ...
        Datum continue_datum =
            SPI_getbinval(step_tuple, step_tupdesc, 6, &isnull);
        if (!isnull)
 ...
 */
static bool workflow_execute_step(Workflow *wf, WorkflowStep *step,
                                  char **error_msg) {
  /* For now, just log the step execution */
  elog(LOG, "executing workflow %s step %d (%s) type=%s", wf->workflow_name,
       step->step_id, step->step_name ? step->step_name : "unnamed",
...
int64 orochi_run_workflow(int64 workflow_id) {
  ...
  if (wf->state == WORKFLOW_STATE_SUSPENDED ||
      wf->state == WORKFLOW_STATE_RUNNING) {
    workflow_free(wf);
    ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                    errmsg("workflow %s is not in a runnable state",
                           wf->workflow_name)));
  }
  ...
  elog(LOG, "started workflow %ld (%s) run %ld with %d steps", workflow_id,
       wf->workflow_name, run_id, wf->num_steps);
  ...
  appendStringInfo(&update_query,
                   ...
                   "WHERE workflow_id = %ld",
                   OROCHI_WORKFLOWS_TABLE, (int)WORKFLOW_STATE_RUNNING,
                   workflow_id);
  ...
  for (int i = 0; i < wf->num_steps; i++) {
    ...
    for (int attempt = 0; attempt <= step->retry_count; attempt++) {
      if (attempt > 0) {
        elog(LOG, "retrying step %d (attempt %d/%d)", step->step_id,
             attempt + 1, step->retry_count + 1);
        /* Wait before retry */
  ...
  appendStringInfo(&update_query,
                   ...
                   final_state == WORKFLOW_STATE_SUCCEEDED ? "successful_runs"
                                                           : "failed_runs",
                   final_state == WORKFLOW_STATE_SUCCEEDED ? "successful_runs"
                                                           : "failed_runs",
                   workflow_id);
  ...
  elog(LOG, "finished workflow %ld run %ld: %s (%d/%d steps completed)",
       workflow_id, run_id, workflow_state_name(final_state), steps_completed,
       wf ? wf->num_steps : 0);
