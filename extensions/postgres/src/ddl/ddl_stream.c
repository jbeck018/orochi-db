 ...
 */
static Stream *stream_alloc(void)
{
 ...
 */
static StreamOffset *stream_offset_alloc(void)
{
 ...
 */
static Bitmapset *get_primary_key_columns(Oid relid)
{
 ...
 */
static void stream_get_table_columns(Stream *stream)
{
    Relation rel;
    TupleDesc tupdesc;
    int i;
 ...
 */
void ddl_parse_stream_options(Stream *stream, const char *options)
{
    ...
    while (*p && *p != ')') {
        ...
        if (strcasecmp(key, "APPEND_ONLY") == 0 || strcasecmp(key, "INSERT_ONLY") == 0) {
            if (strcasecmp(value, "TRUE") == 0 || strcmp(value, "1") == 0)
                stream->stream_type = STREAM_TYPE_APPEND_ONLY;
        } else if (strcasecmp(key, "SHOW_INITIAL_ROWS") == 0) {
            stream->show_initial_rows = (strcasecmp(value, "TRUE") == 0 || strcmp(value, "1") == 0);
 ...
 */
Stream *ddl_parse_create_stream(const char *sql)
{
    Stream *stream;
    const char *p;
    char *stream_name;
 ...
 */
bool ddl_validate_stream(Stream *stream, char **error_msg)
{
    if (stream == NULL) {
    ...
    if (strlen(stream->stream_name) > STREAM_MAX_NAME_LENGTH) {
        *error_msg = psprintf("stream name exceeds maximum length of %d", STREAM_MAX_NAME_LENGTH);
        return false;
    }
 ...
 */
void ddl_catalog_store_stream(Stream *stream)
{
    ...
    appendStringInfo(&query,
                     ...
                     "RETURNING stream_id",
                     OROCHI_STREAMS_TABLE, stream->stream_name,
                     stream->stream_schema ? stream->stream_schema : "public",
                     stream->source_table_oid,
                     stream->source_schema ? stream->source_schema : "public", stream->source_table,
                     (int)stream->stream_type, (int)stream->stream_mode,
                     stream->show_initial_rows ? "TRUE" : "FALSE", (int)stream->state);
    ...
    for (int i = 0; i < stream->num_columns; i++) {
        ...
        appendStringInfo(&query,
                         ...
                         "VALUES (%ld, '%s', %u, %s, %s)",
                         OROCHI_STREAM_COLUMNS_TABLE, stream->stream_id,
                         stream->columns[i].column_name, stream->columns[i].column_type,
 ...
 */
void ddl_catalog_update_stream(Stream *stream)
{
    StringInfoData query;
    ...
    appendStringInfo(
        ...
        "pending_changes = %ld "
        "WHERE stream_id = %ld",
        OROCHI_STREAMS_TABLE, (int)stream->state, stream->has_pending_data ? "TRUE" : "FALSE",
        stream->last_consumed_at ? psprintf("'%s'", timestamptz_to_str(stream->last_consumed_at))
                                 : "NULL",
        stream->total_changes, stream->pending_changes, stream->stream_id);
 ...
 */
void ddl_catalog_delete_stream(int64 stream_id)
{
    StringInfoData query;
    ...
    appendStringInfo(&query, "DELETE FROM orochi.%s WHERE stream_id = %ld",
    ...
    /* Delete stream */
    resetStringInfo(&query);
    appendStringInfo(&query, "DELETE FROM orochi.%s WHERE stream_id = %ld", OROCHI_STREAMS_TABLE,
                     stream_id);
    SPI_execute(query.data, false, 0);
 ...
 */
void ddl_catalog_update_stream_offset(int64 stream_id, StreamOffset *offset)
{
    StringInfoData query;
    ...
    appendStringInfo(&query,
                     ...
                     OROCHI_STREAM_OFFSETS_TABLE, stream_id, (uint32)(offset->lsn >> 32),
                     (uint32)offset->lsn, offset->xid, timestamptz_to_str(offset->timestamp),
                     offset->sequence);
 ...
 * Create a stream
 */
int64 orochi_create_stream(const char *name, Oid source_table, StreamType type, StreamMode mode,
                           bool show_initial_rows)
{
    ...
    elog(LOG, "created stream '%s' on table '%s.%s' with ID %ld", stream->stream_name,
         stream->source_schema, stream->source_table, stream->stream_id);
 ...
 * Drop a stream
 */
bool orochi_drop_stream(int64 stream_id, bool if_exists)
{
 ...
 */
bool orochi_stream_has_data(int64 stream_id)
{
    StringInfoData query;
    bool has_data = false;
    int ret;
    ...
    appendStringInfo(&query,
                     ...
                     "            (SELECT sequence FROM orochi.%s WHERE "
                     "stream_id = s.stream_id), 0)) "
                     "FROM orochi.%s s "
                     "WHERE s.stream_id = %ld",
                     OROCHI_STREAM_OFFSETS_TABLE, OROCHI_STREAM_OFFSETS_TABLE, OROCHI_STREAMS_TABLE,
                     stream_id);
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        ...
        Datum has_pending =
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull1);
        Datum has_new_changes =
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 2, &isnull2);

        /* Stream has data if either flag is set or there are new changes */
        has_data =
            (!isnull1 && DatumGetBool(has_pending)) || (!isnull2 && DatumGetBool(has_new_changes));
    }
 ...
 */
int64 orochi_stream_pending_count(int64 stream_id)
{
    StringInfoData query;
    int64 count = 0;
    int ret;
 ...
 */
bool orochi_stream_advance_offset(int64 stream_id, StreamOffset *offset)
{
 ...
 */
bool orochi_stream_is_stale(int64 stream_id)
{
    StringInfoData query;
    bool is_stale = false;
    int ret;
    ...
    appendStringInfo(&query,
                     ...
                     "WHERE s.stream_id = %ld",
                     (int)STREAM_STATE_STALE, STREAM_DEFAULT_STALENESS_DAYS, OROCHI_STREAMS_TABLE,
                     OROCHI_STREAM_OFFSETS_TABLE, stream_id);
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        bool isnull;
        Datum stale_datum = SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull);
        if (!isnull)
...
const char *stream_type_name(StreamType type)
{
    switch (type) {
    case STREAM_TYPE_STANDARD:
        return "STANDARD";
    case STREAM_TYPE_APPEND_ONLY:
        return "APPEND_ONLY";
    case STREAM_TYPE_INSERT_ONLY:
        return "INSERT_ONLY";
    default:
        return "UNKNOWN";
    }
...
StreamType stream_parse_type(const char *name)
{
    if (strcasecmp(name, "STANDARD") == 0)
        return STREAM_TYPE_STANDARD;
    if (strcasecmp(name, "APPEND_ONLY") == 0)
        return STREAM_TYPE_APPEND_ONLY;
    if (strcasecmp(name, "INSERT_ONLY") == 0)
        return STREAM_TYPE_INSERT_ONLY;
    return STREAM_TYPE_STANDARD;
}
...
const char *stream_mode_name(StreamMode mode)
{
    ...
    case STREAM_MODE_DEFAULT:
        return "DEFAULT";
    case STREAM_MODE_KEYS_ONLY:
        return "KEYS_ONLY";
    case STREAM_MODE_FULL_BEFORE:
        return "FULL_BEFORE";
    default:
        return "UNKNOWN";
    }
...
StreamMode stream_parse_mode(const char *name)
{
    if (strcasecmp(name, "DEFAULT") == 0)
        return STREAM_MODE_DEFAULT;
    if (strcasecmp(name, "KEYS_ONLY") == 0)
        return STREAM_MODE_KEYS_ONLY;
    if (strcasecmp(name, "FULL_BEFORE") == 0)
        return STREAM_MODE_FULL_BEFORE;
    return STREAM_MODE_DEFAULT;
}
...
const char *stream_state_name(StreamState state)
{
    ...
    case STREAM_STATE_ACTIVE:
        return "ACTIVE";
    case STREAM_STATE_STALE:
        return "STALE";
    case STREAM_STATE_PAUSED:
        return "PAUSED";
    case STREAM_STATE_DISABLED:
        return "DISABLED";
    default:
        return "UNKNOWN";
    }
...
int stream_offset_compare(StreamOffset *a, StreamOffset *b)
{
    if (a->lsn < b->lsn)
        return -1;
    if (a->lsn > b->lsn)
...
bool stream_offset_is_valid(StreamOffset *offset)
{
    return offset != NULL && offset->lsn != InvalidXLogRecPtr;
}

StreamOffset *stream_offset_copy(StreamOffset *offset)
{
    StreamOffset *copy;
...
void stream_change_record_free(StreamChangeRecord *record)
{
    if (record == NULL)
        return;
...
Datum orochi_create_stream_sql(PG_FUNCTION_ARGS)
{
    ...
    stream_id =
        orochi_create_stream(name, table_oid, STREAM_TYPE_STANDARD, STREAM_MODE_DEFAULT, false);

    PG_RETURN_INT64(stream_id);
}

PG_FUNCTION_INFO_V1(orochi_drop_stream_sql);
Datum orochi_drop_stream_sql(PG_FUNCTION_ARGS)
{
...
Datum orochi_stream_has_data_sql(PG_FUNCTION_ARGS)
{
    int64 stream_id = PG_GETARG_INT64(0);
...
Datum orochi_stream_status_sql(PG_FUNCTION_ARGS)
{
    int64 stream_id = PG_GETARG_INT64(0);
    StringInfoData query;
    ...
    appendStringInfo(&query, "SELECT state FROM orochi.%s WHERE stream_id = %ld",
                     OROCHI_STREAMS_TABLE, stream_id);
