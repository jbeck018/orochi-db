...
static void stream_get_table_columns(Stream *stream) {
  ...
  for (i = 0; i < tupdesc->natts; i++) {
    ...
    if (!attr->attisdropped) {
      ...
      stream->columns[num_cols].is_key =
          bms_is_member(attr->attnum, pk_columns);
...
void ddl_catalog_store_stream(Stream *stream) {
  ...
  appendStringInfo(
      ...
      stream->source_table, (int)stream->stream_type, (int)stream->stream_mode,
...
void ddl_catalog_update_stream(Stream *stream) {
  ...
  appendStringInfo(
      ...
      "pending_changes = %ld "
      "WHERE stream_id = %ld",
      OROCHI_STREAMS_TABLE, (int)stream->state,
      stream->has_pending_data ? "TRUE" : "FALSE",
      stream->last_consumed_at
          ? psprintf("'%s'", timestamptz_to_str(stream->last_consumed_at))
          : "NULL",
...
void ddl_catalog_update_stream_offset(int64 stream_id, StreamOffset *offset) {
  ...
  appendStringInfo(&query,
                   ...
                   OROCHI_STREAM_OFFSETS_TABLE, stream_id,
                   (uint32)(offset->lsn >> 32), (uint32)offset->lsn,
                   offset->xid, timestamptz_to_str(offset->timestamp),
                   offset->sequence);
 ...
 * Create a stream
 */
int64 orochi_create_stream(const char *name, Oid source_table, StreamType type,
  ...
  elog(LOG, "created stream '%s' on table '%s.%s' with ID %ld",
       stream->stream_name, stream->source_schema, stream->source_table,
       stream->stream_id);
...
bool orochi_stream_has_data(int64 stream_id) {
  ...
  appendStringInfo(&query,
                   ...
                   OROCHI_STREAM_OFFSETS_TABLE, OROCHI_STREAM_OFFSETS_TABLE,
                   OROCHI_STREAMS_TABLE, stream_id);
...
bool orochi_stream_is_stale(int64 stream_id) {
  ...
  appendStringInfo(&query,
                   ...
                   "WHERE s.stream_id = %ld",
                   (int)STREAM_STATE_STALE, STREAM_DEFAULT_STALENESS_DAYS,
                   OROCHI_STREAMS_TABLE, OROCHI_STREAM_OFFSETS_TABLE,
                   stream_id);
...
Datum orochi_create_stream_sql(PG_FUNCTION_ARGS) {
  ...
  stream_id = orochi_create_stream(name, table_oid, STREAM_TYPE_STANDARD,
                                   STREAM_MODE_DEFAULT, false);
