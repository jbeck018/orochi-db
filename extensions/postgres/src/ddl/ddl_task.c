 ...
 */
static int cron_parse_field(const char *field, int current, int min_val, int max_val)
{
    const char *p = field;
    int step = 1;
 ...
 */
TimestampTz cron_next_occurrence(const char *cron_expr, const char *timezone, TimestampTz from)
{
    char fields[5][64];
    int field_idx = 0;
    ...
    if (field_idx < 5) {
        ...
        return DatumGetTimestampTz(DirectFunctionCall2(
            timestamptz_pl_interval, TimestampTzGetDatum(from), PointerGetDatum(&one_hour)));
    }
 ...
 */
TaskSchedule *ddl_parse_cron_schedule(const char *cron_expr, const char *timezone)
{
    TaskSchedule *schedule;
...
Task *ddl_parse_create_task(const char *sql)
{
    ...
    while (*p) {
        ...
        if (strncasecmp(p, "AFTER", 5) == 0) {
            ...
            List *deps = NIL;
            while (*p && strncasecmp(p, "AS", 2) != 0 && strncasecmp(p, "SCHEDULE", 8) != 0 &&
...
bool ddl_validate_task(Task *task, char **error_msg)
{
    ...
    if (strlen(task->task_name) > TASK_MAX_NAME_LENGTH) {
        *error_msg = psprintf("task name exceeds maximum length of %d", TASK_MAX_NAME_LENGTH);
        return false;
    }
    ...
    if (strlen(task->sql_text) > TASK_MAX_SQL_LENGTH) {
        *error_msg = psprintf("task SQL exceeds maximum length of %d", TASK_MAX_SQL_LENGTH);
        return false;
    }
...
void ddl_catalog_store_task(Task *task)
{
    ...
    for (int i = 0; i < task->num_predecessors; i++) {
        ...
        appendStringInfo(&query,
                         ...
                         "VALUES (%ld, '%s')",
                         OROCHI_TASK_DEPS_TABLE, task->task_id, task->predecessor_names[i]);
...
void ddl_catalog_update_task(Task *task)
{
    ...
    appendStringInfo(
        ...
        "next_run_at = %s "
        "WHERE task_id = %ld",
        OROCHI_TASKS_TABLE, task->sql_text, (int)task->state, task->consecutive_failures,
        task->last_run_at ? psprintf("'%s'", timestamptz_to_str(task->last_run_at)) : "NULL",
        task->next_run_at ? psprintf("'%s'", timestamptz_to_str(task->next_run_at)) : "NULL",
        task->task_id);
...
void ddl_catalog_delete_task(int64 task_id)
{
    ...
    appendStringInfo(&query, "DELETE FROM orochi.%s WHERE task_id = %ld", OROCHI_TASK_DEPS_TABLE,
                     task_id);
    ...
    /* Delete runs */
    resetStringInfo(&query);
    appendStringInfo(&query, "DELETE FROM orochi.%s WHERE task_id = %ld", OROCHI_TASK_RUNS_TABLE,
                     task_id);
    SPI_execute(query.data, false, 0);

    /* Delete task */
    resetStringInfo(&query);
    appendStringInfo(&query, "DELETE FROM orochi.%s WHERE task_id = %ld", OROCHI_TASKS_TABLE,
                     task_id);
    SPI_execute(query.data, false, 0);
 ...
 * Finish a task run
 */
void ddl_catalog_finish_task_run(int64 run_id, TaskRunState state, const char *error_msg)
{
    StringInfoData query;
 ...
 * Create a task
 */
int64 orochi_create_task(const char *name, const char *sql, TaskSchedule *schedule,
                         TaskCondition *condition)
{
    Task *task;
    char *error_msg;
...
int64 orochi_execute_task(int64 task_id)
{
    ...
    PG_TRY();
    {
        ...
        if (ret < 0) {
            ...
            elog(LOG, "task %ld run %ld completed successfully, %lu rows affected", task_id, run_id,
...
TimestampTz task_calculate_next_run(Task *task)
{
    ...
    case SCHEDULE_TYPE_CRON:
        return cron_next_occurrence(task->schedule->cron_expression, task->schedule->timezone,
                                    GetCurrentTimestamp());
...
bool task_condition_evaluate(TaskCondition *condition)
{
    ...
    case TASK_CONDITION_STREAM:
        ...
        if (condition->stream_id > 0)
            ...
            initStringInfo(&query);
            appendStringInfo(&query, "SELECT stream_id FROM orochi.%s WHERE stream_name = '%s'",
...
Datum orochi_task_status_sql(PG_FUNCTION_ARGS)
{
    ...
    appendStringInfo(&query, "SELECT state FROM orochi.%s WHERE task_id = %ld", OROCHI_TASKS_TABLE,
                     task_id);
