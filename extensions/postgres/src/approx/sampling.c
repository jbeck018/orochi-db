...
static uint64 sampling_rng_next(void) {
  ...
  const uint64 result =
      rotl(sampling_rng.s[0] + sampling_rng.s[3], 23) + sampling_rng.s[0];
 ...
 */
void bernoulli_sample_stats(BernoulliSampleState *state, int64 *rows_seen,
                            int64 *rows_selected, double *actual_rate) {
  if (rows_seen)
 ...
 */
ReservoirSampleState *reservoir_sample_create(int32 reservoir_size,
                                              int32 num_columns,
                                              Oid *column_types) {
  ...
  sample_context = AllocSetContextCreate(
      CurrentMemoryContext, "ReservoirSampling", ALLOCSET_DEFAULT_SIZES);
  ...
  for (int i = 0; i < num_columns; i++) {
    ...
    get_typlenbyval(column_types[i], &state->column_typlen[i],
 ...
 */
void reservoir_sample_add(ReservoirSampleState *state, Datum *values,
                          bool *nulls) {
  MemoryContext old_context;
  int slot;
  ...
  for (int i = 0; i < state->num_columns; i++) {
    ...
    if (!nulls[i]) {
      /* Copy the datum */
      state->values_array[idx] = datumCopy(values[i], state->column_typbyval[i],
 ...
 */
bool reservoir_sample_get(ReservoirSampleState *state, int32 index,
...
Datum reservoir_sample_final(PG_FUNCTION_ARGS) {
  ...
  result = construct_array(elems, state->num_items, FLOAT8OID, sizeof(float8),
                           FLOAT8PASSBYVAL, 'd');
...
Datum reservoir_sample_combine(PG_FUNCTION_ARGS) {
  ...
  for (int i = 0;
       i < result->reservoir_size && i < state1->num_items + state2->num_items;
       i++) {
...
Datum sample_rows(PG_FUNCTION_ARGS) {
  ...
  while (!sample_state->scan_done) {
    ...
    if (!table_scan_getnextslot(sample_state->scan, ForwardScanDirection,
                                sample_state->slot)) {
      /* No more tuples */
...
Datum reservoir_sample_rows(PG_FUNCTION_ARGS) {
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    /* Create reservoir */
    sample_state->reservoir =
        reservoir_sample_create(sample_size, natts, column_types);
  ...
  if (sample_state->current_index <
      reservoir_sample_count(sample_state->reservoir)) {
    ...
    if (reservoir_sample_get(sample_state->reservoir,
                             sample_state->current_index, values, nulls)) {
