...
void tdigest_compress(TDigest *td)
{
    ...
    for (int i = 1; i < total_count; i++) {
        ...
        if (k_high - k_low <= 1.0 || centroid_weight == 0.0) {
            ...
            k_low = tdigest_scale_function(cumulative_weight / total_weight, td->compression,
                                           td->scale_function);
 ...
 */
void tdigest_quantiles(TDigest *td, double *quantiles, double *results, int count)
{
...
double tdigest_trimmed_mean(TDigest *td, double lower_fraction, double upper_fraction)
{
...
Datum tdigest_add_sql(PG_FUNCTION_ARGS)
{
    ...
    if (PG_ARGISNULL(0))
        ereport(ERROR,
                (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED), errmsg("T-Digest cannot be NULL")));
...
Datum tdigest_info_sql(PG_FUNCTION_ARGS)
{
    ...
    result = psprintf("TDigest(compression=%.0f, centroids=%d, count=%ld, min=%.4g, "
                      "max=%.4g, p50=%.4g)",
                      td->compression, td->num_centroids, td->total_weight, tdigest_min(td),
                      tdigest_max(td), tdigest_quantile(td, 0.5));
...
Datum tdigest_out(PG_FUNCTION_ARGS)
{
    ...
    result = psprintf("TDigest(compression=%.0f, count=%ld)", td->compression, td->total_weight);
