...
static int compare_value_weight(const void *a, const void *b) {
 ...
 */
static double scale_k0(double q, double delta) {
  return delta * (asin(2.0 * q - 1.0) + M_PI_2) / M_PI;
}

static double scale_k0_inverse(double k, double delta) {
...
static double scale_k1_inverse(double k, double delta) {
 ...
 */
static double scale_k2(double q, double delta) {
  if (q <= 1e-15)
    return 0.0;
  if (q >= 1.0 - 1e-15)
    return delta;
...
static double scale_k2_inverse(double k, double delta) {
 ...
 */
static double scale_k3(double q, double delta) {
  if (q <= 1e-15)
    return 0.0;
  if (q >= 1.0 - 1e-15)
    return delta;
...
static double scale_k3_inverse(double k, double delta) {
  if (k < delta / 4.0)
...
double tdigest_scale_function(double q, double compression, int scale_type) {
  switch (scale_type) {
...
double tdigest_scale_inverse(double k, double compression, int scale_type) {
  switch (scale_type) {
  case TDIGEST_SCALE_K0:
 ...
 */
TDigest *tdigest_create(double compression) {
  return tdigest_create_with_scale(compression, TDIGEST_SCALE_K1);
}

TDigest *tdigest_create_with_scale(double compression, int scale_function) {
  TDigest *td;
  int max_centroids;
  Size size;
 ...
 */
void tdigest_sort_centroids(TDigest *td) {
 ...
 */
void tdigest_compress(TDigest *td) {
  Centroid *centroids = TDIGEST_CENTROIDS(td);
  ...
  double k_low =
      tdigest_scale_function(0.0, td->compression, td->scale_function);
  ...
  for (int i = 1; i < total_count; i++) {
    ...
    double k_high =
        tdigest_scale_function(q_high, td->compression, td->scale_function);
 ...
 */
void tdigest_add_weighted(TDigest *td, double value, double weight) {
 ...
 */
void tdigest_add(TDigest *td, double value) {
 ...
 */
void tdigest_add_centroid(TDigest *td, double mean, double weight) {
 ...
 */
double tdigest_quantile(TDigest *td, double q) {
  Centroid *centroids;
  double total_weight;
  ...
  for (int i = 0; i < td->num_centroids; i++) {
    ...
    if (target_weight < next_weight) {
      ...
      double next_mean =
          (i == td->num_centroids - 1) ? td->max_value : centroids[i + 1].mean;
 ...
 */
void tdigest_quantiles(TDigest *td, double *quantiles, double *results,
                       int count) {
 ...
 */
double tdigest_cdf(TDigest *td, double x) {
  Centroid *centroids;
  double total_weight;
  double cumulative_weight;
  ...
  for (int i = 0; i < td->num_centroids; i++) {
    if (x <= centroids[i].mean) {
      ...
      return (prev_weight + fraction * centroids[i].weight / 2.0) /
             total_weight;
    }
 ...
 * Merge two T-Digests.
 */
TDigest *tdigest_merge(TDigest *td1, TDigest *td2) {
  TDigest *result;
  Centroid *centroids1;
  Centroid *centroids2;
  double compression;
 ...
 */
void tdigest_merge_inplace(TDigest *td1, TDigest *td2) {
  Centroid *centroids2;
...
int64 tdigest_count(TDigest *td) {
  tdigest_compress(td);
  return td->total_weight;
}

double tdigest_min(TDigest *td) {
  return td->min_value == DBL_MAX ? NAN : td->min_value;
}

double tdigest_max(TDigest *td) {
  return td->max_value == -DBL_MAX ? NAN : td->max_value;
}

double tdigest_mean(TDigest *td) {
  Centroid *centroids;
  double sum = 0.0;
...
double tdigest_trimmed_mean(TDigest *td, double lower_fraction,
...
bytea *tdigest_serialize(TDigest *td) {
  StringInfoData buf;
  Centroid *centroids;
...
TDigest *tdigest_deserialize(bytea *data) {
...
Size tdigest_size(TDigest *td) { return TDIGEST_SIZE(td->max_centroids); }
...
Datum tdigest_trans(PG_FUNCTION_ARGS) {
...
Datum tdigest_final_percentile(PG_FUNCTION_ARGS) {
  TDigest *state;
  double percentile;
  ...
  percentile = PG_NARGS() > 1 && !PG_ARGISNULL(1) ? PG_GETARG_FLOAT8(1) : 0.5;
...
Datum tdigest_final_agg(PG_FUNCTION_ARGS) {
  TDigest *state;
...
Datum tdigest_create_sql(PG_FUNCTION_ARGS) {
...
Datum tdigest_add_sql(PG_FUNCTION_ARGS) {
  TDigest *td;
  double value;
...
Datum tdigest_quantile_sql(PG_FUNCTION_ARGS) {
  TDigest *td;
  double q;
...
Datum tdigest_percentile_sql(PG_FUNCTION_ARGS) {
  TDigest *td;
  double p;
...
Datum tdigest_merge_sql(PG_FUNCTION_ARGS) {
  TDigest *td1;
  TDigest *td2;
...
Datum tdigest_cdf_sql(PG_FUNCTION_ARGS) {
  TDigest *td;
  double x;
...
Datum tdigest_info_sql(PG_FUNCTION_ARGS) {
  TDigest *td;
  char *result;
  ...
  result =
      psprintf("TDigest(compression=%.0f, centroids=%d, count=%ld, min=%.4g, "
               "max=%.4g, p50=%.4g)",
               td->compression, td->num_centroids, td->total_weight,
               tdigest_min(td), tdigest_max(td), tdigest_quantile(td, 0.5));
...
Datum tdigest_out(PG_FUNCTION_ARGS) {
  ...
  result = psprintf("TDigest(compression=%.0f, count=%ld)", td->compression,
                    td->total_weight);
