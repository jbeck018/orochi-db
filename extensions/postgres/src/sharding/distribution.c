...
bool orochi_is_distributed_table(Oid table_oid)
{
...
bool orochi_is_reference_table(Oid table_oid)
{
    OrochiTableInfo *info;
    ...
    return (info->is_distributed && info->shard_strategy == OROCHI_SHARD_REFERENCE);
}

bool orochi_tables_are_colocated(Oid table1_oid, Oid table2_oid)
{
    int32 group1, group2;
...
int32 orochi_get_colocation_group(Oid table_oid)
{
    OrochiTableInfo *info;
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        ...
        group_id = orochi_create_colocation_group(info->shard_count, info->shard_strategy, NULL);
    }
...
int32 orochi_create_colocation_group(int32 shard_count, OrochiShardStrategy strategy,
...
uint32 orochi_crc32_hash(const void *data, size_t length)
{
...
int32 orochi_hash_datum(Datum value, Oid type_oid)
{
    int32 hash_value;
    ...
    case VARCHAROID: {
        ...
        hash_value = (int32)orochi_crc32_hash(VARDATA_ANY(txt), VARSIZE_ANY_EXHDR(txt));
    } break;
...
int32 orochi_get_shard_index(int32 hash_value, int32 shard_count)
{
...
OrochiShardInfo *orochi_route_to_shard(Oid table_oid, Datum value, Oid type_oid)
{
    int32 hash_value;
...
List *orochi_get_all_shards(Oid table_oid)
{
    return orochi_catalog_get_table_shards(table_oid);
}

List *orochi_get_shards_in_range(Oid table_oid, int32 min_hash, int32 max_hash)
{
    List *all_shards;
    List *matched = NIL;
    ListCell *lc;
...
char *orochi_get_distribution_column(Oid table_oid)
{
    OrochiTableInfo *info;
...
AttrNumber orochi_get_distribution_attnum(Oid table_oid)
{
    OrochiTableInfo *info;
    HeapTuple tp;
...
Oid orochi_get_distribution_type(Oid table_oid)
{
    AttrNumber attnum;
    HeapTuple tp;
...
void orochi_create_reference_table_internal(Oid table_oid)
{
    OrochiTableInfo info;
    ...
    elog(LOG, "Created reference table %s.%s", info.schema_name, info.table_name);
}

bool orochi_can_be_reference_table(Oid table_oid)
{
    /* Check if table is already distributed */
    if (orochi_is_distributed_table(table_oid)) {
        ereport(WARNING, (errmsg("table is already distributed"),
...
OrochiNodeInfo *orochi_get_shard_node(int64 shard_id)
{
 ...
 */
static int64 create_shard_move_record(int64 shard_id, int32 source_node, int32 target_node,
                                      int64 bytes_total)
{
    StringInfoData query;
    int64 move_id = -1;
 ...
 */
static void update_shard_move_progress(int64 move_id, int status, int64 bytes_copied,
                                       const char *error_msg)
{
    ...
    if (status == SHARD_MOVE_COMPLETED || status == SHARD_MOVE_FAILED) {
        ...
        if (error_msg != NULL)
            appendStringInfo(&query, ", error_message = %s", quote_literal_cstr(error_msg));
    } else {
        appendStringInfo(&query,
                         "UPDATE orochi.orochi_shard_moves SET "
...
void orochi_move_shard(int64 shard_id, int32 target_node)
{
    OrochiShardInfo *shard;
    int64 move_id;
    int32 source_node;
    ...
    if (shard == NULL) {
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_OBJECT), errmsg("shard %ld does not exist", shard_id)));
    }
    ...
    elog(DEBUG1, "Moved shard %ld from node %d to node %d", shard_id, source_node, target_node);
}

int32 orochi_select_node_for_shard(void)
{
    List *nodes;
    ListCell *lc;
    int32 best_node = -1;
    ...
    foreach (lc, nodes) {
        ...
        if (node->role == OROCHI_NODE_WORKER && node->shard_count < min_shards) {
...
bool orochi_validate_distribution_column(Oid table_oid, const char *column_name)
{
    HeapTuple tp;
    Oid type_oid;
...
int32 orochi_get_shard_count(Oid table_oid)
{
    OrochiTableInfo *info;
...
Datum orochi_create_distributed_table_sql(PG_FUNCTION_ARGS)
{
    ...
    orochi_catalog_create_shards(table_oid, shard_count, (OrochiShardStrategy)strategy);
...
Datum orochi_create_reference_table_sql(PG_FUNCTION_ARGS)
{
    Oid table_oid = PG_GETARG_OID(0);
...
Datum orochi_undistribute_table_sql(PG_FUNCTION_ARGS)
{
    Oid table_oid = PG_GETARG_OID(0);
 ...
 */
ShardBalanceStats *orochi_get_shard_balance_stats(void)
{
 ...
 */
void orochi_rebalance_table_shards(Oid table_oid)
{
    List *shards;
    ListCell *lc;
    List *nodes;
    ...
    elog(NOTICE, "Rebalancing %ld shards across %d nodes (target: %d per node)", total_shards,
         node_count, target_per_node);
 ...
 */
void orochi_move_shard_with_data(int64 shard_id, int32 target_node)
{
    OrochiShardInfo *shard;
 ...
 */
void orochi_split_shard_impl(int64 shard_id)
{
    OrochiShardInfo *shard;
    int32 mid_hash;
    int64 new_shard_id;
    ...
    new_shard_id = orochi_catalog_create_shard(shard->table_oid, mid_hash + 1, shard->hash_max,
                                               shard->node_id);

    elog(NOTICE, "Split shard %ld into [%d,%d] and new shard %ld [%d,%d]", shard_id,
         shard->hash_min, mid_hash, new_shard_id, mid_hash + 1, shard->hash_max);
}

/*
 * Merge two adjacent shards
 */
void orochi_merge_shards_impl(int64 shard1_id, int64 shard2_id)
{
    ...
    if (shard1 == NULL || shard2 == NULL) {
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_OBJECT), errmsg("one or both shards do not exist")));
    }
    ...
    elog(NOTICE, "Merged shards %ld and %ld into shard %ld [%d,%d]", shard1_id, shard2_id,
         shard1_id, new_min, new_max);
}

/*
 * Get shard placement details
 */
List *orochi_get_shard_placements(Oid table_oid)
{
    List *shards;
    List *placements = NIL;
    ListCell *lc;
...
Datum orochi_rebalance_table_sql(PG_FUNCTION_ARGS)
{
    Oid table_oid = PG_GETARG_OID(0);
...
Datum orochi_move_shard_sql(PG_FUNCTION_ARGS)
{
...
Datum orochi_split_shard_sql(PG_FUNCTION_ARGS)
{
    int64 shard_id = PG_GETARG_INT64(0);
...
Datum orochi_merge_shards_sql(PG_FUNCTION_ARGS)
{
...
Datum orochi_get_rebalance_plan_sql(PG_FUNCTION_ARGS)
{
    ShardBalanceStats *stats;
    ...
    appendStringInfo(&result,
                     ...
                     "  Imbalance ratio: %.2f\n"
                     "  Needs rebalancing: %s",
                     stats->total_nodes, stats->total_shards, stats->min_shards, stats->max_shards,
                     stats->avg_shards_per_node, stats->std_deviation, stats->imbalance_ratio,
