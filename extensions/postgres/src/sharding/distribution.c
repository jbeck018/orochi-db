...
int32 orochi_create_colocation_group(int32 shard_count,
...
int32 orochi_hash_datum(Datum value, Oid type_oid) {
  ...
  case VARCHAROID: {
    ...
    hash_value =
        (int32)orochi_crc32_hash(VARDATA_ANY(txt), VARSIZE_ANY_EXHDR(txt));
  } break;
...
OrochiShardInfo *orochi_route_to_shard(Oid table_oid, Datum value,
                                       Oid type_oid) {
  int32 hash_value;
...
List *orochi_get_shards_in_range(Oid table_oid, int32 min_hash,
                                 int32 max_hash) {
  List *all_shards;
  List *matched = NIL;
  ListCell *lc;
 ...
 */
static int64 create_shard_move_record(int64 shard_id, int32 source_node,
                                      int32 target_node, int64 bytes_total) {
  StringInfoData query;
  int64 move_id = -1;
  int ret;
 ...
 */
static void update_shard_move_progress(int64 move_id, int status,
                                       int64 bytes_copied,
...
void orochi_move_shard(int64 shard_id, int32 target_node) {
  ...
  move_id = create_shard_move_record(shard_id, source_node, target_node,
                                     shard->size_bytes);
  ...
  /* Mark as completed */
  update_shard_move_progress(move_id, SHARD_MOVE_COMPLETED, shard->size_bytes,
                             NULL);

  SPI_finish();

  elog(DEBUG1, "Moved shard %ld from node %d to node %d", shard_id, source_node,
       target_node);
}
...
bool orochi_validate_distribution_column(Oid table_oid,
...
Datum orochi_create_distributed_table_sql(PG_FUNCTION_ARGS) {
  ...
  elog(LOG,
       "Created distributed table %s.%s with %d physical shards on column %s",
...
void orochi_split_shard_impl(int64 shard_id) {
  ...
  new_shard_id = orochi_catalog_create_shard(shard->table_oid, mid_hash + 1,
                                             shard->hash_max, shard->node_id);

  elog(NOTICE, "Split shard %ld into [%d,%d] and new shard %ld [%d,%d]",
       shard_id, shard->hash_min, mid_hash, new_shard_id, mid_hash + 1,
       shard->hash_max);
}
...
void orochi_merge_shards_impl(int64 shard1_id, int64 shard2_id) {
  ...
  new_min =
      shard1->hash_min < shard2->hash_min ? shard1->hash_min : shard2->hash_min;
  new_max =
      shard1->hash_max > shard2->hash_max ? shard1->hash_max : shard2->hash_max;

  orochi_catalog_update_shard_range(shard1_id, new_min, new_max);
  orochi_catalog_delete_shard(shard2_id);

  elog(NOTICE, "Merged shards %ld and %ld into shard %ld [%d,%d]", shard1_id,
       shard2_id, shard1_id, new_min, new_max);
}
...
Datum orochi_get_rebalance_plan_sql(PG_FUNCTION_ARGS) {
  ...
  appendStringInfo(&result,
                   ...
                   "  Imbalance ratio: %.2f\n"
                   "  Needs rebalancing: %s",
                   stats->total_nodes, stats->total_shards, stats->min_shards,
                   stats->max_shards, stats->avg_shards_per_node,
                   stats->std_deviation, stats->imbalance_ratio,
