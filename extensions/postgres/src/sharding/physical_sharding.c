 ...
 */
void orochi_create_physical_shards(Oid table_oid, int shard_count,
  ...
  for (i = 0; i < shard_count; i++) {
    ...
    if (ret != SPI_OK_UTILITY)
      ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR),
                      errmsg("failed to create shard table %s_shard_%d",
                             table_name, i)));
    ...
    appendStringInfo(&query,
                     ...
                     quote_identifier(schema_name), table_name, i, i,
                     shard_count, schema_name, table_name, dist_column);
  ...
  elog(LOG, "Created %d physical shard tables for %s.%s", shard_count,
...
void orochi_drop_physical_shards(Oid table_oid) {
  ...
  elog(LOG, "Dropped %d physical shard tables for %s.%s", info->shard_count,
       schema_name, table_name);
}
...
int64 orochi_redistribute_data(Oid table_oid) {
  ...
  elog(LOG, "Redistributed %ld rows from %s.%s to %d shards", total_moved,
       schema_name, table_name, info->shard_count);
 ...
 */
int32 orochi_compute_shard_for_value(Datum value, Oid type_oid,
                                     int32 shard_count) {
  int32 hash_value;
 ...
 */
void orochi_install_routing_trigger(Oid table_oid, const char *dist_column,
                                    int32 shard_count) {
  StringInfoData query;
  char *schema_name;
  char *table_name;
  ...
  ret = SPI_execute(query.data, false, 0);
  if (ret != SPI_OK_UTILITY)
    elog(WARNING, "Failed to create INSERT routing function for %s.%s",
         schema_name, table_name);
  pfree(query.data);
  ...
  appendStringInfo(
      ...
      "$orochi_trigger$ LANGUAGE plpgsql",
      quote_identifier(schema_name), table_name, quote_identifier(dist_column),
      shard_count, schema_name, table_name, schema_name, table_name,
      dist_column, dist_column);

  ret = SPI_execute(query.data, false, 0);
  if (ret != SPI_OK_UTILITY)
    elog(WARNING, "Failed to create DELETE routing function for %s.%s",
         schema_name, table_name);
  pfree(query.data);

  /* Create BEFORE INSERT trigger */
  initStringInfo(&query);
  appendStringInfo(&query, "DROP TRIGGER IF EXISTS %s_insert_router ON %s.%s",
                   table_name, quote_identifier(schema_name),
                   quote_identifier(table_name));
  SPI_execute(query.data, false, 0);
  pfree(query.data);
  ...
  appendStringInfo(&query,
                   ...
                   table_name, quote_identifier(schema_name),
                   quote_identifier(table_name), quote_identifier(schema_name),
                   table_name);

  ret = SPI_execute(query.data, false, 0);
  if (ret != SPI_OK_UTILITY)
    elog(WARNING, "Failed to create INSERT trigger for %s.%s", schema_name,
         table_name);
  pfree(query.data);

  /* Create BEFORE DELETE trigger */
  initStringInfo(&query);
  appendStringInfo(&query, "DROP TRIGGER IF EXISTS %s_delete_router ON %s.%s",
                   table_name, quote_identifier(schema_name),
                   quote_identifier(table_name));
  SPI_execute(query.data, false, 0);
  pfree(query.data);
  ...
  ret = SPI_execute(query.data, false, 0);
  if (ret != SPI_OK_UTILITY)
    elog(WARNING, "Failed to create DELETE trigger for %s.%s", schema_name,
         table_name);
  pfree(query.data);
...
void orochi_remove_routing_trigger(Oid table_oid) {
  ...
  initStringInfo(&query);
  appendStringInfo(&query, "DROP TRIGGER IF EXISTS %s_update_router ON %s.%s",
                   table_name, quote_identifier(schema_name),
                   quote_identifier(table_name));
  SPI_execute(query.data, false, 0);
  pfree(query.data);
 ...
 */
void orochi_create_union_view(Oid table_oid, const char *dist_column,
                              int32 shard_count) {
  StringInfoData query;
  char *schema_name;
  char *table_name;
  ...
  for (i = 0; i < shard_count; i++) {
    ...
    appendStringInfo(&query, "SELECT * FROM %s.%s_shard_%d",
                     quote_identifier(schema_name), table_name, i);
  }

  ret = SPI_execute(query.data, false, 0);
  if (ret != SPI_OK_UTILITY)
    elog(WARNING, "Failed to create union view for %s.%s", schema_name,
         table_name);
  pfree(query.data);
  ...
  elog(LOG, "Created union view %s.%s_all_shards with INSTEAD OF triggers",
       schema_name, table_name);
}
