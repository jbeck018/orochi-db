 ...
 */
char *orochi_get_shard_table_name(Oid table_oid, int shard_index)
{
 ...
 */
Oid orochi_get_shard_table_oid(Oid parent_table_oid, int shard_index)
{
    char *shard_name;
    Oid namespace_oid;
    Oid shard_oid;
 ...
 */
void orochi_create_physical_shards(Oid table_oid, int shard_count, const char *dist_column)
{
    ...
    for (i = 0; i < shard_count; i++) {
        ...
        initStringInfo(&query);
        appendStringInfo(&query,
                         "COMMENT ON TABLE %s.%s_shard_%d IS 'Orochi shard %d of "
                         "%d for %s.%s, distribution column: %s'",
                         quote_identifier(schema_name), table_name, i, i, shard_count, schema_name,
                         table_name, dist_column);
    ...
    elog(LOG, "Created %d physical shard tables for %s.%s", shard_count, schema_name, table_name);
}

/*
 * Drop all physical shard tables for a distributed table
 */
void orochi_drop_physical_shards(Oid table_oid)
{
    StringInfoData query;
    ...
    for (i = 0; i < info->shard_count; i++) {
        initStringInfo(&query);
        appendStringInfo(&query, "DROP TABLE IF EXISTS %s.%s_shard_%d CASCADE",
                         quote_identifier(schema_name), table_name, i);
    ...
    elog(LOG, "Dropped %d physical shard tables for %s.%s", info->shard_count, schema_name,
         table_name);
}
 ...
 */
int64 orochi_redistribute_data(Oid table_oid)
{
    StringInfoData query;
    OrochiTableInfo *info;
    char *schema_name;
    ...
    for (i = 0; i < info->shard_count; i++) {
        ...
        hash_max =
            (i == info->shard_count - 1) ? INT32_MAX : (INT32_MIN + ((i + 1) * hash_range) - 1);
        ...
        appendStringInfo(&query,
                         ...
                         quote_identifier(schema_name), table_name, i,
                         quote_identifier(schema_name), quote_identifier(table_name),
                         quote_identifier(dist_column), hash_min, quote_identifier(dist_column),
                         hash_max);
    ...
    initStringInfo(&query);
    appendStringInfo(&query, "DELETE FROM %s.%s", quote_identifier(schema_name),
    ...
    elog(LOG, "Redistributed %ld rows from %s.%s to %d shards", total_moved, schema_name,
         table_name, info->shard_count);
 ...
 */
int32 orochi_compute_shard_for_value(Datum value, Oid type_oid, int32 shard_count)
{
    int32 hash_value;
 ...
 */
void orochi_install_routing_trigger(Oid table_oid, const char *dist_column, int32 shard_count)
{
    StringInfoData query;
    char *schema_name;
    char *table_name;
    ...
    /* Create the INSERT routing trigger function using E'' string to handle
     * escaping */
    initStringInfo(&query);
    appendStringInfo(&query,
                     ...
                     "    EXECUTE format('INSERT INTO %s.%s_shard_%%s VALUES ($1.*)', "
                     "shard_idx) USING NEW; "
                     "    RETURN NULL; "
                     "END; "
                     "$orochi_trigger$ LANGUAGE plpgsql",
                     quote_identifier(schema_name), table_name, quote_identifier(dist_column),
                     shard_count, schema_name, table_name);

    ret = SPI_execute(query.data, false, 0);
    if (ret != SPI_OK_UTILITY)
        elog(WARNING, "Failed to create INSERT routing function for %s.%s", schema_name,
             table_name);
    pfree(query.data);
    ...
    appendStringInfo(&query,
                     ...
                     "    EXECUTE format('DELETE FROM %s.%s_shard_%%s WHERE ctid IN (SELECT "
                     "ctid FROM %s.%s_shard_%%s WHERE %s = $1 LIMIT 1)', shard_idx, "
                     "shard_idx) USING OLD.%s; "
                     "    RETURN NULL; "
                     "END; "
                     "$orochi_trigger$ LANGUAGE plpgsql",
                     quote_identifier(schema_name), table_name, quote_identifier(dist_column),
                     shard_count, schema_name, table_name, schema_name, table_name, dist_column,
                     dist_column);

    ret = SPI_execute(query.data, false, 0);
    if (ret != SPI_OK_UTILITY)
        elog(WARNING, "Failed to create DELETE routing function for %s.%s", schema_name,
             table_name);
    pfree(query.data);

    /* Create BEFORE INSERT trigger */
    initStringInfo(&query);
    appendStringInfo(&query, "DROP TRIGGER IF EXISTS %s_insert_router ON %s.%s", table_name,
                     quote_identifier(schema_name), quote_identifier(table_name));
    SPI_execute(query.data, false, 0);
    pfree(query.data);
    ...
    initStringInfo(&query);
    appendStringInfo(&query, "DROP TRIGGER IF EXISTS %s_delete_router ON %s.%s", table_name,
                     quote_identifier(schema_name), quote_identifier(table_name));
    SPI_execute(query.data, false, 0);
    pfree(query.data);
 ...
 */
void orochi_remove_routing_trigger(Oid table_oid)
{
    StringInfoData query;
    char *schema_name;
    char *table_name;
    ...
    initStringInfo(&query);
    appendStringInfo(&query, "DROP TRIGGER IF EXISTS %s_update_router ON %s.%s", table_name,
                     quote_identifier(schema_name), quote_identifier(table_name));
    SPI_execute(query.data, false, 0);
    pfree(query.data);
    ...
    /* Drop functions */
    initStringInfo(&query);
    appendStringInfo(&query, "DROP FUNCTION IF EXISTS %s.%s_route_insert()",
                     quote_identifier(schema_name), table_name);
    SPI_execute(query.data, false, 0);
    pfree(query.data);

    initStringInfo(&query);
    appendStringInfo(&query, "DROP FUNCTION IF EXISTS %s.%s_route_update()",
                     quote_identifier(schema_name), table_name);
    SPI_execute(query.data, false, 0);
    pfree(query.data);

    initStringInfo(&query);
    appendStringInfo(&query, "DROP FUNCTION IF EXISTS %s.%s_route_delete()",
                     quote_identifier(schema_name), table_name);
    SPI_execute(query.data, false, 0);
    pfree(query.data);
 ...
 */
void orochi_create_union_view(Oid table_oid, const char *dist_column, int32 shard_count)
{
    StringInfoData query;
    char *schema_name;
    char *table_name;
    ...
    initStringInfo(&query);
    appendStringInfo(&query, "CREATE OR REPLACE VIEW %s.%s_all_shards AS ",
                     quote_identifier(schema_name), table_name);

    for (i = 0; i < shard_count; i++) {
        if (i > 0)
            appendStringInfoString(&query, " UNION ALL ");
        appendStringInfo(&query, "SELECT * FROM %s.%s_shard_%d", quote_identifier(schema_name),
                         table_name, i);
    }
    ...
    appendStringInfo(&query,
                     ...
                     "    EXECUTE format('DELETE FROM %s.%s_shard_%%s WHERE %s = $1', "
                     "old_shard) USING OLD.%s; "
                     "    EXECUTE format('INSERT INTO %s.%s_shard_%%s VALUES ($1.*)', "
                     "new_shard) USING NEW; "
                     "    RETURN NEW; "
                     "END; "
                     "$orochi_trigger$ LANGUAGE plpgsql",
                     quote_identifier(schema_name), table_name, quote_identifier(dist_column),
                     shard_count, quote_identifier(dist_column), shard_count, schema_name,
                     table_name, dist_column, dist_column, schema_name, table_name);
    ...
    appendStringInfo(&query,
                     ...
                     "BEGIN "
                     "    shard_idx := orochi.get_shard_index(orochi.hash_value(OLD.%s), %d); "
                     "    EXECUTE format('DELETE FROM %s.%s_shard_%%s WHERE ctid = (SELECT "
                     "ctid FROM %s.%s_shard_%%s WHERE %s = $1 LIMIT 1)', shard_idx, "
                     "shard_idx) USING OLD.%s; "
                     "    RETURN OLD; "
                     "END; "
    ...
    appendStringInfo(&query,
                     ...
                     table_name, quote_identifier(schema_name), table_name,
                     quote_identifier(schema_name), table_name);
    ...
    elog(LOG, "Created union view %s.%s_all_shards with INSTEAD OF triggers", schema_name,
         table_name);
}
 ...
 * Drop the union view
 */
void orochi_drop_union_view(Oid table_oid)
{
    StringInfoData query;
    char *schema_name;
    char *table_name;
    ...
    initStringInfo(&query);
    appendStringInfo(&query, "DROP VIEW IF EXISTS %s.%s_all_shards CASCADE",
                     quote_identifier(schema_name), table_name);
    SPI_execute(query.data, false, 0);
    pfree(query.data);
 ...
 */
bool orochi_should_rewrite_query(Oid table_oid)
{
    OrochiTableInfo *info;
 ...
 */
List *orochi_get_target_shards_for_query(Oid table_oid, Node *quals)
{
    OrochiTableInfo *info;
    List *shard_oids = NIL;
 ...
 */
List *orochi_expand_rte_to_shards(RangeTblEntry *rte, Node *quals)
{
    List *shard_rtes = NIL;
    List *shard_oids;
    ListCell *lc;
 ...
 */
Datum orochi_hash_value_sql(PG_FUNCTION_ARGS)
{
...
/*
 * SQL function: orochi.get_shard_index(hash_value integer, shard_count integer)
 * -> integer Returns the shard index (0-based) for a hash value
 */
Datum orochi_get_shard_index_sql(PG_FUNCTION_ARGS)
{
    ...
    if (shard_count <= 0)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("shard_count must be positive")));
...
/*
 * SQL function: orochi.get_shard_for_value(table_oid oid, value anyelement) ->
 * integer Returns the shard index for a value in a specific table
 */
Datum orochi_get_shard_for_value_sql(PG_FUNCTION_ARGS)
{
    ...
    if (info == NULL || !info->is_distributed)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("table is not distributed")));
