...
Size orochi_cdc_shmem_size(void)
{
...
void orochi_cdc_shmem_init(void)
{
    ...
    cdc_shared_state =
        (CDCSharedState *)ShmemInitStruct("Orochi CDC State", sizeof(CDCSharedState), &found);
                                     ...
                                     CDCOutputFormat format, const char *options_json)
{
    ...
    if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR), errmsg("failed to create CDC subscription")));
    }
...
bool orochi_start_cdc_subscription(int64 subscription_id)
{
    ...
    if (!RegisterDynamicBackgroundWorker(&worker, &handle)) {
        ereport(ERROR,
                (errcode(ERRCODE_INSUFFICIENT_RESOURCES), errmsg("could not register CDC worker")));
    }

    /* Wait for worker to start */
    if (WaitForBackgroundWorkerStartup(handle, &pid) != BGWH_STARTED) {
        ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR), errmsg("CDC worker failed to start")));
    }
...
bool orochi_resume_cdc_subscription(int64 subscription_id)
{
    ...
    if (current_state != CDC_SUB_STATE_PAUSED) {
        elog(NOTICE, "CDC subscription %ld is not paused (state: %s)", subscription_id,
             cdc_subscription_state_name(current_state));
        return false;
    }
...
bool orochi_drop_cdc_subscription(int64 subscription_id, bool if_exists)
{
    ...
    /* Delete stats */
    initStringInfo(&query);
    appendStringInfo(&query, "DELETE FROM orochi.orochi_cdc_stats WHERE subscription_id = %ld",
                     subscription_id);
    SPI_execute(query.data, false, 0);
    pfree(query.data);
...
CDCSubscriptionState orochi_get_cdc_subscription_state(int64 subscription_id)
{
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        bool isnull;
        state = (CDCSubscriptionState)DatumGetInt32(
...
List *orochi_list_cdc_subscriptions(void)
{
    ...
    initStringInfo(&query);
    appendStringInfoString(&query, "SELECT subscription_id FROM orochi.orochi_cdc_subscriptions "
 ...
 */
CDCEvent *cdc_create_event(CDCEventType type, Oid table_oid, HeapTuple old_tuple,
                           HeapTuple new_tuple)
{
    CDCEvent *event;
    Relation rel;
...
char *cdc_serialize_event_json(CDCEvent *event, CDCSubscription *sub)
{
    ...
    appendStringInfo(&buf, ", \"timestamp\": \"%s\"", timestamptz_to_str(event->event_time));
...
char *cdc_serialize_event_debezium(CDCEvent *event, CDCSubscription *sub)
{
    ...
    if (event->before != NULL &&
        (event->event_type == CDC_EVENT_UPDATE || event->event_type == CDC_EVENT_DELETE)) {
        appendStringInfo(&buf, "\"before\": {");
        first = true;
        for (i = 0; i < event->num_columns; i++) {
            if (event->before[i].column_name == NULL)
    ...
    /* After image */
    if (event->after != NULL &&
        (event->event_type == CDC_EVENT_INSERT || event->event_type == CDC_EVENT_UPDATE)) {
        appendStringInfo(&buf, "\"after\": {");
        first = true;
        for (i = 0; i < event->num_columns; i++) {
            if (event->after[i].column_name == NULL)
    ...
    appendStringInfo(&buf, ", \"ts_ms\": %ld", timestamptz_to_time_t(event->event_time) * 1000);
    appendStringInfo(&buf, ", \"schema\": \"%s\"", event->schema_name);
    appendStringInfo(&buf, ", \"table\": \"%s\"", event->table_name);
    ...
    appendStringInfo(&buf, ", \"ts_ms\": %ld", timestamptz_to_time_t(GetCurrentTimestamp()) * 1000);
...
bool cdc_event_matches_subscription(CDCEvent *event, CDCSubscription *sub)
{
    ...
    if (sub->num_table_filters > 0) {
        return cdc_table_matches_filter(event->schema_name, event->table_name, sub->table_filters,
                                        sub->num_table_filters);
    }
 ...
 */
bool cdc_table_matches_filter(const char *schema, const char *table, CDCTableFilter *filters,
                              int num_filters)
{
    int i;
...
void cdc_worker_main(Datum main_arg)
{
    ...
    while (!got_sigterm) {
        ...
        PG_CATCH();
        {
            ...
            ecxt = MemoryContextSwitchTo(sub->cdc_context ? sub->cdc_context : TopMemoryContext);
            edata = CopyErrorData();
            FlushErrorState();
...
static CDCSubscription *load_subscription_from_catalog(int64 subscription_id)
{
    ...
    sub->cdc_context =
        AllocSetContextCreate(TopMemoryContext, "CDC Subscription Context", ALLOCSET_DEFAULT_SIZES);
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        ...
        sub->output_format =
            (CDCOutputFormat)DatumGetInt32(SPI_getbinval(tuple, tupdesc, 4, &isnull));
...
static void process_subscription_events(CDCSubscription *sub, void *sink_context)
{
     ...
     */
    elog(DEBUG1, "Processing CDC events for subscription %ld", sub->subscription_id);
}
    ...
    while ((r = JsonbIteratorNext(&it, &key, true)) == WJB_KEY) {
        ...
        if (strcmp(keystr, "brokers") == 0 || strcmp(keystr, "bootstrap_servers") == 0) {
            ...
            if (val.type == jbvNumeric)
                config->partition = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
        } else if (strcmp(keystr, "batch_size") == 0) {
            if (val.type == jbvNumeric)
                config->batch_size = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
        } else if (strcmp(keystr, "linger_ms") == 0) {
            if (val.type == jbvNumeric)
                config->linger_ms = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
            ...
            if (val.type == jbvNumeric)
                config->retries = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
        } else if (strcmp(keystr, "retry_backoff_ms") == 0) {
            if (val.type == jbvNumeric)
                config->retry_backoff_ms = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
        }
...
static CDCWebhookSinkConfig *parse_webhook_sink_config(const char *json)
{
    ...
    while ((r = JsonbIteratorNext(&it, &key, true)) == WJB_KEY) {
        ...
        if (strcmp(keystr, "url") == 0 || strcmp(keystr, "endpoint") == 0) {
                ...
                if (hr == WJB_BEGIN_OBJECT) {
                    ...
                    while ((hr = JsonbIteratorNext(&headers_it, &header_key, true)) == WJB_KEY) {
                        ...
                        if (header_key.type == jbvString && header_val.type == jbvString) {
                            ...
                            appendStringInfo(&hdr, "%.*s=%.*s", header_key.val.string.len,
                                             header_key.val.string.val, header_val.val.string.len,
                    ...
                    while ((hr = JsonbIteratorNext(&headers_it, &header_val, true)) !=
                        ...
                        if (hr == WJB_ELEM && header_val.type == jbvString) {
                            header_list = lappend(header_list, pnstrdup(header_val.val.string.val,
            ...
            if (val.type == jbvNumeric)
                config->timeout_ms = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
        } else if (strcmp(keystr, "retry_count") == 0 || strcmp(keystr, "retries") == 0) {
            if (val.type == jbvNumeric)
                config->retry_count = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
        } else if (strcmp(keystr, "retry_delay_ms") == 0) {
            if (val.type == jbvNumeric)
                config->retry_delay_ms = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
            ...
            if (val.type == jbvNumeric)
                config->batch_timeout_ms = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
        }
...
static CDCFileSinkConfig *parse_file_sink_config(const char *json)
{
    ...
    while ((r = JsonbIteratorNext(&it, &key, true)) == WJB_KEY) {
        ...
        if (strcmp(keystr, "path") == 0 || strcmp(keystr, "file_path") == 0 ||
            ...
            if (val.type == jbvNumeric)
                config->max_file_size = DatumGetInt64(
                    DirectFunctionCall1(numeric_int8, NumericGetDatum(val.val.numeric)));
            else if (val.type == jbvString) {
            ...
            if (val.type == jbvNumeric)
                config->max_files = DatumGetInt32(
                    DirectFunctionCall1(numeric_int4, NumericGetDatum(val.val.numeric)));
...
CDCSinkType cdc_parse_sink_type(const char *name)
{
    ...
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("unknown CDC sink type: %s", name)));
    return CDC_SINK_KAFKA;
}
...
Datum orochi_create_cdc_subscription_sql(PG_FUNCTION_ARGS)
{
    ...
    int64 subscription_id = orochi_create_cdc_subscription(name, sink_type, output_format, options);
