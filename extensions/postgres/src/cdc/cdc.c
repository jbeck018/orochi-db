...
static void process_subscription_events(CDCSubscription *sub,
                                        void *sink_context);
static void update_subscription_stats(CDCSubscription *sub, int64 sent,
                                      int64 failed);
...
Size orochi_cdc_shmem_size(void) { return sizeof(CDCSharedState); }

void orochi_cdc_shmem_init(void) {
  bool found;

  cdc_shared_state = (CDCSharedState *)ShmemInitStruct(
      "Orochi CDC State", sizeof(CDCSharedState), &found);
 ...
 */
int64 orochi_create_cdc_subscription(const char *name, CDCSinkType sink_type,
  ...
  appendStringInfo(
      ...
      quote_literal_cstr(name), (int)sink_type, (int)format,
  ...
  if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0) {
    bool isnull;
    subscription_id = DatumGetInt64(SPI_getbinval(
  ...
  elog(NOTICE, "Created CDC subscription '%s' with ID %ld", name,
       subscription_id);
 ...
 */
bool orochi_start_cdc_subscription(int64 subscription_id) {
  CDCSubscription *sub;
  ...
  if (sub->state == CDC_SUB_STATE_ERROR) {
    elog(WARNING, "CDC subscription %ld is in error state, resetting...",
         subscription_id);
  }
  ...
  elog(NOTICE, "Started CDC subscription %ld with worker PID %d",
       subscription_id, pid);
 ...
 */
bool orochi_pause_cdc_subscription(int64 subscription_id) {
  StringInfoData query;
  int ret;
  ...
  appendStringInfo(&query,
                   ...
                   (int)CDC_SUB_STATE_PAUSED, subscription_id,
                   (int)CDC_SUB_STATE_ACTIVE);
 ...
 */
bool orochi_resume_cdc_subscription(int64 subscription_id) {
 ...
 */
bool orochi_stop_cdc_subscription(int64 subscription_id) {
  StringInfoData query;
  int ret;
  int worker_pid = 0;
  ...
  initStringInfo(&query);
  appendStringInfo(&query,
                   "SELECT worker_pid FROM orochi.orochi_cdc_subscriptions "
                   "WHERE subscription_id = %ld",
                   subscription_id);
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    bool isnull;
    worker_pid = DatumGetInt32(SPI_getbinval(
        SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
  }
  pfree(query.data);
 ...
 */
bool orochi_drop_cdc_subscription(int64 subscription_id, bool if_exists) {
  StringInfoData query;
  int ret;
  ...
  initStringInfo(&query);
  appendStringInfo(&query,
                   "SELECT 1 FROM orochi.orochi_cdc_subscriptions WHERE "
                   "subscription_id = %ld",
                   subscription_id);
  ret = SPI_execute(query.data, true, 1);
  pfree(query.data);
  ...
  initStringInfo(&query);
  appendStringInfo(
      &query, "DELETE FROM orochi.orochi_cdc_stats WHERE subscription_id = %ld",
      subscription_id);
  SPI_execute(query.data, false, 0);
  pfree(query.data);
 ...
 */
CDCSubscription *orochi_get_cdc_subscription(int64 subscription_id) {
 ...
 */
CDCSubscriptionState orochi_get_cdc_subscription_state(int64 subscription_id) {
  StringInfoData query;
  ...
  initStringInfo(&query);
  appendStringInfo(&query,
                   "SELECT state FROM orochi.orochi_cdc_subscriptions WHERE "
                   "subscription_id = %ld",
                   subscription_id);
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    bool isnull;
    state = (CDCSubscriptionState)DatumGetInt32(SPI_getbinval(
        SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
  }
 ...
 */
CDCStats *orochi_get_cdc_stats(int64 subscription_id) {
  CDCStats *stats;
  StringInfoData query;
  int ret;
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    stats->avg_latency_ms =
        DatumGetFloat8(SPI_getbinval(tuple, tupdesc, 6, &isnull));
    stats->throughput_per_sec =
        DatumGetFloat8(SPI_getbinval(tuple, tupdesc, 7, &isnull));
    stats->last_capture_time =
        DatumGetTimestampTz(SPI_getbinval(tuple, tupdesc, 8, &isnull));
    stats->last_send_time =
        DatumGetTimestampTz(SPI_getbinval(tuple, tupdesc, 9, &isnull));
    stats->current_lsn =
        DatumGetUInt64(SPI_getbinval(tuple, tupdesc, 10, &isnull));
    stats->confirmed_lsn =
        DatumGetUInt64(SPI_getbinval(tuple, tupdesc, 11, &isnull));
  }
 ...
 */
List *orochi_list_cdc_subscriptions(void) {
  List *result = NIL;
  StringInfoData query;
  int ret;
  int i;
  ...
  appendStringInfoString(
      &query, "SELECT subscription_id FROM orochi.orochi_cdc_subscriptions "
 ...
 */
CDCEvent *cdc_create_event(CDCEventType type, Oid table_oid,
  ...
  if (old_tuple != NULL &&
      (type == CDC_EVENT_UPDATE || type == CDC_EVENT_DELETE)) {
  ...
  if (new_tuple != NULL &&
      (type == CDC_EVENT_INSERT || type == CDC_EVENT_UPDATE)) {
 ...
 */
void cdc_free_event(CDCEvent *event) {
  int i;
 ...
 */
char *cdc_serialize_event_json(CDCEvent *event, CDCSubscription *sub) {
  StringInfoData buf;
  int i;
  bool first;
  ...
  /* Event metadata */
  appendStringInfo(&buf, "\"event_type\": \"%s\"",
                   cdc_event_type_name(event->event_type));
  appendStringInfo(&buf, ", \"timestamp\": \"%s\"",
                   timestamptz_to_str(event->event_time));
  appendStringInfo(&buf, ", \"lsn\": \"%X/%X\"", LSN_FORMAT_ARGS(event->lsn));
 ...
 */
char *cdc_serialize_event_debezium(CDCEvent *event, CDCSubscription *sub) {
  StringInfoData buf;
  int i;
  bool first;
  const char *op;
  ...
  /* Schema section */
  appendStringInfo(
      &buf, "\"schema\": {\"type\": \"struct\", \"name\": \"%s.%s.Envelope\"}",
 ...
 */
bool cdc_event_matches_subscription(CDCEvent *event, CDCSubscription *sub) {
 ...
 */
bool cdc_table_matches_filter(const char *schema, const char *table,
    ...
    if (filters[i].schema_pattern != NULL) {
      if (strcmp(filters[i].schema_pattern, "*") != 0) {
        ...
        if (strchr(filters[i].schema_pattern, '*') != NULL) {
          ...
          int len = strchr(filters[i].schema_pattern, '*') -
                    filters[i].schema_pattern;
    ...
    if (filters[i].table_pattern != NULL) {
      if (strcmp(filters[i].table_pattern, "*") != 0) {
        if (strchr(filters[i].table_pattern, '*') != NULL) {
          int len =
              strchr(filters[i].table_pattern, '*') - filters[i].table_pattern;
 ...
 */
void cdc_worker_main(Datum main_arg) {
  ...
  if (sub == NULL) {
    elog(ERROR, "Failed to load CDC subscription %ld configuration",
         subscription_id);
    proc_exit(1);
  }
  ...
  if (sink_context == NULL) {
    elog(ERROR, "Failed to initialize CDC sink for subscription %ld",
         subscription_id);
    proc_exit(1);
  }
  ...
  while (!got_sigterm) {
    ...
    rc = WaitLatch(MyLatch, WL_LATCH_SET | WL_TIMEOUT | WL_EXIT_ON_PM_DEATH,
                   orochi_cdc_batch_timeout_ms, PG_WAIT_EXTENSION);
    ...
    PG_CATCH();
    {
      ...
      ecxt = MemoryContextSwitchTo(sub->cdc_context ? sub->cdc_context
                                                    : TopMemoryContext);
      edata = CopyErrorData();
      FlushErrorState();
      MemoryContextSwitchTo(ecxt);

      elog(WARNING, "CDC subscription %ld error: %s", subscription_id,
           edata->message);
...
static CDCSubscription *load_subscription_from_catalog(int64 subscription_id) {
  ...
  sub->cdc_context = AllocSetContextCreate(
      TopMemoryContext, "CDC Subscription Context", ALLOCSET_DEFAULT_SIZES);
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    sub->sink_type =
        (CDCSinkType)DatumGetInt32(SPI_getbinval(tuple, tupdesc, 3, &isnull));
    sub->output_format = (CDCOutputFormat)DatumGetInt32(
        SPI_getbinval(tuple, tupdesc, 4, &isnull));

    options_json = SPI_getvalue(tuple, tupdesc, 5);

    sub->state = (CDCSubscriptionState)DatumGetInt32(
    ...
    sub->include_schema =
        DatumGetBool(SPI_getbinval(tuple, tupdesc, 11, &isnull));
    if (isnull)
      sub->include_schema = true;

    sub->include_before =
        DatumGetBool(SPI_getbinval(tuple, tupdesc, 12, &isnull));
    if (isnull)
      sub->include_before = true;

    sub->include_transaction =
        DatumGetBool(SPI_getbinval(tuple, tupdesc, 13, &isnull));
    if (isnull)
      sub->include_transaction = false;

    sub->start_lsn = DatumGetUInt64(SPI_getbinval(tuple, tupdesc, 14, &isnull));
    sub->confirmed_lsn =
        DatumGetUInt64(SPI_getbinval(tuple, tupdesc, 15, &isnull));
...
static void update_subscription_stats(CDCSubscription *sub, int64 sent,
  ...
  appendStringInfo(
      ...
      "events_captured = orochi.orochi_cdc_stats.events_captured + "
      "EXCLUDED.events_captured, "
      "events_sent = orochi.orochi_cdc_stats.events_sent + "
      "EXCLUDED.events_sent, "
      "events_failed = orochi.orochi_cdc_stats.events_failed + "
      "EXCLUDED.events_failed, "
      "last_send_time = NOW(), "
...
static CDCKafkaSinkConfig *parse_kafka_sink_config(const char *json) {
  ...
  while ((r = JsonbIteratorNext(&it, &key, true)) == WJB_KEY) {
    ...
    if (strcmp(keystr, "brokers") == 0 ||
        strcmp(keystr, "bootstrap_servers") == 0) {
      if (val.type == jbvString)
        config->bootstrap_servers =
            pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "topic") == 0) {
      if (val.type == jbvString)
        config->topic = pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "client_id") == 0 ||
               strcmp(keystr, "key_field") == 0) {
      if (val.type == jbvString)
        config->key_field = pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "partition") == 0) {
      if (val.type == jbvNumeric)
        config->partition = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    } else if (strcmp(keystr, "batch_size") == 0) {
      if (val.type == jbvNumeric)
        config->batch_size = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    } else if (strcmp(keystr, "linger_ms") == 0) {
      if (val.type == jbvNumeric)
        config->linger_ms = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    } else if (strcmp(keystr, "compression_type") == 0 ||
               strcmp(keystr, "compression") == 0) {
      if (val.type == jbvString)
        config->compression_type =
            pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "security_protocol") == 0 ||
               strcmp(keystr, "security") == 0) {
      if (val.type == jbvString)
      ...
      if (val.type == jbvString)
        config->sasl_mechanism =
            pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "sasl_username") == 0 ||
               strcmp(keystr, "username") == 0) {
      if (val.type == jbvString)
        config->sasl_username =
            pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "sasl_password") == 0 ||
               strcmp(keystr, "password") == 0) {
      if (val.type == jbvString)
        config->sasl_password =
            pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "acks") == 0) {
      if (val.type == jbvNumeric)
        config->acks = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
      ...
      if (val.type == jbvNumeric)
        config->retries = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    } else if (strcmp(keystr, "retry_backoff_ms") == 0) {
      if (val.type == jbvNumeric)
        config->retry_backoff_ms = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    }
...
static CDCWebhookSinkConfig *parse_webhook_sink_config(const char *json) {
  ...
  while ((r = JsonbIteratorNext(&it, &key, true)) == WJB_KEY) {
    ...
    if (strcmp(keystr, "url") == 0 || strcmp(keystr, "endpoint") == 0) {
      ...
      if (val.type == jbvBinary) {
        ...
        if (hr == WJB_BEGIN_OBJECT) {
          ...
          while ((hr = JsonbIteratorNext(&headers_it, &header_key, true)) ==
                 WJB_KEY) {
            hr = JsonbIteratorNext(&headers_it, &header_val, true);
            if (header_key.type == jbvString && header_val.type == jbvString) {
              StringInfoData hdr;
              initStringInfo(&hdr);
              appendStringInfo(&hdr, "%.*s=%.*s", header_key.val.string.len,
                               header_key.val.string.val,
                               header_val.val.string.len,
                               header_val.val.string.val);
              header_list = lappend(header_list, hdr.data);
            }
          }
        } else if (hr == WJB_BEGIN_ARRAY) {
          /* Array format: ["Header: Value", ...] */
          while ((hr = JsonbIteratorNext(&headers_it, &header_val, true)) !=
                 WJB_END_ARRAY) {
            if (hr == WJB_ELEM && header_val.type == jbvString) {
              header_list =
                  lappend(header_list, pnstrdup(header_val.val.string.val,
                                                header_val.val.string.len));
            }
          }
        }
    ...
    } else if (strcmp(keystr, "auth_type") == 0 ||
               strcmp(keystr, "auth") == 0) {
      if (val.type == jbvString)
        config->auth_type = pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "auth_credentials") == 0 ||
               strcmp(keystr, "credentials") == 0 ||
               strcmp(keystr, "token") == 0 || strcmp(keystr, "api_key") == 0) {
      if (val.type == jbvString)
        config->auth_credentials =
            pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "timeout_ms") == 0 ||
               strcmp(keystr, "timeout") == 0) {
      if (val.type == jbvNumeric)
        config->timeout_ms = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    } else if (strcmp(keystr, "retry_count") == 0 ||
               strcmp(keystr, "retries") == 0) {
      if (val.type == jbvNumeric)
        config->retry_count = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    } else if (strcmp(keystr, "retry_delay_ms") == 0) {
      if (val.type == jbvNumeric)
        config->retry_delay_ms = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    } else if (strcmp(keystr, "batch_mode") == 0 ||
               strcmp(keystr, "batch") == 0) {
      if (val.type == jbvBool)
      ...
      if (val.type == jbvNumeric)
        config->batch_timeout_ms = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    }
...
static CDCFileSinkConfig *parse_file_sink_config(const char *json) {
  ...
  while ((r = JsonbIteratorNext(&it, &key, true)) == WJB_KEY) {
    ...
    } else if (strcmp(keystr, "format") == 0 ||
               strcmp(keystr, "rotation_pattern") == 0 ||
               strcmp(keystr, "pattern") == 0) {
      if (val.type == jbvString)
        config->rotation_pattern =
            pnstrdup(val.val.string.val, val.val.string.len);
    } else if (strcmp(keystr, "max_file_size") == 0 ||
               strcmp(keystr, "max_size") == 0) {
      if (val.type == jbvNumeric)
        config->max_file_size = DatumGetInt64(DirectFunctionCall1(
            numeric_int8, NumericGetDatum(val.val.numeric)));
      else if (val.type == jbvString) {
    ...
    } else if (strcmp(keystr, "max_files") == 0 ||
               strcmp(keystr, "rotation_count") == 0) {
      if (val.type == jbvNumeric)
        config->max_files = DatumGetInt32(DirectFunctionCall1(
            numeric_int4, NumericGetDatum(val.val.numeric)));
    } else if (strcmp(keystr, "append_mode") == 0 ||
               strcmp(keystr, "append") == 0) {
      if (val.type == jbvBool)
        config->append_mode = val.val.boolean;
    } else if (strcmp(keystr, "flush_immediate") == 0 ||
               strcmp(keystr, "flush") == 0 || strcmp(keystr, "sync") == 0) {
      if (val.type == jbvBool)
        config->flush_immediate = val.val.boolean;
    } else if (strcmp(keystr, "pretty_print") == 0 ||
               strcmp(keystr, "pretty") == 0) {
      if (val.type == jbvBool)
...
const char *cdc_event_type_name(CDCEventType type) {
  switch (type) {
  case CDC_EVENT_INSERT:
    return "insert";
  case CDC_EVENT_UPDATE:
...
CDCEventType cdc_parse_event_type(const char *name) {
  if (pg_strcasecmp(name, "insert") == 0)
    return CDC_EVENT_INSERT;
  if (pg_strcasecmp(name, "update") == 0)
    return CDC_EVENT_UPDATE;
  if (pg_strcasecmp(name, "delete") == 0)
    return CDC_EVENT_DELETE;
  if (pg_strcasecmp(name, "truncate") == 0)
    return CDC_EVENT_TRUNCATE;
  if (pg_strcasecmp(name, "begin") == 0)
    return CDC_EVENT_BEGIN;
  if (pg_strcasecmp(name, "commit") == 0)
    return CDC_EVENT_COMMIT;
  if (pg_strcasecmp(name, "ddl") == 0)
    return CDC_EVENT_DDL;

  return CDC_EVENT_INSERT;
}

const char *cdc_sink_type_name(CDCSinkType type) {
  switch (type) {
  case CDC_SINK_KAFKA:
    return "kafka";
  case CDC_SINK_WEBHOOK:
  ...
  case CDC_SINK_FILE:
    return "file";
  case CDC_SINK_KINESIS:
    return "kinesis";
  case CDC_SINK_PUBSUB:
    return "pubsub";
  default:
    return "unknown";
  }
...
CDCSinkType cdc_parse_sink_type(const char *name) {
  if (pg_strcasecmp(name, "kafka") == 0)
    return CDC_SINK_KAFKA;
  if (pg_strcasecmp(name, "webhook") == 0)
    return CDC_SINK_WEBHOOK;
  if (pg_strcasecmp(name, "file") == 0)
    return CDC_SINK_FILE;
  if (pg_strcasecmp(name, "kinesis") == 0)
    return CDC_SINK_KINESIS;
  if (pg_strcasecmp(name, "pubsub") == 0)
    return CDC_SINK_PUBSUB;
...
const char *cdc_subscription_state_name(CDCSubscriptionState state) {
  switch (state) {
  case CDC_SUB_STATE_CREATED:
    return "created";
  case CDC_SUB_STATE_ACTIVE:
    return "active";
  case CDC_SUB_STATE_PAUSED:
    return "paused";
  case CDC_SUB_STATE_ERROR:
    return "error";
  case CDC_SUB_STATE_DISABLED:
    return "disabled";
  default:
    return "unknown";
  }
...
const char *cdc_output_format_name(CDCOutputFormat format) {
  switch (format) {
  case CDC_FORMAT_JSON:
    return "json";
  case CDC_FORMAT_AVRO:
    return "avro";
  case CDC_FORMAT_DEBEZIUM:
    return "debezium";
  case CDC_FORMAT_PROTOBUF:
    return "protobuf";
  default:
    return "unknown";
  }
...
CDCOutputFormat cdc_parse_output_format(const char *name) {
  if (pg_strcasecmp(name, "json") == 0)
    return CDC_FORMAT_JSON;
  if (pg_strcasecmp(name, "avro") == 0)
    return CDC_FORMAT_AVRO;
  if (pg_strcasecmp(name, "debezium") == 0)
    return CDC_FORMAT_DEBEZIUM;
  if (pg_strcasecmp(name, "protobuf") == 0)
    return CDC_FORMAT_PROTOBUF;
...
Datum orochi_create_cdc_subscription_sql(PG_FUNCTION_ARGS) {
  ...
  int64 subscription_id =
      orochi_create_cdc_subscription(name, sink_type, output_format, options);

  PG_RETURN_INT64(subscription_id);
}

Datum orochi_drop_cdc_subscription_sql(PG_FUNCTION_ARGS) {
...
Datum orochi_start_cdc_subscription_sql(PG_FUNCTION_ARGS) {
  int64 subscription_id = PG_GETARG_INT64(0);
  bool result = orochi_start_cdc_subscription(subscription_id);
  PG_RETURN_BOOL(result);
}

Datum orochi_stop_cdc_subscription_sql(PG_FUNCTION_ARGS) {
  int64 subscription_id = PG_GETARG_INT64(0);
  bool result = orochi_stop_cdc_subscription(subscription_id);
  PG_RETURN_BOOL(result);
}

Datum orochi_list_cdc_subscriptions_sql(PG_FUNCTION_ARGS) {
...
Datum orochi_cdc_subscription_stats_sql(PG_FUNCTION_ARGS) {
  int64 subscription_id = PG_GETARG_INT64(0);
  CDCStats *stats;
  TupleDesc tupdesc;
  Datum values[11];
