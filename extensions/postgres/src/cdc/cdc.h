...
extern bool orochi_update_cdc_subscription(int64 subscription_id,
                                           const char *options_json);

/* Subscription filtering */
extern bool orochi_add_cdc_table_filter(int64 subscription_id,
                                        const char *schema_pattern,
                                        const char *table_pattern,
                                        bool include);
extern bool orochi_remove_cdc_table_filter(int64 subscription_id,
                                           int filter_index);

/* Subscription status */
extern CDCSubscriptionState
...
extern CDCEvent *cdc_create_event(CDCEventType type, Oid table_oid,
                                  HeapTuple old_tuple, HeapTuple new_tuple);
extern void cdc_free_event(CDCEvent *event);

/* Event filtering */
extern bool cdc_event_matches_subscription(CDCEvent *event,
                                           CDCSubscription *sub);
extern bool cdc_table_matches_filter(const char *schema, const char *table,
                                     CDCTableFilter *filters, int num_filters);

/* Event serialization */
extern char *cdc_serialize_event_json(CDCEvent *event, CDCSubscription *sub);
extern char *cdc_serialize_event_debezium(CDCEvent *event,
                                          CDCSubscription *sub);
...
/* Event delivery */
extern bool cdc_sink_send_event(void *sink_context, CDCSinkType sink_type,
                                const char *event_data, int event_size,
                                const char *key, int key_size);
extern bool cdc_sink_send_batch(void *sink_context, CDCSinkType sink_type,
...
extern bool cdc_kafka_sink_send(void *producer, const char *topic,
                                const char *key, int key_size,
