...
static bool cdc_webhook_sink_send_internal(WebhookSinkContext *ctx,
 ...
 */
bool cdc_sink_send_event(void *sink_context, CDCSinkType sink_type,
  ...
  case CDC_SINK_KAFKA: {
    ...
    return cdc_kafka_sink_send(ctx, ctx->config->topic, key, key_size,
 ...
 */
bool cdc_sink_send_batch(void *sink_context, CDCSinkType sink_type,
                         char **events, int *event_sizes, int count) {
  int i;
  bool success = true;

  for (i = 0; i < count; i++) {
    if (!cdc_sink_send_event(sink_context, sink_type, events[i], event_sizes[i],
                             NULL, 0)) {
      success = false;
    }
  }
...
void *cdc_kafka_sink_init(CDCKafkaSinkConfig *config) {
  ...
  kafka_context = AllocSetContextCreate(TopMemoryContext, "CDC Kafka Sink",
...
#ifdef HAVE_LIBRDKAFKA
  {
    ...
    /* Configure batching */
    {
      ...
      rd_kafka_conf_set(conf, "batch.num.messages", batch_str, errstr,
                        sizeof(errstr));
    ...
    /* Configure retries */
    {
      ...
      snprintf(retries_str, sizeof(retries_str), "%d",
               config->retry_backoff_ms);
      rd_kafka_conf_set(conf, "retry.backoff.ms", retries_str, errstr,
                        sizeof(errstr));
    }
 ...
 */
bool cdc_kafka_sink_send(void *producer, const char *topic, const char *key,
                         int key_size, const char *value, int value_size) {
...
#ifdef HAVE_LIBRDKAFKA
  {
    int partition = ctx->config->partition >= 0 ? ctx->config->partition
                                                : RD_KAFKA_PARTITION_UA;
    int err;

    err = rd_kafka_produce(ctx->topic, partition, RD_KAFKA_MSG_F_COPY,
                           (void *)value, value_size, key, key_size, ctx);
  ...
  ctx->last_send_time = GetCurrentTimestamp();
  elog(DEBUG1, "CDC Kafka sink stub: sent %d bytes to topic %s", value_size,
       topic);
  return true;
#endif
}
...
void *cdc_webhook_sink_init(CDCWebhookSinkConfig *config) {
  ...
  /* Create memory context */
  webhook_context = AllocSetContextCreate(TopMemoryContext, "CDC Webhook Sink",
                                          ALLOCSET_DEFAULT_SIZES);
...
#ifdef HAVE_LIBCURL
  {
    ...
    ctx->headers = curl_slist_append(
        ctx->headers, psprintf("Content-Type: %s", config->content_type));
    ...
    if (config->auth_type && config->auth_credentials) {
      if (pg_strcasecmp(config->auth_type, "basic") == 0) {
        ...
        ctx->headers =
            curl_slist_append(ctx->headers, psprintf("Authorization: Bearer %s",
...
void *cdc_file_sink_init(CDCFileSinkConfig *config) {
  ...
  /* Create memory context */
  file_context = AllocSetContextCreate(TopMemoryContext, "CDC File Sink",
                                       ALLOCSET_DEFAULT_SIZES);
  ...
  ctx->current_file_path =
      file_sink_generate_path(ctx, ctx->current_file_index);
...
bool cdc_file_sink_write(void *context, const char *data, int size) {
  ...
  if (written != size) {
    elog(WARNING, "File sink write failed: wrote %zu of %d bytes", written,
         size);
    return false;
  }
...
static bool file_sink_rotate(FileSinkContext *ctx) {
  ...
  if (ctx->config->max_files > 0 &&
      ctx->current_file_index >= ctx->config->max_files - 1) {
