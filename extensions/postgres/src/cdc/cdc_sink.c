 ...
 */
void *
cdc_sink_init(CDCSubscription *sub)
{
    if (sub == NULL)
        return NULL;
 ...
 */
void
cdc_sink_close(void *sink_context, CDCSinkType sink_type)
{
 ...
 */
bool
cdc_sink_send_event(void *sink_context, CDCSinkType sink_type,
                    const char *event_data, int event_size,
        ...
        case CDC_SINK_KAFKA:
            {
                ...
                return cdc_kafka_sink_send(ctx, ctx->config->topic,
                                           key, key_size,
 ...
 */
bool
cdc_sink_send_batch(void *sink_context, CDCSinkType sink_type,
                    char **events, int *event_sizes, int count)
{
    int i;
    bool success = true;
    ...
    for (i = 0; i < count; i++)
    {
        if (!cdc_sink_send_event(sink_context, sink_type,
                                 events[i], event_sizes[i],
                                 NULL, 0))
        {
            success = false;
        }
 ...
 */
bool
cdc_sink_flush(void *sink_context, CDCSinkType sink_type)
{
    if (sink_context == NULL)
        return false;
        ...
        case CDC_SINK_WEBHOOK:
            {
                ...
                if (ctx->batch_count > 0 && ctx->batch_buffer != NULL)
                {
                    ...
                    return cdc_webhook_sink_send(ctx, ctx->batch_buffer->data,
...
#ifdef HAVE_LIBRDKAFKA
static void
kafka_delivery_callback(rd_kafka_t *rk, const rd_kafka_message_t *msg, void *opaque)
{
    ...
    if (msg->err)
    {
        snprintf(ctx->last_error, sizeof(ctx->last_error),
                 "Kafka delivery failed: %s", rd_kafka_err2str(msg->err));
        ctx->messages_failed++;
 ...
 */
void *
cdc_kafka_sink_init(CDCKafkaSinkConfig *config)
{
    ...
    kafka_context = AllocSetContextCreate(TopMemoryContext,
                                          "CDC Kafka Sink",
...
#ifdef HAVE_LIBRDKAFKA
    {
        ...
        {
            rd_kafka_conf_set(conf, "compression.type", config->compression_type,
        ...
        {
            rd_kafka_conf_set(conf, "sasl.mechanism", config->sasl_mechanism,
                              errstr, sizeof(errstr));
        }

        if (config->sasl_username)
        {
            rd_kafka_conf_set(conf, "sasl.username", config->sasl_username,
                              errstr, sizeof(errstr));
        }
        ...
        elog(LOG, "CDC Kafka sink initialized: brokers=%s, topic=%s",
 ...
 */
bool
cdc_kafka_sink_send(void *producer, const char *topic,
                    const char *key, int key_size,
...
#ifdef HAVE_LIBRDKAFKA
    {
        int partition = ctx->config->partition >= 0 ?
                        ctx->config->partition : RD_KAFKA_PARTITION_UA;
        int err;

        err = rd_kafka_produce(ctx->topic,
                               partition,
                               RD_KAFKA_MSG_F_COPY,
 ...
 */
void
cdc_kafka_sink_close(void *producer)
{
    KafkaSinkContext *ctx = (KafkaSinkContext *)producer;
    ...
    if (ctx->producer)
    {
        ...
        elog(LOG, "CDC Kafka sink closed: sent=%ld, failed=%ld, bytes=%ld",
             ctx->messages_sent, ctx->messages_failed, ctx->bytes_sent);
    }
#endif
...
#ifdef HAVE_LIBCURL
static size_t
webhook_write_callback(void *contents, size_t size, size_t nmemb, void *userp)
{
 ...
 */
void *
cdc_webhook_sink_init(CDCWebhookSinkConfig *config)
{
    ...
    /* Create memory context */
    webhook_context = AllocSetContextCreate(TopMemoryContext,
                                            "CDC Webhook Sink",
                                            ALLOCSET_DEFAULT_SIZES);
...
#ifdef HAVE_LIBCURL
    {
        ...
        ctx->headers = curl_slist_append(ctx->headers,
                                         psprintf("Content-Type: %s", config->content_type));
 ...
 */
bool
cdc_webhook_sink_send(void *context, const char *data, int size)
{
    ...
    if (ctx->config->batch_mode && ctx->batch_buffer != NULL)
    {
            ...
            (now - ctx->batch_start_time) >= ctx->config->batch_timeout_ms * 1000)
        {
            ...
            bool result = cdc_webhook_sink_send_internal(ctx,
                                                         ctx->batch_buffer->data,
                                                         ctx->batch_buffer->len);
 ...
 */
static bool
cdc_webhook_sink_send_internal(WebhookSinkContext *ctx, const char *data, int size)
{
#ifdef HAVE_LIBCURL
    CURLcode res;
    int retry;
    ...
    elog(WARNING, "Webhook send failed after %d retries: %s",
         ctx->config->retry_count, ctx->last_error);
    ctx->requests_failed++;
    ctx->consecutive_errors++;
    return false;
#else
 ...
 */
void
cdc_webhook_sink_close(void *context)
{
    WebhookSinkContext *ctx = (WebhookSinkContext *)context;
    ...
    if (ctx->curl)
    {
        ...
        elog(LOG, "CDC Webhook sink closed: sent=%ld, failed=%ld, bytes=%ld",
             ctx->requests_sent, ctx->requests_failed, ctx->bytes_sent);
    }
#endif
 ...
 */
void *
cdc_file_sink_init(CDCFileSinkConfig *config)
{
    FileSinkContext *ctx;
    MemoryContext oldcontext;
    ...
    /* Create memory context */
    file_context = AllocSetContextCreate(TopMemoryContext,
                                         "CDC File Sink",
                                         ALLOCSET_DEFAULT_SIZES);
 ...
 */
bool
cdc_file_sink_write(void *context, const char *data, int size)
{
 ...
 */
static bool
file_sink_rotate(FileSinkContext *ctx)
{
    char *new_path;
    int i;
 ...
 */
static char *
file_sink_generate_path(FileSinkContext *ctx, int index)
{
    StringInfoData path;
 ...
 */
void
cdc_file_sink_close(void *context)
{
    FileSinkContext *ctx = (FileSinkContext *)context;
    ...
    if (ctx->file)
    {
        ...
        elog(LOG, "CDC File sink closed: events=%ld, bytes=%ld, path=%s",
             ctx->events_written, ctx->bytes_written, ctx->current_file_path);
    }
