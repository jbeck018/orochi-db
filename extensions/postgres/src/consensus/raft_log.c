...
static bool raft_log_write_wal_record(RaftLog *log, int fd, RaftWALRecordType type,
...
static bool raft_log_read_wal_record(int fd, RaftWALRecord *record, char **data);
 ...
 */
RaftLog *raft_log_init(const char *wal_path)
{
    RaftLog *log;
    ...
    elog(DEBUG1, "Raft log initialized (capacity=%d, persistence=%s)", log->capacity,
         log->persistence_enabled ? "enabled" : "disabled");
 ...
 */
void raft_log_free(RaftLog *log)
{
    uint64 i;
    uint64 entry_count;
 ...
 */
bool raft_log_append(RaftLog *log, RaftLogEntry *entries, int32 count)
{
    int32 i;
    uint64 next_index;
    ...
    elog(DEBUG1, "Raft log: appended %d entries (last_index=%lu)", count, log->last_index);
 ...
 */
RaftLogEntry *raft_log_get(RaftLog *log, uint64 index)
{
    uint64 array_index;
 ...
 */
RaftLogEntry *raft_log_get_range(RaftLog *log, uint64 start_index, uint64 end_index, int32 *count)
{
    RaftLogEntry *result;
    uint64 i;
    int32 result_count;
    ...
    *count = 0;
    ...
    /* Validate range */
    if (start_index > end_index || start_index < log->first_index || end_index > log->last_index) {
        return NULL;
    }
 ...
 */
void raft_log_truncate(RaftLog *log, uint64 after_index)
{
    uint64 i;
    ...
    elog(DEBUG1, "Raft log: truncated after index %lu (new last_index=%lu)", after_index,
         log->last_index);
}
 ...
 */
uint64 raft_log_term_at(RaftLog *log, uint64 index)
{
    RaftLogEntry *entry;
...
static void raft_log_ensure_capacity(RaftLog *log, int32 needed)
{
    ...
    log->entries = (RaftLogEntry *)repalloc(log->entries, sizeof(RaftLogEntry) * log->capacity);
 ...
 */
void raft_log_compact(RaftLog *log, uint64 up_to_index)
{
    ...
    if (up_to_index < log->first_index) {
        elog(DEBUG1, "Raft log: nothing to compact (up_to=%lu, first=%lu)", up_to_index,
             log->first_index);
        return;
    }
    ...
    memset(&log->entries[remaining_entries], 0, sizeof(RaftLogEntry) * entries_to_remove);

    /* Update first_index */
    log->first_index = up_to_index + 1;

    elog(LOG, "Raft log: compacted %lu entries (new first_index=%lu)", entries_to_remove,
         log->first_index);
}
 ...
 */
void raft_log_persist(RaftLog *log)
{
    StringInfoData path;
    int fd;
    uint64 i;
    ...
    initStringInfo(&path);
    appendStringInfo(&path, "%s/raft_wal_%lu.log", log->wal_path, log->first_index);
    ...
    for (i = log->first_index; i <= log->last_index; i++) {
        ...
        /* Write record */
        if (!raft_log_write_wal_record(log, fd, RAFT_WAL_ENTRY, entry_buf.data, entry_buf.len,
                                       entry->index)) {
    ...
    if (!raft_log_write_wal_record(log, fd, RAFT_WAL_COMMIT, (char *)&log->commit_index,
    ...
    elog(DEBUG1, "Raft log: persisted entries %lu-%lu to WAL", log->first_index, log->last_index);
}

/*
 * raft_log_recover - Recover log from WAL on startup
 */
void raft_log_recover(RaftLog *log)
{
    StringInfoData path;
    int fd;
    ...
    while (raft_log_read_wal_record(fd, &record, &data)) {
        ...
        case RAFT_WAL_COMMIT: {
            ...
            if (commit_index > log->commit_index && commit_index <= log->last_index) {
    ...
    elog(LOG, "Raft log: recovered %lu entries from WAL (index %lu-%lu)",
         log->last_index - log->first_index + 1, log->first_index, log->last_index);
}
 ...
 */
void raft_log_create_snapshot(RaftLog *log, uint64 last_included_index, const char *snapshot_data,
                              int32 snapshot_size)
{
    StringInfoData path;
    int fd;
    ...
    initStringInfo(&path);
    appendStringInfo(&path, "%s/raft_snapshot_%lu.snap", log->wal_path, last_included_index);
    ...
    elog(LOG, "Raft log: created snapshot at index %lu, compacted log", last_included_index);
}
 ...
 */
char *raft_log_load_snapshot(RaftLog *log, uint64 *last_included_index, uint64 *last_included_term,
                             int32 *data_size)
{
    ...
    *data_size = snap_size;
    ...
    elog(LOG, "Raft log: loaded snapshot at index %lu (term %lu, %d bytes)", snap_index, snap_term,
         snap_size);
 ...
 */
void raft_log_get_stats(RaftLog *log, uint64 *first_index, uint64 *last_index, uint64 *commit_index,
                        uint64 *last_applied, int32 *entry_count, int32 *capacity)
{
    if (first_index)
    ...
    if (entry_count)
        *entry_count =
            (int32)(log->last_index >= log->first_index ? log->last_index - log->first_index + 1
                                                        : 0);
    if (capacity)
 ...
 */
void raft_log_dump(RaftLog *log, StringInfo buf)
{
    uint64 i;

    appendStringInfo(buf, "Raft Log: first=%lu, last=%lu, commit=%lu, applied=%lu\n",
                     log->first_index, log->last_index, log->commit_index, log->last_applied);
