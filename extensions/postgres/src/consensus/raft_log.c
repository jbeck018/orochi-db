...
static bool raft_log_write_wal_record(RaftLog *log, int fd,
                                      RaftWALRecordType type, const char *data,
                                      int32 size, uint64 index);
...
bool raft_log_append(RaftLog *log, RaftLogEntry *entries, int32 count) {
  ...
  raft_log_ensure_capacity(log,
                           (log->last_index - log->first_index + 1) + count);
  ...
  for (i = 0; i < count; i++) {
    ...
    dest->type = src->type;
    dest->created_at =
        src->created_at ? src->created_at : GetCurrentTimestamp();
  ...
  elog(DEBUG1, "Raft log: appended %d entries (last_index=%lu)", count,
       log->last_index);
 ...
 */
RaftLogEntry *raft_log_get_range(RaftLog *log, uint64 start_index,
  ...
  *count = 0;
  ...
  /* Validate range */
  if (start_index > end_index || start_index < log->first_index ||
...
void raft_log_persist(RaftLog *log) {
  ...
  initStringInfo(&path);
  appendStringInfo(&path, "%s/raft_wal_%lu.log", log->wal_path,
                   log->first_index);
  ...
  for (i = log->first_index; i <= log->last_index; i++) {
    ...
    appendBinaryStringInfo(&entry_buf, (char *)&entry->command_size,
                           sizeof(int32));
    appendBinaryStringInfo(&entry_buf, (char *)&entry->created_at,
                           sizeof(TimestampTz));

    if (entry->command_size > 0 && entry->command_data != NULL)
      appendBinaryStringInfo(&entry_buf, entry->command_data,
                             entry->command_size);

    /* Write record */
    if (!raft_log_write_wal_record(log, fd, RAFT_WAL_ENTRY, entry_buf.data,
                                   entry_buf.len, entry->index)) {
      elog(WARNING, "Failed to write Raft WAL entry at index %lu: %m",
           entry->index);
  ...
  if (!raft_log_write_wal_record(log, fd, RAFT_WAL_COMMIT,
                                 (char *)&log->commit_index, sizeof(uint64),
                                 log->commit_index)) {
  ...
  elog(DEBUG1, "Raft log: persisted entries %lu-%lu to WAL", log->first_index,
       log->last_index);
}
...
void raft_log_recover(RaftLog *log) {
  ...
  elog(LOG, "Raft log: recovered %lu entries from WAL (index %lu-%lu)",
       log->last_index - log->first_index + 1, log->first_index,
       log->last_index);
}
 ...
 */
void raft_log_create_snapshot(RaftLog *log, uint64 last_included_index,
  ...
  initStringInfo(&path);
  appendStringInfo(&path, "%s/raft_snapshot_%lu.snap", log->wal_path,
                   last_included_index);
 ...
 */
char *raft_log_load_snapshot(RaftLog *log, uint64 *last_included_index,
 ...
 */
void raft_log_get_stats(RaftLog *log, uint64 *first_index, uint64 *last_index,
...
void raft_log_dump(RaftLog *log, StringInfo buf) {
  ...
  appendStringInfo(
      buf, "Raft Log: first=%lu, last=%lu, commit=%lu, applied=%lu\n",
