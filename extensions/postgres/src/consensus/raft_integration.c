...
static void transaction_tracker_update_status(const char *gid,
 ...
 */
Size orochi_raft_shmem_size(void) { return sizeof(RaftSharedState); }
...
void orochi_raft_shmem_init(void) {
  ...
  raft_shared_state = (RaftSharedState *)ShmemInitStruct(
      "Orochi Raft State", sizeof(RaftSharedState), &found);
...
void orochi_raft_worker_main(Datum main_arg) {
  ...
  StartTransactionCommand();
  {
    ...
    foreach (lc, nodes) {
      ...
      if (node->node_id != orochi_raft_node_id) {
        raft_add_peer(local_raft_node, node->node_id, node->hostname,
                      node->port);
      }
    }
  }
  ...
  while (!got_sigterm) {
    ...
    rc = WaitLatch(MyLatch, WL_LATCH_SET | WL_TIMEOUT | WL_EXIT_ON_PM_DEATH,
                   wait_time_ms, PG_WAIT_EXTENSION);
...
static void raft_apply_entry_callback(RaftLogEntry *entry, void *context) {
  ...
  if (entry->command_data != NULL && entry->command_size > 0) {
    ...
    elog(DEBUG1, "Transaction %s status updated to %d on apply", gid,
         new_status);

    /* Call user-registered callback if present */
    if (apply_callback != NULL) {
      apply_callback(gid, entry->command_data, entry->command_size,
...
bool orochi_raft_submit_transaction(const char *gid, List *participant_nodes,
  ...
  /* Submit to Raft log */
  log_index =
      raft_submit_command(local_raft_node, full_command.data, full_command.len);
...
bool orochi_raft_wait_for_commit(const char *gid, int timeout_ms) {
  ...
  if (entry == NULL) {
    elog(WARNING,
         "Transaction %s not found in tracker - cannot wait for commit", gid);
    return false;
  }
  ...
  while (GetCurrentTimestamp() < deadline) {
    ...
    if (current_committed >= txn_log_index) {
      ...
      if (local_raft_node != NULL) {
        ...
        elog(DEBUG1,
             "Transaction %s presumed committed at index %lu (no local node)",
             gid, txn_log_index);
        return true;
      }
    }
...
bool orochi_raft_prepare_transaction(const char *gid, List *shard_ids) {
  ...
  /* Submit via Raft */
  bool result =
      orochi_raft_submit_transaction(gid, shard_ids, command.data, command.len);
  pfree(command.data);
...
bool orochi_raft_commit_transaction(const char *gid) {
  ...
  bool result =
      orochi_raft_submit_transaction(gid, NIL, command.data, command.len);
  pfree(command.data);
 ...
 */
void orochi_raft_add_cluster_node(int32 node_id, const char *hostname,
                                  int port) {
  if (local_raft_node != NULL) {
 ...
 */
void orochi_raft_set_apply_callback(RaftTransactionApplyCallback callback,
                                    void *context) {
...
static void transaction_tracker_init(void) {
  ...
  transaction_tracker =
      hash_create("Raft Transaction Tracker", MAX_TRACKED_TRANSACTIONS,
...
transaction_tracker_add(const char *gid, uint64 log_index, uint64 term,
  ...
  entry = (RaftTransactionEntry *)hash_search(transaction_tracker, gid,
                                              HASH_ENTER, &found);
  ...
  if (!found) {
    ...
    elog(DEBUG1, "Transaction %s added to tracker at index %lu, term %lu", gid,
         log_index, term);
  } else {
...
static bool transaction_tracker_verify_commit(const char *gid, RaftNode *node) {
  ...
  if (entry->log_index > node->log->commit_index) {
    elog(DEBUG2,
         "Transaction %s at index %lu not yet committed (commit_index=%lu)",
  ...
  if (log_entry == NULL) {
    elog(WARNING, "Transaction %s: log entry at index %lu not found", gid,
         entry->log_index);
    return false;
  }
  ...
  if (log_entry->term != entry->term) {
    elog(WARNING,
         "Transaction %s: term mismatch at index %lu (expected %lu, got %lu)",
  ...
  if (stored_hash != entry->command_hash ||
    ...
    elog(WARNING, "Transaction %s: command mismatch at index %lu", gid,
         entry->log_index);
    transaction_tracker_update_status(gid, RAFT_TXN_ABORTED);
    return false;
  }
  ...
  elog(DEBUG1, "Transaction %s verified committed at index %lu", gid,
       entry->log_index);
...
Datum orochi_raft_status(PG_FUNCTION_ARGS) {
  ...
  if (raft_shared_state == NULL) {
    ...
    values[1] =
        CStringGetTextDatum(raft_state_name(raft_shared_state->current_state));
...
Datum orochi_raft_cluster_info(PG_FUNCTION_ARGS) {
  ...
  if (raft_shared_state == NULL) {
    ...
    values[3] =
        BoolGetDatum(raft_shared_state->current_state == RAFT_STATE_LEADER);
