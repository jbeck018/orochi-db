...
int32 orochi_raft_get_leader_id(void) {
...
void orochi_raft_register_apply_callback(RaftTransactionApplyCallback callback,
                                         void *context) {
  /* Stub: no-op */
...
Datum orochi_raft_status(PG_FUNCTION_ARGS) {
  ereport(NOTICE, (errmsg("Raft consensus not enabled (stub implementation)")));
  PG_RETURN_NULL();
}
