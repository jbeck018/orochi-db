...
static bool raft_is_log_up_to_date(RaftNode *node, uint64 last_log_index, uint64 last_log_term);
 ...
 */
RaftNode *raft_init(int32 node_id, const char *wal_path)
{
    ...
    raft_context = AllocSetContextCreate(TopMemoryContext, "RaftContext", ALLOCSET_DEFAULT_SIZES);
 ...
 */
void raft_shutdown(RaftNode *node)
{
    if (node == NULL)
        return;
...
static void raft_become_follower(RaftNode *node, uint64 term)
{
    elog(DEBUG1, "Raft node %d becoming follower (term=%lu)", node->node_id, term);
...
static void raft_become_candidate(RaftNode *node)
{
    ...
    elog(DEBUG1, "Raft node %d becoming candidate (term=%lu)", node->node_id, node->current_term);
}
...
static void raft_become_leader(RaftNode *node)
{
    ...
    elog(LOG, "Raft node %d becoming leader (term=%lu)", node->node_id, node->current_term);
...
/*
 * raft_initialize_leader_state - Initialize next_index and match_index for all
 * peers
 */
static void raft_initialize_leader_state(RaftNode *node)
{
...
void raft_start_election(RaftNode *node)
{
    ...
    elog(DEBUG1, "Raft node %d starting election for term %lu", node->node_id, node->current_term);
    ...
    for (i = 0; i < node->cluster->peer_count; i++) {
        ...
        if (response.vote_granted) {
            ...
            elog(DEBUG1, "Raft node %d received vote from node %d", node->node_id, peer->node_id);
        }
    }
    ...
    if (node->votes_received >= votes_needed) {
        ...
        elog(DEBUG1, "Raft node %d election failed: got %lu votes, needed %d", node->node_id,
             node->votes_received, votes_needed);
    }
}
 ...
 */
RequestVoteResponse raft_request_vote(RaftNode *node, RequestVoteRequest *request)
{
    RequestVoteResponse response;
    ...
    if (request->term < node->current_term) {
        elog(DEBUG1, "Raft node %d rejecting vote for node %d: stale term %lu < %lu", node->node_id,
             request->candidate_id, request->term, node->current_term);
        return response;
    }
    ...
    if ((node->voted_for == -1 || node->voted_for == request->candidate_id) &&
        ...
        elog(DEBUG1, "Raft node %d granting vote to node %d for term %lu", node->node_id,
             request->candidate_id, request->term);
    } else {
        elog(DEBUG1,
             "Raft node %d rejecting vote for node %d: "
...
/*
 * raft_is_log_up_to_date - Check if candidate's log is at least as up-to-date
 * as ours
 */
static bool raft_is_log_up_to_date(RaftNode *node, uint64 last_log_index, uint64 last_log_term)
{
 ...
 */
void raft_heartbeat(RaftNode *node)
{
    if (!node->is_leader)
        return;
 ...
 */
void raft_replicate(RaftNode *node)
{
    int i;
    ...
    for (i = 0; i < node->cluster->peer_count; i++) {
        ...
        if (peer->next_index <= node->log->last_index) {
            uint64 end_index =
                Min(peer->next_index + RAFT_MAX_ENTRIES_PER_APPEND - 1, node->log->last_index);
            entries = raft_log_get_range(node->log, peer->next_index, end_index, &entries_count);
        }
 ...
 */
AppendEntriesResponse raft_append_entries(RaftNode *node, AppendEntriesRequest *request)
{
    AppendEntriesResponse response;
    int i;
    ...
    if (request->term < node->current_term) {
        elog(DEBUG1, "Raft node %d rejecting AppendEntries: stale term %lu < %lu", node->node_id,
             request->term, node->current_term);
        return response;
    }
    ...
    if (request->prev_log_index > 0) {
        ...
        if (request->prev_log_index > node->log->last_index ||
            ...
            elog(DEBUG1,
                 "Raft node %d rejecting AppendEntries: log inconsistency "
                 "at index %lu (expected term %lu, have %lu)",
                 node->node_id, request->prev_log_index, request->prev_log_term, term_at_prev);
    ...
    if (request->entries_count > 0) {
        ...
        elog(DEBUG1, "Raft node %d appended %d entries (last_index now %lu)", node->node_id,
             request->entries_count, node->log->last_index);
    }
 ...
 */
void raft_update_commit_index(RaftNode *node)
{
    uint64 new_commit;
    int i;
    ...
    for (new_commit = node->log->last_index; new_commit > node->log->commit_index; new_commit--) {
        ...
        if (replicated_count >= node->cluster->quorum_size) {
            ...
            elog(DEBUG1, "Raft node %d advanced commit_index to %lu", node->node_id, new_commit);
            break;
        }
    }
}
...
static void raft_apply_committed_entries(RaftNode *node)
{
    while (node->log->last_applied < node->log->commit_index) {
        ...
        if (entry == NULL) {
            elog(WARNING, "Raft node %d: missing log entry at index %lu", node->node_id,
                 node->log->last_applied);
            continue;
        }
        ...
        elog(DEBUG1, "Raft node %d applied entry at index %lu", node->node_id,
             node->log->last_applied);
    }
}
 ...
 * too far behind, the leader sends a snapshot instead of individual log
 * entries.
 */
InstallSnapshotResponse raft_install_snapshot(RaftNode *node, InstallSnapshotRequest *request)
{
    ...
    if (request->term < node->current_term) {
        elog(DEBUG1, "Raft node %d rejecting snapshot: stale term %lu < %lu", node->node_id,
             request->term, node->current_term);
        return response;
    }
    ...
    if (request->offset == 0) {
        ...
        elog(DEBUG1,
             "Raft node %d starting new snapshot reception "
             "(last_included_index=%lu, last_included_term=%lu)",
             node->node_id, request->last_included_index, request->last_included_term);
    }

    /* Validate offset continuity */
    if (request->offset != snapshot_offset) {
        elog(WARNING,
             "Raft node %d: snapshot chunk out of order "
    ...
    elog(DEBUG1,
         "Raft node %d received snapshot chunk "
         "(offset=%d, size=%d, total=%d, done=%d)",
         node->node_id, request->offset, request->data_size, snapshot_offset, request->done);
    ...
    if (request->done) {
        ...
        if (node->log->persistence_enabled && node->log->wal_path != NULL) {
            ...
            appendStringInfo(&snapshot_path, "%s/raft_snapshot_%lu.snap", node->log->wal_path,
                             request->last_included_index);

            fd = open(snapshot_path.data, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
            if (fd >= 0) {
                ...
                elog(LOG, "Raft node %d saved snapshot to %s (%d bytes)", node->node_id,
                     snapshot_path.data, snapshot_offset);
            } else {
                int saved_errno = errno;
                elog(WARNING, "Raft node %d failed to save snapshot: %s", node->node_id,
        ...
        if (node->snapshot_apply_callback != NULL) {
            snapshot_applied = node->snapshot_apply_callback(
                request->last_included_index, request->last_included_term, snapshot_buffer,
                snapshot_offset, node->snapshot_apply_context);

            if (!snapshot_applied) {
                elog(WARNING, "Raft node %d: snapshot apply callback failed", node->node_id);
            }
        } else {
        ...
        if (snapshot_applied) {
            ...
            elog(LOG, "Raft node %d applied snapshot at index %lu (term %lu)", node->node_id,
 ...
 */
bool raft_send_install_snapshot(RaftNode *node, RaftPeer *peer)
{
    ...
    snapshot_data = raft_log_load_snapshot(node->log, &last_included_index, &last_included_term,
                                           &snapshot_size);

    if (snapshot_data == NULL && snapshot_size == 0 && last_included_index == 0) {
        /* No snapshot available */
        elog(WARNING, "Raft node %d: no snapshot available for peer %d", node->node_id,
             peer->node_id);
        return false;
    }

    elog(LOG,
         "Raft node %d sending snapshot to peer %d "
         "(index=%lu, term=%lu, size=%d bytes)",
         node->node_id, peer->node_id, last_included_index, last_included_term, snapshot_size);
    ...
    if (offset >= snapshot_size) {
        ...
        elog(LOG, "Raft node %d completed snapshot transfer to peer %d", node->node_id,
             peer->node_id);
    }
 ...
 */
InstallSnapshotResponse raft_send_install_snapshot_rpc(RaftPeer *peer,
                                                       InstallSnapshotRequest *request)
{
    InstallSnapshotResponse response;
    PGresult *res;
    ...
    snprintf(param_last_idx, sizeof(param_last_idx), "%lu",
             (unsigned long)request->last_included_index);
    snprintf(param_last_term, sizeof(param_last_term), "%lu",
             (unsigned long)request->last_included_term);
    ...
    if (request->data_size > 0 && request->data != NULL) {
        ...
        size_t escaped_len;
        escaped_data = (char *)PQescapeByteaConn(peer->connection, (unsigned char *)request->data,
                                                 request->data_size, &escaped_len);
    ...
    if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0) {
        ...
        elog(WARNING, "InstallSnapshot RPC to peer %d failed: %s", peer->node_id,
             PQerrorMessage(peer->connection));
 ...
 */
void raft_set_snapshot_callback(RaftNode *node,
 ...
 */
bool raft_create_snapshot(RaftNode *node, char *(*get_state)(void *context, int32 *size),
                          void *context)
{
    char *snapshot_data;
    int32 snapshot_size;
    ...
    if (snapshot_index < node->log->first_index) {
        elog(DEBUG1, "Raft node %d: nothing to snapshot (applied=%lu, first=%lu)", node->node_id,
             snapshot_index, node->log->first_index);
        return false;
    }
    ...
    if (snapshot_data == NULL && snapshot_size > 0) {
        elog(WARNING, "Raft node %d: failed to get state for snapshot", node->node_id);
        return false;
    }

    /* Create the snapshot */
    raft_log_create_snapshot(node->log, snapshot_index, snapshot_data, snapshot_size);

    if (snapshot_data != NULL)
        pfree(snapshot_data);

    elog(LOG, "Raft node %d created snapshot at index %lu (%d bytes)", node->node_id,
 ...
 */
bool raft_should_snapshot(RaftNode *node)
{
 ...
 */
uint64 raft_read_index(RaftNode *node, int timeout_ms)
{
    uint64 read_index = 0;
    ...
    if (node->is_leader) {
        ...
        elog(DEBUG1, "Raft node %d: read_index failed, only %d/%d acks", node->node_id,
             acks_received, acks_needed);
        return 0;
    } else {
        /*
         ...
         */
        if (node->leader_id < 0) {
            elog(DEBUG1, "Raft node %d: cannot perform read - no known leader", node->node_id);
            return 0;
        }
        ...
        while (node->log->last_applied < read_index) {
            if (GetCurrentTimestamp() >= deadline) {
                elog(DEBUG1, "Raft node %d: read_index timeout waiting for apply", node->node_id);
                return 0;
            }
 ...
 */
uint64 raft_request_read_index_from_leader(RaftPeer *leader, RaftNode *node)
{
    PGresult *res;
    uint64 read_index = 0;
    ...
    res = PQexecParams(leader->connection, "SELECT orochi.raft_leader_read_index($1::integer)", 1,
    ...
    if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0) {
        ...
        elog(WARNING, "Read index request to leader %d failed: %s", leader->node_id,
             PQerrorMessage(leader->connection));
    }
 ...
 * the requested index.
 */
bool raft_can_serve_read(RaftNode *node, uint64 read_index)
{
 ...
 */
bool raft_follower_read(RaftNode *node, int max_staleness_ms)
{
    TimestampTz now = GetCurrentTimestamp();
    int64 staleness_us;
    ...
    if (staleness_us > (int64)max_staleness_ms * 1000L) {
        elog(DEBUG1, "Raft node %d: follower read denied - too stale (%ld ms)", node->node_id,
             staleness_us / 1000);
        return false;
    }
 ...
 */
uint64 raft_lease_read(RaftNode *node)
{
    TimestampTz now;
 ...
 */
uint64 raft_submit_command(RaftNode *node, const char *command, int32 command_size)
{
    RaftLogEntry entry;
    MemoryContext old_context;

    if (!node->is_leader) {
        elog(DEBUG1, "Raft node %d: cannot submit command - not leader", node->node_id);
        return 0;
    }
    ...
    elog(DEBUG1, "Raft node %d accepted command at index %lu", node->node_id,
         node->log->last_index);
 ...
 */
bool raft_is_committed(RaftNode *node, uint64 log_index)
{
 ...
 */
bool raft_wait_for_commit(RaftNode *node, uint64 log_index, int timeout_ms)
{
 ...
 */
void raft_set_apply_callback(RaftNode *node, void (*callback)(RaftLogEntry *entry, void *context),
                             void *context)
{
 ...
 */
void raft_add_peer(RaftNode *node, int32 peer_id, const char *hostname, int port)
{
    RaftPeer *peer;
    MemoryContext old_context;
    ...
    elog(LOG, "Raft node %d added peer %d at %s:%d (quorum=%d)", node->node_id, peer_id, hostname,
         port, node->cluster->quorum_size);
}

/*
 * raft_remove_peer - Remove a peer from the cluster
 */
void raft_remove_peer(RaftNode *node, int32 peer_id)
{
    int i, j;
    ...
    for (i = 0; i < node->cluster->peer_count; i++) {
        if (node->cluster->peers[i].node_id == peer_id) {
            ...
            elog(LOG, "Raft node %d removed peer %d (quorum=%d)", node->node_id, peer_id,
                 node->cluster->quorum_size);
            return;
        }
    }
 ...
 */
void raft_connect_peers(RaftNode *node)
{
    int i;
    ...
    for (i = 0; i < node->cluster->peer_count; i++) {
        ...
        if (peer->connection == NULL || PQstatus(peer->connection) != CONNECTION_OK) {
            ...
            if (peer->is_active) {
                elog(DEBUG1, "Raft node %d connected to peer %d", node->node_id, peer->node_id);
            } else {
                elog(WARNING, "Raft node %d failed to connect to peer %d", node->node_id,
                     peer->node_id);
            }
        }
    }
 ...
 */
void raft_disconnect_peers(RaftNode *node)
{
    int i;
...
static PGconn *raft_get_peer_connection(RaftPeer *peer)
{
    ...
    appendStringInfo(&connstr, "host=%s port=%d dbname=%s connect_timeout=5", peer->hostname,
                     peer->port, get_database_name(MyDatabaseId));

    conn = PQconnectdb(connstr.data);
    pfree(connstr.data);

    if (PQstatus(conn) != CONNECTION_OK) {
        elog(WARNING, "Failed to connect to Raft peer %d at %s:%d: %s", peer->node_id,
             peer->hostname, peer->port, PQerrorMessage(conn));
        PQfinish(conn);
        return NULL;
    }
 ...
 */
RequestVoteResponse raft_send_request_vote(RaftPeer *peer, RequestVoteRequest *request)
{
    RequestVoteResponse response;
    PGresult *res;
    ...
    snprintf(param_last_log_index, sizeof(param_last_log_index), "%lu",
             (unsigned long)request->last_log_index);
    snprintf(param_last_log_term, sizeof(param_last_log_term), "%lu",
             (unsigned long)request->last_log_term);
    ...
    res = PQexecParams(peer->connection,
                       ...
                       paramValues, NULL, /* param lengths (not needed for text) */
    ...
    if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0) {
        ...
        elog(WARNING, "RequestVote RPC to peer %d failed: %s", peer->node_id,
             PQerrorMessage(peer->connection));
        peer->is_active = false;
    }
 ...
 * Security: Uses parameterized queries (PQexecParams) to prevent SQL injection
 */
AppendEntriesResponse raft_send_append_entries(RaftPeer *peer, AppendEntriesRequest *request)
{
    AppendEntriesResponse response;
    PGresult *res;
    /* Parameter buffers for parameterized query */
    ...
    snprintf(param_term, sizeof(param_term), "%lu", (unsigned long)request->term);
    snprintf(param_leader_id, sizeof(param_leader_id), "%d", request->leader_id);
    snprintf(param_prev_log_index, sizeof(param_prev_log_index), "%lu",
             (unsigned long)request->prev_log_index);
    snprintf(param_prev_log_term, sizeof(param_prev_log_term), "%lu",
             (unsigned long)request->prev_log_term);
    snprintf(param_leader_commit, sizeof(param_leader_commit), "%lu",
             (unsigned long)request->leader_commit);

    if (request->entries_count == 0) {
        /* Heartbeat - no entries, use parameterized query with NULL */
        const char *paramValues[5] = { param_term, param_leader_id, param_prev_log_index,
        ...
        for (i = 0; i < request->entries_count; i++) {
            ...
            appendStringInfo(&entries_json, "{\"term\":%lu,\"type\":%d,\"data\":\"%s\"}",
        ...
        res = PQexecParams(peer->connection,
                           "SELECT term, success, match_index FROM orochi.raft_append_entries("
                           "$1::bigint, $2::integer, $3::bigint, $4::bigint, $5::jsonb, "
                           "$6::bigint)",
    ...
    if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0) {
        ...
        elog(WARNING, "AppendEntries RPC to peer %d failed: %s", peer->node_id,
             PQerrorMessage(peer->connection));
        peer->is_active = false;
    }
 ...
 */
void raft_persist_state(RaftNode *node)
{
    StringInfoData path;
    FILE *fp;
    ...
    initStringInfo(&path);
    appendStringInfo(&path, "%s/raft_state_%d.dat", DataDir, node->node_id);
    ...
    if (fp != NULL) {
        ...
        if (write_success) {
            elog(DEBUG1, "Raft node %d persisted state (term=%lu, voted_for=%d)", node->node_id,
 ...
 */
void raft_load_state(RaftNode *node)
{
    StringInfoData path;
    FILE *fp;
    ...
    if (fp != NULL) {
        if (fread(&node->current_term, sizeof(uint64), 1, fp) == 1 &&
            ...
            elog(LOG, "Raft node %d recovered state (term=%lu, voted_for=%d)", node->node_id,
                 node->current_term, node->voted_for);
        }
        fclose(fp);
    }
 ...
 */
int32 raft_get_leader(RaftNode *node)
{
 ...
 */
bool raft_is_leader(RaftNode *node)
{
 ...
 */
uint64 raft_get_term(RaftNode *node)
{
 ...
 */
void raft_get_stats(RaftNode *node, uint64 *term, RaftState *state, int32 *leader_id,
 ...
 */
int raft_random_election_timeout(void)
{
 ...
 */
char *raft_serialize_request_vote(RequestVoteRequest *req, int32 *size)
{
    char *buffer;
    char *ptr;
 ...
 */
RequestVoteRequest *raft_deserialize_request_vote(const char *data, int32 size)
{
 ...
 */
char *raft_serialize_append_entries(AppendEntriesRequest *req, int32 *size)
{
    StringInfoData buf;
    int i;
 ...
 */
AppendEntriesRequest *raft_deserialize_append_entries(const char *data, int32 size)
{
