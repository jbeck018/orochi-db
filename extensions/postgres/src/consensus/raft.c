...
RaftNode *raft_init(int32 node_id, const char *wal_path) {
  ...
  raft_context = AllocSetContextCreate(TopMemoryContext, "RaftContext",
  ...
  node->cluster->peers =
      (RaftPeer *)palloc0(sizeof(RaftPeer) * RAFT_MAX_CLUSTER_SIZE);
 ...
 */
static void raft_become_follower(RaftNode *node, uint64 term) {
  elog(DEBUG1, "Raft node %d becoming follower (term=%lu)", node->node_id,
       term);
...
static void raft_become_candidate(RaftNode *node) {
  ...
  elog(DEBUG1, "Raft node %d becoming candidate (term=%lu)", node->node_id,
       node->current_term);
}
...
static void raft_become_leader(RaftNode *node) {
  ...
  elog(LOG, "Raft node %d becoming leader (term=%lu)", node->node_id,
       node->current_term);
...
void raft_start_election(RaftNode *node) {
  ...
  elog(DEBUG1, "Raft node %d starting election for term %lu", node->node_id,
       node->current_term);
  ...
  for (i = 0; i < node->cluster->peer_count; i++) {
    ...
    if (response.vote_granted) {
      ...
      elog(DEBUG1, "Raft node %d received vote from node %d", node->node_id,
           peer->node_id);
    }
  }
 ...
 */
RequestVoteResponse raft_request_vote(RaftNode *node,
                                      RequestVoteRequest *request) {
  RequestVoteResponse response;
  ...
  if (request->term < node->current_term) {
    elog(DEBUG1,
         "Raft node %d rejecting vote for node %d: stale term %lu < %lu",
  ...
  if ((node->voted_for == -1 || node->voted_for == request->candidate_id) &&
      raft_is_log_up_to_date(node, request->last_log_index,
  ...
  for (i = 0; i < node->cluster->peer_count; i++) {
    ...
    if (peer->next_index <= node->log->last_index) {
      uint64 end_index = Min(peer->next_index + RAFT_MAX_ENTRIES_PER_APPEND - 1,
                             node->log->last_index);
      entries = raft_log_get_range(node->log, peer->next_index, end_index,
                                   &entries_count);
    }
 ...
 */
AppendEntriesResponse raft_append_entries(RaftNode *node,
                                          AppendEntriesRequest *request) {
  AppendEntriesResponse response;
  int i;
  ...
  if (request->prev_log_index > 0) {
    ...
    if (request->prev_log_index > node->log->last_index ||
      ...
      elog(DEBUG1,
           ...
           node->node_id, request->prev_log_index, request->prev_log_term,
           term_at_prev);
      response.match_index =
          Min(node->log->last_index, request->prev_log_index - 1);
      return response;
    }
  }
  ...
  if (request->leader_commit > node->log->commit_index) {
    node->log->commit_index =
        Min(request->leader_commit, node->log->last_index);
  }
...
void raft_update_commit_index(RaftNode *node) {
  ...
  for (new_commit = node->log->last_index; new_commit > node->log->commit_index;
       new_commit--) {
    ...
    if (replicated_count >= node->cluster->quorum_size) {
      ...
      elog(DEBUG1, "Raft node %d advanced commit_index to %lu", node->node_id,
           new_commit);
      break;
    }
  }
 ...
 * entries.
 */
InstallSnapshotResponse raft_install_snapshot(RaftNode *node,
  ...
  if (request->data_size > 0 && request->data != NULL) {
    ...
    memcpy(snapshot_buffer + snapshot_offset, request->data,
           request->data_size);
  ...
  elog(DEBUG1,
       ...
       node->node_id, request->offset, request->data_size, snapshot_offset,
       request->done);
  ...
  if (request->done) {
    ...
    if (node->snapshot_apply_callback != NULL) {
      snapshot_applied = node->snapshot_apply_callback(
          ...
          snapshot_buffer, snapshot_offset, node->snapshot_apply_context);
...
bool raft_send_install_snapshot(RaftNode *node, RaftPeer *peer) {
  ...
  snapshot_data = raft_log_load_snapshot(node->log, &last_included_index,
                                         &last_included_term, &snapshot_size);
  ...
  elog(LOG,
       ...
       node->node_id, peer->node_id, last_included_index, last_included_term,
       snapshot_size);
  ...
  while (offset < snapshot_size || (offset == 0 && snapshot_size == 0)) {
    ...
    if (!response.success) {
      elog(WARNING,
           "Raft node %d: snapshot chunk to peer %d failed at offset %d",
...
raft_send_install_snapshot_rpc(RaftPeer *peer,
                               InstallSnapshotRequest *request) {
  InstallSnapshotResponse response;
  PGresult *res;
...
void raft_set_snapshot_callback(RaftNode *node,
                                bool (*callback)(uint64 last_index,
                                                 uint64 last_term,
 ...
 */
bool raft_create_snapshot(RaftNode *node,
  ...
  /* Create the snapshot */
  raft_log_create_snapshot(node->log, snapshot_index, snapshot_data,
                           snapshot_size);
  ...
  if (node->is_leader) {
    ...
    for (i = 0; i < node->cluster->peer_count; i++) {
      ...
      request.prev_log_term =
          raft_log_term_at(node->log, request.prev_log_index);
...
uint64 raft_request_read_index_from_leader(RaftPeer *leader, RaftNode *node) {
  ...
  if (leader->connection == NULL ||
      PQstatus(leader->connection) != CONNECTION_OK) {
  ...
  res = PQexecParams(leader->connection,
                     "SELECT orochi.raft_leader_read_index($1::integer)", 1,
...
uint64 raft_lease_read(RaftNode *node) {
   ...
   */
  if (lease_remaining_us >
      (RAFT_ELECTION_TIMEOUT_MIN_MS * 500L)) /* Half of min */
  {
 ...
 */
uint64 raft_submit_command(RaftNode *node, const char *command,
                           int32 command_size) {
  RaftLogEntry entry;
  MemoryContext old_context;
 ...
 */
void raft_set_apply_callback(RaftNode *node,
 ...
 */
void raft_add_peer(RaftNode *node, int32 peer_id, const char *hostname,
                   int port) {
  RaftPeer *peer;
  MemoryContext old_context;
  ...
  elog(LOG, "Raft node %d added peer %d at %s:%d (quorum=%d)", node->node_id,
       peer_id, hostname, port, node->cluster->quorum_size);
}
...
void raft_remove_peer(RaftNode *node, int32 peer_id) {
  ...
  for (i = 0; i < node->cluster->peer_count; i++) {
    if (node->cluster->peers[i].node_id == peer_id) {
      ...
      elog(LOG, "Raft node %d removed peer %d (quorum=%d)", node->node_id,
           peer_id, node->cluster->quorum_size);
      return;
    }
  }
...
void raft_connect_peers(RaftNode *node) {
  ...
  for (i = 0; i < node->cluster->peer_count; i++) {
    ...
    if (peer->connection == NULL ||
        PQstatus(peer->connection) != CONNECTION_OK) {
      peer->connection = raft_get_peer_connection(peer);
      peer->is_active = (peer->connection != NULL);

      if (peer->is_active) {
        elog(DEBUG1, "Raft node %d connected to peer %d", node->node_id,
             peer->node_id);
      } else {
...
static PGconn *raft_get_peer_connection(RaftPeer *peer) {
  ...
  appendStringInfo(&connstr, "host=%s port=%d dbname=%s connect_timeout=5",
                   peer->hostname, peer->port, get_database_name(MyDatabaseId));
 ...
 */
RequestVoteResponse raft_send_request_vote(RaftPeer *peer,
                                           RequestVoteRequest *request) {
  RequestVoteResponse response;
  PGresult *res;
  ...
  snprintf(param_term, sizeof(param_term), "%lu", (unsigned long)request->term);
  snprintf(param_candidate_id, sizeof(param_candidate_id), "%d",
 ...
 * Security: Uses parameterized queries (PQexecParams) to prevent SQL injection
 */
AppendEntriesResponse raft_send_append_entries(RaftPeer *peer,
                                               AppendEntriesRequest *request) {
  AppendEntriesResponse response;
  PGresult *res;
  /* Parameter buffers for parameterized query */
  ...
  if (request->entries_count == 0) {
    ...
    const char *paramValues[5] = {param_term, param_leader_id,
                                  param_prev_log_index, param_prev_log_term,
                                  param_leader_commit};
...
void raft_persist_state(RaftNode *node) {
  ...
  if (fp != NULL) {
    ...
    elog(WARNING, "Failed to open Raft state file for writing: %s (%m)",
         path.data);
  }
 ...
 */
int32 raft_get_leader(RaftNode *node) { return node->leader_id; }

/*
 * raft_is_leader - Check if this node is the leader
 */
bool raft_is_leader(RaftNode *node) { return node->is_leader; }

/*
 * raft_get_term - Get current term
 */
uint64 raft_get_term(RaftNode *node) { return node->current_term; }
 ...
 */
void raft_get_stats(RaftNode *node, uint64 *term, RaftState *state,
...
int raft_random_election_timeout(void) {
  ...
  return RAFT_ELECTION_TIMEOUT_MIN_MS +
         (rand() %
          (RAFT_ELECTION_TIMEOUT_MAX_MS - RAFT_ELECTION_TIMEOUT_MIN_MS));
}
 ...
 */
RequestVoteRequest *raft_deserialize_request_vote(const char *data,
                                                  int32 size) {
 ...
 */
AppendEntriesRequest *raft_deserialize_append_entries(const char *data,
                                                      int32 size) {
