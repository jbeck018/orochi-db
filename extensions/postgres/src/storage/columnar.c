...
static bool apply_skip_list_filter(ColumnarReadState *state, ColumnarChunkGroup *chunk_group);

/* ============================================================
 * Compression Implementation
 * ============================================================ */

int64 columnar_compress_buffer(const char *input, int64 input_size, char *output,
                               int64 max_output_size, OrochiCompressionType type, int level)
{
...
int64 columnar_decompress_buffer(const char *input, int64 input_size, char *output,
                                 int64 expected_size, OrochiCompressionType type)
{
    ...
    if (decompressed_size != expected_size)
        elog(WARNING, "Decompressed size mismatch: got %ld, expected %ld", decompressed_size,
             expected_size);

    return decompressed_size;
}

OrochiCompressionType columnar_select_compression(Oid type_oid, Datum *sample_values,
                                                  int sample_count)
{
    /*
    ...
    case BPCHAROID:
        ...
        if (sample_values != NULL && sample_count > 0) {
            ...
            for (i = 0; i < sample_count; i++) {
                ...
                hash_val =
                    DatumGetUInt32(hash_any((unsigned char *)DatumGetPointer(sample_values[i]),
...
ColumnarWriteState *columnar_begin_write(Relation relation, ColumnarOptions *options)
{
    ...
    state->write_context =
        AllocSetContextCreate(CurrentMemoryContext, "ColumnarWriteContext", ALLOCSET_DEFAULT_SIZES);
    state->stripe_context = AllocSetContextCreate(state->write_context, "ColumnarStripeContext",
    ...
    for (i = 0; i < tupdesc->natts; i++) {
        ...
        state->column_buffers[i].values = palloc(sizeof(Datum) * state->column_buffers[i].capacity);
        state->column_buffers[i].nulls = palloc(sizeof(bool) * state->column_buffers[i].capacity);
            ...
            if (buf->count == 0) {
                ...
                if (OidIsValid(typentry->cmp_proc)) {
                    ...
                    cmp_result = DatumGetInt32(FunctionCall2Coll(
                        &typentry->cmp_proc_finfo, attr->attcollation, curr_value, buf->min_value));
                    if (cmp_result < 0)
                        buf->min_value = curr_value;

                    /* Compare with max value */
                    cmp_result = DatumGetInt32(FunctionCall2Coll(
                        &typentry->cmp_proc_finfo, attr->attcollation, curr_value, buf->max_value));
                    if (cmp_result > 0)
...
void columnar_write_rows(ColumnarWriteState *state, TupleTableSlot **slots, int count)
{
    int i;
...
static void serialize_chunk_group(ColumnarWriteState *state)
{
    ...
    for (i = 0; i < tupdesc->natts; i++) {
        ...
        compressed_size = columnar_compress_buffer(raw_buffer, value_buffer_size, compressed_buffer,
                                                   value_buffer_size, compression,
...
static void flush_stripe(ColumnarWriteState *state)
{
    ...
    stripe_id = orochi_catalog_create_stripe(
        RelationGetRelid(state->relation), state->current_stripe->first_row_number,
        state->stripe_row_count, state->current_stripe->column_count);
     ...
     */
    {
        ...
        for (cg_idx = 0; cg_idx < stripe->chunk_group_count; cg_idx++) {
            ...
            for (col_idx = 0; col_idx < state->tupdesc->natts; col_idx++) {
                ...
                orochi_catalog_create_column_chunk_with_data(stripe_id, cg_idx, col_idx,
                                                             &chunk_meta, col_buf->compressed_data,
                                                             col_buf->compressed_size);

                /* Update min/max statistics for skip list */
                orochi_catalog_update_column_stats(stripe_id, col_idx, col_buf->min_value,
                                                   col_buf->max_value, col_buf->attr_type);
    ...
    orochi_catalog_update_stripe_status(stripe_id, true, stripe->data_size, stripe->metadata_size);
                                       ...
                                       Bitmapset *columns_needed)
{
    ...
    state->read_context =
        AllocSetContextCreate(CurrentMemoryContext, "ColumnarReadContext", ALLOCSET_DEFAULT_SIZES);
    state->stripe_context = AllocSetContextCreate(state->read_context, "ColumnarStripeReadContext",
                                                  ALLOCSET_DEFAULT_SIZES);
...
static void load_chunk_group(ColumnarReadState *state, int chunk_group_index)
{
    ...
    for (col_idx = 0; col_idx < tupdesc->natts; col_idx++) {
         ...
         */
        {
            ...
            compressed_data = orochi_catalog_read_chunk_data(stripe->stripe_id, chunk_group_index,
            ...
            if (compressed_data != NULL && compressed_size > 0) {
                ...
                if (compression_type != OROCHI_COMPRESS_NONE) {
                    ...
                    int64 actual_size = columnar_decompress_buffer(
                        compressed_data, compressed_size, decompressed_buffer, decompressed_size,
                        compression_type);

                    if (actual_size > 0 && actual_size != decompressed_size) {
                        elog(DEBUG1, "Column %d decompression: expected %ld, got %ld", col_idx,
                ...
                if (attr->attbyval && attr->attlen > 0) {
                    ...
                    int64 j;
                    for (j = 0; j < chunk_meta->value_count && j * attr->attlen < decompressed_size;
                         j++) {
                        memcpy(&col_buf->values[j], decompressed_buffer + (j * attr->attlen),
                               attr->attlen);
...
int columnar_read_next_vector(ColumnarReadState *state, TupleTableSlot *slot, int max_rows)
{
    int rows_read = 0;
 ...
 */
List *columnar_get_matching_chunks(ColumnarSkipList *skip_list, List *predicates)
{
    List *result = NIL;
    int i;
...
TableScanDesc columnar_beginscan(Relation relation, Snapshot snapshot, int nkeys,
                                 struct ScanKeyData *keys, ParallelTableScanDesc parallel_scan,
                                 uint32 flags)
{
...
void columnar_rescan(TableScanDesc sscan, struct ScanKeyData *key, bool set_params,
...
bool columnar_getnextslot(TableScanDesc sscan, ScanDirection direction, TupleTableSlot *slot)
{
    ColumnarScanDesc scan = (ColumnarScanDesc)sscan;
...
void columnar_tuple_insert(Relation relation, TupleTableSlot *slot, CommandId cid, int options,
