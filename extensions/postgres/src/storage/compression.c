...
int64 compress_lz4(const char *input, int64 input_size, char *output,
                   int64 max_output_size) {
#ifdef HAVE_LZ4
  int compressed_size;

  compressed_size = LZ4_compress_default(input, output, (int)input_size,
                                         (int)max_output_size);
...
int64 decompress_lz4(const char *input, int64 input_size, char *output,
                     int64 expected_size) {
#ifdef HAVE_LZ4
  int decompressed_size;

  decompressed_size =
      LZ4_decompress_safe(input, output, (int)input_size, (int)expected_size);
...
int64 compress_zstd(const char *input, int64 input_size, char *output,
                    int64 max_output_size, int level) {
#ifdef HAVE_ZSTD
  size_t compressed_size;

  compressed_size = ZSTD_compress(output, (size_t)max_output_size, input,
                                  (size_t)input_size, level);
...
int64 decompress_zstd(const char *input, int64 input_size, char *output,
                      int64 expected_size) {
#ifdef HAVE_ZSTD
  size_t decompressed_size;

  decompressed_size =
      ZSTD_decompress(output, (size_t)expected_size, input, (size_t)input_size);
...
int64 compress_pglz(const char *input, int64 input_size, char *output,
                    int64 max_output_size) {
...
int64 decompress_pglz(const char *input, int64 input_size, char *output,
                      int64 expected_size) {
...
int64 compress_delta(const char *input, int64 input_size, char *output,
                     int64 max_output_size) {
...
int64 decompress_delta(const char *input, int64 input_size, char *output,
                       int64 expected_size) {
...
int64 compress_gorilla(const char *input, int64 input_size, char *output,
                       int64 max_output_size) {
...
#define WRITE_BITS(val, nbits)                                                 \
  do {                                                                         \
    ...
    while (_bits_remaining > 0) {                                              \
      ...
      int bits_to_write =                                                      \
          (_bits_remaining < bits_in_byte) ? _bits_remaining : bits_in_byte;   \
  ...
  for (i = 0; i < count; i++) {
    ...
    if (xor_result == 0) {
      ...
      if (i > 0 && leading >= prev_leading && trailing >= prev_trailing) {
         ...
         * bits */
        uint64 significant =
            (xor_result >> trailing) & ((1ULL << meaningful_bits) - 1);
...
int64 decompress_gorilla(const char *input, int64 input_size, char *output,
                         int64 expected_size) {
  const uint8 *in = (const uint8 *)input;
...
#define READ_BITS(nbits, result)                                               \
  do {                                                                         \
    ...
    while (_bits_needed > 0) {                                                 \
      int bits_in_byte = 8 - bit_pos;                                          \
      int bits_to_read =                                                       \
          (_bits_needed < bits_in_byte) ? _bits_needed : bits_in_byte;         \
      ...
      for (i = 0; i < count && in_idx < input_size; i++) {
        ...
        if (first_bit == 0) {
          ...
          if (second_bit == 0) {
            ...
            uint64 xor_val = significant
                             << (64 - prev_leading - prev_meaningful);
...
int64 compress_dictionary(const char *input, int64 input_size, char *output,
                          int64 max_output_size) {
  ...
  while (ptr < end) {
    ...
    while (hash_table[h] != (uint32)-1) {
      ...
      if (dict[idx].len == str_len &&
          memcmp(dict[idx].str, ptr, str_len) == 0) {
        found_idx = idx;
        break;
      }
      h = (h + 1) % hash_size;
...
int64 decompress_dictionary(const char *input, int64 input_size, char *output,
                            int64 expected_size) {
...
int64 compress_rle(const char *input, int64 input_size, char *output,
                   int64 max_output_size) {
  const uint8 *in = (const uint8 *)input;
  uint8 *out = (uint8 *)output;
  int64 out_idx = 0;
  ...
  while (i < input_size && out_idx + 2 <= max_output_size) {
    ...
    while (i + run_length < input_size && in[i + run_length] == val &&
           run_length < 255) {
      run_length++;
    }
...
int64 decompress_rle(const char *input, int64 input_size, char *output,
                     int64 expected_size) {
  const uint8 *in = (const uint8 *)input;
  uint8 *out = (uint8 *)output;
  int64 out_idx = 0;
