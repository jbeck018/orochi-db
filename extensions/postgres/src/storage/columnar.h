 ...
 */
extern int64 columnar_compress_buffer(const char *input, int64 input_size, char *output,
                                      int64 max_output_size, OrochiCompressionType type, int level);

/*
 * Decompress a buffer
 */
extern int64 columnar_decompress_buffer(const char *input, int64 input_size, char *output,
                                        int64 expected_size, OrochiCompressionType type);

/*
 * Select optimal compression for column type
 */
extern OrochiCompressionType columnar_select_compression(Oid type_oid, Datum *sample_values,
                                                         int sample_count);
 ...
 */
extern ColumnarWriteState *columnar_begin_write(Relation relation, ColumnarOptions *options);

/*
 * Write a single row
 */
extern void columnar_write_row(ColumnarWriteState *state, Datum *values, bool *nulls);

/*
 * Write multiple rows (batch)
 */
extern void columnar_write_rows(ColumnarWriteState *state, TupleTableSlot **slots, int count);
 ...
 */
extern ColumnarReadState *columnar_begin_read(Relation relation, Snapshot snapshot,
                                              Bitmapset *columns_needed);

/*
 * Read next row
 */
extern bool columnar_read_next_row(ColumnarReadState *state, TupleTableSlot *slot);

/*
 * Read next vector of rows
 */
extern int columnar_read_next_vector(ColumnarReadState *state, TupleTableSlot *slot, int max_rows);
 ...
 */
extern bool columnar_can_skip_chunk(ColumnarSkipListEntry *entry, List *predicates);

/*
 * Get chunks matching predicate
 */
extern List *columnar_get_matching_chunks(ColumnarSkipList *skip_list, List *predicates);
 ...
 * Scan functions
 */
extern TableScanDesc columnar_beginscan(Relation relation, Snapshot snapshot, int nkeys,
                                        struct ScanKeyData *keys,
                                        ParallelTableScanDesc parallel_scan, uint32 flags);
extern void columnar_endscan(TableScanDesc scan);
extern void columnar_rescan(TableScanDesc scan, struct ScanKeyData *key, bool set_params,
                            bool allow_strat, bool allow_sync, bool allow_pagemode);
extern bool columnar_getnextslot(TableScanDesc scan, ScanDirection direction, TupleTableSlot *slot);

/*
 * Tuple operations
 */
extern void columnar_tuple_insert(Relation relation, TupleTableSlot *slot, CommandId cid,
                                  int options, struct BulkInsertStateData *bistate);
extern void columnar_tuple_insert_speculative(Relation relation, TupleTableSlot *slot,
                                              CommandId cid, int options,
                                              struct BulkInsertStateData *bistate,
                                              uint32 specToken);
extern void columnar_tuple_complete_speculative(Relation relation, TupleTableSlot *slot,
                                                uint32 specToken, bool succeeded);
extern TM_Result columnar_tuple_delete(Relation relation, ItemPointer tid, CommandId cid,
                                       Snapshot snapshot, Snapshot crosscheck, bool wait,
                                       TM_FailureData *tmfd, bool changingPart);
extern TM_Result columnar_tuple_update(Relation relation, ItemPointer otid, TupleTableSlot *slot,
                                       CommandId cid, Snapshot snapshot, Snapshot crosscheck,
                                       bool wait, TM_FailureData *tmfd, LockTupleMode *lockmode,
                                       TU_UpdateIndexes *update_indexes);
extern TM_Result columnar_tuple_lock(Relation relation, ItemPointer tid, Snapshot snapshot,
 ...
 * Index support
 */
extern bool columnar_fetch_row_version(Relation relation, ItemPointer tid, Snapshot snapshot,
                                       TupleTableSlot *slot);
 ...
 * Planner support
 */
extern void columnar_estimate_rel_size(Relation rel, int32 *attr_widths, BlockNumber *pages,
