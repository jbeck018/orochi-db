...
static char *build_connection_string(OrochiNodeInfo *node)
{
    ...
    appendStringInfo(&connstr, "host=%s port=%d dbname=%s connect_timeout=%d", node->hostname,
                     node->port, get_database_name(MyDatabaseId), CONNECTION_TIMEOUT_SEC);
...
void orochi_execute_distributed_plan(DistributedPlan *plan, DistributedExecutorState *state)
{
    ListCell *lc;
...
void *orochi_get_worker_connection(int32 node_id)
{
    ...
    if (PQstatus(conn) != CONNECTION_OK) {
        elog(WARNING, "Failed to connect to node %d: %s", node_id, PQerrorMessage(conn));
        PQfinish(conn);
        return NULL;
    }
...
void orochi_execute_remote_task(RemoteTask *task)
{
    ...
    elog(DEBUG1, "Executing task on shard %ld (node %d)", task->fragment->shard_id, node_id);
...
void orochi_adaptive_execute(DistributedExecutorState *state)
{
    ...
    while (!state->all_completed && !state->has_error) {
        ...
        foreach (lc, state->tasks) {
            ...
            if (task->state == TASK_PENDING && state->running_count < active_connections) {
        ...
        /* Check completion */
        state->all_completed =
            (state->completed_count + state->failed_count == list_length(state->tasks));
...
void orochi_adjust_parallelism(DistributedExecutorState *state)
{
    ...
    foreach (lc, state->tasks) {
        ...
        if (task->state == TASK_COMPLETED && task->completed_at > task->started_at) {
    ...
    elog(DEBUG1, "Adjusted parallelism to %d (avg latency: %ld ms)", state->max_parallel,
         avg_latency_ms);
}
...
void orochi_start_async_task(RemoteTask *task)
{
    ...
    elog(DEBUG1, "Started async task on shard %ld (node %d)", task->fragment->shard_id, node_id);
}
 ...
 */
void orochi_execute_aggregate_pushdown(DistributedExecutorState *state, AggPushdownPlan *agg_plan)
{
    ListCell *lc;
    ListCell *task_lc;
    ...
    forboth(lc, agg_plan->worker_queries, task_lc, state->tasks)
    {
        ...
        if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0) {
            ...
            elog(DEBUG1, "Partial aggregate from shard %ld: count=%ld, sum=%ld", partial->shard_id,
                 partial->count, partial->sum);
        } else {
            elog(WARNING, "Aggregate query failed on shard %ld: %s", partial->shard_id,
                 PQerrorMessage(conn));
        }
...
void orochi_init_query_cache(void)
{
    ...
    query_cache =
        hash_create("OrochiQueryCache", 1024, &hash_ctl, HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
}
 ...
 * Security: Uses LWLock to prevent race conditions
 */
void orochi_cache_store(const char *query_string, const char *result_data, int64 result_size)
{
    uint64 hash;
    QueryCacheEntry *entry;
    bool found;
...
void orochi_cache_invalidate(Oid table_oid)
{
    ...
    if (cache_dependencies != NULL) {
        ...
        if (found && dep_entry->num_queries > 0) {
            ...
            elog(DEBUG1, "Invalidated %d cache entries for table OID %u", invalidated, table_oid);
            return;
        }
    }
    ...
    elog(DEBUG1, "No dependency info for table OID %u, clearing entire cache", table_oid);
 ...
 * Security: Uses LWLock to prevent race conditions
 */
void orochi_cache_store_with_deps(const char *query_string, const char *result_data,
 ...
 */
StreamingResultState *orochi_init_streaming_result(DistributedExecutorState *state)
{
...
List *orochi_get_next_batch(StreamingResultState *stream)
{
    ...
    while (stream->current_shard < list_length(stream->executor_state->tasks)) {
        RemoteTask *task =
            (RemoteTask *)list_nth(stream->executor_state->tasks, stream->current_shard);
        PGresult *res;
        int ntuples;
        int nfields;
        int row_start;
...
bool orochi_prepare_distributed_transaction(DistributedExecutorState *state)
{
    ...
    foreach (lc, current_dtxn->participants) {
        ...
        resetStringInfo(&cmd);
        appendStringInfo(&cmd, "PREPARE TRANSACTION '%s_%d'", current_dtxn->gid, node_id);
    ...
    if (all_prepared) {
        ...
        if (orochi_use_raft_consensus && orochi_raft_is_leader()) {
            if (!orochi_raft_prepare_transaction(current_dtxn->gid, shard_ids)) {
                elog(WARNING, "Failed to log prepare decision to Raft for %s", current_dtxn->gid);
...
void orochi_commit_distributed_transaction(DistributedExecutorState *state)
{
    ...
    foreach (lc, current_dtxn->participants) {
        ...
        resetStringInfo(&cmd);
        appendStringInfo(&cmd, "COMMIT PREPARED '%s_%d'", current_dtxn->gid, node_id);
...
void orochi_rollback_distributed_transaction(DistributedExecutorState *state)
{
    ...
    if (orochi_use_raft_consensus && current_dtxn->prepared) {
        if (orochi_raft_is_leader()) {
            if (!orochi_raft_abort_transaction(current_dtxn->gid)) {
                elog(WARNING, "Failed to log abort decision to Raft for %s", current_dtxn->gid);
            } else {
    ...
    if (current_dtxn->prepared) {
        foreach (lc, current_dtxn->participants) {
            ...
            resetStringInfo(&cmd);
            appendStringInfo(&cmd, "ROLLBACK PREPARED '%s_%d'", current_dtxn->gid, node_id);
...
void orochi_handle_task_failure(RemoteTask *task, DistributedExecutorState *state)
{
    if (orochi_is_retryable_error(task->error_message) && orochi_retry_task(task)) {
...
bool orochi_is_retryable_error(const char *error_message)
{
    ...
    if (strstr(error_message, "connection") != NULL || strstr(error_message, "timeout") != NULL)
        return true;
