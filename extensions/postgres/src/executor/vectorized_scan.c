...
VectorizedScanState *vectorized_columnar_scan_begin(Relation relation,
                                                    Snapshot snapshot,
  ...
  state->columnar_state =
      columnar_begin_read(relation, snapshot, columns_needed);
  ...
  state->current_batch =
      vector_batch_create(VECTORIZED_BATCH_SIZE, state->projection_count);
...
void vectorized_columnar_scan_end(VectorizedScanState *state) {
  ...
  /* Log statistics */
  elog(DEBUG1,
       "Vectorized scan complete: batches=%ld rows=%ld chunks_skipped=%ld",
  ...
  while (rows_loaded < VECTORIZED_BATCH_SIZE) {
    ...
    if (columnar->current_chunk_group == NULL ||
      ...
      if (columnar->current_stripe == NULL ||
        ...
        if (columnar->current_stripe_index >=
            list_length(columnar->stripe_list)) {
          /* No more data */
          break;
        }
      ...
      /* This triggers decompression and populates column_buffers */
      {
        ...
        for (int col_idx = 0; col_idx < state->tupdesc->natts; col_idx++) {
          ...
          compressed_data = orochi_catalog_read_chunk_data(
              columnar->current_stripe->stripe_id,
              columnar->current_chunk_group_index, col_idx, &compressed_size);
          ...
          if (compressed_data != NULL && compressed_size > 0) {
            ...
            if (attr->attbyval && attr->attlen > 0) {
              ...
              for (int64 j = 0;
                   j < row_count && j * attr->attlen < decompressed_size; j++) {
    ...
    int64 rows_available = columnar->current_chunk_group->row_count -
                           columnar->current_row_in_chunk;
    int32 rows_to_load =
        (int32)Min(rows_available, VECTORIZED_BATCH_SIZE - rows_loaded);
 ...
 */
void vectorized_apply_predicate(VectorizedScanState *state,
                                VectorBatch *batch) {
  int i;
  ...
  if (state->predicates != NIL) {
    ...
    foreach (lc, state->predicates) {
      ...
      if (can_pushdown_predicate(pred, &column_index, &op, &value,
                                 &data_type)) {
...
static void load_int64_column_avx2(int64 *dest, const int64 *src, int32 count,
  ...
  for (i = 0; i + VECTORIZED_INT64_LANES <= count;
       i += VECTORIZED_INT64_LANES) {
 ...
 */
static void load_float64_column_avx2(double *dest, const double *src,
                                     int32 count, const uint8 *null_bitmap,
                                     uint64 *dest_nulls) {
  int32 i;

  /* Process 4 double values at a time */
  for (i = 0; i + VECTORIZED_FLOAT64_LANES <= count;
...
static void load_int32_column_avx2(int32 *dest, const int32 *src, int32 count,
  ...
  for (i = 0; i + VECTORIZED_INT32_LANES <= count;
       i += VECTORIZED_INT32_LANES) {
    __m256i data = _mm256_loadu_si256((__m256i *)(src + i));
    _mm256_storeu_si256((__m256i *)(dest + i), data);
  }
 ...
 */
static void load_column_scalar(void *dest, const void *src, int32 count,
                               int32 elem_size, const uint8 *null_bitmap,
                               uint64 *dest_nulls) {
 ...
 */
void vectorized_load_column_data(VectorColumn *column, const void *src_data,
  ...
  case VECTORIZED_TYPE_INT64:
  case VECTORIZED_TYPE_TIMESTAMP:
    load_int64_column_avx2((int64 *)column->data, (const int64 *)src_data,
                           row_count, null_bitmap, column->nulls);
    break;

  case VECTORIZED_TYPE_FLOAT64:
    load_float64_column_avx2((double *)column->data, (const double *)src_data,
                             row_count, null_bitmap, column->nulls);
    break;

  case VECTORIZED_TYPE_INT32:
  case VECTORIZED_TYPE_DATE:
    load_int32_column_avx2((int32 *)column->data, (const int32 *)src_data,
                           row_count, null_bitmap, column->nulls);
    break;
