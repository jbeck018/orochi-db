...
VectorBatch *vector_batch_create(int32 capacity, int32 column_count) {
  ...
  batch_context = AllocSetContextCreate(
      CurrentMemoryContext, "VectorBatchContext", ALLOCSET_DEFAULT_SIZES);
 ...
 */
static void filter_int64_avx2(VectorBatch *batch, int32 column_index,
                              VectorizedCompareOp op, int64 value) {
  ...
  for (i = 0; i + VECTORIZED_INT64_LANES <= batch->count;
 ...
 */
static void filter_float64_avx2(VectorBatch *batch, int32 column_index,
                                VectorizedCompareOp op, double value) {
  VectorColumn *column = batch->columns[column_index];
...
void vectorized_filter_int64(VectorBatch *batch, int32 column_index,
                             VectorizedCompareOp op, int64 value) {
  if (batch == NULL || column_index < 0 || column_index >= batch->column_count)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
void vectorized_filter_float64(VectorBatch *batch, int32 column_index,
                               VectorizedCompareOp op, double value) {
  if (batch == NULL || column_index < 0 || column_index >= batch->column_count)
    ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                    errmsg("invalid batch or column index")));
...
void vectorized_filter_int32(VectorBatch *batch, int32 column_index,
...
#ifdef __AVX2__
  {
    ...
    for (i = 0; i + VECTORIZED_INT32_LANES <= batch->count;
...
void vectorized_filter_float32(VectorBatch *batch, int32 column_index,
...
void vectorized_filter_generic(VectorBatch *batch,
...
void vectorized_selection_and(VectorBatch *batch, uint32 *other_selection,
                              int32 other_count) {
...
void vectorized_selection_or(VectorBatch *batch, uint32 *other_selection,
                             int32 other_count) {
  uint32 *new_selection;
  int32 new_count = 0;
  int32 i, j;
...
int64 vectorized_min_int64(VectorBatch *batch, int32 column_index,
                           bool *found) {
  VectorColumn *column;
  int64 *data;
...
int64 vectorized_max_int64(VectorBatch *batch, int32 column_index,
                           bool *found) {
  VectorColumn *column;
  int64 *data;
...
double vectorized_min_float64(VectorBatch *batch, int32 column_index,
                              bool *found) {
  VectorColumn *column;
  double *data;
...
double vectorized_max_float64(VectorBatch *batch, int32 column_index,
                              bool *found) {
  VectorColumn *column;
  double *data;
...
int vectorized_batch_to_slots(VectorBatch *batch, TupleTableSlot **slots,
...
void vectorized_batch_debug_print(VectorBatch *batch) {
  ...
  elog(DEBUG1, "VectorBatch: capacity=%d count=%d columns=%d", batch->capacity,
       batch->count, batch->column_count);
  elog(DEBUG1, "  selection: has=%s count=%d",
       batch->has_selection ? "true" : "false", batch->selected_count);
 ...
 */
void vectorized_filter_with_jit(VectorBatch *batch,
                                VectorizedFilterState *filter,
 ...
 */
void vectorized_aggregate_with_jit(VectorizedAggState *state,
                                   VectorBatch *batch,
...
vectorized_compile_filter_jit(VectorizedFilterState *filter) {
  ...
  return jit_compile_simple_filter(ctx, filter->column_index,
                                   filter->compare_op, filter->const_value,
                                   filter->data_type);
}
