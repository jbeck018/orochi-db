...
#define OROCHI_MODULE_MAGIC PG_MODULE_MAGIC_EXT(.name = "orochi", .version = OROCHI_VERSION_STRING)
#else
 ...
 */
#define OROCHI_LWLOCK_REGISTER_TRANCHE(id, tranche) LWLockRegisterTranche(id, tranche)
 ...
 */
#define OROCHI_HASH_CREATE(name, nelem, info, flags) hash_create(name, nelem, info, flags)
 ...
 */
#define OROCHI_BGW_FLAGS (BGWORKER_SHMEM_ACCESS | BGWORKER_BACKEND_DATABASE_CONNECTION)
