...
void orochi_workload_shmem_startup(void)
{
    ...
    WorkloadState = (WorkloadSharedState *)ShmemInitStruct("Orochi Workload State",
                                                           sizeof(WorkloadSharedState), &found);
...
void orochi_workload_init(void)
{
    ...
    /* Define GUCs */
    DefineCustomBoolVariable("orochi.workload_enabled", "Enable workload management", NULL,
                             &orochi_workload_enabled, true, PGC_SUSET, 0, NULL, NULL, NULL);

    DefineCustomIntVariable(
        "orochi.default_pool_concurrency", "Default concurrent queries per pool (0 = unlimited)",
        NULL, &orochi_default_pool_concurrency, 0, 0, 10000, PGC_SUSET, 0, NULL, NULL, NULL);

    DefineCustomIntVariable(
        "orochi.default_query_timeout_ms", "Default query timeout in milliseconds", NULL,
        &orochi_default_query_timeout_ms, 30000, 0, 3600000, PGC_SUSET, 0, NULL, NULL, NULL);

    DefineCustomBoolVariable("orochi.workload_log_rejections", "Log rejected queries", NULL,
...
static void init_default_pools(void)
{
    ...
    for (int i = 0; i < WorkloadState->num_pools; i++) {
        if (strcmp(WorkloadState->pools[i].pool_name, WORKLOAD_DEFAULT_POOL_NAME) == 0 ||
            strcmp(WorkloadState->pools[i].pool_name, WORKLOAD_ADMIN_POOL_NAME) == 0) {
                                  ...
                                  int priority_weight)
{
    ...
    if (pool_name == NULL || strlen(pool_name) == 0) {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("pool name cannot be empty")));
    }

    if (strlen(pool_name) >= WORKLOAD_MAX_POOL_NAME) {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("pool name too long (max %d characters)", WORKLOAD_MAX_POOL_NAME - 1)));
    }
    ...
    if (pool == NULL) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INSUFFICIENT_RESOURCES),
                 errmsg("maximum number of resource pools reached (%d)", WORKLOAD_MAX_POOLS)));
    }
 ...
 */
bool orochi_alter_resource_pool(int64 pool_id, ResourceLimits *new_limits, int priority_weight)
{
    ...
    if (new_limits != NULL) {
        if (!orochi_validate_limits(new_limits)) {
            LWLockRelease(WorkloadState->main_lock);
            ereport(ERROR,
                    (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("invalid resource limits")));
        }
    ...
    elog(DEBUG1, "Altered resource pool \"%s\" (id=%ld)", pool->pool_name, pool_id);
...
bool orochi_drop_resource_pool(int64 pool_id, bool if_exists)
{
    ...
    if (pool->is_internal) {
        ...
        ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
                        errmsg("cannot drop internal resource pool \"%s\"", pool->pool_name)));
    }
    ...
    if (pool->active_queries > 0) {
        ...
        ereport(ERROR, (errcode(ERRCODE_OBJECT_IN_USE),
                        errmsg("resource pool \"%s\" has %d active queries", pool->pool_name,
                               pool->active_queries)));
    }

    elog(DEBUG1, "Dropped resource pool \"%s\" (id=%ld)", pool->pool_name, pool_id);
                                   ...
                                   QueryPriority default_priority, ResourceLimits *limits)
{
    ...
    if (class_name == NULL || strlen(class_name) == 0) {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("class name cannot be empty")));
    }
    ...
    if (wclass == NULL) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INSUFFICIENT_RESOURCES),
                 errmsg("maximum number of workload classes reached (%d)", WORKLOAD_MAX_CLASSES)));
    }
    ...
    elog(DEBUG1, "Created workload class \"%s\" (id=%ld) in pool %ld", class_name, class_id,
         pool_id);
 ...
 */
bool orochi_alter_workload_class(int64 class_id, int64 pool_id, QueryPriority priority,
...
bool orochi_drop_workload_class(int64 class_id, bool if_exists)
{
    ...
    elog(DEBUG1, "Dropped workload class \"%s\" (id=%ld)", wclass->class_name, class_id);
...
static bool check_pool_resources(ResourcePool *pool, int64 memory_requested)
{
    ...
    if (pool->limits.max_concurrency > 0 && pool->active_queries >= pool->limits.max_concurrency) {
        return false;
    }

    /* Check memory limit */
    if (pool->limits.memory_mb > 0) {
        int64 available = (pool->limits.memory_mb * 1024 * 1024) - pool->usage.memory_bytes;
                        ...
                        int64 estimated_cost, int64 memory_requested)
{
    ...
    if (pool == NULL) {
        LWLockRelease(WorkloadState->main_lock);
        ereport(WARNING, (errmsg("resource pool %ld not found, using default", pool_id)));
    ...
    if (check_pool_resources(pool, memory_requested)) {
        ...
        elog(DEBUG2,
             ...
             "(active=%d, memory=%ld)",
             backend_pid, pool->pool_name, pool->active_queries, pool->usage.memory_bytes);
    } else {
        ...
        if (orochi_workload_log_rejections) {
            elog(LOG,
                 ...
                 "(active=%d/%d)",
                 backend_pid, pool->pool_name, pool->active_queries, pool->limits.max_concurrency);
        }
    }
                        ...
                        int64 estimated_cost)
{
    ...
    if (pool->limits.max_queue_size > 0 && pool->queued_queries >= pool->limits.max_queue_size) {
        LWLockRelease(WorkloadState->main_lock);
        pool->usage.queries_rejected++;
        if (orochi_workload_log_rejections) {
            elog(LOG, "Query rejected: pool \"%s\" queue full (%d/%d)", pool->pool_name,
    ...
    elog(DEBUG2, "Queued query for backend %d in pool %ld (position=%d)", backend_pid, pool_id,
         pool->queued_queries);
...
void orochi_release_query(int backend_pid)
{
    ...
    for (int i = 0; i < WORKLOAD_MAX_POOLS; i++) {
        if (WorkloadState->pools[i].pool_id != 0 && WorkloadState->pools[i].active_queries > 0) {
...
bool orochi_dequeue_next_query(int64 pool_id, int *backend_pid)
{
    ...
    for (int i = 0; i < WORKLOAD_QUEUE_SIZE; i++) {
        ...
        if (entry->backend_pid != 0 && entry->pool_id == pool_id && !entry->is_active) {
...
void orochi_log_pool_stats(int64 pool_id)
{
    ...
    if (pool != NULL) {
        elog(LOG,
             ...
             pool->pool_name, pool->pool_id, pool->active_queries, pool->queued_queries,
             pool->usage.cpu_time_us, pool->usage.memory_bytes, pool->usage.queries_executed,
             pool->usage.queries_rejected);
        pfree(pool);
    }
}
...
void orochi_log_workload_status(void)
{
    ...
    elog(LOG,
         ...
         WorkloadState->num_pools, WorkloadState->num_classes, WorkloadState->total_active_queries,
         WorkloadState->total_queued_queries);
...
bool orochi_assign_workload_class_by_name(int backend_pid, const char *class_name)
{
...
bool orochi_assign_workload_class(int backend_pid, int64 class_id)
{
    ...
    elog(DEBUG1, "Assigned backend %d to workload class %ld (found=%d)", backend_pid, class_id,
         found);
...
bool orochi_register_session(int backend_pid, int64 pool_id, int64 class_id)
{
    ...
    if (session != NULL) {
        ...
        if (session != NULL) {
            ...
            elog(DEBUG1, "Registered new session for backend %d in pool %ld class %ld", backend_pid,
                 pool_id, class_id);
        } else {
            elog(WARNING,
...
void orochi_track_session_cpu(int backend_pid, int64 cpu_time_us)
{
    ...
    if (session != NULL) {
        ...
        elog(DEBUG2, "Tracked %ld us CPU for backend %d (total: %ld us)", cpu_time_us, backend_pid,
             session->usage.cpu_time_us);
    }
...
void orochi_track_session_memory(int backend_pid, int64 memory_bytes)
{
    ...
    if (session != NULL) {
        ...
        elog(DEBUG2, "Tracked %ld bytes memory for backend %d (peak: %ld)", memory_bytes,
             backend_pid, session->usage.peak_memory_bytes);
    }
 ...
 */
void orochi_track_session_io(int backend_pid, int64 read_bytes, int64 write_bytes)
{
    SessionResourceTracker *session;
    ...
    if (session != NULL) {
        ...
        elog(DEBUG2, "Tracked I/O for backend %d: read=%ld, write=%ld", backend_pid, read_bytes,
             write_bytes);
    }
...
List *orochi_get_queued_queries(int64 pool_id)
{
    ...
    for (int i = 0; i < WORKLOAD_QUEUE_SIZE; i++) {
        ...
        if (entry->backend_pid != 0 && (pool_id == 0 || entry->pool_id == pool_id) &&
            !entry->is_active) {
