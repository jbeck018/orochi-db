 ...
 */
void
orochi_governor_init(void)
{
    bool found;

    LWLockAcquire(AddinShmemInitLock, LW_EXCLUSIVE);

    GovernorState = (GovernorSharedState *)
        ShmemInitStruct("Orochi Resource Governor",
                        sizeof(GovernorSharedState),
                        &found);
 ...
 */
static SessionResourceState *
find_session_state(int backend_pid)
{
    for (int i = 0; i < MAX_TRACKED_SESSIONS; i++)
    {
 ...
 */
static SessionResourceState *
find_or_create_session_state(int backend_pid)
{
 ...
 */
static void
cleanup_inactive_sessions(void)
{
 ...
 */
void
orochi_governor_track_cpu(int backend_pid, int64 cpu_time_us)
{
    SessionResourceState *session;
 ...
 */
bool
orochi_governor_check_cpu_limit(int backend_pid)
{
    SessionResourceState *session;
    bool exceeded = false;
 ...
 */
int64
orochi_governor_get_cpu_usage(int backend_pid)
{
    SessionResourceState *session;
    int64 cpu_time = 0;
 ...
 */
bool
orochi_governor_request_memory(int backend_pid, int64 bytes)
{
    SessionResourceState *session;
    bool allowed = true;
    ...
    if (session != NULL)
    {
        ...
        if (session->memory_limit_bytes > 0)
        {
            ...
            {
                allowed = false;
                elog(DEBUG1, "Memory request denied for backend %d: "
 ...
 */
void
orochi_governor_release_memory(int backend_pid, int64 bytes)
{
    SessionResourceState *session;
 ...
 */
bool
orochi_governor_check_memory_limit(int backend_pid)
{
    SessionResourceState *session;
    bool exceeded = false;
 ...
 */
int64
orochi_governor_get_memory_usage(int backend_pid)
{
    SessionResourceState *session;
    int64 memory = 0;
 ...
 */
bool
orochi_governor_check_io_limit(int backend_pid, int64 bytes, bool is_write)
{
    SessionResourceState *session;
    bool allowed = true;
    ...
    if (session != NULL)
    {
        ...
        int64 limit_bps = is_write ? session->io_write_limit_bps
                                   : session->io_read_limit_bps;

        if (limit_bps > 0)
        {
            int64 window_bytes = is_write ? session->io_write_window
                                          : session->io_read_window;
            int64 allowed_bytes;
 ...
 */
void
orochi_governor_track_io(int backend_pid, int64 bytes, bool is_write)
{
    SessionResourceState *session;
 ...
 */
void
orochi_governor_throttle_io(int backend_pid, int delay_ms)
{
    if (delay_ms > 0 && delay_ms < 10000)  /* Max 10 second delay */
    {
        elog(DEBUG2, "Throttling I/O for backend %d: %d ms delay",
             backend_pid, delay_ms);
 ...
 */
bool
orochi_governor_can_start_query(int64 pool_id)
{
    ResourcePool *pool;
    bool can_start = true;
 ...
 */
void
orochi_governor_query_started(int64 pool_id)
{
 ...
 */
void
orochi_governor_query_finished(int64 pool_id)
{
    /* This is handled in workload.c pool management */
 ...
 */
int
orochi_governor_get_active_queries(int64 pool_id)
{
    ResourcePool *pool;
    int count = 0;
 ...
 */
bool
orochi_governor_check_timeout(int backend_pid)
{
    SessionResourceState *session;
        ...
        session->query_start_time != 0)
    {
        ...
        {
            timed_out = true;
            elog(DEBUG1, "Query timeout for backend %d: %ld ms >= %d ms",
                 backend_pid, elapsed_ms, session->query_timeout_ms);
        }
    }
 ...
 */
void
orochi_governor_set_timeout(int backend_pid, int timeout_ms)
{
    SessionResourceState *session;
 ...
 */
void
orochi_governor_clear_timeout(int backend_pid)
{
    SessionResourceState *session;
 ...
 */
static int64
get_pool_cpu_limit(int64 pool_id)
{
    ResourcePool *pool;
    int64 limit = 0;
 ...
 */
static int64
get_pool_memory_limit(int64 pool_id)
{
    ResourcePool *pool;
    int64 limit = 0;
 ...
 */
static int64
get_pool_io_limit(int64 pool_id, bool is_write)
{
    ResourcePool *pool;
    int64 limit = 0;
 ...
 */
static int
get_pool_concurrency_limit(int64 pool_id)
{
    ResourcePool *pool;
    int limit = 0;
 ...
 */
void
orochi_governor_get_stats(int64 *total_cpu_us, int64 *total_memory_bytes,
 ...
 */
void
orochi_governor_reset_session_stats(int backend_pid)
{
    SessionResourceState *session;
 ...
 */
void
orochi_governor_set_session_limits(int backend_pid, int64 pool_id,
                                   int64 class_id, ResourceLimits *limits)
{
    SessionResourceState *session;
    ...
    if (session != NULL)
    {
        ...
        if (limits != NULL)
        {
            ...
            session->cpu_limit_us = (limits->cpu_percent > 0 && limits->cpu_percent < 100)
