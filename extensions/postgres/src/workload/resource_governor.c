...
void orochi_governor_init(void)
{
    ...
    GovernorState = (GovernorSharedState *)ShmemInitStruct("Orochi Resource Governor",
                                                           sizeof(GovernorSharedState), &found);
...
static SessionResourceState *find_or_create_session_state(int backend_pid)
{
    ...
    elog(WARNING, "Cannot track session for backend %d: no free slots", backend_pid);
    return NULL;
}
...
static void cleanup_inactive_sessions(void)
{
    ...
    for (int i = 0; i < MAX_TRACKED_SESSIONS; i++) {
        ...
        if (session->is_active && session->last_activity < cutoff && session->active_queries == 0) {
        ...
        if (session->memory_limit_bytes > 0) {
            if (session->memory_bytes + bytes > session->memory_limit_bytes) {
                ...
                elog(DEBUG1,
                     ...
                     backend_pid, session->memory_bytes, bytes, session->memory_limit_bytes);
            }
        }
 ...
 */
bool orochi_governor_check_io_limit(int backend_pid, int64 bytes, bool is_write)
{
    SessionResourceState *session;
    bool allowed = true;
    ...
    if (session != NULL) {
        ...
        int64 limit_bps = is_write ? session->io_write_limit_bps : session->io_read_limit_bps;

        if (limit_bps > 0) {
            int64 window_bytes = is_write ? session->io_write_window : session->io_read_window;
            int64 allowed_bytes;
...
void orochi_governor_throttle_io(int backend_pid, int delay_ms)
{
    ...
    {
        elog(DEBUG2, "Throttling I/O for backend %d: %d ms delay", backend_pid, delay_ms);
...
bool orochi_governor_check_timeout(int backend_pid)
{
    ...
    session = find_session_state(backend_pid);
    if (session != NULL && session->query_timeout_ms > 0 && session->query_start_time != 0) {
        TimestampTz now = GetCurrentTimestamp();
        int64 elapsed_ms = (now - session->query_start_time) / 1000;

        if (elapsed_ms >= session->query_timeout_ms) {
            timed_out = true;
            elog(DEBUG1, "Query timeout for backend %d: %ld ms >= %d ms", backend_pid, elapsed_ms,
                 session->query_timeout_ms);
        }
    }
 ...
 */
void orochi_governor_set_session_limits(int backend_pid, int64 pool_id, int64 class_id,
