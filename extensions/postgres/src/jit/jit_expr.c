...
static int64 jit_eval_int64_internal(VectorBatch *batch, JitExprNode *expr, int32 row_idx,
                                     bool *is_null);
static int32 jit_eval_int32_internal(VectorBatch *batch, JitExprNode *expr, int32 row_idx,
                                     bool *is_null);
static double jit_eval_float64_internal(VectorBatch *batch, JitExprNode *expr, int32 row_idx,
                                        bool *is_null);
static float jit_eval_float32_internal(VectorBatch *batch, JitExprNode *expr, int32 row_idx,
                                       bool *is_null);
static bool jit_eval_bool_internal(VectorBatch *batch, JitExprNode *expr, int32 row_idx,
                                   bool *is_null);
...
void jit_extract_column_refs(JitExprNode *expr, int32 *columns, int32 *count, int32 max_columns)
{
    int i;
...
JitExprNode *jit_optimize_expr(JitContext *ctx, JitExprNode *expr)
{
    ...
    if (expr->left != NULL && expr->right != NULL && jit_expr_is_const(expr->left) &&
        jit_expr_is_const(expr->right)) {
        ...
        case JIT_EXPR_ARITH_MOD: {
            ...
            if (expr->result_type == VECTORIZED_TYPE_INT64) {
                int64 left_val =
                    jit_convert_to_int64(expr->left->const_value, expr->left->result_type);
                int64 right_val =
                    jit_convert_to_int64(expr->right->const_value, expr->right->result_type);
                int64 result = 0;
                ...
                double left_val =
                    jit_convert_to_float64(expr->left->const_value, expr->left->result_type);
                double right_val =
                    jit_convert_to_float64(expr->right->const_value, expr->right->result_type);
                double result = 0.0;
        ...
        case JIT_EXPR_BOOL_AND: {
            bool left_val = jit_convert_to_bool(expr->left->const_value, expr->left->result_type);
            bool right_val =
                jit_convert_to_bool(expr->right->const_value, expr->right->result_type);
    ...
    if (expr->type == JIT_EXPR_BOOL_AND) {
        if (jit_expr_is_const(expr->left)) {
            bool val = jit_convert_to_bool(expr->left->const_value, expr->left->result_type);
            if (!val)
        ...
        if (jit_expr_is_const(expr->right)) {
            bool val = jit_convert_to_bool(expr->right->const_value, expr->right->result_type);
            if (!val)
                                     ...
                                     bool *is_null)
{
    ...
    *is_null = false;
    ...
    case JIT_EXPR_COLUMN: {
        ...
        if (col->has_nulls && (col->nulls[row_idx / 64] & (1ULL << (row_idx % 64)))) {
            *is_null = true;
            return 0;
        }
...
static int64 jit_wrapper_eval_int64(const void *data, int32 row_idx, const uint64 *nulls,
                                    bool *is_null)
{
...
static int32 jit_wrapper_eval_int32(const void *data, int32 row_idx, const uint64 *nulls,
                                    bool *is_null)
{
    VectorBatch *batch = (VectorBatch *)data;
...
static double jit_wrapper_eval_float64(const void *data, int32 row_idx, const uint64 *nulls,
                                       bool *is_null)
{
    VectorBatch *batch = (VectorBatch *)data;
...
static float jit_wrapper_eval_float32(const void *data, int32 row_idx, const uint64 *nulls,
                                      bool *is_null)
{
    VectorBatch *batch = (VectorBatch *)data;
...
static bool jit_wrapper_eval_bool(const void *data, int32 row_idx, const uint64 *nulls,
                                  bool *is_null)
{
    VectorBatch *batch = (VectorBatch *)data;
...
static void jit_batch_filter_internal(VectorBatch *batch, JitExprNode *expr, bool *out_mask)
{
    int32 i;
    bool is_null;
