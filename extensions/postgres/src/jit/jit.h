 ...
 */
typedef int64 (*JitEvalInt64Fn)(const void *data, int32 row_idx, const uint64 *nulls,
                                bool *is_null);
typedef int32 (*JitEvalInt32Fn)(const void *data, int32 row_idx, const uint64 *nulls,
                                bool *is_null);
typedef double (*JitEvalFloat64Fn)(const void *data, int32 row_idx, const uint64 *nulls,
                                   bool *is_null);
typedef float (*JitEvalFloat32Fn)(const void *data, int32 row_idx, const uint64 *nulls,
                                  bool *is_null);
typedef bool (*JitEvalBoolFn)(const void *data, int32 row_idx, const uint64 *nulls, bool *is_null);

/*
 * Vectorized evaluation function types
 * These operate on entire batches for better performance
 */
typedef void (*JitFilterFn)(VectorBatch *batch, int32 column_index, Datum const_value,
                            bool *out_mask);
...
typedef void (*JitFilterExprFn)(VectorBatch *batch, JitExprNode *expr, bool *out_mask);
 ...
 */
extern JitExprNode *jit_expr_between(JitContext *ctx, JitExprNode *value, JitExprNode *low,
                                     JitExprNode *high);
 ...
 */
extern JitCompiledFilter *jit_compile_simple_filter(JitContext *ctx, int32 column_index,
 ...
 */
extern JitCompiledAgg *jit_compile_filtered_agg(JitContext *ctx, VectorizedAggType agg_type,
                                                int32 column_index, VectorizedDataType data_type,
 ...
 */
extern void jit_execute_agg_update(JitCompiledAgg *agg, VectorizedAggState *state,
                                   VectorBatch *batch);
 ...
 */
extern void jit_vectorized_filter(VectorBatch *batch, JitCompiledFilter *filter);

/*
 * Execute JIT-compiled aggregation on batch
 * Falls back to interpreted execution if JIT unavailable
 */
extern void jit_vectorized_aggregate(VectorizedAggState *state, VectorBatch *batch,
                                     JitCompiledAgg *agg);
