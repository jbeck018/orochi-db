...
JitContext *jit_context_create_with_options(const JitOptions *options)
{
    ...
    jit_ctx = AllocSetContextCreate(CurrentMemoryContext, "JitContext", ALLOCSET_DEFAULT_SIZES);
    ...
    ctx->cache_count = 0;
    ctx->expr_cache = MemoryContextAllocZero(jit_ctx, sizeof(JitCompiledExpr *) * ctx->cache_size);
...
JitExprNode *jit_expr_column(JitContext *ctx, int32 column_index, VectorizedDataType data_type)
{
...
static JitExprNode *jit_expr_binary(JitContext *ctx, JitExprType type, JitExprNode *left,
                                    JitExprNode *right, VectorizedDataType result_type)
{
...
static JitExprNode *jit_expr_unary(JitContext *ctx, JitExprType type, JitExprNode *operand,
                                   VectorizedDataType result_type)
{
    JitExprNode *node = jit_expr_alloc(ctx, type, result_type);
    node->left = operand;
...
static VectorizedDataType jit_arith_result_type(VectorizedDataType left, VectorizedDataType right)
{
    /* Float64 dominates */
...
JitExprNode *jit_expr_add(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    VectorizedDataType result_type = jit_arith_result_type(left->result_type, right->result_type);
    return jit_expr_binary(ctx, JIT_EXPR_ARITH_ADD, left, right, result_type);
}

JitExprNode *jit_expr_sub(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    VectorizedDataType result_type = jit_arith_result_type(left->result_type, right->result_type);
    return jit_expr_binary(ctx, JIT_EXPR_ARITH_SUB, left, right, result_type);
}

JitExprNode *jit_expr_mul(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    VectorizedDataType result_type = jit_arith_result_type(left->result_type, right->result_type);
    return jit_expr_binary(ctx, JIT_EXPR_ARITH_MUL, left, right, result_type);
}

JitExprNode *jit_expr_div(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
...
JitExprNode *jit_expr_mod(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
...
JitExprNode *jit_expr_eq(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    return jit_expr_binary(ctx, JIT_EXPR_CMP_EQ, left, right, VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_ne(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    return jit_expr_binary(ctx, JIT_EXPR_CMP_NE, left, right, VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_lt(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    return jit_expr_binary(ctx, JIT_EXPR_CMP_LT, left, right, VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_le(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    return jit_expr_binary(ctx, JIT_EXPR_CMP_LE, left, right, VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_gt(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    return jit_expr_binary(ctx, JIT_EXPR_CMP_GT, left, right, VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_ge(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    return jit_expr_binary(ctx, JIT_EXPR_CMP_GE, left, right, VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_and(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
    return jit_expr_binary(ctx, JIT_EXPR_BOOL_AND, left, right, VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_or(JitContext *ctx, JitExprNode *left, JitExprNode *right)
{
...
JitExprNode *jit_expr_between(JitContext *ctx, JitExprNode *value, JitExprNode *low,
                              JitExprNode *high)
{
    JitExprNode *node;
    MemoryContext old_ctx;
...
JitExprNode *jit_expr_cast(JitContext *ctx, JitExprNode *operand, VectorizedDataType target_type)
{
...
JitCompiledExpr *jit_compile_expression(JitContext *ctx, JitExprNode *expr)
{
    ...
    if (orochi_jit_cache_enabled) {
        for (i = 0; i < ctx->cache_count; i++) {
            if (ctx->expr_cache[i] != NULL && ctx->expr_cache[i]->expr_hash == expr_hash) {
                ctx->cache_hits++;
...
JitCompiledFilter *jit_compile_simple_filter(JitContext *ctx, int32 column_index,
...
JitCompiledFilter *jit_compile_filter(JitContext *ctx, JitExprNode *predicate)
{
    ...
    jit_extract_column_refs(predicate, filter->column_indices, &filter->column_count, JIT_MAX_ARGS);
...
void jit_execute_filter(JitCompiledFilter *filter, VectorBatch *batch)
{
    ...
    if (filter->filter_fn != NULL) {
        ...
        filter->filter_fn(batch, filter->column_indices[0], filter->predicate->const_value, mask);
...
JitCompiledAgg *jit_compile_agg(JitContext *ctx, VectorizedAggType agg_type, int32 column_index,
                                VectorizedDataType data_type)
{
    JitCompiledAgg *agg;
    MemoryContext old_ctx;
...
JitCompiledAgg *jit_compile_filtered_agg(JitContext *ctx, VectorizedAggType agg_type,
...
void jit_execute_agg_update(JitCompiledAgg *agg, VectorizedAggState *state, VectorBatch *batch)
{
...
void jit_vectorized_aggregate(VectorizedAggState *state, VectorBatch *batch, JitCompiledAgg *agg)
{
...
void jit_get_stats(JitContext *ctx, JitStats *stats)
{
    ...
    if (ctx->expressions_compiled > 0) {
        stats->avg_compile_time_us = (double)ctx->total_compile_time_us / ctx->expressions_compiled;
    }
}
...
void jit_expr_debug_print(JitExprNode *expr, int indent)
{
    ...
    elog(DEBUG1, "%s (type=%s, cost=%d, depth=%d)", jit_expr_type_name(expr->type),
         vectorized_type_name(expr->result_type), expr->cost, expr->depth);
