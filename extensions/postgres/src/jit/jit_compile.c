...
static void jit_filter_int64_eq(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int64_ne(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int64_lt(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int64_le(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int64_gt(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int64_ge(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);

/* Float64 filter templates */
static void jit_filter_float64_eq(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float64_ne(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float64_lt(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float64_le(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float64_gt(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float64_ge(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);

/* Int32 filter templates */
static void jit_filter_int32_eq(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int32_ne(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int32_lt(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int32_le(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int32_gt(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);
static void jit_filter_int32_ge(VectorBatch *batch, int32 col_idx, Datum val,
                                bool *mask);

/* Float32 filter templates */
static void jit_filter_float32_eq(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float32_ne(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float32_lt(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float32_le(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float32_gt(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);
static void jit_filter_float32_ge(VectorBatch *batch, int32 col_idx, Datum val,
                                  bool *mask);

/* Forward declarations - Aggregation template functions */
static void jit_agg_sum_int64_update(VectorizedAggState *state,
                                     VectorBatch *batch);
static void jit_agg_sum_int32_update(VectorizedAggState *state,
                                     VectorBatch *batch);
static void jit_agg_sum_float64_update(VectorizedAggState *state,
                                       VectorBatch *batch);
static void jit_agg_sum_float32_update(VectorizedAggState *state,
                                       VectorBatch *batch);
static void jit_agg_count_update(VectorizedAggState *state, VectorBatch *batch);
static void jit_agg_min_int64_update(VectorizedAggState *state,
                                     VectorBatch *batch);
static void jit_agg_max_int64_update(VectorizedAggState *state,
                                     VectorBatch *batch);
static void jit_agg_min_float64_update(VectorizedAggState *state,
                                       VectorBatch *batch);
static void jit_agg_max_float64_update(VectorizedAggState *state,
                                       VectorBatch *batch);
...
JitContext *jit_context_create_with_options(const JitOptions *options) {
  JitContext *ctx;
  MemoryContext jit_ctx;

  /* Create dedicated memory context */
  jit_ctx = AllocSetContextCreate(CurrentMemoryContext, "JitContext",
  ...
  ctx->cache_count = 0;
  ctx->expr_cache = MemoryContextAllocZero(jit_ctx, sizeof(JitCompiledExpr *) *
                                                        ctx->cache_size);
...
void jit_context_reset(JitContext *ctx) {
  int i;
...
JitContext *jit_get_global_context(void) {
...
static JitExprNode *jit_expr_alloc(JitContext *ctx, JitExprType type,
...
JitExprNode *jit_expr_const_int64(JitContext *ctx, int64 value) {
  JitExprNode *node =
      jit_expr_alloc(ctx, JIT_EXPR_CONST, VECTORIZED_TYPE_INT64);
...
JitExprNode *jit_expr_const_int32(JitContext *ctx, int32 value) {
  JitExprNode *node =
      jit_expr_alloc(ctx, JIT_EXPR_CONST, VECTORIZED_TYPE_INT32);
...
JitExprNode *jit_expr_const_float64(JitContext *ctx, double value) {
  JitExprNode *node =
      jit_expr_alloc(ctx, JIT_EXPR_CONST, VECTORIZED_TYPE_FLOAT64);
...
JitExprNode *jit_expr_const_float32(JitContext *ctx, float value) {
  JitExprNode *node =
      jit_expr_alloc(ctx, JIT_EXPR_CONST, VECTORIZED_TYPE_FLOAT32);
...
JitExprNode *jit_expr_const_bool(JitContext *ctx, bool value) {
...
JitExprNode *jit_expr_const_null(JitContext *ctx, VectorizedDataType type) {
...
JitExprNode *jit_expr_column(JitContext *ctx, int32 column_index,
...
static JitExprNode *jit_expr_binary(JitContext *ctx, JitExprType type,
                                    JitExprNode *left, JitExprNode *right,
                                    VectorizedDataType result_type) {
...
static JitExprNode *jit_expr_unary(JitContext *ctx, JitExprType type,
                                   JitExprNode *operand,
                                   VectorizedDataType result_type) {
  JitExprNode *node = jit_expr_alloc(ctx, type, result_type);
  node->left = operand;
...
static VectorizedDataType jit_arith_result_type(VectorizedDataType left,
                                                VectorizedDataType right) {
  /* Float64 dominates */
...
JitExprNode *jit_expr_add(JitContext *ctx, JitExprNode *left,
                          JitExprNode *right) {
  VectorizedDataType result_type =
      jit_arith_result_type(left->result_type, right->result_type);
  return jit_expr_binary(ctx, JIT_EXPR_ARITH_ADD, left, right, result_type);
}

JitExprNode *jit_expr_sub(JitContext *ctx, JitExprNode *left,
                          JitExprNode *right) {
  VectorizedDataType result_type =
      jit_arith_result_type(left->result_type, right->result_type);
...
JitExprNode *jit_expr_mul(JitContext *ctx, JitExprNode *left,
                          JitExprNode *right) {
  VectorizedDataType result_type =
      jit_arith_result_type(left->result_type, right->result_type);
...
JitExprNode *jit_expr_div(JitContext *ctx, JitExprNode *left,
                          JitExprNode *right) {
  /* Division always returns float64 for accuracy */
  return jit_expr_binary(ctx, JIT_EXPR_ARITH_DIV, left, right,
                         VECTORIZED_TYPE_FLOAT64);
}

JitExprNode *jit_expr_mod(JitContext *ctx, JitExprNode *left,
                          JitExprNode *right) {
  /* Modulo operates on integers */
  return jit_expr_binary(ctx, JIT_EXPR_ARITH_MOD, left, right,
                         VECTORIZED_TYPE_INT64);
}

JitExprNode *jit_expr_neg(JitContext *ctx, JitExprNode *operand) {
  return jit_expr_unary(ctx, JIT_EXPR_ARITH_NEG, operand, operand->result_type);
}

JitExprNode *jit_expr_eq(JitContext *ctx, JitExprNode *left,
                         JitExprNode *right) {
  return jit_expr_binary(ctx, JIT_EXPR_CMP_EQ, left, right,
                         VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_ne(JitContext *ctx, JitExprNode *left,
                         JitExprNode *right) {
  return jit_expr_binary(ctx, JIT_EXPR_CMP_NE, left, right,
                         VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_lt(JitContext *ctx, JitExprNode *left,
                         JitExprNode *right) {
  return jit_expr_binary(ctx, JIT_EXPR_CMP_LT, left, right,
                         VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_le(JitContext *ctx, JitExprNode *left,
                         JitExprNode *right) {
  return jit_expr_binary(ctx, JIT_EXPR_CMP_LE, left, right,
                         VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_gt(JitContext *ctx, JitExprNode *left,
                         JitExprNode *right) {
  return jit_expr_binary(ctx, JIT_EXPR_CMP_GT, left, right,
                         VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_ge(JitContext *ctx, JitExprNode *left,
                         JitExprNode *right) {
  return jit_expr_binary(ctx, JIT_EXPR_CMP_GE, left, right,
                         VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_and(JitContext *ctx, JitExprNode *left,
                          JitExprNode *right) {
  return jit_expr_binary(ctx, JIT_EXPR_BOOL_AND, left, right,
                         VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_or(JitContext *ctx, JitExprNode *left,
                         JitExprNode *right) {
  return jit_expr_binary(ctx, JIT_EXPR_BOOL_OR, left, right,
                         VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_not(JitContext *ctx, JitExprNode *operand) {
  return jit_expr_unary(ctx, JIT_EXPR_BOOL_NOT, operand, VECTORIZED_TYPE_BOOL);
}

JitExprNode *jit_expr_is_null(JitContext *ctx, JitExprNode *operand) {
  JitExprNode *node =
      jit_expr_unary(ctx, JIT_EXPR_IS_NULL, operand, VECTORIZED_TYPE_BOOL);
  node->is_nullable = false; /* IS NULL result is never null */
  return node;
}

JitExprNode *jit_expr_is_not_null(JitContext *ctx, JitExprNode *operand) {
  JitExprNode *node =
      jit_expr_unary(ctx, JIT_EXPR_IS_NOT_NULL, operand, VECTORIZED_TYPE_BOOL);
  node->is_nullable = false; /* IS NOT NULL result is never null */
  return node;
}

JitExprNode *jit_expr_between(JitContext *ctx, JitExprNode *value,
...
JitExprNode *jit_expr_cast(JitContext *ctx, JitExprNode *operand,
...
void jit_expr_free(JitContext *ctx, JitExprNode *node) {
  int i;
...
bool jit_can_compile_expression(JitExprNode *expr) {
  if (expr == NULL)
    return false;
...
int32 jit_estimate_expression_cost(JitExprNode *expr) {
  if (expr == NULL)
    return 0;
...
JitCompiledExpr *jit_compile_expression(JitContext *ctx, JitExprNode *expr) {
...
JitCompiledFilter *jit_compile_simple_filter(JitContext *ctx,
                                             int32 column_index,
...
JitCompiledFilter *jit_compile_filter(JitContext *ctx, JitExprNode *predicate) {
  JitCompiledFilter *filter;
  ...
  jit_extract_column_refs(predicate, filter->column_indices,
                          &filter->column_count, JIT_MAX_ARGS);
...
void jit_execute_filter(JitCompiledFilter *filter, VectorBatch *batch) {
  bool *mask;
  int32 i;
...
void jit_filter_free(JitContext *ctx, JitCompiledFilter *filter) {
  if (filter == NULL)
    return;
...
JitCompiledAgg *jit_compile_agg(JitContext *ctx, VectorizedAggType agg_type,
                                int32 column_index,
                                VectorizedDataType data_type) {
  JitCompiledAgg *agg;
  MemoryContext old_ctx;
...
JitCompiledAgg *jit_compile_filtered_agg(JitContext *ctx,
...
void jit_execute_agg_update(JitCompiledAgg *agg, VectorizedAggState *state,
                            VectorBatch *batch) {
  ...
  agg->total_rows +=
      batch->has_selection ? batch->selected_count : batch->count;
}

void jit_agg_free(JitContext *ctx, JitCompiledAgg *agg) {
  if (agg == NULL)
    return;
...
void jit_vectorized_filter(VectorBatch *batch, JitCompiledFilter *filter) {
...
void jit_vectorized_aggregate(VectorizedAggState *state, VectorBatch *batch,
                              JitCompiledAgg *agg) {
...
bool jit_should_use_jit(VectorBatch *batch, int32 expr_cost) {
...
static void jit_agg_count_update(VectorizedAggState *state,
                                 VectorBatch *batch) {
...
static void jit_agg_avg_update(VectorizedAggState *state, VectorBatch *batch) {
  ...
  case VECTORIZED_TYPE_TIMESTAMP:
    state->sum_float64 +=
        (double)vectorized_sum_int64(batch, state->column_index);
    break;
  case VECTORIZED_TYPE_INT32:
  case VECTORIZED_TYPE_DATE:
    state->sum_float64 +=
        (double)vectorized_sum_int32(batch, state->column_index);
    break;
  case VECTORIZED_TYPE_FLOAT64:
    state->sum_float64 += vectorized_sum_float64(batch, state->column_index);
    break;
...
void jit_get_stats(JitContext *ctx, JitStats *stats) {
  ...
  if (ctx->expressions_compiled > 0) {
    stats->avg_compile_time_us =
        (double)ctx->total_compile_time_us / ctx->expressions_compiled;
  }
}

void jit_reset_stats(JitContext *ctx) {
  if (ctx == NULL)
    return;
...
const char *jit_expr_type_name(JitExprType type) {
  switch (type) {
  case JIT_EXPR_CONST:
    return "CONST";
  case JIT_EXPR_COLUMN:
...
void jit_expr_debug_print(JitExprNode *expr, int indent) {
  int i;
  ...
  elog(DEBUG1, "%s (type=%s, cost=%d, depth=%d)",
       jit_expr_type_name(expr->type), vectorized_type_name(expr->result_type),
       expr->cost, expr->depth);

  if (expr->type == JIT_EXPR_CONST) {
    elog(DEBUG1, "  value=%ld, is_null=%s", (long)expr->const_value,
