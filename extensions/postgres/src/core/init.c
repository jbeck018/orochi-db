...
void _PG_init(void)
{
    ...
    if (process_shared_preload_libraries_in_progress) {
        ...
        ereport(WARNING, (errmsg("orochi extension should be loaded via shared_preload_libraries"),
...
static void orochi_define_gucs(void)
{
    ...
    DefineCustomIntVariable("orochi.default_shard_count",
                            "Default number of shards for distributed tables", NULL,
                            &orochi_default_shard_count, 32, /* default */
                            1,                               /* min */
                            1024,                            /* max */
                            PGC_USERSET, 0, NULL, NULL, NULL);

    /* Columnar storage configuration */
    DefineCustomIntVariable("orochi.stripe_row_limit", "Maximum rows per columnar stripe", NULL,
                            &orochi_stripe_row_limit, 150000, 1000, 10000000, PGC_USERSET, 0, NULL,
                            NULL, NULL);

    /* Time-series configuration */
    DefineCustomIntVariable("orochi.chunk_time_interval_hours",
                            "Default chunk interval for hypertables (hours)", NULL,
                            &orochi_chunk_time_interval_hours, 24, 1, 8760, /* 1 year */
                            PGC_USERSET, 0, NULL, NULL, NULL);

    /* Compression configuration */
    DefineCustomIntVariable("orochi.compression_level", "Default compression level (1-19 for ZSTD)",
                            NULL, &orochi_compression_level, 3, 1, 19, PGC_USERSET, 0, NULL, NULL,
                            NULL);

    /* S3 configuration */
    DefineCustomStringVariable("orochi.s3_endpoint", "S3 endpoint URL for cold storage", NULL,
                               &orochi_s3_endpoint, NULL, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomStringVariable("orochi.s3_bucket", "S3 bucket for cold storage", NULL,
                               &orochi_s3_bucket, NULL, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomStringVariable("orochi.s3_access_key", "S3 access key", NULL, &orochi_s3_access_key,
                               NULL, PGC_SIGHUP, GUC_SUPERUSER_ONLY, NULL, NULL, NULL);

    DefineCustomStringVariable("orochi.s3_secret_key", "S3 secret key", NULL, &orochi_s3_secret_key,
                               NULL, PGC_SIGHUP, GUC_SUPERUSER_ONLY, NULL, NULL, NULL);

    DefineCustomStringVariable("orochi.s3_region", "S3 region", NULL, &orochi_s3_region,
                               "us-east-1", PGC_SIGHUP, 0, NULL, NULL, NULL);
    ...
    DefineCustomBoolVariable("orochi.enable_parallel_query",
                             ...
                             &orochi_enable_parallel_query, true, PGC_USERSET, 0, NULL, NULL, NULL);

    DefineCustomIntVariable("orochi.parallel_workers", "Number of parallel workers per shard", NULL,
                            &orochi_parallel_workers, 4, 1, 64, PGC_USERSET, 0, NULL, NULL, NULL);

    /* Tiered storage configuration */
    DefineCustomBoolVariable("orochi.enable_tiering", "Enable automatic data tiering", NULL,
                             &orochi_enable_tiering, true, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomIntVariable(
        "orochi.hot_threshold_hours", "Hours until data moves from hot to warm tier", NULL,
        &orochi_hot_threshold_hours, 24, 1, 720, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomIntVariable(
        "orochi.warm_threshold_days", "Days until data moves from warm to cold tier", NULL,
        &orochi_warm_threshold_days, 7, 1, 365, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomIntVariable(
        "orochi.cold_threshold_days", "Days until data moves from cold to frozen tier", NULL,
        &orochi_cold_threshold_days, 30, 7, 3650, PGC_SIGHUP, 0, NULL, NULL, NULL);
...
static void orochi_shmem_startup(void)
{
    ...
    ShmemInitStruct("orochi_columnar_cache", (Size)orochi_cache_size_mb * 1024 * 1024, &found);
...
Datum orochi_initialize_sql(PG_FUNCTION_ARGS)
{
    ...
    shmem_available = process_shared_preload_libraries_in_progress || (orochi_shmem_size > 0);
