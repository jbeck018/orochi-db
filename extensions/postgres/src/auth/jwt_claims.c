...
static const char *reserved_claims[] = {
 ...
 */
static bool
is_reserved_claim(const char *claim_name)
{
    const char **reserved;
 ...
 */
static bool
validate_claim_name(const char *claim_name)
{
    size_t len;
    const char *p;
 ...
 */
static Jsonb *
extract_jsonb_path(Jsonb *jsonb, const char *path)
{
    char *path_copy;
    char *token;
    char *saveptr;
 ...
 */
static ClaimSourceType
source_type_from_string(const char *str)
{
    if (str == NULL)
 ...
 */
static Jsonb *
create_empty_claims(void)
{
    return DatumGetJsonbP(
 ...
 */
static Jsonb *
add_string_claim(Jsonb *claims, const char *name, const char *value)
{
 ...
 */
static Jsonb *
add_numeric_claim(Jsonb *claims, const char *name, int64 value)
{
    JsonbParseState *state = NULL;
    JsonbIterator *it;
    JsonbValue v;
    ...
    v.type = jbvNumeric;
    v.val.numeric = DatumGetNumeric(
        DirectFunctionCall1(int8_numeric, Int64GetDatum(value)));
    pushJsonbValue(&state, WJB_VALUE, &v);
 ...
 */
static Jsonb *
add_bool_claim(Jsonb *claims, const char *name, bool value)
{
    JsonbParseState *state = NULL;
    JsonbIterator *it;
    JsonbValue v;
 ...
 */
static Jsonb *
add_jsonb_claim(Jsonb *claims, const char *name, Jsonb *value)
{
    JsonbParseState *state = NULL;
    JsonbIterator *it;
    JsonbValue v;
 ...
 */
static Jsonb *
remove_claim(Jsonb *claims, const char *name)
{
    JsonbParseState *state = NULL;
    JsonbIterator *it;
    JsonbValue v;
 ...
 */
static Jsonb *
merge_jsonb(Jsonb *base, Jsonb *overlay)
{
    Datum result;
    ...
    result = DirectFunctionCall2(jsonb_concat,
                                  JsonbPGetDatum(base),
                                  JsonbPGetDatum(overlay));
 ...
 */
static List *
load_custom_claims_config(void)
{
    List *configs = NIL;
    int ret;
    uint64 proc;
    ...
    ret = SPI_execute(
        "SELECT claim_name, source_type, source_path, source_function, "
 ...
 */
static Jsonb *
load_user_claims(const char *user_id)
{
 ...
 */
static Jsonb *
execute_claim_function(const char *function_name, const char *user_id)
{
    int ret;
    char *query;
    Datum result;
 ...
 */
static Jsonb *
resolve_custom_claim(CustomClaimConfig *config, Jsonb *user_metadata,
 ...
 */
static Jsonb *
build_custom_claims(const char *user_id, Jsonb *user_metadata, Jsonb *app_metadata,
                    bool for_access_token)
{
    ...
    foreach(lc, configs)
    {
        ...
        {
            ereport(WARNING,
                    (errmsg("Required custom claim '%s' could not be resolved",
                            config->claim_name)));
        }
    }
 ...
 *
 * Usage: SELECT orochi_jwt_build_claims(user_id, email, role, session_id, exp_seconds)
 */
Datum
orochi_jwt_build_claims(PG_FUNCTION_ARGS)
{
    text *user_id_text;
    text *email_text;
    text *role_text;
 ...
 */
Datum
orochi_jwt_merge_claims(PG_FUNCTION_ARGS)
{
    Jsonb *base;
    Jsonb *overlay;
    Jsonb *result;
 ...
 */
Datum
orochi_jwt_add_claim(PG_FUNCTION_ARGS)
{
    Jsonb *claims;
    text *name_text;
    Jsonb *value;
    ...
    {
        ereport(WARNING,
                (errmsg("Overwriting reserved claim: %s", name)));
    }
 ...
 */
Datum
orochi_jwt_remove_claim(PG_FUNCTION_ARGS)
{
    Jsonb *claims;
    text *name_text;
    char *name;
    ...
    {
        ereport(WARNING,
                (errmsg("Removing reserved claim: %s", name)));
    }
 ...
 */
Datum
orochi_jwt_validate_claims(PG_FUNCTION_ARGS)
{
    Jsonb *claims;
    ...
    if (exp_val == NULL)
        ereport(WARNING,
                (errmsg("Claims do not contain 'exp' (expiration) claim")));
 ...
 *
 * Usage: SELECT orochi_jwt_get_custom_claims(user_id, user_metadata, app_metadata, for_access_token)
 */
Datum
orochi_jwt_get_custom_claims(PG_FUNCTION_ARGS)
{
    text *user_id_text;
    Jsonb *user_metadata;
    Jsonb *app_metadata;
    ...
    custom_claims = build_custom_claims(user_id, user_metadata, app_metadata,
                                         for_access_token);
 ...
 */
Datum
orochi_jwt_extract_claim_path(PG_FUNCTION_ARGS)
{
    Jsonb *jsonb;
    text *path_text;
    char *path;
