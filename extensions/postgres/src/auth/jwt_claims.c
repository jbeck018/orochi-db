...
static const char *reserved_claims[] = {"iss", "sub", "aud", "exp",
                                        "nbf", "iat", "jti", NULL};
...
static Jsonb *extract_jsonb_path(Jsonb *jsonb, const char *path) {
  ...
  while (token != NULL) {
    current =
        getKeyJsonValueFromContainer(container, token, strlen(token), NULL);
 ...
 */
static Jsonb *add_string_claim(Jsonb *claims, const char *name,
                               const char *value) {
...
static Jsonb *add_numeric_claim(Jsonb *claims, const char *name, int64 value) {
  ...
  v.type = jbvNumeric;
  v.val.numeric =
      DatumGetNumeric(DirectFunctionCall1(int8_numeric, Int64GetDatum(value)));
  pushJsonbValue(&state, WJB_VALUE, &v);
...
static Jsonb *merge_jsonb(Jsonb *base, Jsonb *overlay) {
  ...
  result = DirectFunctionCall2(jsonb_concat, JsonbPGetDatum(base),
 ...
 */
static Jsonb *execute_claim_function(const char *function_name,
                                     const char *user_id) {
  int ret;
  char *query;
  Datum result;
 ...
 */
static Jsonb *resolve_custom_claim(CustomClaimConfig *config,
 ...
 */
static Jsonb *build_custom_claims(const char *user_id, Jsonb *user_metadata,
                                  Jsonb *app_metadata, bool for_access_token) {
  List *configs;
  ListCell *lc;
  Jsonb *custom_claims;
  Jsonb *user_claims;
...
Datum orochi_jwt_build_claims(PG_FUNCTION_ARGS) {
  ...
  if (!PG_ARGISNULL(3)) {
    ...
    claims = add_string_claim(claims, "session_id",
                              text_to_cstring(session_id_text));
  }
