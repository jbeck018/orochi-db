...
OrochiJwtClaims *orochi_jwt_parse_claims(const char *json_str) {
  ...
  if (found)
    claims->standard.exp =
        orochi_unix_to_timestamptz(claims->standard.exp_unix);

  claims->standard.nbf_unix = extract_jsonb_int64(jsonb, "nbf", &found);
  if (found)
    claims->standard.nbf =
        orochi_unix_to_timestamptz(claims->standard.nbf_unix);

  claims->standard.iat_unix = extract_jsonb_int64(jsonb, "iat", &found);
  if (found)
    claims->standard.iat =
        orochi_unix_to_timestamptz(claims->standard.iat_unix);
 ...
 */
static bool verify_hmac_signature(const char *data, Size data_len,
 ...
 */
static bool verify_rsa_signature(const char *data, Size data_len,
                                 const unsigned char *signature, Size sig_len,
 ...
 */
static unsigned char *ecdsa_sig_to_der(const unsigned char *jwt_sig,
                                       Size jwt_sig_len, int coord_size,
                                       Size *der_len) {
  ECDSA_SIG *sig = NULL;
 ...
 */
static bool verify_ecdsa_signature(const char *data, Size data_len,
                                   const unsigned char *signature, Size sig_len,
                                   const char *public_key_pem,
                                   OrochiJwtAlgorithm alg) {
  const EVP_MD *md;
 ...
 */
bool orochi_jwt_verify_signature(const char *header_payload,
                                 const char *signature_b64, const char *secret,
                                 OrochiJwtAlgorithm alg) {
...
OrochiJwt *orochi_jwt_parse(const char *token) {
  ...
  *second_dot = '\0';
  ...
  /* Decode signature */
  jwt->signature =
      orochi_base64url_decode(jwt->signature_b64, &jwt->signature_len);
 ...
 */
OrochiJwt *orochi_jwt_decode(const char *token, const char *secret,
...
OrochiJwt *orochi_jwt_decode_with_options(const char *token, const char *secret,
  ...
  if (options->verify_exp &&
      orochi_jwt_is_expired(jwt, options->clock_skew_seconds)) {
  ...
  if (options->verify_nbf &&
      orochi_jwt_is_not_yet_valid(jwt, options->clock_skew_seconds)) {
 ...
 */
int64 orochi_jwt_get_claim_int(OrochiJwt *jwt, const char *claim_name,
                               bool *is_null) {
  if (jwt == NULL || jwt->claims == NULL || jwt->claims->raw_payload == NULL) {
    *is_null = true;
    return 0;
 ...
 */
bool orochi_jwt_get_claim_bool(OrochiJwt *jwt, const char *claim_name,
                               bool *is_null) {
  JsonbValue *val;
...
void orochi_jwt_set_session_context(OrochiJwt *jwt) {
  ...
  set_session_variable(JWT_CLAIM_PREFIX "role", jwt->claims->custom.role
                                                    ? jwt->claims->custom.role
                                                    : "authenticated");
  set_session_variable(JWT_CLAIM_PREFIX "session_id",
                       jwt->claims->custom.session_id);
  set_session_variable(JWT_CLAIM_PREFIX "aal", jwt->claims->custom.aal
                                                   ? jwt->claims->custom.aal
                                                   : "aal1");
...
Datum orochi_jwt_encode(PG_FUNCTION_ARGS) {
  ...
  (void)JsonbToCString(&payload_json, &claims_jsonb->root,
                       VARSIZE(claims_jsonb));
  ...
  if (HMAC(md, secret, strlen(secret), (unsigned char *)header_payload,
           strlen(header_payload), hmac_result, &hmac_len) == NULL) {
    pfree(header_json.data);
    pfree(payload_json.data);
    pfree(header_b64);
    pfree(payload_b64);
