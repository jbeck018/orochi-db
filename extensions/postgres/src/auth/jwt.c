 ...
 */
char *
orochi_base64url_decode(const char *input, Size *output_len)
{
    Size input_len;
    Size padding;
    Size decoded_len;
 ...
 */
char *
orochi_base64url_encode(const unsigned char *input, Size input_len)
{
    Size output_len;
    char *output;
    Size i, j;
 ...
 */
OrochiJwtAlgorithm
orochi_jwt_algorithm_from_string(const char *alg_str)
{
    if (alg_str == NULL)
 ...
 */
const char *
orochi_jwt_algorithm_to_string(OrochiJwtAlgorithm alg)
{
    switch (alg)
    {
 ...
 */
const char *
orochi_jwt_status_message(OrochiJwtStatus status)
{
    switch (status)
    {
 ...
 */
TimestampTz
orochi_unix_to_timestamptz(int64 unix_timestamp)
{
 ...
 */
int64
orochi_timestamptz_to_unix(TimestampTz ts)
{
    int64 pg_epoch_offset = (POSTGRES_EPOCH_JDATE - UNIX_EPOCH_JDATE) * SECS_PER_DAY;
 ...
 */
static int64
get_current_unix_timestamp(void)
{
 ...
 */
OrochiJwtHeader *
orochi_jwt_parse_header(const char *json_str)
{
 ...
 */
static char *
extract_jsonb_string(Jsonb *jsonb, const char *key)
{
    JsonbValue *val;
 ...
 */
static int64
extract_jsonb_int64(Jsonb *jsonb, const char *key, bool *found)
{
    JsonbValue *val;
 ...
 */
static Jsonb *
extract_jsonb_object(Jsonb *jsonb, const char *key)
{
    JsonbValue *val;
    JsonbValue jbv;
    ...
    if (val != NULL && (val->type == jbvObject || val->type == jbvArray ||
                        val->type == jbvBinary))
    {
 ...
 */
OrochiJwtClaims *
orochi_jwt_parse_claims(const char *json_str)
{
 ...
 */
static const EVP_MD *
get_hmac_algorithm(OrochiJwtAlgorithm alg)
{
    switch (alg)
    {
        case OROCHI_JWT_HS256:
 ...
 */
static bool
verify_hmac_signature(const char *data, Size data_len,
 ...
 */
bool
orochi_jwt_verify_signature(const char *header_payload,
 ...
 */
OrochiJwt *
orochi_jwt_parse(const char *token)
{
    OrochiJwt *jwt;
    char *token_copy;
 ...
 */
bool
orochi_jwt_is_expired(OrochiJwt *jwt, int64 clock_skew_seconds)
{
    int64 now;
    int64 exp;
 ...
 */
bool
orochi_jwt_is_not_yet_valid(OrochiJwt *jwt, int64 clock_skew_seconds)
{
    int64 now;
    int64 nbf;
 ...
 */
OrochiJwt *
orochi_jwt_decode(const char *token,
                  const char *secret,
 ...
 */
OrochiJwt *
orochi_jwt_decode_with_options(const char *token,
                                const char *secret,
                                OrochiJwtAlgorithm expected_alg,
                                OrochiJwtValidationOptions *options)
{
    ...
    if (options->verify_signature)
    {
        ...
        if (!orochi_jwt_verify_signature(header_payload, jwt->signature_b64,
                                         secret, expected_alg))
        {
 ...
 */
char *
orochi_jwt_get_claim_text(OrochiJwt *jwt, const char *claim_name)
{
 ...
 */
Jsonb *
orochi_jwt_get_claim_jsonb(OrochiJwt *jwt, const char *claim_name)
{
    JsonbValue *val;
    ...
    val = getKeyJsonValueFromContainer(&jwt->claims->raw_payload->root,
                                        claim_name, strlen(claim_name), NULL);
    if (val == NULL)
        return NULL;
 ...
 */
bool
orochi_jwt_has_claim(OrochiJwt *jwt, const char *claim_name)
{
    JsonbValue *val;
 ...
 */
int64
orochi_jwt_get_claim_int(OrochiJwt *jwt, const char *claim_name, bool *is_null)
{
    if (jwt == NULL || jwt->claims == NULL || jwt->claims->raw_payload == NULL)
    {
        *is_null = true;
 ...
 */
bool
orochi_jwt_get_claim_bool(OrochiJwt *jwt, const char *claim_name, bool *is_null)
{
    JsonbValue *val;
 ...
 */
static void
set_session_variable(const char *name, const char *value)
{
    if (value != NULL)
 ...
 */
void
orochi_jwt_set_session_context(OrochiJwt *jwt)
{
 ...
 */
void
orochi_jwt_clear_session_context(void)
{
 ...
 */
char *
orochi_jwt_get_session_claim(const char *claim_name)
{
    char *setting_name;
    const char *value;
 ...
 */
Datum
orochi_jwt_verify(PG_FUNCTION_ARGS)
{
    text *token_text;
    text *secret_text;
    char *token;
 ...
 */
Datum
orochi_jwt_get_claim(PG_FUNCTION_ARGS)
{
    text *token_text;
 ...
 */
Datum
orochi_jwt_clear_auth_context(PG_FUNCTION_ARGS)
{
    orochi_jwt_clear_session_context();
    PG_RETURN_VOID();
}
 ...
 */
Datum
orochi_jwt_encode(PG_FUNCTION_ARGS)
{
    Jsonb *claims_jsonb;
    text *secret_text;
    text *alg_text;
    ...
    header_b64 = orochi_base64url_encode((unsigned char *)header_json.data,
                                          header_json.len);
    payload_b64 = orochi_base64url_encode((unsigned char *)payload_json.data,
                                           payload_json.len);
    ...
    if (md == NULL)
    {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
                 errmsg("Unsupported JWT algorithm: %s",
                        orochi_jwt_algorithm_to_string(alg))));
    }
