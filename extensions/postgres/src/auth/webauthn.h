...
extern WebAuthnChallenge *webauthn_create_authentication_challenge(
    const char *user_email,
    const char *rp_id,
    const char *origin);

extern bool webauthn_validate_challenge(
    pg_uuid_t challenge_id,
...
extern bool webauthn_validate_sign_count(
    OrochiWebAuthnCredential *credential,
    int64 new_sign_count);

/* Credential management */
extern OrochiWebAuthnCredential *webauthn_get_credential(
    pg_uuid_t credential_id);

extern OrochiWebAuthnCredential *webauthn_get_credential_by_webauthn_id(
    const uint8 *webauthn_credential_id,
    int32 credential_id_len);

extern List *webauthn_get_user_credentials(pg_uuid_t user_id);

extern bool webauthn_revoke_credential(
    pg_uuid_t user_id,
    pg_uuid_t credential_id,
    const char *reason);

extern bool webauthn_suspend_credential(
    pg_uuid_t credential_id,
    const char *reason);
...
extern int32 webauthn_extract_sign_count(
    const uint8 *authenticator_data,
    int32 data_len);

extern bool webauthn_extract_flags(
    const uint8 *authenticator_data,
    int32 data_len,
    bool *user_present,
    bool *user_verified,
