...
void webauthn_init(void) {
  if (WebAuthnContext == NULL) {
    WebAuthnContext = AllocSetContextCreate(TopMemoryContext, "WebAuthnContext",
 ...
 */
Size webauthn_shmem_size(void) { return sizeof(WebAuthnSharedState); }
 ...
 * registration
 */
WebAuthnChallenge *webauthn_create_registration_challenge(
  ...
  challenge->expires_at = TimestampTzPlusMilliseconds(
      challenge->created_at, WEBAUTHN_CHALLENGE_TTL_SECONDS * 1000);
 ...
 * authentication
 */
WebAuthnChallenge *webauthn_create_authentication_challenge(
    const char *user_email, const char *rp_id, const char *origin) {
  MemoryContext old_context;
  WebAuthnChallenge *challenge;
  StringInfoData query;
  pg_uuid_t user_id;
  ...
  if (user_found) {
    ...
    appendStringInfo(&query,
                     ...
                     "NOW() + INTERVAL '%d seconds')",
                     uuid_to_string(user_id), rp_id, origin,
                     WEBAUTHN_CHALLENGE_TTL_SECONDS);
 ...
 */
bool webauthn_validate_challenge(pg_uuid_t challenge_id,
  ...
  appendStringInfo(&query,
                   ...
                   expected_type == WEBAUTHN_CHALLENGE_REGISTRATION
                       ? "registration"
                       : "authentication");
...
WebAuthnRegistrationResult *webauthn_complete_registration(
  ...
  result =
      (WebAuthnRegistrationResult *)palloc0(sizeof(WebAuthnRegistrationResult));
  ...
  elog(LOG, "WebAuthn credential registered for user %s",
       uuid_to_string(user_id));
 ...
 */
WebAuthnAuthenticationResult *webauthn_verify_authentication(
    pg_uuid_t challenge_id, const uint8 *credential_id, int32 credential_id_len,
  ...
  result = (WebAuthnAuthenticationResult *)palloc0(
      sizeof(WebAuthnAuthenticationResult));
  ...
  credential =
      webauthn_get_credential_by_webauthn_id(credential_id, credential_id_len);
  ...
  if (memcmp(&credential->user_id, &challenge_user_id, sizeof(pg_uuid_t)) !=
      0) {
    result->result = WEBAUTHN_VERIFY_CREDENTIAL_NOT_FOUND;
  ...
  /* Verify signature */
  if (!webauthn_verify_signature(
          credential->public_key_spki, credential->public_key_len,
          credential->public_key_algorithm, authenticator_data,
          authenticator_data_len, client_data_hash, signature, signature_len)) {
  ...
  new_sign_count =
      webauthn_extract_sign_count(authenticator_data, authenticator_data_len);
 ...
 */
bool webauthn_verify_signature(const uint8 *public_key, int32 public_key_len,
                               int32 algorithm, const uint8 *authenticator_data,
                               int32 authenticator_data_len,
                               const uint8 *client_data_hash,
                               const uint8 *signature, int32 signature_len) {
  EVP_PKEY *pkey = NULL;
 ...
 */
bool webauthn_validate_sign_count(OrochiWebAuthnCredential *credential,
                                  int64 new_sign_count) {
  /*
  ...
  if (credential->sign_count > 0 && new_sign_count <= credential->sign_count) {
    ...
    elog(WARNING,
         "WebAuthn sign count validation failed: stored=%ld, received=%ld",
 ...
 */
int32 webauthn_extract_sign_count(const uint8 *authenticator_data,
                                  int32 data_len) {
  /*
 ...
 */
bool webauthn_extract_flags(const uint8 *authenticator_data, int32 data_len,
                            bool *user_present, bool *user_verified,
                            bool *backup_eligible, bool *backup_state) {
  uint8 flags;
    ...
    /* Parse webauthn_credential_id (bytea) */
    {
      ...
      if (!isnull) {
        ...
        credential->webauthn_credential_id =
            palloc(credential->credential_id_len);
 ...
 */
bool webauthn_revoke_credential(pg_uuid_t user_id, pg_uuid_t credential_id,
                                const char *reason) {
  StringInfoData query;
  int ret;
  int active_count;
...
void webauthn_cleanup_expired_challenges(void) {
  ...
  appendStringInfoString(
      &query, "DELETE FROM auth.webauthn_challenges WHERE expires_at < NOW()");
...
Datum webauthn_begin_registration(PG_FUNCTION_ARGS) {
  ...
  challenge = webauthn_create_registration_challenge(
      *user_id, text_to_cstring(rp_id), text_to_cstring(rp_name),
      text_to_cstring(origin), WEBAUTHN_UV_PREFERRED);
  ...
  appendStringInfo(&result,
                   "\", \"rp_id\": \"%s\", \"rp_name\": \"%s\", "
                   "\"user_id\": \"%s\", \"timeout\": %d}",
                   challenge->rp_id, challenge->rp_name,
                   uuid_to_string(*user_id),
...
Datum webauthn_begin_authentication(PG_FUNCTION_ARGS) {
  ...
  challenge = webauthn_create_authentication_challenge(
      email ? text_to_cstring(email) : NULL, text_to_cstring(rp_id),
  ...
  appendStringInfo(&result, "\", \"rp_id\": \"%s\", \"timeout\": %d}",
                   challenge->rp_id, WEBAUTHN_CHALLENGE_TTL_SECONDS * 1000);
...
Datum webauthn_revoke_credential_sql(PG_FUNCTION_ARGS) {
  ...
  bool success = webauthn_revoke_credential(
      *user_id, *credential_id, reason ? text_to_cstring(reason) : NULL);
