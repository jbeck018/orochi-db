...
webauthn_init(void)
{
    ...
    {
        WebAuthnContext = AllocSetContextCreate(TopMemoryContext,
                                                 "WebAuthnContext",
...
/*
 * webauthn_create_registration_challenge - Create challenge for credential registration
 */
WebAuthnChallenge *
webauthn_create_registration_challenge(pg_uuid_t user_id,
                                       const char *rp_id,
                                       const char *rp_name,
                                       const char *origin,
                                       WebAuthnUserVerification user_verification)
{
    ...
    challenge->expires_at = TimestampTzPlusMilliseconds(challenge->created_at,
    ...
    appendStringInfo(&query,
        ...
        user_verification == WEBAUTHN_UV_PREFERRED ? "preferred" : "discouraged",
    ...
    elog(DEBUG1, "Created WebAuthn registration challenge for user %s",
         uuid_to_string(user_id));
...
/*
 * webauthn_create_authentication_challenge - Create challenge for authentication
 */
WebAuthnChallenge *
                                         ...
                                         const char *origin)
{
    ...
    if (user_email != NULL && user_email[0] != '\0')
    {
        initStringInfo(&query);
        appendStringInfo(&query,
            "SELECT user_id FROM auth.users WHERE email = '%s' AND status = 'active'",
            user_email);
 ...
 */
bool
webauthn_validate_challenge(pg_uuid_t challenge_id, WebAuthnChallengeType expected_type)
{
    StringInfoData query;
    bool is_valid = false;
    int ret;
    ...
    appendStringInfo(&query,
        ...
        expected_type == WEBAUTHN_CHALLENGE_REGISTRATION ? "registration" : "authentication");
 ...
 */
void
webauthn_consume_challenge(pg_uuid_t challenge_id)
{
    StringInfoData query;
...
webauthn_complete_registration(pg_uuid_t challenge_id,
                               const uint8 *credential_id,
                               int32 credential_id_len,
                               const uint8 *public_key,
                               int32 public_key_len,
                               int32 algorithm,
                               int64 sign_count,
                               const WebAuthnTransport *transports,
                               int transport_count,
                               const char *device_name)
{
    ...
    for (i = 0; i < transport_count; i++)
    {
        ...
        appendStringInfo(&transports_array, "'%s'",
                        webauthn_transport_name(transports[i]));
    }
    ...
    appendStringInfo(&query,
        ...
        "public_key_algorithm, sign_count, transports, device_name, status, created_at) "
        "VALUES ('%s', '%s', E'\\\\x%s', E'\\\\x%s', %d, %ld, %s, %s, 'active', NOW())",
        uuid_to_string(new_credential_id),
        uuid_to_string(user_id),
        cred_hex.data,
        pubkey_hex.data,
        algorithm,
        (long)sign_count,
        transports_array.data,
...
webauthn_verify_authentication(pg_uuid_t challenge_id,
                               const uint8 *credential_id,
                               int32 credential_id_len,
                               const uint8 *authenticator_data,
                               int32 authenticator_data_len,
                               ...
                               const char *user_agent)
{
    ...
    if (credential == NULL)
    {
        ...
        appendStringInfo(&query,
            ...
            "'partial_cred_id', '%s'))",
            uuid_to_string(challenge_user_id),
    ...
    webauthn_sha256((const uint8 *)client_data_json, strlen(client_data_json),
                    client_data_hash);

    /* Verify signature */
    if (!webauthn_verify_signature(credential->public_key_spki,
                                   credential->public_key_len,
    ...
    elog(LOG, "WebAuthn authentication successful for user %s",
         uuid_to_string(result->user_id));
 ...
 */
bool
webauthn_verify_signature(const uint8 *public_key,
                          int32 public_key_len,
                          int32 algorithm,
                          const uint8 *authenticator_data,
                          int32 authenticator_data_len,
                          const uint8 *client_data_hash,
                          const uint8 *signature,
                          int32 signature_len)
{
    EVP_PKEY *pkey = NULL;
 ...
 */
bool
webauthn_validate_sign_count(OrochiWebAuthnCredential *credential, int64 new_sign_count)
{
    /*
     * Per WebAuthn spec, if stored sign_count is non-zero, new count must be greater.
     * A sign_count of 0 from the authenticator means counter is not supported.
     */
    if (new_sign_count == 0)
    {
 ...
 */
int32
webauthn_extract_sign_count(const uint8 *authenticator_data, int32 data_len)
{
    /*
    ...
    return ((int32)authenticator_data[33] << 24) |
           ((int32)authenticator_data[34] << 16) |
           ((int32)authenticator_data[35] << 8) |
           ((int32)authenticator_data[36]);
}
 ...
 */
bool
webauthn_extract_flags(const uint8 *authenticator_data,
                       int32 data_len,
                       bool *user_present,
                       bool *user_verified,
 ...
 */
void
webauthn_sha256(const uint8 *data, int32 data_len, uint8 *hash_out)
{
    SHA256_CTX ctx;
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0)
    {
        ...
        /* Parse public_key_spki (bytea) */
        {
            ...
            if (!isnull)
            {
                ...
                memcpy(credential->public_key_spki, VARDATA_ANY(ba),
                       credential->public_key_len);
            }
        }
...
/*
 * webauthn_get_credential_by_webauthn_id - Retrieve by authenticator credential ID
 */
OrochiWebAuthnCredential *
                                       ...
                                       int32 credential_id_len)
{
    ...
    appendStringInfo(&query,
        ...
        "sign_count, backup_eligible, backup_state, status, last_used_at, "
        "last_used_ip, created_at, revoked_at, revoked_reason "
        "FROM auth.webauthn_credentials WHERE webauthn_credential_id = E'\\\\x%s'",
        cred_hex.data);
 ...
 */
bool
webauthn_revoke_credential(pg_uuid_t user_id, pg_uuid_t credential_id, const char *reason)
{
    StringInfoData query;
    int ret;
    int active_count;
    ...
    appendStringInfo(&query,
        ...
        uuid_to_string(user_id),
        reason ? quote_literal_cstr(reason) : "NULL",
        uuid_to_string(credential_id),
        uuid_to_string(user_id));
    ...
    elog(LOG, "WebAuthn credential %s revoked for user %s",
 ...
 */
bool
webauthn_suspend_credential(pg_uuid_t credential_id, const char *reason)
{
    StringInfoData query;
    int ret;
 ...
 */
bool
webauthn_reactivate_credential(pg_uuid_t credential_id)
{
    StringInfoData query;
    int ret;
 ...
 */
int
webauthn_get_active_credential_count(pg_uuid_t user_id)
{
    StringInfoData query;
    int ret;
    int count = 0;
 ...
 */
const char *
webauthn_credential_type_name(WebAuthnCredentialType type)
{
    switch (type)
    {
 ...
 */
const char *
webauthn_device_type_name(WebAuthnDeviceType type)
{
    switch (type)
    {
 ...
 */
const char *
webauthn_status_name(WebAuthnCredentialStatus status)
{
    switch (status)
    {
 ...
 */
const char *
webauthn_transport_name(WebAuthnTransport transport)
{
    switch (transport)
    {
 ...
 */
const char *
webauthn_verify_result_message(WebAuthnVerifyResult result)
{
    switch (result)
    {
 ...
 */
void
webauthn_cleanup_expired_challenges(void)
{
    StringInfoData query;
    int ret;
    ...
    appendStringInfoString(&query,
        "DELETE FROM auth.webauthn_challenges WHERE expires_at < NOW()");
 ...
 */
void
webauthn_free_credential(OrochiWebAuthnCredential *credential)
{
    if (credential == NULL)
        return;
 ...
 */
void
webauthn_free_challenge(WebAuthnChallenge *challenge)
{
    if (challenge == NULL)
        return;
 ...
 */
static bool
parse_uuid(const char *str, pg_uuid_t *uuid)
{
 ...
 */
Datum
webauthn_begin_registration(PG_FUNCTION_ARGS)
{
    ...
    challenge = webauthn_create_registration_challenge(
        *user_id,
        text_to_cstring(rp_id),
        text_to_cstring(rp_name),
        text_to_cstring(origin),
        WEBAUTHN_UV_PREFERRED);

    /* Build JSON response */
    initStringInfo(&result);
    appendStringInfo(&result,
        "{\"challenge_id\": \"%s\", \"challenge\": \"",
        uuid_to_string(challenge->challenge_id));
 ...
 */
Datum
webauthn_begin_authentication(PG_FUNCTION_ARGS)
{
    ...
    challenge = webauthn_create_authentication_challenge(
        email ? text_to_cstring(email) : NULL,
        text_to_cstring(rp_id),
        text_to_cstring(origin));
 ...
 */
Datum
webauthn_list_credentials(PG_FUNCTION_ARGS)
{
