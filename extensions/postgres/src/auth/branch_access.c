...
static BranchPermissionLevel resolve_inherited_permission(pg_uuid_t user_id, pg_uuid_t branch_id,
                                                          int depth);
static bool check_ip_restriction(OrochiBranchPermission *perm, const char *ip_address);
static bool check_time_restriction(OrochiBranchPermission *perm, TimestampTz check_time);
 ...
 */
Size branch_access_shmem_size(void)
{
 ...
 */
BranchPermissionLevel branch_get_permission(pg_uuid_t user_id, pg_uuid_t branch_id)
{
 ...
 */
BranchPermissionLevel branch_get_effective_permission(pg_uuid_t user_id, pg_uuid_t branch_id,
                                                      const char *ip_address)
{
    ...
    appendStringInfo(&cache_key, "%s:%s", uuid_to_string(user_id), uuid_to_string(branch_id));

    /* Check cache first */
    if (BranchPermissionCache != NULL) {
        cache_entry = hash_search(BranchPermissionCache, cache_key.data, HASH_FIND, &found);
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        char *level_str = SPI_getvalue(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1);
    ...
    if (branch->inherit_permissions && branch->parent_branch_id != NULL) {
        permission = resolve_inherited_permission(user_id, *branch->parent_branch_id, 0);
...
cache_and_return:
    /* Cache the result */
    if (BranchPermissionCache != NULL) {
        cache_entry = hash_search(BranchPermissionCache, cache_key.data, HASH_ENTER, &found);

        strncpy(cache_entry->cache_key, cache_key.data, sizeof(cache_entry->cache_key) - 1);
                                                          ...
                                                          int depth)
{
    ...
    if (branch != NULL && branch->inherit_permissions && branch->parent_branch_id != NULL) {
        permission = resolve_inherited_permission(user_id, *branch->parent_branch_id, depth + 1);
    }
 ...
 */
BranchAccessContext *branch_create_access_context(pg_uuid_t user_id, pg_uuid_t branch_id)
{
    MemoryContext old_context;
    ...
    branch = branch_get_info(branch_id);
    if (branch != NULL) {
        ctx->is_owner = (memcmp(&branch->owner_user_id, &user_id, sizeof(pg_uuid_t)) == 0);
        branch_free_info(branch);
    }
                             ...
                             TimestampTz valid_until)
{
    ...
    elog(LOG, "Granted %s permission on branch %s to user %s", branch_permission_level_name(level),
         uuid_to_string(branch_id), uuid_to_string(grantee_user_id));
 ...
 */
bool branch_revoke_permission(pg_uuid_t branch_id, pg_uuid_t user_id, pg_uuid_t revoked_by)
{
    StringInfoData query;
    int ret;
    ...
    elog(LOG, "Revoked permission on branch %s from user %s", uuid_to_string(branch_id),
 ...
 */
bool branch_update_protection(pg_uuid_t branch_id, BranchProtectionLevel level)
{
    StringInfoData query;
    int ret;
    ...
    appendStringInfo(&query,
                     ...
                     "updated_at = NOW() "
                     "WHERE branch_id = '%s'",
                     branch_protection_level_name(level), uuid_to_string(branch_id));
 ...
 */
bool branch_validate_cross_branch_access(pg_uuid_t user_id, pg_uuid_t source_branch_id,
                                         pg_uuid_t target_branch_id, CrossBranchOperation operation)
{
    ...
    case CROSS_BRANCH_COPY:
        ...
        return (source_perm >= BRANCH_PERM_READ && target_perm >= BRANCH_PERM_WRITE);
    ...
    case CROSS_BRANCH_MERGE:
        /* Need write on both */
        return (source_perm >= BRANCH_PERM_WRITE && target_perm >= BRANCH_PERM_WRITE);
...
Datum branch_validate_operation_sql(PG_FUNCTION_ARGS)
{
    ...
    valid = branch_validate_cross_branch_access(*user_id, *source_branch_id, *target_branch_id, op);
