...
extern BranchPermissionLevel branch_get_permission(pg_uuid_t user_id, pg_uuid_t branch_id);

extern BranchPermissionLevel branch_get_effective_permission(pg_uuid_t user_id, pg_uuid_t branch_id,
                                                             const char *ip_address);

extern BranchAccessContext *branch_create_access_context(pg_uuid_t user_id, pg_uuid_t branch_id);

extern bool branch_check_permission(pg_uuid_t user_id, pg_uuid_t branch_id,
                                    BranchPermissionLevel required_level);

/* Permission management */
extern bool branch_grant_permission(pg_uuid_t branch_id, pg_uuid_t grantee_user_id,
...
extern bool branch_revoke_permission(pg_uuid_t branch_id, pg_uuid_t user_id, pg_uuid_t revoked_by);

extern bool branch_revoke_all_permissions(pg_uuid_t branch_id, pg_uuid_t revoked_by);

/* Branch management */
extern OrochiBranchInfo *branch_get_info(pg_uuid_t branch_id);
extern OrochiBranchInfo *branch_get_by_slug(pg_uuid_t cluster_id, const char *slug);

extern pg_uuid_t branch_create(pg_uuid_t cluster_id, pg_uuid_t *parent_branch_id, const char *name,
                               const char *slug, pg_uuid_t owner_user_id, bool inherit_permissions);
...
extern bool branch_update_protection(pg_uuid_t branch_id, BranchProtectionLevel level);

/* Protection rules */
extern bool branch_add_protection_rule(pg_uuid_t branch_id, BranchProtectionRuleType rule_type,
                                       const char *rule_config);

extern bool branch_remove_protection_rule(pg_uuid_t branch_id, BranchProtectionRuleType rule_type);

extern bool branch_check_protection_rule(pg_uuid_t branch_id, BranchProtectionRuleType rule_type,
                                         const char *context);

extern List *branch_get_protection_rules(pg_uuid_t branch_id);

/* Cross-branch operations */
extern bool branch_validate_cross_branch_access(pg_uuid_t user_id, pg_uuid_t source_branch_id,
...
/* Shard isolation */
extern OrochiBranchShardMapping *branch_get_shard_mapping(pg_uuid_t branch_id, Oid table_oid,
                                                          int32 hash_value);

extern bool branch_register_shard_mapping(pg_uuid_t branch_id, int64 shard_id, Oid table_oid,
                                          int64 base_shard_id);

extern bool branch_update_cow_state(pg_uuid_t branch_id, int64 shard_id, BranchCowState new_state);
...
extern const char *branch_protection_rule_type_name(BranchProtectionRuleType type);
