...
static char *compute_hmac_signature(const char *payload, const char *secret,
  ...
  if (!HMAC(EVP_sha256(), secret, strlen(secret),
            (unsigned char *)signing_input, strlen(signing_input), hmac_result,
            &hmac_len)) {
    pfree(signing_input);
 ...
 */
bool orochi_auth_verify_webhook_signature(const char *payload,
  ...
  pg_timestamp =
      (timestamp - ((POSTGRES_EPOCH_JDATE - UNIX_EPOCH_JDATE) * SECS_PER_DAY)) *
      USECS_PER_SEC;
  ...
  valid =
      (strlen(expected_signature) == strlen(signature)) &&
...
orochi_auth_create_webhook_event(AuthWebhookEventType event_type,
  ...
  if (user) {
    snprintf(
        ...
        user->id.data[8], user->id.data[9], user->id.data[10],
        user->id.data[11], user->id.data[12], user->id.data[13],
        user->id.data[14], user->id.data[15]);
    ...
    payload->record = DatumGetJsonbP(
        DirectFunctionCall1(jsonb_in, CStringGetDatum(record_json.data)));
  ...
  if (old_user && event_type == AUTH_WEBHOOK_USER_UPDATED) {
    ...
    if (old_user->email)
      appendStringInfo(&old_record_json, ", \"email\": \"%s\"",
                       old_user->email);
    else
      appendStringInfoString(&old_record_json, ", \"email\": null");

    if (old_user->phone)
      appendStringInfo(&old_record_json, ", \"phone\": \"%s\"",
                       old_user->phone);
    else
    ...
    payload->old_record = DatumGetJsonbP(
        DirectFunctionCall1(jsonb_in, CStringGetDatum(old_record_json.data)));
...
char *orochi_auth_serialize_webhook_payload(AuthWebhookPayload *payload) {
  ...
  snprintf(
      event_uuid_str, sizeof(event_uuid_str),
      "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
  ...
  snprintf(
      user_uuid_str, sizeof(user_uuid_str),
      "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
...
bool orochi_auth_deliver_webhook_event(AuthWebhookPayload *payload) {
  ...
  appendStringInfoString(
      &query, "SELECT endpoint_id, url, secret, timeout_ms, max_retries, "
  ...
  for (i = 0; i < SPI_processed; i++) {
    ...
    for (attempt = 1; attempt <= endpoint.max_retries && !delivered;
         attempt++) {
...
static void log_delivery_attempt(AuthWebhookDelivery *delivery) {
  ...
  appendStringInfo(
      ...
      delivery->endpoint_id, uuid_str, delivery->attempt_number,
      delivery->http_status, delivery->success ? "TRUE" : "FALSE",
      delivery->duration_ms,
      delivery->error_message ? quote_literal_cstr(delivery->error_message)
                              : "NULL");
 ...
 */
int64 orochi_auth_create_webhook_endpoint(const char *url, const char *secret,
                                          bool *events, int timeout_ms,
                                          int max_retries) {
  StringInfoData query;
  int64 endpoint_id = 0;
  int ret;
  ...
  appendStringInfo(
      ...
      quote_literal_cstr(url), quote_literal_cstr(secret), timeout_ms,
      max_retries, events ? (events[0] ? "TRUE" : "FALSE") : "TRUE",
  ...
  if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0) {
    bool isnull;
    endpoint_id = DatumGetInt64(SPI_getbinval(
...
void orochi_auth_emit_user_created(OrochiAuthUser *user) {
  ...
  payload =
      orochi_auth_create_webhook_event(AUTH_WEBHOOK_USER_CREATED, user, NULL);
 ...
 */
void orochi_auth_emit_user_updated(OrochiAuthUser *user,
                                   OrochiAuthUser *old_user) {
  AuthWebhookPayload *payload;
  ...
  payload = orochi_auth_create_webhook_event(AUTH_WEBHOOK_USER_UPDATED, user,
                                             old_user);
  orochi_auth_deliver_webhook_event(payload);
}
...
void orochi_auth_emit_user_deleted(OrochiAuthUser *user) {
  ...
  payload =
      orochi_auth_create_webhook_event(AUTH_WEBHOOK_USER_DELETED, user, NULL);
  orochi_auth_deliver_webhook_event(payload);
}
...
void orochi_auth_emit_user_signed_in(OrochiAuthUser *user) {
  ...
  payload =
      orochi_auth_create_webhook_event(AUTH_WEBHOOK_USER_SIGNED_IN, user, NULL);
  orochi_auth_deliver_webhook_event(payload);
}
...
void orochi_auth_emit_user_signed_out(OrochiAuthUser *user) {
  ...
  payload = orochi_auth_create_webhook_event(AUTH_WEBHOOK_USER_SIGNED_OUT, user,
                                             NULL);
  orochi_auth_deliver_webhook_event(payload);
}
...
Datum orochi_auth_create_webhook_endpoint_sql(PG_FUNCTION_ARGS) {
  ...
  int timeout_ms =
      PG_ARGISNULL(2) ? AUTH_WEBHOOK_TIMEOUT_MS : PG_GETARG_INT32(2);
  int max_retries =
      PG_ARGISNULL(3) ? AUTH_WEBHOOK_MAX_RETRIES : PG_GETARG_INT32(3);

  char *url = text_to_cstring(url_text);
  char *secret = secret_text ? text_to_cstring(secret_text) : NULL;

  int64 endpoint_id = orochi_auth_create_webhook_endpoint(
      url, secret, NULL, timeout_ms, max_retries);
...
Datum orochi_auth_verify_webhook_signature_sql(PG_FUNCTION_ARGS) {
  ...
  bool valid = orochi_auth_verify_webhook_signature(payload, signature, secret,
                                                    timestamp);
...
Datum orochi_auth_list_webhook_endpoints_sql(PG_FUNCTION_ARGS) {
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    initStringInfo(&query);
    appendStringInfoString(
        &query, "SELECT endpoint_id, url, enabled, timeout_ms, max_retries, "
