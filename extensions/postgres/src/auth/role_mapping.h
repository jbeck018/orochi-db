...
extern bool role_parse_name(const char *role_name, RoleIdentityType *type, char *hash_out);

/* Template management */
extern OrochiRoleTemplate *role_get_template(pg_uuid_t template_id);
extern OrochiRoleTemplate *role_get_template_by_name(pg_uuid_t *org_id, const char *template_name);

extern List *role_get_system_templates(void);
extern List *role_get_org_templates(pg_uuid_t org_id);

extern pg_uuid_t role_create_template(pg_uuid_t *org_id, const char *template_name,
                                      const char *display_name, const char *base_privileges);

extern bool role_update_template(pg_uuid_t template_id, const char *base_privileges,
                                 const char *schema_privileges, const char *table_privileges);
...
extern OrochiRoleMapping *role_get_mapping_for_user(pg_uuid_t user_id, pg_uuid_t cluster_id,
                                                    pg_uuid_t *branch_id);

extern OrochiRoleMapping *role_get_mapping_for_team(pg_uuid_t team_id, pg_uuid_t cluster_id);

extern pg_uuid_t role_create_mapping(pg_uuid_t user_id, pg_uuid_t cluster_id, pg_uuid_t *branch_id,
                                     pg_uuid_t template_id, const char *custom_privileges);

extern pg_uuid_t role_create_team_mapping(pg_uuid_t team_id, pg_uuid_t cluster_id,
                                          pg_uuid_t template_id);

extern pg_uuid_t role_create_service_mapping(pg_uuid_t service_account_id, pg_uuid_t cluster_id,
                                             pg_uuid_t template_id);
...
extern BranchPermissionLevel role_get_permission_for_branch(pg_uuid_t user_id, pg_uuid_t branch_id);
