...
static bool rate_limit_check(pg_uuid_t user_id, OrochiAuthTokenType token_type, int limit_per_hour);
static void queue_email_notification(const char *email, const char *template_name,
                                     Jsonb *template_data);
static void queue_sms_notification(const char *phone, const char *message, const char *channel);
 ...
 */
OrochiMagicLink *orochi_auth_send_magic_link(const char *email, const char *redirect_to)
{
    ...
    /* Check rate limiting */
    if (!rate_limit_check(user->id, OROCHI_TOKEN_CONFIRMATION, config->rate_limit_email_sent)) {
    ...
    magic_link_context =
        AllocSetContextCreate(TopMemoryContext, "MagicLink", ALLOCSET_DEFAULT_SIZES);
    ...
    initStringInfo(&query);
    {
        ...
        appendStringInfo(&query,
                         ...
                         "VALUES ('%s', 'confirmation_token', %s, %s, NOW(), NOW() + INTERVAL "
                         "'%d seconds') "
                         "RETURNING id, created_at, expires_at",
                         uuid_str, quote_literal_cstr(token_hash), quote_literal_cstr(email),
    ...
    if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0) {
        ...
        magic_link->created_at = DatumGetTimestampTz(
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 2, &isnull));
        magic_link->expires_at = DatumGetTimestampTz(
    ...
    link_url = psprintf("%s/auth/v1/verify?token=%s&type=magiclink&redirect_to=%s", site_url, token,
    ...
    appendStringInfo(&template_json,
                     ...
                     link_url, token, email, redirect_to ? redirect_to : site_url,
                     OROCHI_AUTH_MAGIC_LINK_EXP);

    template_data =
        DatumGetJsonbP(DirectFunctionCall1(jsonb_in, CStringGetDatum(template_json.data)));
 ...
 */
OrochiMagicLink *orochi_auth_send_phone_otp(const char *phone, const char *channel)
{
    ...
    /* Check rate limiting */
    if (!rate_limit_check(user->id, OROCHI_TOKEN_CONFIRMATION, config->rate_limit_sms_sent)) {
        orochi_auth_free_user(user);
        ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
    ...
    otp_context = AllocSetContextCreate(TopMemoryContext, "PhoneOTP", ALLOCSET_DEFAULT_SIZES);
    ...
    initStringInfo(&query);
    {
        ...
        appendStringInfo(&query,
                         ...
                         "'%d seconds') "
                         "RETURNING id, created_at, expires_at",
                         uuid_str, quote_literal_cstr(otp_hash), quote_literal_cstr(phone),
                         OROCHI_AUTH_OTP_EXP);
    }
    ...
    if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0) {
        ...
        otp_token->created_at = DatumGetTimestampTz(
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 2, &isnull));
        otp_token->expires_at = DatumGetTimestampTz(
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 3, &isnull));
    }
    ...
    /* Build SMS message */
    message = psprintf("Your verification code is: %s. Valid for %d minutes.", otp,
                       OROCHI_AUTH_OTP_EXP / 60);
 ...
 */
OrochiAuthResult *orochi_auth_verify_otp(const char *token, const char *token_type,
                                         const char *email, const char *phone)
{
    ...
    initStringInfo(&query);
    appendStringInfoString(&query, "SELECT user_id, relates_to FROM auth.one_time_tokens "
                                   "WHERE token_hash = $1 "
                                   "AND expires_at > NOW() "
                                   "AND used_at IS NULL");
    ...
    /* Extract user_id */
    {
        ...
        char *relates_to = SPI_getvalue(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 2);
        Datum uuid_datum;
    ...
    initStringInfo(&query);
    {
        ...
        if (strcmp(token_type, "magiclink") == 0 || strcmp(token_type, "signup") == 0) {
            ...
            appendStringInfo(&query,
                             "UPDATE auth.users SET phone_confirmed_at = "
                             "COALESCE(phone_confirmed_at, NOW()), "
                             "last_sign_in_at = NOW(), updated_at = NOW() WHERE id = '%s'",
                             uuid_str);
        } else {
            appendStringInfo(&query,
 ...
 */
OrochiMagicLink *orochi_auth_send_recovery(const char *email)
{
    ...
    /* Check rate limiting */
    if (!rate_limit_check(user->id, OROCHI_TOKEN_RECOVERY, config->rate_limit_email_sent)) {
        orochi_auth_free_user(user);
        ereport(ERROR, (errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
    ...
    /* Create memory context */
    recovery_context =
        AllocSetContextCreate(TopMemoryContext, "RecoveryToken", ALLOCSET_DEFAULT_SIZES);
    ...
    initStringInfo(&query);
    {
        ...
        appendStringInfo(&query,
                         ...
                         quote_literal_cstr(token_hash), uuid_str);
    }
    SPI_execute(query.data, false, 0);
    pfree(query.data);
    ...
    initStringInfo(&query);
    {
        ...
        appendStringInfo(&query,
                         "INSERT INTO auth.one_time_tokens "
                         "(user_id, token_type, token_hash, relates_to, created_at, expires_at) "
                         "VALUES ('%s', 'recovery_token', %s, %s, NOW(), NOW() + INTERVAL '%d "
                         "seconds') "
                         "RETURNING id, created_at, expires_at",
                         uuid_str, quote_literal_cstr(token_hash), quote_literal_cstr(email),
                         OROCHI_AUTH_MAGIC_LINK_EXP);
    ...
    if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0) {
        bool isnull;
        recovery_token->created_at = DatumGetTimestampTz(
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 2, &isnull));
        recovery_token->expires_at = DatumGetTimestampTz(
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 3, &isnull));
    }
    ...
    link_url = psprintf("%s/auth/v1/verify?token=%s&type=recovery", site_url, token);
 ...
 */
bool orochi_auth_verify_recovery(const char *token, const char *new_password)
{
    StringInfoData query;
    int ret;
    char *token_hash;
    ...
    /* Find token */
    initStringInfo(&query);
    appendStringInfoString(&query, "SELECT user_id FROM auth.one_time_tokens "
    ...
    initStringInfo(&query);
    {
        ...
        appendStringInfo(&query,
                         ...
                         "WHERE id = '%s'",
                         quote_literal_cstr(encrypted_password), uuid_str);
    }
    SPI_execute(query.data, false, 0);
    pfree(query.data);
    ...
    initStringInfo(&query);
    {
        ...
        appendStringInfo(&query, "DELETE FROM auth.sessions WHERE user_id = '%s'", uuid_str);
    }
    SPI_execute(query.data, false, 0);
    pfree(query.data);
                                     ...
                                     Jsonb *template_data)
{
    ...
    appendStringInfo(&payload, "{\"type\": \"%s\", \"email\": \"%s\", \"data\": %s}", template_name,
                     email, template_str.data);
    ...
    /* Use pg_notify to send to email worker */
    {
        ...
        appendStringInfo(&notify_query, "SELECT pg_notify('auth_email', %s)",
...
static void queue_sms_notification(const char *phone, const char *message, const char *channel)
{
    ...
    initStringInfo(&payload);
    appendStringInfo(&payload, "{\"phone\": \"%s\", \"message\": \"%s\", \"channel\": \"%s\"}",
    ...
    /* Use pg_notify to send to SMS worker */
    {
        ...
        initStringInfo(&notify_query);
        appendStringInfo(&notify_query, "SELECT pg_notify('auth_sms', %s)",
                         quote_literal_cstr(payload.data));
...
Datum orochi_auth_send_magic_link_sql(PG_FUNCTION_ARGS)
{
...
Datum orochi_auth_send_recovery_sql(PG_FUNCTION_ARGS)
{
    text *email_text = PG_GETARG_TEXT_PP(0);
    char *email = text_to_cstring(email_text);
