...
static bool rate_limit_check(pg_uuid_t user_id, OrochiAuthTokenType token_type,
                             int limit_per_hour);
static void queue_email_notification(const char *email,
 ...
 */
OrochiMagicLink *orochi_auth_send_magic_link(const char *email,
  ...
  magic_link_context = AllocSetContextCreate(TopMemoryContext, "MagicLink",
  ...
  magic_link->redirect_to =
      redirect_to ? pstrdup(redirect_to) : pstrdup(site_url);
  ...
  link_url =
      psprintf("%s/auth/v1/verify?token=%s&type=magiclink&redirect_to=%s",
               site_url, token, redirect_to ? redirect_to : site_url);
  ...
  template_data = DatumGetJsonbP(
      DirectFunctionCall1(jsonb_in, CStringGetDatum(template_json.data)));
 ...
 */
OrochiMagicLink *orochi_auth_send_phone_otp(const char *phone,
                                            const char *channel) {
  ...
  otp_context = AllocSetContextCreate(TopMemoryContext, "PhoneOTP",
                                      ALLOCSET_DEFAULT_SIZES);
 ...
 */
OrochiAuthResult *orochi_auth_verify_otp(const char *token,
  ...
  if (strcmp(token_type, "magiclink") == 0 ||
      strcmp(token_type, "signup") == 0 || strcmp(token_type, "sms") == 0) {
  ...
  result->access_token =
      orochi_auth_generate_jwt(user, session, orochi_auth_jwt_exp);
  result->expires_at =
...
OrochiMagicLink *orochi_auth_send_recovery(const char *email) {
  ...
  /* Create memory context */
  recovery_context = AllocSetContextCreate(TopMemoryContext, "RecoveryToken",
                                           ALLOCSET_DEFAULT_SIZES);
  ...
  link_url =
      psprintf("%s/auth/v1/verify?token=%s&type=recovery", site_url, token);
...
bool orochi_auth_verify_recovery(const char *token, const char *new_password) {
  ...
  appendStringInfoString(
      &query, "SELECT user_id FROM auth.one_time_tokens "
  ...
  /* Extract user_id */
  {
    char *user_id_str =
        SPI_getvalue(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1);
    Datum uuid_datum =
        DirectFunctionCall1(uuid_in, CStringGetDatum(user_id_str));
    memcpy(&user_id, DatumGetUUIDP(uuid_datum), sizeof(pg_uuid_t));
  }
  ...
  initStringInfo(&query);
  {
    ...
    appendStringInfo(&query, "DELETE FROM auth.sessions WHERE user_id = '%s'",
                     uuid_str);
  }
  SPI_execute(query.data, false, 0);
  pfree(query.data);
...
static bool rate_limit_check(pg_uuid_t user_id, OrochiAuthTokenType token_type,
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    bool isnull;
    recent_count = DatumGetInt64(SPI_getbinval(
...
static void queue_email_notification(const char *email,
  ...
  (void)JsonbToCString(&template_str, &template_data->root,
                       VARSIZE(template_data));
...
static void queue_sms_notification(const char *phone, const char *message,
  ...
  initStringInfo(&payload);
  appendStringInfo(
      &payload, "{\"phone\": \"%s\", \"message\": \"%s\", \"channel\": \"%s\"}",
...
Datum orochi_auth_send_magic_link_sql(PG_FUNCTION_ARGS) {
  ...
  PG_RETURN_JSONB_P(DirectFunctionCall1(
      jsonb_in, CStringGetDatum("{\"message\": \"Magic link sent\"}")));
}
...
Datum orochi_auth_send_phone_otp_sql(PG_FUNCTION_ARGS) {
  ...
  PG_RETURN_JSONB_P(DirectFunctionCall1(
      jsonb_in, CStringGetDatum("{\"message\": \"OTP sent\"}")));
}
...
Datum orochi_auth_send_recovery_sql(PG_FUNCTION_ARGS) {
  ...
  PG_RETURN_JSONB_P(DirectFunctionCall1(
      jsonb_in, CStringGetDatum("{\"message\": \"If the email exists, a "
