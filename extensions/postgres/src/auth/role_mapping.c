...
void role_mapping_init(void) {
  ...
  if (RoleMappingContext == NULL) {
    RoleMappingContext = AllocSetContextCreate(
        TopMemoryContext, "RoleMappingContext", ALLOCSET_DEFAULT_SIZES);
  }
  ...
  RoleMappingCache = hash_create("RoleMappingCache", ROLE_CACHE_SIZE, &hash_ctl,
                                 HASH_ELEM | HASH_CONTEXT);

  elog(LOG, "Role mapping module initialized (cache size: %d)",
       ROLE_CACHE_SIZE);
}
 ...
 */
Size role_mapping_shmem_size(void) { return sizeof(RoleMappingSharedState); }
 ...
 */
bool role_parse_name(const char *role_name, RoleIdentityType *type,
                     char *hash_out) {
  if (role_name == NULL)
    return false;
  ...
  if (strncmp(role_name, ROLE_PREFIX_SERVICE, strlen(ROLE_PREFIX_SERVICE)) ==
      0) {
 ...
 */
OrochiRoleTemplate *role_get_template_by_name(pg_uuid_t *org_id,
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    }
    {
      ...
      if (!isnull) {
        ...
        memcpy(mapping->service_account_id, DatumGetUUIDP(d),
               sizeof(pg_uuid_t));
      }
    }
 ...
 */
OrochiRoleMapping *role_get_mapping_for_user(pg_uuid_t user_id,
  ...
  appendStringInfo(&cache_key, "%s:%s:%s", uuid_to_string(user_id),
  ...
  if (RoleMappingCache != NULL) {
    cache_entry =
        hash_search(RoleMappingCache, cache_key.data, HASH_FIND, &found);
    if (found && cache_entry->is_valid) {
      int64 age_sec =
          (GetCurrentTimestamp() - cache_entry->cached_at) / 1000000;
      if (age_sec < ROLE_CACHE_TTL_SEC) {
        ...
        mapping = palloc0(sizeof(OrochiRoleMapping));
        strncpy(mapping->pg_role_name, cache_entry->pg_role_name,
                ROLE_NAME_MAX_LEN);
  ...
  if (branch_id != NULL)
    appendStringInfo(&query, "AND branch_id = '%s' ",
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    if (!isnull) {
      ...
      if (mapping != NULL && RoleMappingCache != NULL) {
        cache_entry =
            hash_search(RoleMappingCache, cache_key.data, HASH_ENTER, &found);
        strncpy(cache_entry->cache_key, cache_key.data, 255);
        strncpy(cache_entry->pg_role_name, mapping->pg_role_name,
                ROLE_NAME_MAX_LEN);
 ...
 */
pg_uuid_t role_create_mapping(pg_uuid_t user_id, pg_uuid_t cluster_id,
                              pg_uuid_t *branch_id, pg_uuid_t template_id,
...
bool role_create_db_role(pg_uuid_t mapping_id) {
  ...
  initStringInfo(&sql);
  appendStringInfo(&sql, "CREATE ROLE %s",
...
int role_batch_create(pg_uuid_t cluster_id, pg_uuid_t *user_ids, int count) {
  ...
  elog(LOG, "Batch created %d roles for cluster %s", success_count,
       uuid_to_string(cluster_id));
 ...
 */
char *role_lookup_for_connection(pg_uuid_t user_id, pg_uuid_t cluster_id,
                                 pg_uuid_t *branch_id) {
  OrochiRoleMapping *mapping;
  char *role_name;
...
int role_cleanup_user_roles(pg_uuid_t user_id) {
  ...
  elog(LOG, "Cleaned up %d role mappings for user %s", count,
       uuid_to_string(user_id));
...
void role_invalidate_cache(pg_uuid_t cluster_id) {
  ...
  snprintf(cluster_str, sizeof(cluster_str),
           ":%s:", uuid_to_string(cluster_id));
...
Datum role_create_mapping_sql(PG_FUNCTION_ARGS) {
  ...
  mapping_id =
      role_create_mapping(*user_id, *cluster_id, branch_id, *template_id,
...
Datum role_lookup_sql(PG_FUNCTION_ARGS) {
  ...
  char *role_name =
      role_lookup_for_connection(*user_id, *cluster_id, branch_id);
...
Datum role_batch_create_sql(PG_FUNCTION_ARGS) {
  ...
  deconstruct_array(user_ids_array, UUIDOID, 16, false, 'c', &datums, &nulls,
                    &count);
