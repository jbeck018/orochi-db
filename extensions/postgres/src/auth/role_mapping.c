 ...
 */
void
role_mapping_init(void)
{
    HASHCTL hash_ctl;

    if (RoleMappingContext == NULL)
    {
        RoleMappingContext = AllocSetContextCreate(TopMemoryContext,
                                                    "RoleMappingContext",
    ...
    RoleMappingCache = hash_create("RoleMappingCache",
                                    ROLE_CACHE_SIZE,
                                    &hash_ctl,
 ...
 */
char *
role_generate_name(RoleIdentityType type, pg_uuid_t identity_id)
{
    char *hash;
    char *role_name;
    const char *prefix;
 ...
 */
char *
role_generate_branch_name(pg_uuid_t branch_id)
{
 ...
 */
bool
role_parse_name(const char *role_name, RoleIdentityType *type, char *hash_out)
{
    if (role_name == NULL)
        return false;
 ...
 */
OrochiRoleTemplate *
role_get_template(pg_uuid_t template_id)
{
    ...
    appendStringInfo(&query,
        ...
        "base_privileges, schema_privileges, table_privileges, function_privileges, "
 ...
 */
OrochiRoleTemplate *
role_get_template_by_name(pg_uuid_t *org_id, const char *template_name)
{
    OrochiRoleTemplate *tmpl = NULL;
    StringInfoData query;
    int ret;
 ...
 */
OrochiRoleMapping *
role_get_mapping(pg_uuid_t mapping_id)
{
    MemoryContext old_context;
 ...
 */
OrochiRoleMapping *
role_get_mapping_for_user(pg_uuid_t user_id,
                          ...
                          pg_uuid_t *branch_id)
{
    ...
    appendStringInfo(&cache_key, "%s:%s:%s",
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0)
    {
        ...
        if (!isnull)
        {
            ...
            {
                cache_entry = hash_search(RoleMappingCache, cache_key.data,
                                          HASH_ENTER, &found);
 ...
 */
pg_uuid_t
role_create_mapping(pg_uuid_t user_id,
                    pg_uuid_t cluster_id,
                    pg_uuid_t *branch_id,
                    pg_uuid_t template_id,
                    const char *custom_privileges)
{
    ...
    appendStringInfo(&query,
        ...
        uuid_to_string(template_id),
        custom_privileges ? quote_literal_cstr(custom_privileges) : "NULL",
        pg_role_name);
    ...
    if (ret != SPI_OK_INSERT && ret != SPI_OK_UPDATE)
    {
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("failed to create role mapping")));
    }

    elog(LOG, "Created role mapping %s for user %s",
         uuid_to_string(mapping_id), uuid_to_string(user_id));
 ...
 */
bool
role_create_db_role(pg_uuid_t mapping_id)
{
 ...
 */
bool
role_apply_privileges(pg_uuid_t mapping_id)
{
    OrochiRoleMapping *mapping;
    OrochiRoleTemplate *tmpl = NULL;
    StringInfoData sql;
    ...
    if (tmpl->sequence_privileges != NULL)
    {
        initStringInfo(&sql);
        appendStringInfo(&sql,
            "GRANT USAGE ON ALL SEQUENCES IN SCHEMA public TO %s",
            quote_identifier(mapping->pg_role_name));
 ...
 */
bool
role_sync_to_cluster(pg_uuid_t mapping_id)
{
    bool success;
 ...
 */
int
role_batch_create(pg_uuid_t cluster_id, pg_uuid_t *user_ids, int count)
{
    int success_count = 0;
    int batch_size;
    ...
    elog(LOG, "Batch created %d roles for cluster %s",
         success_count, uuid_to_string(cluster_id));
 ...
 */
char *
role_lookup_for_connection(pg_uuid_t user_id,
                           pg_uuid_t cluster_id,
                           pg_uuid_t *branch_id)
{
    OrochiRoleMapping *mapping;
 ...
 */
int
role_cleanup_user_roles(pg_uuid_t user_id)
{
    StringInfoData query;
    int ret;
    int count = 0;
    ...
    elog(LOG, "Cleaned up %d role mappings for user %s",
         count, uuid_to_string(user_id));
 ...
 */
int
role_suspend_user_roles(pg_uuid_t user_id)
{
    StringInfoData query;
    int ret;
    int count = 0;
 ...
 */
int
role_reactivate_user_roles(pg_uuid_t user_id)
{
    StringInfoData query;
    int ret;
    int count = 0;
 ...
 */
void
role_invalidate_cache(pg_uuid_t cluster_id)
{
    ...
    snprintf(cluster_str, sizeof(cluster_str), ":%s:",
             uuid_to_string(cluster_id));
 ...
 */
void
role_invalidate_user_cache(pg_uuid_t user_id)
{
    HASH_SEQ_STATUS status;
    RoleCacheEntry *entry;
    char user_str[64];
...
const char *
role_identity_type_name(RoleIdentityType type)
{
    switch (type)
    {
        case ROLE_IDENTITY_USER:
...
const char *
role_mapping_status_name(RoleMappingStatus status)
{
    switch (status)
    {
...
const char *
role_sync_operation_name(RoleSyncOperation op)
{
    switch (op)
    {
...
RoleIdentityType
role_parse_identity_type(const char *str)
{
    if (str == NULL)
...
RoleMappingStatus
role_parse_mapping_status(const char *str)
{
    if (str == NULL)
...
void
role_free_template(OrochiRoleTemplate *tmpl)
{
    if (tmpl == NULL)
        return;
...
void
role_free_mapping(OrochiRoleMapping *mapping)
{
    if (mapping == NULL)
        return;
...
void
role_free_statistics(RoleStatistics *stats)
{
    if (stats != NULL)
        pfree(stats);
}
...
static char *
generate_hash_from_uuid(pg_uuid_t uuid)
{
...
Datum
role_create_mapping_sql(PG_FUNCTION_ARGS)
{
    ...
    mapping_id = role_create_mapping(
        *user_id,
        *cluster_id,
        branch_id,
        *template_id,
...
Datum
role_sync_mapping_sql(PG_FUNCTION_ARGS)
{
...
Datum
role_lookup_sql(PG_FUNCTION_ARGS)
{
    pg_uuid_t *user_id = PG_GETARG_UUID_P(0);
    pg_uuid_t *cluster_id = PG_GETARG_UUID_P(1);
    pg_uuid_t *branch_id = PG_ARGISNULL(2) ? NULL : PG_GETARG_UUID_P(2);
...
Datum
role_batch_create_sql(PG_FUNCTION_ARGS)
{
    ...
    deconstruct_array(user_ids_array, UUIDOID, 16, false, 'c',
                      &datums, &nulls, &count);
