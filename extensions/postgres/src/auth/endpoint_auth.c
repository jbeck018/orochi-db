...
void endpoint_auth_init(void) {
  ...
  if (EndpointAuthContext == NULL) {
    EndpointAuthContext = AllocSetContextCreate(
        TopMemoryContext, "EndpointAuthContext", ALLOCSET_DEFAULT_SIZES);
  }
  ...
  EndpointTokenCache =
      hash_create("EndpointTokenCache", ENDPOINT_TOKEN_CACHE_SIZE, &hash_ctl,
 ...
 */
Size endpoint_auth_shmem_size(void) { return sizeof(EndpointAuthSharedState); }
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    }
    {
      ...
      endpoint->max_connections =
          isnull ? ENDPOINT_DEFAULT_MAX_CONN : DatumGetInt32(d);
    }
 ...
 */
EndpointTokenResult *endpoint_generate_token(pg_uuid_t endpoint_id,
                                             pg_uuid_t user_id,
                                             const char *db_role,
  ...
  if (!branch_check_permission(user_id, endpoint->branch_id,
                               BRANCH_PERM_CONNECT)) {
  ...
  appendStringInfo(
      ...
      uuid_to_string(credential_id), uuid_to_string(endpoint_id),
      result->token_id, db_role, branch_permission_level_name(permission),
      valid_duration_sec, uuid_to_string(user_id));
 ...
 */
char *endpoint_generate_connection_string(pg_uuid_t credential_id,
  ...
  if (driver_type == NULL || strcmp(driver_type, "psql") == 0) {
    ...
    appendStringInfo(&conn_str,
                     ...
                     signed_token, endpoint->endpoint_host,
                     endpoint->endpoint_port, database, ssl_mode_str);
  } else if (strcmp(driver_type, "jdbc") == 0) {
    /* JDBC connection string */
    appendStringInfo(
        &conn_str,
        "jdbc:postgresql://%s:%d/%s?user=orochi_token&password=%s&sslmode=%s",
        endpoint->endpoint_host, endpoint->endpoint_port, database,
        signed_token, ssl_mode_str);
 ...
 */
EndpointAuthResult *endpoint_validate_token(const char *token,
  ...
  if (cached_claims != NULL) {
    ...
    if (cached_claims->exp > now) {
      ...
      memcpy(&result->user_id,
             DatumGetUUIDP(DirectFunctionCall1(
                 uuid_in, CStringGetDatum(cached_claims->sub))),
             sizeof(pg_uuid_t));
      memcpy(&result->endpoint_id, &cached_claims->endpoint_id,
             sizeof(pg_uuid_t));
 ...
 */
EndpointAuthResult *endpoint_validate_password(const char *username,
                                               const char *password,
                                               pg_uuid_t endpoint_id) {
  MemoryContext old_context;
  EndpointAuthResult *result;
 ...
 */
OrochiEndpointCredential *
endpoint_get_credential_by_username(pg_uuid_t endpoint_id,
                                    const char *username) {
  OrochiEndpointCredential *credential = NULL;
  StringInfoData query;
  int ret;
 ...
 */
char **endpoint_get_startup_commands(EndpointSessionContext *context,
                                     int *command_count) {
  char **commands;
  ...
  *command_count = 4;
  ...
  /* SET orochi.user_id */
  commands[2] =
      psprintf("SET orochi.user_id = '%s'", uuid_to_string(context->user_id));
...
Datum endpoint_generate_token_sql(PG_FUNCTION_ARGS) {
  ...
  int valid_duration =
      PG_ARGISNULL(4) ? ENDPOINT_DEFAULT_TOKEN_TTL_SEC : PG_GETARG_INT32(4);
  ...
  result = endpoint_generate_token(*endpoint_id, *user_id,
                                   text_to_cstring(db_role_text), permission,
                                   valid_duration);
  ...
  if (result->success) {
    appendStringInfo(&json_result,
                     ...
                     uuid_to_string(result->credential_id), result->token_id,
                     valid_duration);
  } else {
