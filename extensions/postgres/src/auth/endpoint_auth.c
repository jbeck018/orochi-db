 ...
 */
void endpoint_auth_init(void)
{
    HASHCTL hash_ctl;

    if (EndpointAuthContext == NULL) {
        EndpointAuthContext =
            AllocSetContextCreate(TopMemoryContext, "EndpointAuthContext", ALLOCSET_DEFAULT_SIZES);
    }
    ...
    EndpointTokenCache = hash_create("EndpointTokenCache", ENDPOINT_TOKEN_CACHE_SIZE, &hash_ctl,
 ...
 */
Size endpoint_auth_shmem_size(void)
{
 ...
 */
OrochiComputeEndpoint *endpoint_get(pg_uuid_t endpoint_id)
{
    ...
    appendStringInfo(&query,
                     "SELECT endpoint_id, cluster_id, branch_id, endpoint_name, "
                     "endpoint_host, "
 ...
 */
OrochiComputeEndpoint *endpoint_get_by_host(const char *host)
{
    OrochiComputeEndpoint *endpoint = NULL;
    StringInfoData query;
    int ret;
 ...
 */
bool endpoint_update_status(pg_uuid_t endpoint_id, EndpointStatus new_status)
{
    StringInfoData query;
    int ret;
    ...
    appendStringInfo(&query,
                     ...
                     "WHERE endpoint_id = '%s'",
                     endpoint_status_name(new_status), uuid_to_string(endpoint_id));
 ...
 */
EndpointTokenResult *endpoint_generate_token(pg_uuid_t endpoint_id, pg_uuid_t user_id,
                                             const char *db_role, BranchPermissionLevel permission,
                                             int valid_duration_sec)
{
    ...
    if (db_role == NULL) {
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("db_role is required")));
    }
    ...
    appendStringInfo(&query,
                     ...
                     uuid_to_string(credential_id), uuid_to_string(endpoint_id), result->token_id,
                     db_role, branch_permission_level_name(permission), valid_duration_sec,
    ...
    elog(LOG, "Generated endpoint token for user %s on endpoint %s", uuid_to_string(user_id),
         uuid_to_string(endpoint_id));
 ...
 */
char *endpoint_generate_connection_string(pg_uuid_t credential_id, const char *signed_token,
                                          const char *database, const char *driver_type)
{
    ...
    if (credential == NULL) {
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("Credential not found")));
    }

    endpoint = endpoint_get(credential->endpoint_id);
    if (endpoint == NULL) {
        endpoint_free_credential(credential);
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("Endpoint not found")));
    }
    ...
    if (driver_type == NULL || strcmp(driver_type, "psql") == 0) {
        ...
        appendStringInfo(&conn_str, "postgresql://orochi_token:%s@%s:%d/%s?sslmode=%s",
                         signed_token, endpoint->endpoint_host, endpoint->endpoint_port, database,
                         ssl_mode_str);
    } else if (strcmp(driver_type, "jdbc") == 0) {
        /* JDBC connection string */
        appendStringInfo(
            &conn_str, "jdbc:postgresql://%s:%d/%s?user=orochi_token&password=%s&sslmode=%s",
            endpoint->endpoint_host, endpoint->endpoint_port, database, signed_token, ssl_mode_str);
        ...
        appendStringInfo(&conn_str,
                         ...
                         endpoint->endpoint_host, endpoint->endpoint_port, database, signed_token,
                         ssl_mode_str);
 ...
 */
EndpointAuthResult *endpoint_validate_token(const char *token, const char *endpoint_host,
                                            const char *client_ip)
{
    ...
    if (cached_claims != NULL) {
        ...
        if (cached_claims->exp > now) {
            ...
            memcpy(&result->user_id,
                   DatumGetUUIDP(DirectFunctionCall1(uuid_in, CStringGetDatum(cached_claims->sub))),
                   sizeof(pg_uuid_t));
    ...
    if (endpoint_host != NULL && strcmp(endpoint->endpoint_host, endpoint_host) != 0) {
        result->validation = ENDPOINT_TOKEN_INVALID_ENDPOINT;
 ...
 */
EndpointAuthResult *endpoint_validate_password(const char *username, const char *password,
                                               pg_uuid_t endpoint_id)
{
    MemoryContext old_context;
    EndpointAuthResult *result;
 ...
 */
OrochiEndpointCredential *endpoint_get_credential(pg_uuid_t credential_id)
{
    MemoryContext old_context;
 ...
 */
OrochiEndpointCredential *endpoint_get_credential_by_token_id(const char *token_id)
{
    OrochiEndpointCredential *credential = NULL;
    StringInfoData query;
    int ret;
 ...
 */
OrochiEndpointCredential *endpoint_get_credential_by_username(pg_uuid_t endpoint_id,
                                                              const char *username)
{
    OrochiEndpointCredential *credential = NULL;
    StringInfoData query;
    ...
    appendStringInfo(&query,
                     ...
                     uuid_to_string(endpoint_id), username);
 ...
 */
bool endpoint_revoke_credential(pg_uuid_t credential_id, pg_uuid_t revoked_by)
{
    StringInfoData query;
    int ret;
    ...
    appendStringInfo(&query,
                     ...
                     uuid_to_string(revoked_by), uuid_to_string(credential_id));
 ...
 */
void endpoint_record_credential_use(pg_uuid_t credential_id)
{
    StringInfoData query;
 ...
 */
EndpointSessionContext *endpoint_create_session_context(EndpointJWTClaims *claims)
{
    ...
    if (claims->sub != NULL) {
        pg_uuid_t *uuid = DatumGetUUIDP(DirectFunctionCall1(uuid_in, CStringGetDatum(claims->sub)));
 ...
 */
char **endpoint_get_startup_commands(EndpointSessionContext *context, int *command_count)
{
    char **commands;
    ...
    *command_count = 4;
    ...
    commands[1] = psprintf("SET orochi.branch_id = '%s'", uuid_to_string(context->branch_id));

    /* SET orochi.user_id */
    commands[2] = psprintf("SET orochi.user_id = '%s'", uuid_to_string(context->user_id));
 ...
 */
bool endpoint_set_session_variables(EndpointSessionContext *context)
{
    char **commands;
    int command_count;
    int i;
 ...
 */
void endpoint_cache_token(const char *token_id, EndpointJWTClaims *claims)
{
 ...
 */
EndpointJWTClaims *endpoint_get_cached_token(const char *token_id)
{
    TokenCacheEntry *entry;
    bool found;
 ...
 */
void endpoint_invalidate_token_cache(const char *token_id)
{
    TokenCacheEntry *entry;
    bool found;
 ...
 */
void endpoint_clear_token_cache(void)
{
...
const char *endpoint_credential_type_name(EndpointCredentialType type)
{
    switch (type) {
...
const char *endpoint_pooler_mode_name(EndpointPoolerMode mode)
{
    switch (mode) {
...
const char *endpoint_ssl_mode_name(EndpointSSLMode mode)
{
    switch (mode) {
...
const char *endpoint_status_name(EndpointStatus status)
{
    switch (status) {
...
const char *endpoint_token_validation_message(EndpointTokenValidation result)
{
    switch (result) {
...
EndpointPoolerMode endpoint_parse_pooler_mode(const char *str)
{
    if (str == NULL)
...
EndpointSSLMode endpoint_parse_ssl_mode(const char *str)
{
    if (str == NULL)
...
EndpointStatus endpoint_parse_status(const char *str)
{
    if (str == NULL)
...
void endpoint_free(OrochiComputeEndpoint *endpoint)
{
    if (endpoint == NULL)
        return;
...
void endpoint_free_credential(OrochiEndpointCredential *cred)
{
    if (cred == NULL)
        return;
...
void endpoint_free_auth_result(EndpointAuthResult *result)
{
    if (result == NULL)
        return;
...
void endpoint_free_token_result(EndpointTokenResult *result)
{
    if (result == NULL)
        return;
...
void endpoint_free_session_context(EndpointSessionContext *context)
{
    if (context != NULL)
        pfree(context);
}
...
Datum endpoint_generate_token_sql(PG_FUNCTION_ARGS)
{
    ...
    result = endpoint_generate_token(*endpoint_id, *user_id, text_to_cstring(db_role_text),
    ...
    if (result->success) {
        appendStringInfo(&json_result,
                         ...
                         uuid_to_string(result->credential_id), result->token_id, valid_duration);
    } else {
        appendStringInfo(&json_result, "{\"success\": false, \"error\": \"%s\"}",
...
Datum endpoint_validate_token_sql(PG_FUNCTION_ARGS)
{
    ...
    result =
        endpoint_validate_token(text_to_cstring(token_text),
    ...
    if (result->authenticated) {
        appendStringInfo(&json_result,
                         ...
                         result->db_role, branch_permission_level_name(result->permission),
                         uuid_to_string(result->branch_id));
    } else {
        appendStringInfo(&json_result, "{\"valid\": false, \"error\": \"%s\"}",
...
Datum endpoint_get_connection_string_sql(PG_FUNCTION_ARGS)
{
    ...
    char *conn_string = endpoint_generate_connection_string(
        *credential_id, text_to_cstring(token_text),
...
Datum endpoint_revoke_credential_sql(PG_FUNCTION_ARGS)
{
    pg_uuid_t *credential_id = PG_GETARG_UUID_P(0);
