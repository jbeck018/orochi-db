...
void orochi_auth_shmem_init(void) {
  ...
  OrochiAuthSharedState =
      ShmemInitStruct("orochi_auth_cache", orochi_auth_shmem_size(), &found);
  ...
  if (!found) {
    ...
    LWLockRegisterTranche(OrochiAuthLWLockTrancheId,
                          OROCHI_AUTH_LWLOCK_TRANCHE_NAME);
 ...
 *    Add token to cache
 */
void orochi_auth_cache_token(OrochiTokenValidation *validation,
  ...
  for (probe_count = 0; probe_count < 16; probe_count++) {
    ...
    if (entry->token_hash[0] == '\0' || entry->expires_at < now ||
      ...
      strlcpy(entry->tenant_id, validation->tenant_id,
              sizeof(entry->tenant_id));
      strlcpy(entry->session_id, validation->session_id,
      ...
      if (OrochiAuthSharedState->token_cache_count <
          OROCHI_AUTH_MAX_CACHED_TOKENS)
  ...
  OrochiAuthSharedState->token_cache_head =
      (OrochiAuthSharedState->token_cache_head + 1) %
      OROCHI_AUTH_MAX_CACHED_TOKENS;
...
void orochi_auth_cache_session(OrochiSession *session) {
  ...
  for (probe_count = 0; probe_count < 16; probe_count++) {
    ...
    if (entry->session_id[0] == '\0' ||
      ...
      if (OrochiAuthSharedState->session_cache_count <
  ...
  OrochiAuthSharedState->session_cache_head =
      (OrochiAuthSharedState->session_cache_head + 1) %
      OROCHI_AUTH_MAX_CACHED_SESSIONS;
...
void orochi_auth_cache_signing_key(OrochiSigningKey *key) {
  ...
  for (i = 0; i < OrochiAuthSharedState->signing_key_count; i++) {
    if (strcmp(OrochiAuthSharedState->signing_keys[i].key_id, key->key_id) ==
        0) {
  ...
  if (entry == NULL &&
      OrochiAuthSharedState->signing_key_count < OROCHI_AUTH_MAX_SIGNING_KEYS) {
    entry = &OrochiAuthSharedState
                 ->signing_keys[OrochiAuthSharedState->signing_key_count];
...
void orochi_auth_check_key_rotation(void) {
  ...
  rotation_threshold =
      now - (orochi_auth_key_rotation_days * 24 * 60 * 60 * USECS_PER_SEC);
 ...
 */
bool orochi_auth_check_rate_limit(const char *key, const char *action,
                                  int limit) {
  uint32 slot;
  int probe_count;
  ...
  for (probe_count = 0; probe_count < 8; probe_count++) {
    ...
    if (strncmp(entry->key, key, strlen(key)) == 0) {
      window_end =
          entry->window_start + (entry->window_seconds * USECS_PER_SEC);
 ...
 */
void orochi_auth_audit_log(const char *tenant_id, const char *user_id,
  ...
  elog(LOG, "Orochi Auth Audit: event=%s tenant=%s user=%s session=%s ip=%s",
       ...
       tenant_id ? tenant_id : "(none)", user_id ? user_id : "(none)",
       session_id ? session_id : "(none)", ip_address ? ip_address : "(none)");
  ...
  PG_TRY();
  {
    ...
    appendStringInfoString(
        &query, "INSERT INTO orochi.orochi_auth_audit_log "
    ...
    /* Execute with 9 parameters (resource_id added inline) */
    {
      ...
      ret = SPI_execute_with_args(query.data, 9, all_argtypes, all_values,
                                  all_nulls, false, 0);
    }
  ...
  PG_CATCH();
  {
    ...
    elog(WARNING,
         "Orochi Auth: audit logging failed, continuing without audit");
    FlushErrorState();
  }
  PG_END_TRY();
}
...
void orochi_auth_ensure_audit_partitions(void) {
  ...
  PG_TRY();
  {
    ...
    if (ret != SPI_OK_CONNECT) {
      elog(WARNING,
           "Orochi Auth: SPI_connect failed for partition creation: %d", ret);
      PG_RE_THROW();
    }
    connected = true;
    ...
    elog(
        DEBUG1,
        "Orochi Auth: ensured audit log partitions for %04d-%02d and %04d-%02d",
...
void orochi_auth_archive_audit_partitions(void) {
  ...
  now = GetCurrentTimestamp();
  retention_cutoff =
      now - ((int64)orochi_auth_audit_retention_days * USECS_PER_DAY);
  ...
  PG_TRY();
  {
    ...
    if (ret != SPI_OK_CONNECT) {
      elog(WARNING,
           "Orochi Auth: SPI_connect failed for partition archival: %d", ret);
      PG_RE_THROW();
    }
    connected = true;
    ...
    for (i = 0; i < SPI_processed; i++) {
      ...
      if (sscanf(partition_name, "orochi_auth_audit_log_y%4dm%2d", &part_year,
                 &part_month) != 2) {
        elog(DEBUG1, "Orochi Auth: skipping non-matching partition: %s",
             partition_name);
        continue;
      }
      ...
      if (part_year < cutoff_year ||
        ...
        if (ret == SPI_OK_UTILITY) {
          ...
          ret = SPI_execute(detach_query.data, false, 0);
          if (ret == SPI_OK_UTILITY) {
            elog(LOG, "Orochi Auth: dropped archived partition %s",
                 partition_name);
          } else {
