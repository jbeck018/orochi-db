...
void orochi_auth_client_authentication_hook(Port *port, int status) {
  ...
  if (orochi_auth_rate_limit_enabled) {
    if (!orochi_auth_check_rate_limit(ip_address, "login",
  ...
  if (jwt_token != NULL && strlen(jwt_token) > 0) {
    ...
    if (!validation.is_valid) {
      ...
      orochi_auth_audit_log(tenant_id, NULL, NULL, OROCHI_AUDIT_LOGIN_FAILED,
                            ip_address, user_agent, "jwt", NULL);

      ereport(FATAL, (errcode(ERRCODE_INVALID_AUTHORIZATION_SPECIFICATION),
                      errmsg("JWT token validation failed: %s",
    ...
    orochi_auth_audit_log(validation.tenant_id, validation.user_id,
                          validation.session_id, OROCHI_AUDIT_LOGIN_SUCCESS,
                          ip_address, user_agent, "jwt", NULL);
  ...
  if (api_key != NULL && strlen(api_key) > 0) {
    ...
    if (!validation.is_valid) {
      /* Log failed authentication */
      orochi_auth_audit_log(tenant_id, NULL, NULL, OROCHI_AUDIT_LOGIN_FAILED,
                            ip_address, user_agent, "api_key", NULL);

      ereport(FATAL, (errcode(ERRCODE_INVALID_AUTHORIZATION_SPECIFICATION),
                      errmsg("API key validation failed: %s",
                             validation.error_message),
                      errdetail("Error code: %s", validation.error_code)));
    }
  ...
  if (tenant_id != NULL && strlen(tenant_id) > 0) {
    SetConfigOption("orochi.auth_tenant_id", tenant_id, PGC_USERSET,
                    PGC_S_SESSION);
  }
}
...
static void orochi_auth_ensure_api_keys_table(void) {
  ...
  ret = SPI_execute("SELECT 1 FROM pg_namespace WHERE nspname = 'orochi'", true,
                    1);
 ...
 */
int64 orochi_auth_store_api_key(const char *user_id, const char *tenant_id,
                                const char *name, const char *prefix,
                                const char *key_hash, int64 scopes,
 ...
 */
bool orochi_auth_revoke_api_key_db(int64 key_id, const char *prefix,
                                   const char *user_id) {
  StringInfoData query;
  int ret;
  bool success = false;
  ...
  if (key_id <= 0 && (prefix == NULL || strlen(prefix) == 0)) {
    elog(WARNING,
         "orochi_auth_revoke_api_key_db: must specify key_id or prefix");
    return false;
  }
...
int orochi_auth_list_api_keys_db(const char *user_id, bool include_revoked,
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    *keys_out =
        (OrochiApiKeyInfo *)palloc0(sizeof(OrochiApiKeyInfo) * key_count);
...
Datum orochi_auth_create_api_key(PG_FUNCTION_ARGS) {
  ...
  /* Audit log */
  orochi_auth_audit_log(tenant_id, user_id, NULL, OROCHI_AUDIT_API_KEY_CREATED,
                        NULL, NULL, "api_key", name);
...
Datum orochi_auth_revoke_api_key(PG_FUNCTION_ARGS) {
  ...
  orochi_auth_audit_log(orochi_auth_get_current_tenant_id(), user_id, NULL,
                        OROCHI_AUDIT_API_KEY_REVOKED, NULL, NULL, "api_key",
                        key_id_str);
...
Datum orochi_auth_list_api_keys(PG_FUNCTION_ARGS) {
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    key_count =
        orochi_auth_list_api_keys_db(user_id, false, /* don't include revoked */
