...
extern pg_uuid_t endpoint_create(pg_uuid_t cluster_id, pg_uuid_t branch_id,
                                 const char *name,
                                 EndpointPoolerMode pooler_mode,
                                 int max_connections);

extern bool endpoint_update_status(pg_uuid_t endpoint_id,
                                   EndpointStatus new_status);

extern bool endpoint_delete(pg_uuid_t endpoint_id);

/* Token generation */
extern EndpointTokenResult *
endpoint_generate_token(pg_uuid_t endpoint_id, pg_uuid_t user_id,
                        const char *db_role, BranchPermissionLevel permission,
                        int valid_duration_sec);

extern char *endpoint_generate_connection_string(pg_uuid_t credential_id,
                                                 const char *signed_token,
                                                 const char *database,
                                                 const char *driver_type);

/* Token validation */
extern EndpointAuthResult *endpoint_validate_token(const char *token,
                                                   const char *endpoint_host,
                                                   const char *client_ip);

extern EndpointAuthResult *endpoint_validate_password(const char *username,
                                                      const char *password,
                                                      pg_uuid_t endpoint_id);

extern bool endpoint_validate_jwt_signature(const char *token,
                                            const uint8 *public_key,
                                            int public_key_len);

/* Credential management */
extern OrochiEndpointCredential *
...
extern bool
endpoint_create_password_credential(pg_uuid_t endpoint_id, const char *username,
                                    const char *password, const char *db_role,
                                    BranchPermissionLevel permission,
                                    pg_uuid_t created_by);

extern bool endpoint_revoke_credential(pg_uuid_t credential_id,
                                       pg_uuid_t revoked_by);

extern bool endpoint_rotate_token(pg_uuid_t credential_id,
                                  pg_uuid_t rotated_by);

extern void endpoint_record_credential_use(pg_uuid_t credential_id);

/* Session context */
extern EndpointSessionContext *
endpoint_create_session_context(EndpointJWTClaims *claims);

extern char **endpoint_get_startup_commands(EndpointSessionContext *context,
                                            int *command_count);
...
/* Caching */
extern void endpoint_cache_token(const char *token_id,
                                 EndpointJWTClaims *claims);
