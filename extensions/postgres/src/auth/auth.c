 ...
 */
bool orochi_auth_check_algorithm(char **newval, void **extra,
                                 GucSource source) {
...
void orochi_auth_define_gucs(void) {
  ...
  DefineCustomBoolVariable(
      "orochi.auth_enabled", "Enable Orochi authentication system", NULL,
      &orochi_auth_enabled, true, PGC_SIGHUP, 0, NULL, NULL, NULL);

  /* JWT Configuration */
  DefineCustomStringVariable(
      "orochi.jwt_secret", "JWT signing secret (deprecated, use signing keys)",
      NULL, &orochi_auth_jwt_secret, NULL, PGC_SIGHUP, GUC_SUPERUSER_ONLY, NULL,
      NULL, NULL);

  DefineCustomIntVariable(
      "orochi.token_expiry", "Default token expiry time in seconds", NULL,
      &orochi_auth_token_expiry, OROCHI_AUTH_ACCESS_TOKEN_TTL_SEC, 60, 86400,
      PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomIntVariable("orochi.max_sessions", "Maximum cached sessions",
                          NULL, &orochi_auth_max_sessions,
                          OROCHI_AUTH_MAX_CACHED_SESSIONS, 100, 100000,
                          PGC_POSTMASTER, 0, NULL, NULL, NULL);

  DefineCustomIntVariable(
      "orochi.auth_access_token_ttl", "Access token time-to-live in seconds",
      NULL, &orochi_auth_access_token_ttl, OROCHI_AUTH_ACCESS_TOKEN_TTL_SEC, 60,
      86400, PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomIntVariable(
      "orochi.auth_refresh_token_ttl", "Refresh token time-to-live in seconds",
      NULL, &orochi_auth_refresh_token_ttl, OROCHI_AUTH_REFRESH_TOKEN_TTL_SEC,
      3600, 2592000, /* 30 days */
      PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomIntVariable("orochi.auth_session_ttl",
                          "Session time-to-live in seconds", NULL,
                          &orochi_auth_session_ttl, OROCHI_AUTH_SESSION_TTL_SEC,
                          3600, 604800, /* 7 days */
                          PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomIntVariable("orochi.auth_max_sessions_per_user",
                          "Maximum concurrent sessions per user", NULL,
                          &orochi_auth_max_sessions_per_user, 10, 1, 1000,
                          PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomStringVariable("orochi.auth_jwt_algorithm",
                             "JWT signing algorithm (RS256, ES256, HS256)",
                             NULL, &orochi_auth_jwt_algorithm, "RS256",
                             PGC_SIGHUP, 0, orochi_auth_check_algorithm, NULL,
                             NULL);

  DefineCustomIntVariable(
      "orochi.auth_jwt_leeway", "JWT clock skew tolerance in seconds", NULL,
      &orochi_auth_jwt_leeway, 60, 0, 300, PGC_SIGHUP, 0, NULL, NULL, NULL);

  /* MFA Configuration */
  DefineCustomBoolVariable(
      "orochi.auth_mfa_required", "Require MFA for all users", NULL,
      &orochi_auth_mfa_required, false, PGC_SIGHUP, 0, NULL, NULL, NULL);

  /* Rate Limiting */
  DefineCustomBoolVariable("orochi.auth_rate_limit_enabled",
                           "Enable authentication rate limiting", NULL,
                           &orochi_auth_rate_limit_enabled, true, PGC_SIGHUP, 0,
                           NULL, NULL, NULL);

  DefineCustomIntVariable("orochi.auth_rate_limit_login",
                          "Maximum login attempts per window", NULL,
                          &orochi_auth_rate_limit_login, 5, 1, 100, PGC_SIGHUP,
                          0, NULL, NULL, NULL);

  DefineCustomIntVariable("orochi.auth_rate_limit_api",
                          "Maximum API requests per window", NULL,
                          &orochi_auth_rate_limit_api, 100, 1, 10000,
                          PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomIntVariable("orochi.auth_rate_limit_window",
                          "Rate limit window in seconds", NULL,
                          &orochi_auth_rate_limit_window, 60, 1, 3600,
                          PGC_SIGHUP, 0, NULL, NULL, NULL);

  /* Password Configuration */
  DefineCustomIntVariable("orochi.auth_password_min_length",
                          "Minimum password length", NULL,
                          &orochi_auth_password_min_length, 8, 6, 128,
                          PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomBoolVariable("orochi.auth_password_require_uppercase",
                           "Require uppercase letter in password", NULL,
                           &orochi_auth_password_require_uppercase, true,
                           PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomBoolVariable("orochi.auth_password_require_number",
                           "Require number in password", NULL,
                           &orochi_auth_password_require_number, true,
                           PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomBoolVariable("orochi.auth_password_require_special",
                           "Require special character in password", NULL,
                           &orochi_auth_password_require_special, false,
                           PGC_SIGHUP, 0, NULL, NULL, NULL);

  /* Audit Configuration */
  DefineCustomBoolVariable(
      "orochi.auth_audit_enabled", "Enable authentication audit logging", NULL,
      &orochi_auth_audit_enabled, true, PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomIntVariable("orochi.auth_audit_retention_days",
                          "Audit log retention period in days", NULL,
                          &orochi_auth_audit_retention_days, 90, 7, 3650,
                          PGC_SIGHUP, 0, NULL, NULL, NULL);

  /* Key Management */
  DefineCustomIntVariable(
      "orochi.auth_key_rotation_days", "Signing key rotation interval in days",
      NULL, &orochi_auth_key_rotation_days, OROCHI_AUTH_KEY_ROTATION_DAYS, 7,
      365, PGC_SIGHUP, 0, NULL, NULL, NULL);

  /* Session variables (set per connection, read by RLS policies) */
  DefineCustomStringVariable("orochi.auth_user_id",
                             "Current authenticated user ID", NULL,
                             &orochi_auth_current_user_id, "", PGC_USERSET,
                             GUC_NOT_IN_SAMPLE, NULL, NULL, NULL);

  DefineCustomStringVariable("orochi.auth_tenant_id", "Current tenant ID", NULL,
                             &orochi_auth_current_tenant_id, "", PGC_USERSET,
                             GUC_NOT_IN_SAMPLE, NULL, NULL, NULL);

  DefineCustomStringVariable("orochi.auth_role", "Current user role", NULL,
                             &orochi_auth_current_role, "anon", PGC_USERSET,
                             GUC_NOT_IN_SAMPLE, NULL, NULL, NULL);

  DefineCustomStringVariable("orochi.auth_session_id", "Current session ID",
                             NULL, &orochi_auth_current_session_id, "",
 ...
 */
OrochiTokenValidation orochi_auth_validate_token(const char *token,
 ...
 */
OrochiTokenValidation orochi_auth_validate_jwt_full(const char *token,
                                                    const char *tenant_id) {
  OrochiTokenValidation result;
  ...
  if (!orochi_auth_verify_jwt_signature(token, key)) {
    ...
    if (OrochiAuthSharedState != NULL) {
      pg_atomic_fetch_add_u64(&OrochiAuthSharedState->failed_authentications,
                              1);
    }
...
OrochiTokenValidation orochi_auth_validate_api_key(const char *api_key) {
  ...
  if (api_key == NULL ||
      strlen(api_key) < OROCHI_AUTH_API_KEY_PREFIX_SIZE + 1) {
    result.is_valid = false;
 ...
 */
bool orochi_auth_parse_jwt_claims(const char *token,
                                  OrochiTokenValidation *result) {
  ...
  if (header_end == NULL) {
    ...
    strlcpy(result->error_message,
            "Invalid JWT format: missing first separator",
            sizeof(result->error_message));
    return false;
  }
  ...
  if (payload_end == NULL) {
    strlcpy(result->error_code, "INVALID_FORMAT", sizeof(result->error_code));
    strlcpy(result->error_message,
            "Invalid JWT format: missing second separator",
            sizeof(result->error_message));
    return false;
  }
  ...
  if (jval != NULL && jval->type == jbvNumeric) {
    ...
    result->expires_at =
        GetCurrentTimestamp() + (orochi_auth_access_token_ttl * USECS_PER_SEC);
  }
  ...
  if (jval != NULL && jval->type == jbvBinary) {
    ...
    while ((r = JsonbIteratorNext(&it, &elem, true)) != WJB_DONE) {
      if (r == WJB_ELEM && elem.type == jbvString &&
        ...
        memcpy(result->roles[result->role_count], elem.val.string.val,
               copy_len);
  ...
  if (result->tenant_id[0] == '\0') {
    ...
    if (jval != NULL && jval->type == jbvBinary) {
      ...
      if (tenant_val != NULL && tenant_val->type == jbvString) {
        int copy_len =
            Min(tenant_val->val.string.len, OROCHI_AUTH_TENANT_ID_SIZE);
 ...
 */
static unsigned char *auth_base64url_decode_internal(const char *input,
                                                     Size *output_len) {
  Size input_len;
  Size decoded_len;
...
static bool verify_hmac_signature_internal(const char *data, Size data_len,
                                           ...
                                           Size sig_len, const char *secret,
                                           Size secret_len,
  ...
  /* Compute HMAC */
  if (HMAC(md, secret, (int)secret_len, (unsigned char *)data, data_len,
 ...
 */
static unsigned char *convert_ecdsa_jwt_sig_to_der(const unsigned char *jwt_sig,
                                                   Size jwt_sig_len,
                                                   Size *der_len,
                                                   OrochiJwtAlgorithm alg) {
...
static OrochiJwtAlgorithm parse_jwt_header_algorithm(const char *token) {
  ...
  PG_TRY();
  {
    ...
    if (alg_val != NULL && alg_val->type == jbvString) {
      char *alg_str =
          pnstrdup(alg_val->val.string.val, alg_val->val.string.len);
 ...
 */
bool orochi_auth_verify_jwt_signature(const char *token,
                                      OrochiSigningKey *key) {
  const char *first_dot;
  ...
  if (token_alg != key->algorithm) {
    elog(DEBUG1,
         "JWT verification failed: algorithm mismatch (token: %d, key: %d)",
  ...
  case OROCHI_JWT_ALG_HS512:
    ...
    if (key->public_key[0] == '\0') {
      ...
      result = verify_hmac_signature_internal(
          header_payload, header_payload_len, signature, sig_len,
          key->public_key, strlen(key->public_key), key->algorithm);
    }
    break;
  ...
  case OROCHI_JWT_ALG_RS512:
     ...
     */
    result = verify_rsa_signature_internal(header_payload, header_payload_len,
                                           signature, sig_len, key->public_key,
                                           key->algorithm);
    break;
  ...
  case OROCHI_JWT_ALG_ES512:
     ...
     */
    result = verify_ecdsa_signature_internal(header_payload, header_payload_len,
                                             signature, sig_len,
 ...
 */
OrochiSession *orochi_auth_create_session(const char *user_id,
                                          const char *tenant_id,
  ...
  if (orochi_auth_audit_enabled) {
    orochi_auth_audit_log(tenant_id, user_id, session->session_id,
                          OROCHI_AUDIT_SESSION_CREATED, ip_address, user_agent,
...
void orochi_auth_revoke_session(const char *session_id) {
  ...
  if (orochi_auth_audit_enabled && session != NULL) {
    orochi_auth_audit_log(session->tenant_id, session->user_id, session_id,
                          OROCHI_AUDIT_LOGOUT, session->ip_address,
...
void orochi_auth_set_session_context(OrochiTokenValidation *auth) {
  ...
  /* Set user ID */
  SetConfigOption("orochi.auth_user_id", auth->user_id, PGC_USERSET,
                  PGC_S_SESSION);

  /* Set tenant ID */
  SetConfigOption("orochi.auth_tenant_id", auth->tenant_id, PGC_USERSET,
                  PGC_S_SESSION);

  /* Set session ID */
  SetConfigOption("orochi.auth_session_id", auth->session_id, PGC_USERSET,
                  PGC_S_SESSION);

  /* Set role for RLS */
  if (auth->role_count > 0) {
    SetConfigOption("orochi.auth_role", auth->roles[0], PGC_USERSET,
                    PGC_S_SESSION);
  } else {
    SetConfigOption("orochi.auth_role", "authenticated", PGC_USERSET,
                    PGC_S_SESSION);
  }
}
 ...
 */
void orochi_auth_get_stats(uint64 *total_requests, uint64 *cache_hits,
                           uint64 *cache_misses, uint64 *failed_auths,
                           uint64 *tokens_issued, uint64 *tokens_revoked) {
  if (OrochiAuthSharedState == NULL || !OrochiAuthSharedState->is_initialized) {
    if (total_requests)
      *total_requests = 0;
    if (cache_hits)
  ...
  if (total_requests)
    *total_requests =
        pg_atomic_read_u64(&OrochiAuthSharedState->total_auth_requests);
  if (cache_hits)
  ...
  if (failed_auths)
    *failed_auths =
        pg_atomic_read_u64(&OrochiAuthSharedState->failed_authentications);
  if (tokens_issued)
    *tokens_issued = pg_atomic_read_u64(&OrochiAuthSharedState->tokens_issued);
  if (tokens_revoked)
    *tokens_revoked =
        pg_atomic_read_u64(&OrochiAuthSharedState->tokens_revoked);
}
