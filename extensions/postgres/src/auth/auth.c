 ...
 */
bool orochi_auth_check_algorithm(char **newval, void **extra, GucSource source)
{
    if (*newval == NULL || strlen(*newval) == 0)
        return true; /* Allow empty/NULL */

    /* Check for valid algorithm strings */
    if (strcmp(*newval, "HS256") == 0 || strcmp(*newval, "HS384") == 0 ||
        strcmp(*newval, "HS512") == 0 || strcmp(*newval, "RS256") == 0 ||
        strcmp(*newval, "RS384") == 0 || strcmp(*newval, "RS512") == 0 ||
        strcmp(*newval, "ES256") == 0 || strcmp(*newval, "ES384") == 0 ||
        strcmp(*newval, "ES512") == 0) {
        return true;
    }

    GUC_check_errdetail("Valid algorithms are: HS256, HS384, HS512, RS256, "
 ...
 */
void orochi_auth_define_gucs(void)
{
    /* Main enable/disable */
    DefineCustomBoolVariable("orochi.auth_enabled", "Enable Orochi authentication system", NULL,
                             &orochi_auth_enabled, true, PGC_SIGHUP, 0, NULL, NULL, NULL);

    /* JWT Configuration */
    DefineCustomStringVariable(
        "orochi.jwt_secret", "JWT signing secret (deprecated, use signing keys)", NULL,
        &orochi_auth_jwt_secret, NULL, PGC_SIGHUP, GUC_SUPERUSER_ONLY, NULL, NULL, NULL);

    DefineCustomIntVariable("orochi.token_expiry", "Default token expiry time in seconds", NULL,
                            &orochi_auth_token_expiry, OROCHI_AUTH_ACCESS_TOKEN_TTL_SEC, 60, 86400,
                            PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomIntVariable("orochi.max_sessions", "Maximum cached sessions", NULL,
                            &orochi_auth_max_sessions, OROCHI_AUTH_MAX_CACHED_SESSIONS, 100, 100000,
                            PGC_POSTMASTER, 0, NULL, NULL, NULL);

    DefineCustomIntVariable("orochi.auth_access_token_ttl", "Access token time-to-live in seconds",
                            NULL, &orochi_auth_access_token_ttl, OROCHI_AUTH_ACCESS_TOKEN_TTL_SEC,
                            60, 86400, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomIntVariable("orochi.auth_refresh_token_ttl",
                            "Refresh token time-to-live in seconds", NULL,
                            &orochi_auth_refresh_token_ttl, OROCHI_AUTH_REFRESH_TOKEN_TTL_SEC, 3600,
                            2592000, /* 30 days */
                            PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomIntVariable("orochi.auth_session_ttl", "Session time-to-live in seconds", NULL,
                            &orochi_auth_session_ttl, OROCHI_AUTH_SESSION_TTL_SEC, 3600,
                            604800, /* 7 days */
                            PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomIntVariable(
        "orochi.auth_max_sessions_per_user", "Maximum concurrent sessions per user", NULL,
        &orochi_auth_max_sessions_per_user, 10, 1, 1000, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomStringVariable("orochi.auth_jwt_algorithm",
                               "JWT signing algorithm (RS256, ES256, HS256)", NULL,
                               &orochi_auth_jwt_algorithm, "RS256", PGC_SIGHUP, 0,
                               orochi_auth_check_algorithm, NULL, NULL);

    DefineCustomIntVariable("orochi.auth_jwt_leeway", "JWT clock skew tolerance in seconds", NULL,
                            &orochi_auth_jwt_leeway, 60, 0, 300, PGC_SIGHUP, 0, NULL, NULL, NULL);

    /* MFA Configuration */
    DefineCustomBoolVariable("orochi.auth_mfa_required", "Require MFA for all users", NULL,
                             &orochi_auth_mfa_required, false, PGC_SIGHUP, 0, NULL, NULL, NULL);

    /* Rate Limiting */
    DefineCustomBoolVariable(
        "orochi.auth_rate_limit_enabled", "Enable authentication rate limiting", NULL,
        &orochi_auth_rate_limit_enabled, true, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomIntVariable("orochi.auth_rate_limit_login", "Maximum login attempts per window",
                            NULL, &orochi_auth_rate_limit_login, 5, 1, 100, PGC_SIGHUP, 0, NULL,
                            NULL, NULL);

    DefineCustomIntVariable("orochi.auth_rate_limit_api", "Maximum API requests per window", NULL,
                            &orochi_auth_rate_limit_api, 100, 1, 10000, PGC_SIGHUP, 0, NULL, NULL,
                            NULL);

    DefineCustomIntVariable("orochi.auth_rate_limit_window", "Rate limit window in seconds", NULL,
                            &orochi_auth_rate_limit_window, 60, 1, 3600, PGC_SIGHUP, 0, NULL, NULL,
                            NULL);

    /* Password Configuration */
    DefineCustomIntVariable("orochi.auth_password_min_length", "Minimum password length", NULL,
                            &orochi_auth_password_min_length, 8, 6, 128, PGC_SIGHUP, 0, NULL, NULL,
                            NULL);

    DefineCustomBoolVariable(
        "orochi.auth_password_require_uppercase", "Require uppercase letter in password", NULL,
        &orochi_auth_password_require_uppercase, true, PGC_SIGHUP, 0, NULL, NULL, NULL);

    DefineCustomBoolVariable("orochi.auth_password_require_number", "Require number in password",
                             NULL, &orochi_auth_password_require_number, true, PGC_SIGHUP, 0, NULL,
                             NULL, NULL);

    DefineCustomBoolVariable(
        "orochi.auth_password_require_special", "Require special character in password", NULL,
        &orochi_auth_password_require_special, false, PGC_SIGHUP, 0, NULL, NULL, NULL);

    /* Audit Configuration */
    DefineCustomBoolVariable("orochi.auth_audit_enabled", "Enable authentication audit logging",
                             NULL, &orochi_auth_audit_enabled, true, PGC_SIGHUP, 0, NULL, NULL,
                             NULL);

    DefineCustomIntVariable(
        "orochi.auth_audit_retention_days", "Audit log retention period in days", NULL,
        &orochi_auth_audit_retention_days, 90, 7, 3650, PGC_SIGHUP, 0, NULL, NULL, NULL);

    /* Key Management */
    DefineCustomIntVariable("orochi.auth_key_rotation_days",
                            "Signing key rotation interval in days", NULL,
                            &orochi_auth_key_rotation_days, OROCHI_AUTH_KEY_ROTATION_DAYS, 7, 365,
                            PGC_SIGHUP, 0, NULL, NULL, NULL);

    /* Session variables (set per connection, read by RLS policies) */
    DefineCustomStringVariable("orochi.auth_user_id", "Current authenticated user ID", NULL,
                               &orochi_auth_current_user_id, "", PGC_USERSET, GUC_NOT_IN_SAMPLE,
                               NULL, NULL, NULL);

    DefineCustomStringVariable("orochi.auth_tenant_id", "Current tenant ID", NULL,
                               &orochi_auth_current_tenant_id, "", PGC_USERSET, GUC_NOT_IN_SAMPLE,
                               NULL, NULL, NULL);

    DefineCustomStringVariable("orochi.auth_role", "Current user role", NULL,
                               &orochi_auth_current_role, "anon", PGC_USERSET, GUC_NOT_IN_SAMPLE,
                               NULL, NULL, NULL);

    DefineCustomStringVariable("orochi.auth_session_id", "Current session ID", NULL,
                               &orochi_auth_current_session_id, "", PGC_USERSET, GUC_NOT_IN_SAMPLE,
                               NULL, NULL, NULL);
}
 ...
 */
OrochiTokenValidation orochi_auth_validate_token(const char *token, const char *tenant_id)
{
    ...
    if (!orochi_auth_enabled) {
        ...
        strlcpy(result.error_message, "Authentication is disabled", sizeof(result.error_message));
        return result;
    }
    ...
    if (token == NULL || strlen(token) == 0) {
        ...
        strlcpy(result.error_message, "No token provided", sizeof(result.error_message));
        return result;
    }
 ...
 */
bool orochi_auth_validate_token_fast(const char *token, int token_len,
                                     OrochiTokenValidation *result)
{
    char token_hash[OROCHI_AUTH_TOKEN_HASH_SIZE + 1];
    OrochiToken *cached;
    ...
    if (cached != NULL) {
        ...
        if (cached->expires_at < now) {
            ...
            strlcpy(result->error_message, "Token has expired", sizeof(result->error_message));
        ...
        if (cached->is_revoked) {
            ...
            strlcpy(result->error_message, "Token has been revoked", sizeof(result->error_message));
            LWLockRelease(OrochiAuthSharedState->token_cache_lock);
 ...
 */
OrochiTokenValidation orochi_auth_validate_jwt_full(const char *token, const char *tenant_id)
{
    OrochiTokenValidation result;
 ...
 *    Validate an API key
 */
OrochiTokenValidation orochi_auth_validate_api_key(const char *api_key)
{
    OrochiTokenValidation result;
    ...
    if (api_key == NULL || strlen(api_key) < OROCHI_AUTH_API_KEY_PREFIX_SIZE + 1) {
        ...
        strlcpy(result.error_message, "Invalid API key format", sizeof(result.error_message));
        return result;
    }
    ...
    if (key_info == NULL) {
        ...
        strlcpy(result.error_message, "API key not found", sizeof(result.error_message));
        return result;
    }
    ...
    if (key_info->is_revoked) {
        ...
        strlcpy(result.error_message, "API key has been revoked", sizeof(result.error_message));
        pfree(key_info);
        return result;
    }
    ...
    if (key_info->has_expires && key_info->expires_at < now) {
        ...
        strlcpy(result.error_message, "API key has expired", sizeof(result.error_message));
        pfree(key_info);
        return result;
    }
 ...
 */
bool orochi_auth_parse_jwt_claims(const char *token, OrochiTokenValidation *result)
{
    ...
    if (payload_b64_len <= 0 || payload_b64_len > OROCHI_AUTH_JWT_MAX_SIZE) {
        ...
        strlcpy(result->error_message, "Invalid JWT payload size", sizeof(result->error_message));
        return false;
    }
    ...
    PG_TRY();
    {
        jsonb = DatumGetJsonbP(DirectFunctionCall1(jsonb_in, CStringGetDatum(payload_json)));
    }
    PG_CATCH();
    {
        pfree(payload_json);
    ...
    if (jval != NULL && jval->type == jbvNumeric) {
        int64 exp_unix =
            DatumGetInt64(DirectFunctionCall1(numeric_int8, NumericGetDatum(jval->val.numeric)));
 ...
 */
static unsigned char *auth_base64url_decode_internal(const char *input, Size *output_len)
{
    Size input_len;
    Size decoded_len;
 ...
 */
static const EVP_MD *get_evp_md_for_algorithm(OrochiJwtAlgorithm alg)
{
    switch (alg) {
 ...
 */
static bool verify_hmac_signature_internal(const char *data, Size data_len,
                                           ...
                                           OrochiJwtAlgorithm alg)
{
    ...
    if (sig_len != hmac_len) {
        elog(DEBUG1, "HMAC signature length mismatch: expected %u, got %zu", hmac_len, sig_len);
        return false;
    }
 ...
 */
static bool verify_rsa_signature_internal(const char *data, Size data_len,
                                          const unsigned char *signature, Size sig_len,
                                          const char *public_key_pem, OrochiJwtAlgorithm alg)
{
    BIO *bio = NULL;
    EVP_PKEY *pkey = NULL;
 ...
 */
static unsigned char *convert_ecdsa_jwt_sig_to_der(const unsigned char *jwt_sig, Size jwt_sig_len,
 ...
 */
static bool verify_ecdsa_signature_internal(const char *data, Size data_len,
                                            const unsigned char *signature, Size sig_len,
                                            const char *public_key_pem, OrochiJwtAlgorithm alg)
{
    BIO *bio = NULL;
 ...
 */
static OrochiJwtAlgorithm parse_jwt_header_algorithm(const char *token)
{
    ...
    PG_TRY();
    {
        jsonb = DatumGetJsonbP(DirectFunctionCall1(jsonb_in, CStringGetDatum(header_json)));
 ...
 */
bool orochi_auth_verify_jwt_signature(const char *token, OrochiSigningKey *key)
{
    const char *first_dot;
    ...
    if (token_alg != key->algorithm) {
        elog(DEBUG1, "JWT verification failed: algorithm mismatch (token: %d, key: %d)", token_alg,
             key->algorithm);
        return false;
    }
    ...
    case OROCHI_JWT_ALG_HS512:
        ...
        if (key->public_key[0] == '\0') {
            ...
            result = verify_hmac_signature_internal(header_payload, header_payload_len, signature,
                                                    sig_len, key->public_key,
                                                    strlen(key->public_key), key->algorithm);
        }
        break;
    ...
    case OROCHI_JWT_ALG_RS512:
         ...
         */
        result = verify_rsa_signature_internal(header_payload, header_payload_len, signature,
                                               sig_len, key->public_key, key->algorithm);
        break;
    ...
    case OROCHI_JWT_ALG_ES512:
         ...
         */
        result = verify_ecdsa_signature_internal(header_payload, header_payload_len, signature,
                                                 sig_len, key->public_key, key->algorithm);
        break;

    default:
        elog(DEBUG1, "JWT verification failed: unsupported algorithm %d", key->algorithm);
        result = false;
        break;
    }
 ...
 */
void orochi_auth_generate_session_id(char *session_id_out)
{
    pg_uuid_t uuid;
    ...
    if (!pg_strong_random(&uuid, sizeof(pg_uuid_t))) {
        ...
        snprintf(session_id_out, OROCHI_AUTH_SESSION_ID_SIZE + 1, "%016lx%016lx",
                 (unsigned long)now, (unsigned long)MyProcPid);
        return;
    }

    /* Format as hex string without dashes */
    snprintf(session_id_out, OROCHI_AUTH_SESSION_ID_SIZE + 1,
             "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", uuid.data[0],
             uuid.data[1], uuid.data[2], uuid.data[3], uuid.data[4], uuid.data[5], uuid.data[6],
             uuid.data[7], uuid.data[8], uuid.data[9], uuid.data[10], uuid.data[11], uuid.data[12],
             uuid.data[13], uuid.data[14], uuid.data[15]);
}
 ...
 */
void orochi_auth_generate_token(char *token_out, int length)
{
    uint8 *random_bytes;
    int bytes_needed;
    int i;
 ...
 */
OrochiSession *orochi_auth_create_session(const char *user_id, const char *tenant_id,
                                          OrochiAuthMethod auth_method, const char *ip_address,
                                          const char *user_agent)
{
    ...
    if (orochi_auth_audit_enabled) {
        orochi_auth_audit_log(tenant_id, user_id, session->session_id, OROCHI_AUDIT_SESSION_CREATED,
 ...
 *    Get session by ID
 */
OrochiSession *orochi_auth_get_session(const char *session_id)
{
 ...
 */
void orochi_auth_touch_session(const char *session_id)
{
    OrochiSession *session;
 ...
 *    Revoke a session
 */
void orochi_auth_revoke_session(const char *session_id)
{
    OrochiSession *session;
    ...
    if (orochi_auth_audit_enabled && session != NULL) {
        orochi_auth_audit_log(session->tenant_id, session->user_id, session_id, OROCHI_AUDIT_LOGOUT,
                              session->ip_address, session->user_agent, "session", session_id);
    }
}
 ...
 */
void orochi_auth_set_session_context(OrochiTokenValidation *auth)
{
    if (auth == NULL || !auth->is_valid)
        return;

    /* Set user ID */
    SetConfigOption("orochi.auth_user_id", auth->user_id, PGC_USERSET, PGC_S_SESSION);

    /* Set tenant ID */
    SetConfigOption("orochi.auth_tenant_id", auth->tenant_id, PGC_USERSET, PGC_S_SESSION);

    /* Set session ID */
    SetConfigOption("orochi.auth_session_id", auth->session_id, PGC_USERSET, PGC_S_SESSION);

    /* Set role for RLS */
    if (auth->role_count > 0) {
        SetConfigOption("orochi.auth_role", auth->roles[0], PGC_USERSET, PGC_S_SESSION);
    } else {
        SetConfigOption("orochi.auth_role", "authenticated", PGC_USERSET, PGC_S_SESSION);
    }
}
 ...
 */
void orochi_auth_clear_session_context(void)
{
    SetConfigOption("orochi.auth_user_id", "", PGC_USERSET, PGC_S_SESSION);
    SetConfigOption("orochi.auth_tenant_id", "", PGC_USERSET, PGC_S_SESSION);
    SetConfigOption("orochi.auth_session_id", "", PGC_USERSET, PGC_S_SESSION);
    SetConfigOption("orochi.auth_role", "anon", PGC_USERSET, PGC_S_SESSION);
}
 ...
 */
char *orochi_auth_get_current_user_id(void)
{
    const char *user_id;
 ...
 */
char *orochi_auth_get_current_tenant_id(void)
{
 ...
 *    Get current role
 */
char *orochi_auth_get_current_role(void)
{
    const char *role;
 ...
 */
OrochiJwtAlgorithm orochi_auth_algorithm_from_string(const char *alg)
{
    if (alg == NULL)
        return OROCHI_JWT_ALG_RS256;

    if (strcmp(alg, "HS256") == 0)
        return OROCHI_JWT_ALG_HS256;
    if (strcmp(alg, "HS384") == 0)
        return OROCHI_JWT_ALG_HS384;
    if (strcmp(alg, "HS512") == 0)
        return OROCHI_JWT_ALG_HS512;
    if (strcmp(alg, "RS256") == 0)
        return OROCHI_JWT_ALG_RS256;
    if (strcmp(alg, "RS384") == 0)
        return OROCHI_JWT_ALG_RS384;
    if (strcmp(alg, "RS512") == 0)
        return OROCHI_JWT_ALG_RS512;
    if (strcmp(alg, "ES256") == 0)
        return OROCHI_JWT_ALG_ES256;
    if (strcmp(alg, "ES384") == 0)
        return OROCHI_JWT_ALG_ES384;
    if (strcmp(alg, "ES512") == 0)
        return OROCHI_JWT_ALG_ES512;
 ...
 */
const char *orochi_auth_algorithm_to_string(OrochiJwtAlgorithm alg)
{
    switch (alg) {
    case OROCHI_JWT_ALG_HS256:
        return "HS256";
 ...
 */
const char *orochi_auth_method_name(OrochiAuthMethod method)
{
    switch (method) {
    case OROCHI_AUTH_NONE:
        return "none";
    case OROCHI_AUTH_PASSWORD:
        return "password";
    case OROCHI_AUTH_API_KEY:
        return "api_key";
    case OROCHI_AUTH_JWT:
        return "jwt";
    case OROCHI_AUTH_OAUTH:
        return "oauth";
    case OROCHI_AUTH_MAGIC_LINK:
        return "magic_link";
    case OROCHI_AUTH_PHONE_OTP:
        return "phone_otp";
    case OROCHI_AUTH_SAML:
        return "saml";
    case OROCHI_AUTH_CERTIFICATE:
        return "certificate";
    case OROCHI_AUTH_CONNECTION_STRING:
 ...
 */
const char *orochi_auth_session_state_name(OrochiSessionState state)
{
    switch (state) {
    case OROCHI_SESSION_ACTIVE:
        return "active";
    case OROCHI_SESSION_IDLE:
        return "idle";
    case OROCHI_SESSION_EXPIRED:
        return "expired";
    case OROCHI_SESSION_REVOKED:
        return "revoked";
    case OROCHI_SESSION_MFA_REQUIRED:
        return "mfa_required";
    default:
        return "unknown";
    }
 ...
 */
const char *orochi_auth_audit_event_name(OrochiAuditEvent event)
{
    switch (event) {
    case OROCHI_AUDIT_LOGIN_SUCCESS:
        return "login_success";
    case OROCHI_AUDIT_LOGIN_FAILED:
        return "login_failed";
    case OROCHI_AUDIT_LOGOUT:
        return "logout";
    case OROCHI_AUDIT_TOKEN_ISSUED:
        return "token_issued";
    case OROCHI_AUDIT_TOKEN_REVOKED:
        return "token_revoked";
    case OROCHI_AUDIT_TOKEN_REFRESHED:
        return "token_refreshed";
    case OROCHI_AUDIT_PASSWORD_CHANGED:
        return "password_changed";
    case OROCHI_AUDIT_PASSWORD_RESET:
        return "password_reset";
    case OROCHI_AUDIT_MFA_ENROLLED:
        return "mfa_enrolled";
    case OROCHI_AUDIT_MFA_VERIFIED:
        return "mfa_verified";
    case OROCHI_AUDIT_MFA_FAILED:
        return "mfa_failed";
    case OROCHI_AUDIT_API_KEY_CREATED:
        return "api_key_created";
    case OROCHI_AUDIT_API_KEY_REVOKED:
        return "api_key_revoked";
    case OROCHI_AUDIT_USER_CREATED:
        return "user_created";
    case OROCHI_AUDIT_USER_UPDATED:
        return "user_updated";
    case OROCHI_AUDIT_USER_DELETED:
        return "user_deleted";
    case OROCHI_AUDIT_ROLE_ASSIGNED:
        return "role_assigned";
    case OROCHI_AUDIT_ROLE_REVOKED:
        return "role_revoked";
    case OROCHI_AUDIT_PERMISSION_DENIED:
        return "permission_denied";
    case OROCHI_AUDIT_RATE_LIMITED:
        return "rate_limited";
    case OROCHI_AUDIT_SUSPICIOUS_ACTIVITY:
        return "suspicious_activity";
    case OROCHI_AUDIT_KEY_ROTATED:
        return "key_rotated";
    case OROCHI_AUDIT_SESSION_CREATED:
        return "session_created";
    case OROCHI_AUDIT_SESSION_EXPIRED:
 ...
 */
void orochi_auth_get_stats(uint64 *total_requests, uint64 *cache_hits, uint64 *cache_misses,
                           uint64 *failed_auths, uint64 *tokens_issued, uint64 *tokens_revoked)
{
    if (OrochiAuthSharedState == NULL || !OrochiAuthSharedState->is_initialized) {
        if (total_requests)
            *total_requests = 0;
 ...
 */
Datum orochi_auth_uid(PG_FUNCTION_ARGS)
{
    char *user_id;
 ...
 */
Datum orochi_auth_role(PG_FUNCTION_ARGS)
{
    char *role;
 ...
 */
Datum orochi_auth_stats(PG_FUNCTION_ARGS)
{
    ...
    orochi_auth_get_stats(&total_requests, &cache_hits, &cache_misses, &failed_auths,
                          &tokens_issued, &tokens_revoked);
