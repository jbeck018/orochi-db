...
OrochiAuthResult *orochi_auth_sign_in_anonymous(void) {
  ...
  /* Emit webhook event */
  {
    ...
    appendStringInfo(&payload_json,
                     "{\"user_id\": \"%s\", \"is_anonymous\": true}", uuid_str);

    payload = DatumGetJsonbP(
        DirectFunctionCall1(jsonb_in, CStringGetDatum(payload_json.data)));
 ...
 */
OrochiAuthResult *orochi_auth_link_email(pg_uuid_t user_id, const char *email,
                                         const char *password) {
  OrochiAuthResult *result;
  OrochiAuthUser *user;
  OrochiAuthConfig *config;
  ...
  if (password != NULL && strlen(password) > 0) {
    if (strlen(password) < OROCHI_AUTH_MIN_PASSWORD_LENGTH) {
      ...
      result->error_message =
          psprintf("Password must be at least %d characters",
 ...
 */
OrochiAuthResult *orochi_auth_link_oauth_identity(pg_uuid_t user_id,
  ...
  if (!user->is_anonymous) {
    ...
    result->error_code = "not_anonymous";
    result->error_message =
        "Only anonymous users can link new identities via this method";
    return result;
  }
  ...
  if (identity_data) {
    ...
    (void)JsonbToCString(&identity_json, &identity_data->root,
                         VARSIZE(identity_data));
  } else {
    initStringInfo(&identity_json);
  ...
  appendStringInfo(
      ...
      uuid_str, quote_literal_cstr(provider_id),
      quote_literal_cstr(provider_name),
...
static void emit_webhook_event(const char *event_type, pg_uuid_t user_id,
  ...
  appendStringInfo(&notify_payload,
                   ...
                   "\"%s\", \"data\": %s}",
                   event_type, uuid_str,
                   timestamptz_to_str(GetCurrentTimestamp()), payload_str.data);
...
Datum orochi_auth_link_email_sql(PG_FUNCTION_ARGS) {
  ...
  OrochiAuthResult *result =
      orochi_auth_link_email(*user_uuid, email, password);
  ...
  if (result->success)
    ...
    PG_RETURN_JSONB_P(
        DirectFunctionCall1(jsonb_in, CStringGetDatum(error_json.data)));
  }
}
