...
static void emit_webhook_event(const char *event_type, pg_uuid_t user_id,
                                Jsonb *payload);
 ...
 */
OrochiAuthResult *
orochi_auth_sign_in_anonymous(void)
{
    ...
    /* Emit webhook event */
    {
        ...
        appendStringInfo(&payload_json,
            "{\"user_id\": \"%s\", \"is_anonymous\": true}",
            uuid_str);

        payload = DatumGetJsonbP(DirectFunctionCall1(jsonb_in,
                                  CStringGetDatum(payload_json.data)));
 ...
 */
bool
orochi_auth_is_anonymous(pg_uuid_t user_id)
{
 ...
 */
OrochiAuthResult *
orochi_auth_link_email(pg_uuid_t user_id, const char *email, const char *password)
{
    OrochiAuthResult *result;
    OrochiAuthUser *user;
    OrochiAuthConfig *config;
    ...
    appendStringInfo(&query,
        ...
        "(user_id, provider_id, provider, identity_data, created_at, updated_at) "
        "VALUES ('%s', %s, 'email', "
        "'{\"sub\": \"%s\", \"email\": \"%s\", \"email_verified\": false}'::jsonb, "
        "NOW(), NOW()) "
    ...
    /* Emit webhook event */
    {
        ...
        initStringInfo(&payload_json);
        appendStringInfo(&payload_json,
            "{\"user_id\": \"%s\", \"email\": \"%s\", \"linked_from_anonymous\": true}",
            uuid_str, email);
 ...
 */
OrochiAuthResult *
orochi_auth_link_phone(pg_uuid_t user_id, const char *phone)
{
    OrochiAuthResult *result;
    OrochiAuthUser *user;
    StringInfoData query;
    ...
    appendStringInfo(&query,
        ...
        "(user_id, provider_id, provider, identity_data, created_at, updated_at) "
        "VALUES ('%s', %s, 'phone', "
        "'{\"sub\": \"%s\", \"phone\": \"%s\", \"phone_verified\": false}'::jsonb, "
        "NOW(), NOW()) "
        "ON CONFLICT (provider, provider_id) DO UPDATE SET "
        "identity_data = EXCLUDED.identity_data, updated_at = NOW()",
        uuid_str,
    ...
    /* Emit webhook event */
    {
        ...
        initStringInfo(&payload_json);
        appendStringInfo(&payload_json,
            "{\"user_id\": \"%s\", \"phone\": \"%s\", \"linked_from_anonymous\": true}",
            uuid_str, phone);
 ...
 */
OrochiAuthResult *
orochi_auth_link_oauth_identity(pg_uuid_t user_id,
                                 OrochiAuthProvider provider,
                                 ...
                                 Jsonb *identity_data)
{
    ...
    appendStringInfo(&query,
        ...
        quote_literal_cstr(provider_name),
        quote_literal_cstr(provider_id));
    ...
    /* Emit webhook event */
    {
        ...
        initStringInfo(&payload_json);
        appendStringInfo(&payload_json,
            "{\"user_id\": \"%s\", \"provider\": \"%s\", \"linked_from_anonymous\": true}",
            uuid_str, provider_name);
 ...
 */
bool
orochi_auth_convert_anonymous_to_permanent(pg_uuid_t user_id)
{
    OrochiAuthUser *user;
    StringInfoData query;
    int ret;
 ...
 */
static void
emit_webhook_event(const char *event_type, pg_uuid_t user_id, Jsonb *payload)
{
    ...
    appendStringInfo(&notify_payload,
        "{\"event\": \"%s\", \"user_id\": \"%s\", \"timestamp\": \"%s\", \"data\": %s}",
        event_type,
        uuid_str,
    ...
    if (SPI_connect() == SPI_OK_CONNECT)
    {
        ...
        appendStringInfo(&notify_query,
            "SELECT pg_notify('auth_webhook', %s)",
...
Datum
orochi_auth_sign_in_anonymous_sql(PG_FUNCTION_ARGS)
{
    OrochiAuthResult *result;
    TupleDesc tupdesc;
    Datum values[4];
...
Datum
orochi_auth_link_email_sql(PG_FUNCTION_ARGS)
{
    ...
    if (result->success)
        PG_RETURN_JSONB_P(DirectFunctionCall1(jsonb_in,
            CStringGetDatum("{\"success\": true, \"message\": \"Email linked successfully\"}")));
    else
    {
...
Datum
orochi_auth_link_phone_sql(PG_FUNCTION_ARGS)
{
    pg_uuid_t *user_uuid = PG_GETARG_UUID_P(0);
    ...
    if (result->success)
        PG_RETURN_JSONB_P(DirectFunctionCall1(jsonb_in,
            CStringGetDatum("{\"success\": true, \"message\": \"Phone linked successfully\"}")));
    else
    {
        StringInfoData error_json;
        initStringInfo(&error_json);
...
Datum
orochi_auth_is_anonymous_sql(PG_FUNCTION_ARGS)
{
    pg_uuid_t *user_uuid = PG_GETARG_UUID_P(0);
