...
static Jsonb *build_jwt_payload(OrochiAuthUser *user, OrochiAuthSession *session,
                                 int exp_seconds);
...
orochi_auth_shmem_init(void)
{
    ...
    auth_shared_state = (OrochiAuthSharedState *)
        ShmemInitStruct("Orochi Auth State",
                        sizeof(OrochiAuthSharedState),
                        &found);
 ...
 */
char *
orochi_auth_generate_token(int length)
{
 ...
 */
char *
orochi_auth_generate_otp(int length)
{
    char *otp;
    ...
    random_value = (random_bytes[0] << 24) | (random_bytes[1] << 16) |
 ...
 */
char *
orochi_auth_hash_token(const char *token)
{
 ...
 */
bool
orochi_auth_verify_token_hash(const char *token, const char *hash)
{
    char *computed_hash;
    bool result;
...
static char *
base64url_encode(const unsigned char *input, int input_len)
{
    int encoded_len;
    char *encoded;
    int i;
...
base64url_decode(const char *input, int *output_len)
{
 ...
 */
char *
orochi_auth_hash_password(const char *password)
{
 ...
 */
bool
orochi_auth_verify_password(const char *password, const char *encrypted_password)
{
    StringInfoData query;
    int ret;
    bool verified = false;
    ...
    {
        bool isnull;
        verified = DatumGetBool(SPI_getbinval(SPI_tuptable->vals[0],
 ...
 */
OrochiAuthUser *
orochi_auth_create_user(const char *email,
                         const char *phone,
                         const char *password,
                         Jsonb *user_metadata)
{
    ...
    if (email == NULL && phone == NULL)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("email or phone is required")));

    if (email != NULL && strlen(email) > OROCHI_AUTH_MAX_EMAIL_LENGTH)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("email too long (max %d characters)",
                        OROCHI_AUTH_MAX_EMAIL_LENGTH)));

    if (phone != NULL && strlen(phone) > OROCHI_AUTH_MAX_PHONE_LENGTH)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("phone too long (max %d characters)",
                        OROCHI_AUTH_MAX_PHONE_LENGTH)));
    ...
    user_context = AllocSetContextCreate(TopMemoryContext,
                                          "AuthUser",
 ...
 */
OrochiAuthUser *
orochi_auth_get_user(pg_uuid_t user_id)
{
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0)
    {
        ...
        if (user->email) user->email = pstrdup(user->email);

        user->phone = SPI_getvalue(tuple, tupdesc, 3);
        if (user->phone) user->phone = pstrdup(user->phone);

        user->encrypted_password = SPI_getvalue(tuple, tupdesc, 4);
        if (user->encrypted_password) user->encrypted_password = pstrdup(user->encrypted_password);

        user->role = SPI_getvalue(tuple, tupdesc, 5);
        if (user->role) user->role = pstrdup(user->role);
 ...
 */
OrochiAuthUser *
orochi_auth_get_user_by_email(const char *email)
{
    OrochiAuthUser *user = NULL;
    StringInfoData query;
    int ret;
 ...
 */
OrochiAuthUser *
orochi_auth_get_user_by_phone(const char *phone)
{
    OrochiAuthUser *user = NULL;
    StringInfoData query;
    int ret;
 ...
 */
bool
orochi_auth_update_user(pg_uuid_t user_id,
                         const char *email,
                         const char *phone,
                         const char *password,
                         Jsonb *user_metadata,
                         Jsonb *app_metadata)
{
    ...
    if (phone != NULL)
    {
        if (!first_update) appendStringInfoString(&query, ", ");
    ...
    {
        if (!first_update) appendStringInfoString(&query, ", ");
        appendStringInfo(&query, "encrypted_password = %s",
 ...
 */
bool
orochi_auth_delete_user(pg_uuid_t user_id, bool soft_delete)
{
    StringInfoData query;
    int ret;
    bool deleted = false;
 ...
 */
void
orochi_auth_free_user(OrochiAuthUser *user)
{
    if (user == NULL)
        return;
 ...
 */
OrochiAuthSession *
orochi_auth_create_session(pg_uuid_t user_id,
                            ...
                            OrochiAuthAAL aal)
{
    ...
    session_context = AllocSetContextCreate(TopMemoryContext,
                                             "AuthSession",
                                             ALLOCSET_DEFAULT_SIZES);
    ...
    if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0)
    {
        ...
        session->created_at = DatumGetTimestampTz(SPI_getbinval(
        ...
        initStringInfo(&query);
        appendStringInfo(&query,
            "INSERT INTO auth.refresh_tokens (session_id, user_id, token, created_at) "
            "VALUES ('%s', '%s', %s, NOW())",
 ...
 */
bool
orochi_auth_revoke_session(pg_uuid_t session_id)
{
    StringInfoData query;
    int ret;
    bool revoked = false;
 ...
 */
void
orochi_auth_free_session(OrochiAuthSession *session)
{
    if (session == NULL)
        return;
 ...
 */
OrochiAuthResult *
orochi_auth_sign_in_password(const char *email, const char *password)
{
    ...
    /* Generate JWT */
    result->access_token = orochi_auth_generate_jwt(user, result->session,
                                                     orochi_auth_jwt_exp);
    result->expires_at = GetCurrentTimestamp() +
                         (orochi_auth_jwt_exp * USECS_PER_SEC);
 ...
 */
OrochiAuthResult *
orochi_auth_refresh_token(const char *refresh_token)
{
    OrochiAuthResult *result;
    StringInfoData query;
    int ret;
    ...
    initStringInfo(&query);
    appendStringInfoString(&query,
        "SELECT rt.user_id, rt.session_id FROM auth.refresh_tokens rt "
    ...
    /* Extract user_id and session_id */
    {
        char *user_id_str = SPI_getvalue(SPI_tuptable->vals[0],
                                          SPI_tuptable->tupdesc, 1);
        char *session_id_str = SPI_getvalue(SPI_tuptable->vals[0],
    ...
    /* Generate new refresh token and rotate */
    {
        ...
        appendStringInfo(&query,
            "UPDATE auth.refresh_tokens SET revoked = TRUE, updated_at = NOW() "
            "WHERE token = %s", quote_literal_cstr(refresh_token));
        SPI_execute(query.data, false, 0);
        pfree(query.data);

        initStringInfo(&query);
        appendStringInfo(&query,
            "INSERT INTO auth.refresh_tokens (session_id, user_id, token, parent, created_at) "
            "VALUES ((SELECT session_id FROM auth.refresh_tokens WHERE token = %s), "
            "(SELECT user_id FROM auth.refresh_tokens WHERE token = %s), "
            "%s, %s, NOW())",
            quote_literal_cstr(refresh_token),
            quote_literal_cstr(refresh_token),
            quote_literal_cstr(new_refresh_token),
            quote_literal_cstr(refresh_token));
        SPI_execute(query.data, false, 0);
        pfree(query.data);
        ...
        appendStringInfo(&query,
            ...
            "updated_at = NOW() WHERE id = (SELECT session_id FROM auth.refresh_tokens "
    ...
    result->access_token = orochi_auth_generate_jwt(result->user, result->session,
                                                     orochi_auth_jwt_exp);
    result->expires_at = GetCurrentTimestamp() +
                         (orochi_auth_jwt_exp * USECS_PER_SEC);
 ...
 */
bool
orochi_auth_sign_out(pg_uuid_t session_id)
{
 ...
 */
static Jsonb *
build_jwt_payload(OrochiAuthUser *user, OrochiAuthSession *session, int exp_seconds)
{
    ...
    appendStringInfo(&payload, ", \"role\": \"%s\"",
                     user->role ? user->role : "authenticated");

    appendStringInfo(&payload, ", \"aal\": \"aal%d\"",
                     session ? (int)session->aal : 1);
    ...
    if (session)
    {
        ...
        snprintf(session_uuid_str, sizeof(session_uuid_str),
                 ...
                 session->id.data[8], session->id.data[9], session->id.data[10], session->id.data[11],
                 session->id.data[12], session->id.data[13], session->id.data[14], session->id.data[15]);

        appendStringInfo(&payload, ", \"session_id\": \"%s\"", session_uuid_str);
    }

    appendStringInfo(&payload, ", \"is_anonymous\": %s",
 ...
 */
static char *
sign_jwt(const char *header, const char *payload, const char *secret)
{
    char *header_b64;
    char *payload_b64;
    char *signing_input;
 ...
 */
char *
orochi_auth_generate_jwt(OrochiAuthUser *user, OrochiAuthSession *session,
                          int exp_seconds)
{
    ...
    if (secret == NULL || strlen(secret) == 0)
    {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("JWT secret not configured")));
    }
...
orochi_auth_get_config(void)
{
    ...
    initStringInfo(&query);
    appendStringInfoString(&query,
        "SELECT jwt_secret, jwt_exp, site_url, disable_signup, "
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0)
    {
        ...
        if (config->jwt_secret) config->jwt_secret = pstrdup(config->jwt_secret);

        config->jwt_exp = DatumGetInt32(SPI_getbinval(tuple, tupdesc, 2, &isnull));
        if (isnull) config->jwt_exp = OROCHI_AUTH_DEFAULT_JWT_EXP;

        config->site_url = SPI_getvalue(tuple, tupdesc, 3);
        if (config->site_url) config->site_url = pstrdup(config->site_url);
        ...
        if (isnull) config->enable_anonymous_sign_in = true;

        config->password_min_length = DatumGetInt32(SPI_getbinval(tuple, tupdesc, 6, &isnull));
        if (isnull) config->password_min_length = OROCHI_AUTH_MIN_PASSWORD_LENGTH;
    }
...
const char *
orochi_auth_aal_name(OrochiAuthAAL aal)
{
    switch (aal)
    {
        case OROCHI_AAL_1: return "aal1";
        case OROCHI_AAL_2: return "aal2";
        case OROCHI_AAL_3: return "aal3";
        default: return "aal1";
    }
}

const char *
orochi_auth_factor_type_name(OrochiAuthFactorType type)
{
    switch (type)
    {
        case OROCHI_FACTOR_TOTP: return "totp";
        case OROCHI_FACTOR_PHONE: return "phone";
        case OROCHI_FACTOR_WEBAUTHN: return "webauthn";
        default: return "unknown";
    }
}

const char *
orochi_auth_token_type_name(OrochiAuthTokenType type)
{
    switch (type)
    {
        case OROCHI_TOKEN_CONFIRMATION: return "confirmation";
        case OROCHI_TOKEN_RECOVERY: return "recovery";
        case OROCHI_TOKEN_EMAIL_CHANGE_NEW: return "email_change_new";
        case OROCHI_TOKEN_EMAIL_CHANGE_OLD: return "email_change_old";
        case OROCHI_TOKEN_PHONE_CHANGE: return "phone_change";
        case OROCHI_TOKEN_REAUTHENTICATION: return "reauthentication";
        default: return "unknown";
    }
}

const char *
orochi_auth_provider_name(OrochiAuthProvider provider)
{
    switch (provider)
    {
        case OROCHI_PROVIDER_EMAIL: return "email";
        case OROCHI_PROVIDER_PHONE: return "phone";
        case OROCHI_PROVIDER_ANONYMOUS: return "anonymous";
        case OROCHI_PROVIDER_GOOGLE: return "google";
        case OROCHI_PROVIDER_GITHUB: return "github";
        case OROCHI_PROVIDER_FACEBOOK: return "facebook";
        case OROCHI_PROVIDER_APPLE: return "apple";
        case OROCHI_PROVIDER_AZURE: return "azure";
        case OROCHI_PROVIDER_SAML: return "saml";
        default: return "unknown";
    }
}

OrochiAuthProvider
orochi_auth_parse_provider(const char *name)
{
    if (pg_strcasecmp(name, "email") == 0) return OROCHI_PROVIDER_EMAIL;
    if (pg_strcasecmp(name, "phone") == 0) return OROCHI_PROVIDER_PHONE;
    if (pg_strcasecmp(name, "anonymous") == 0) return OROCHI_PROVIDER_ANONYMOUS;
    if (pg_strcasecmp(name, "google") == 0) return OROCHI_PROVIDER_GOOGLE;
    if (pg_strcasecmp(name, "github") == 0) return OROCHI_PROVIDER_GITHUB;
    if (pg_strcasecmp(name, "facebook") == 0) return OROCHI_PROVIDER_FACEBOOK;
    if (pg_strcasecmp(name, "apple") == 0) return OROCHI_PROVIDER_APPLE;
    if (pg_strcasecmp(name, "azure") == 0) return OROCHI_PROVIDER_AZURE;
    if (pg_strcasecmp(name, "saml") == 0) return OROCHI_PROVIDER_SAML;
...
Datum
orochi_auth_create_user_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_auth_sign_in_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_auth_sign_out_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_auth_refresh_token_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_auth_uid_sql(PG_FUNCTION_ARGS)
{
...
Datum
orochi_auth_role_sql(PG_FUNCTION_ARGS)
{
