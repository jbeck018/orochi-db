...
static Jsonb *build_jwt_payload(OrochiAuthUser *user,
                                OrochiAuthSession *session, int exp_seconds);
static char *sign_jwt(const char *header, const char *payload,
                      const char *secret);
...
Size orochi_auth_shmem_size(void) { return sizeof(OrochiAuthSharedState); }
...
static char *base64url_encode(const unsigned char *input, int input_len) {
  ...
  encoded_len =
      pg_b64_encode((const char *)input, input_len, encoded, encoded_len);
 ...
 */
bool orochi_auth_verify_password(const char *password,
 ...
 */
OrochiAuthUser *orochi_auth_create_user(const char *email, const char *phone,
                                        const char *password,
                                        Jsonb *user_metadata) {
  OrochiAuthUser *user;
  StringInfoData query;
  ...
  user_context = AllocSetContextCreate(TopMemoryContext, "AuthUser",
  ...
  if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0) {
    ...
    ereport(ERROR,
            (errcode(ERRCODE_INTERNAL_ERROR), errmsg("failed to create user")));
  }
...
OrochiAuthUser *orochi_auth_get_user(pg_uuid_t user_id) {
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    user->updated_at =
        DatumGetTimestampTz(SPI_getbinval(tuple, tupdesc, 12, &isnull));
    user->last_sign_in_at =
        DatumGetTimestampTz(SPI_getbinval(tuple, tupdesc, 13, &isnull));
    user->banned_until =
        DatumGetTimestampTz(SPI_getbinval(tuple, tupdesc, 14, &isnull));
    user->deleted_at =
 ...
 */
bool orochi_auth_update_user(pg_uuid_t user_id, const char *email,
                             const char *phone, const char *password,
                             Jsonb *user_metadata, Jsonb *app_metadata) {
  StringInfoData query;
  int ret;
  bool updated = false;
  char *encrypted_password = NULL;
...
bool orochi_auth_delete_user(pg_uuid_t user_id, bool soft_delete) {
  ...
  if (soft_delete) {
    appendStringInfoString(
        &query, "UPDATE auth.users SET deleted_at = NOW(), updated_at = NOW() "
 ...
 */
OrochiAuthSession *orochi_auth_create_session(pg_uuid_t user_id,
  ...
  session_context = AllocSetContextCreate(TopMemoryContext, "AuthSession",
                                          ALLOCSET_DEFAULT_SIZES);
 ...
 */
OrochiAuthResult *orochi_auth_sign_in_password(const char *email,
                                               const char *password) {
  ...
  /* Create session */
  result->session =
      orochi_auth_create_session(user->id, NULL, NULL, OROCHI_AAL_1);
  result->user = user;
  result->refresh_token = pstrdup(result->session->refresh_token);

  /* Generate JWT */
  result->access_token =
      orochi_auth_generate_jwt(user, result->session, orochi_auth_jwt_exp);
  result->expires_at =
      GetCurrentTimestamp() + (orochi_auth_jwt_exp * USECS_PER_SEC);
  ...
  SPI_connect();
  {
    ...
    initStringInfo(&query);
    appendStringInfoString(
        &query, "UPDATE auth.users SET last_sign_in_at = NOW() WHERE id = $1");
...
OrochiAuthResult *orochi_auth_refresh_token(const char *refresh_token) {
  ...
  initStringInfo(&query);
  appendStringInfoString(
      &query, "SELECT rt.user_id, rt.session_id FROM auth.refresh_tokens rt "
...
static Jsonb *build_jwt_payload(OrochiAuthUser *user,
  ...
  snprintf(
      ...
      user->id.data[12], user->id.data[13], user->id.data[14],
      user->id.data[15]);
  ...
  if (session) {
    ...
    snprintf(
        session_uuid_str, sizeof(session_uuid_str),
        "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
        session->id.data[0], session->id.data[1], session->id.data[2],
        session->id.data[3], session->id.data[4], session->id.data[5],
        session->id.data[6], session->id.data[7], session->id.data[8],
        session->id.data[9], session->id.data[10], session->id.data[11],
        session->id.data[12], session->id.data[13], session->id.data[14],
        session->id.data[15]);
...
static char *sign_jwt(const char *header, const char *payload,
  ...
  if (!HMAC(EVP_sha256(), secret, strlen(secret),
            (unsigned char *)signing_input, strlen(signing_input), signature,
            &sig_len)) {
    pfree(header_b64);
    pfree(payload_b64);
    pfree(signing_input);
    ereport(ERROR,
            (errcode(ERRCODE_INTERNAL_ERROR), errmsg("failed to sign JWT")));
  }
 ...
 */
char *orochi_auth_generate_jwt(OrochiAuthUser *user, OrochiAuthSession *session,
                               int exp_seconds) {
  ...
  (void)JsonbToCString(&payload_str, &payload_jsonb->root,
                       VARSIZE(payload_jsonb));
...
OrochiAuthConfig *orochi_auth_get_config(void) {
  ...
  initStringInfo(&query);
  appendStringInfoString(
      &query, "SELECT jwt_secret, jwt_exp, site_url, disable_signup, "
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    config->disable_signup =
        DatumGetBool(SPI_getbinval(tuple, tupdesc, 4, &isnull));
    config->enable_anonymous_sign_in =
        DatumGetBool(SPI_getbinval(tuple, tupdesc, 5, &isnull));
    if (isnull)
      config->enable_anonymous_sign_in = true;
