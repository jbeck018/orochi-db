 ...
 */
float vector_l2_norm(Vector *v) { return vector_l2_norm_simd(v->x, v->dim); }
...
void vector_batch_l2_distances(Vector *query, Vector **vectors, int count,
                               float *distances) {
  int i;
 ...
 */
void vector_knn(Vector *query, Vector **vectors, int count, int k, int *indices,
...
void orochi_create_vector_index(Oid table_oid, const char *column_name,
  ...
  elog(NOTICE, "Created %s vector index on %s.%s", index_type,
       get_rel_name(table_oid), column_name);
