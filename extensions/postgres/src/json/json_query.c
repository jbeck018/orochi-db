...
static void ensure_query_context(void) {
  if (JsonQueryContext == NULL) {
    JsonQueryContext = AllocSetContextCreate(
        TopMemoryContext, "JsonQueryContext", ALLOCSET_DEFAULT_SIZES);
  }
}
...
JsonQueryExpr *json_query_build_expr(Node *pg_expr, Oid table_oid,
                                     int16 column_attnum) {
  ...
  case T_OpExpr: {
    ...
    if (list_length(op->args) >= 1) {
      result->left =
          json_query_build_expr(linitial(op->args), table_oid, column_attnum);
    }
    if (list_length(op->args) >= 2) {
      result->right =
          json_query_build_expr(lsecond(op->args), table_oid, column_attnum);
    }
    break;
  }
  ...
  case T_BoolExpr: {
    ...
    foreach (lc, b->args) {
      JsonQueryExpr *child =
          json_query_build_expr(lfirst(lc), table_oid, column_attnum);
      if (child != NULL)
...
JsonQueryExpr *json_query_optimize(JsonQueryExpr *expr, Oid table_oid,
                                   int16 column_attnum) {
  JsonPathStats *stats;
  ListCell *lc;
  ...
  case JSON_EXPR_OPERATOR:
    ...
    if (expr->left != NULL && expr->left->type == JSON_EXPR_PATH) {
      stats =
          json_index_get_path_stats(table_oid, column_attnum, expr->left->path);
      if (stats != NULL) {
        expr->selectivity =
            estimate_operator_selectivity(expr->operator, stats);
        pfree(stats);
      }
    }
...
JsonQueryPlan *json_query_plan(JsonQueryExpr *expr, Oid table_oid,
                               int16 column_attnum) {
  JsonQueryPlan *plan;
  List *indexes;
  ListCell *lc;
  ...
  if (expr->can_use_index && plan->usable_indexes != NIL) {
    plan->selected_index =
        json_query_select_index(expr, table_oid, column_attnum);
  }
  ...
  if (expr->type == JSON_EXPR_PATH ||
    ...
    if (columns != NIL) {
      char *target_path =
          (expr->type == JSON_EXPR_PATH) ? expr->path : expr->left->path;

      foreach (lc, columns) {
        JsonExtractedColumn *col = lfirst(lc);
        if (strcmp(col->path, target_path) == 0) {
          plan->extracted_columns =
              lappend_int(plan->extracted_columns, col->column_id);
  ...
  /* Estimate costs */
  json_query_estimate_cost(expr, table_oid, column_attnum, &plan->startup_cost,
                           &plan->total_cost, NULL, &plan->estimated_rows);
...
JsonQueryExpr *json_query_apply_pushdown(JsonQueryExpr *expr,
...
bool json_query_index_compatible(JsonQueryExpr *expr, Oid index_oid) {
  ...
  /* Hash indexes support equality only */
    ...
    compatible =
        (expr->type == JSON_EXPR_OPERATOR && expr->operator == JSON_OP_EQ);
  }
...
Oid json_query_select_index(JsonQueryExpr *expr, Oid table_oid,
                            int16 column_attnum) {
  List *indexes;
  ListCell *lc;
...
void json_query_build_index_scan(JsonQueryExpr *expr, Oid index_oid,
...
JsonQueryState *json_query_begin(JsonQueryPlan *plan, EState *estate) {
  ...
  state->plan = plan;
  state->estate = estate;
  state->query_context = AllocSetContextCreate(
      JsonQueryContext, "JsonQueryExec", ALLOCSET_DEFAULT_SIZES);
  ...
  if (plan->selected_index != InvalidOid) {
    ...
    json_query_build_index_scan(plan->root_expr, plan->selected_index, &keys,
                                &nkeys);

#if PG_VERSION_NUM >= 180000
    state->index_scan = index_beginscan(
        relation, index_rel, GetActiveSnapshot(), NULL, /* instrument */
        nkeys, 0);
#else
    state->index_scan =
        index_beginscan(relation, index_rel, GetActiveSnapshot(), nkeys, 0);
#endif
...
bool json_query_next(JsonQueryState *state, TupleTableSlot *result_slot) {
  ...
  while (!found) {
    ...
    /* Get next tuple */
    if (state->use_index) {
      got_tuple = index_getnext_slot(state->index_scan, ForwardScanDirection,
                                     state->slot);
    } else {
      got_tuple = table_scan_getnextslot(state->scan, ForwardScanDirection,
                                         state->slot);
    }
...
int json_query_next_batch(JsonQueryState *state, TupleTableSlot **slots,
                          int max_slots) {
  int count = 0;
...
bool json_query_eval(JsonQueryExpr *expr, Jsonb *value, bool *is_null) {
  ...
  *is_null = false;
  ...
  case JSON_EXPR_OPERATOR: {
    ...
    case JSON_OP_CONTAINS: {
      ...
      return DatumGetBool(DirectFunctionCall2(
          jsonb_contains, JsonbPGetDatum(value), JsonbPGetDatum(pattern)));
    }
    ...
    case JSON_OP_EXISTS: {
      ...
      return DatumGetBool(DirectFunctionCall2(
          jsonb_exists, JsonbPGetDatum(value), PointerGetDatum(key)));
    }
      ...
      if (expr->left->type == JSON_EXPR_PATH) {
        ...
        PG_TRY();
        {
          left_val = DirectFunctionCall2(jsonb_object_field_text,
                                         JsonbPGetDatum(value), path_datum);
    ...
    return eval_operator(expr->operator, left_val, right_val, left_type,
                         right_type);
  }
  ...
  case JSON_EXPR_NOT: {
    ...
    bool child_null;
    bool child_result =
        json_query_eval(linitial(expr->children), value, &child_null);
    if (child_null) {
      *is_null = true;
      return false;
    }
...
Datum json_query_eval_datum(JsonQueryExpr *expr, Jsonb *value, Oid *result_type,
                            bool *is_null) {
  if (expr == NULL) {
    *is_null = true;
...
void json_query_eval_batch(JsonQueryExpr *expr, Jsonb **values, int count,
                           bool *results) {
...
BatchParseContext *json_query_batch_init(int batch_size) {
  ...
  ctx->parse_context = AllocSetContextCreate(JsonQueryContext, "BatchParse",
                                             ALLOCSET_DEFAULT_SIZES);
  ctx->batch_size = batch_size;
  ctx->cache_size = JSON_QUERY_PATH_CACHE_SIZE;
  ctx->path_cache = MemoryContextAllocZero(
      ctx->parse_context, ctx->cache_size * sizeof(struct PathCache));
  ctx->cache_count = 0;
  ctx->values_parsed = 0;
  ctx->cache_hits = 0;
  ctx->cache_misses = 0;
...
void json_query_batch_parse(BatchParseContext *ctx, Datum *json_datums,
                            int count, const char **paths, int path_count,
...
void json_query_batch_extract(BatchParseContext *ctx, Jsonb **values, int count,
  ...
  for (int i = 0; i < count; i++) {
    ...
    PG_TRY();
    {
      results[i] = DirectFunctionCall2(jsonb_object_field_text,
                                       JsonbPGetDatum(values[i]), path_datum);
...
void json_query_cache_path(BatchParseContext *ctx, const char *path) {
  ...
  ctx->path_cache[ctx->cache_count].path =
      MemoryContextStrdup(ctx->parse_context, path);
...
void json_query_estimate_cost(JsonQueryExpr *expr, Oid table_oid,
...
void json_query_estimate_index_cost(JsonQueryExpr *expr, Oid index_oid,
  ...
  *total_cost =
      random_page_cost * 10 + cpu_index_tuple_cost * 100 * index_selectivity;
}
...
List *json_query_reorder_paths(List *path_exprs,
...
Datum orochi_json_query(PG_FUNCTION_ARGS) {
...

  {
    ...
    TupleTableSlot *slot =
        MakeSingleTupleTableSlot(funcctx->tuple_desc, &TTSOpsVirtual);
...
Datum orochi_json_extract_batch(PG_FUNCTION_ARGS) {
  ...
  deconstruct_array(json_array, JSONBOID, -1, false, TYPALIGN_INT, &json_datums,
                    &json_nulls, &count);
  ...
  /* Build result array */
  result_array =
      construct_array(results, count, TEXTOID, -1, false, TYPALIGN_INT);
