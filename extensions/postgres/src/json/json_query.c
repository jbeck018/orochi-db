...
static double estimate_operator_selectivity(JsonQueryOperator op, JsonPathStats *stats);
static bool eval_operator(JsonQueryOperator op, Datum left, Datum right, Oid left_type,
                          Oid right_type);
...
static void ensure_query_context(void)
{
    if (JsonQueryContext == NULL) {
        JsonQueryContext =
            AllocSetContextCreate(TopMemoryContext, "JsonQueryContext", ALLOCSET_DEFAULT_SIZES);
    }
}
...
const char *json_query_operator_name(JsonQueryOperator op)
{
    switch (op) {
...
JsonQueryOperator json_query_parse_operator(const char *name)
{
    if (strcmp(name, "@>") == 0)
    ...
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("unknown JSON operator: %s", name)));
...
JsonQueryExpr *json_query_parse(const char *query_text)
{
...
JsonQueryExpr *json_query_build_expr(Node *pg_expr, Oid table_oid, int16 column_attnum)
{
    ...
    case T_OpExpr: {
        ...
        if (list_length(op->args) >= 2) {
            result->right = json_query_build_expr(lsecond(op->args), table_oid, column_attnum);
        }
        break;
    }
    ...
    case T_BoolExpr: {
        ...
        result->children = NIL;
        foreach (lc, b->args) {
            JsonQueryExpr *child = json_query_build_expr(lfirst(lc), table_oid, column_attnum);
            if (child != NULL)
    ...
    case T_FuncExpr: {
        ...
        foreach (lc, f->args) {
            JsonQueryExpr *arg = json_query_build_expr(lfirst(lc), table_oid, column_attnum);
            if (arg != NULL)
...
JsonQueryExpr *json_query_optimize(JsonQueryExpr *expr, Oid table_oid, int16 column_attnum)
{
    JsonPathStats *stats;
    ListCell *lc;
    ...
    case JSON_EXPR_PATH:
        ...
        stats = json_index_get_path_stats(table_oid, column_attnum, expr->path);
        if (stats != NULL) {
    ...
    case JSON_EXPR_OPERATOR:
        ...
        expr->left = json_query_optimize(expr->left, table_oid, column_attnum);
        expr->right = json_query_optimize(expr->right, table_oid, column_attnum);
        ...
        case JSON_OP_EXISTS_ALL:
            ...
            break;
        case JSON_OP_EQ:
            expr->can_use_index = (expr->left != NULL && expr->left->can_use_index);
            break;
        default:
            expr->can_use_index = false;
        }
...
JsonQueryPlan *json_query_plan(JsonQueryExpr *expr, Oid table_oid, int16 column_attnum)
{
    JsonQueryPlan *plan;
    List *indexes;
    ListCell *lc;
    ...
    foreach (lc, indexes) {
        ...
        if (json_query_index_compatible(expr, idx->index_oid)) {
            plan->usable_indexes = lappend_oid(plan->usable_indexes, idx->index_oid);
        }
    ...
    if (expr->can_use_index && plan->usable_indexes != NIL) {
        plan->selected_index = json_query_select_index(expr, table_oid, column_attnum);
    }

    /* Check for extracted columns */
    plan->extracted_columns = NIL;
    if (expr->type == JSON_EXPR_PATH || (expr->type == JSON_EXPR_OPERATOR && expr->left != NULL &&
                                         expr->left->type == JSON_EXPR_PATH)) {
        List *columns = json_columnar_get_columns(table_oid, column_attnum);
        if (columns != NIL) {
            char *target_path = (expr->type == JSON_EXPR_PATH) ? expr->path : expr->left->path;

            foreach (lc, columns) {
                JsonExtractedColumn *col = lfirst(lc);
                if (strcmp(col->path, target_path) == 0) {
                    plan->extracted_columns = lappend_int(plan->extracted_columns, col->column_id);
    ...
    /* Estimate costs */
    json_query_estimate_cost(expr, table_oid, column_attnum, &plan->startup_cost, &plan->total_cost,
                             NULL, &plan->estimated_rows);

    return plan;
}

void json_query_free_expr(JsonQueryExpr *expr)
{
    ListCell *lc;
...
void json_query_free_plan(JsonQueryPlan *plan)
{
    if (plan == NULL)
        return;
...
bool json_query_can_pushdown(JsonQueryExpr *expr)
{
    if (expr == NULL)
        return false;
...
List *json_query_extract_pushable(JsonQueryExpr *expr)
{
    List *result = NIL;
    ListCell *lc;
...
JsonQueryExpr *json_query_apply_pushdown(JsonQueryExpr *expr, List *pushed_predicates)
{
...
ScanKey json_query_to_scankey(JsonQueryExpr *expr, int *nkeys)
{
    ScanKeyData *keys;
    int key_count = 0;
    ...
    case JSON_EXPR_OPERATOR:
        if (expr->right != NULL && expr->right->type == JSON_EXPR_CONST &&
            ...
            if (proc != InvalidOid && key_count < max_keys) {
                ScanKeyInit(&keys[key_count], 1, /* Attribute number (first index column) */
                            strategy, proc, expr->right->const_value);
                key_count++;
            }
        }
        break;
...
bool json_query_index_compatible(JsonQueryExpr *expr, Oid index_oid)
{
    Relation index_rel;
...
Oid json_query_select_index(JsonQueryExpr *expr, Oid table_oid, int16 column_attnum)
{
    List *indexes;
    ListCell *lc;
...
void json_query_build_index_scan(JsonQueryExpr *expr, Oid index_oid, ScanKey *keys, int *nkeys)
{
    Relation index_rel;
    ScanKey base_keys;
    int base_nkeys;
    ...
    /* Hash indexes only support equality */
        ...
        for (int i = 0; i < base_nkeys; i++) {
            if (base_keys[i].sk_strategy == BTEqualStrategyNumber ||
                ...
                if (i != valid_count)
                    memcpy(&base_keys[valid_count], &base_keys[i], sizeof(ScanKeyData));
                valid_count++;
            }
        }
...
JsonQueryState *json_query_begin(JsonQueryPlan *plan, EState *estate)
{
    ...
    state->estate = estate;
    state->query_context =
        AllocSetContextCreate(JsonQueryContext, "JsonQueryExec", ALLOCSET_DEFAULT_SIZES);
    ...
    if (plan->selected_index != InvalidOid) {
        ...
        json_query_build_index_scan(plan->root_expr, plan->selected_index, &keys, &nkeys);

#if PG_VERSION_NUM >= 180000
        state->index_scan =
            index_beginscan(relation, index_rel, GetActiveSnapshot(), NULL, /* instrument */
                            nkeys, 0);
#else
        state->index_scan = index_beginscan(relation, index_rel, GetActiveSnapshot(), nkeys, 0);
#endif
...
bool json_query_next(JsonQueryState *state, TupleTableSlot *result_slot)
{
    Relation relation;
    bool found = false;
    ...
    while (!found) {
        ...
        /* Get next tuple */
        if (state->use_index) {
            got_tuple = index_getnext_slot(state->index_scan, ForwardScanDirection, state->slot);
        } else {
            got_tuple = table_scan_getnextslot(state->scan, ForwardScanDirection, state->slot);
        }
        ...
        json_datum = slot_getattr(state->slot, state->plan->column_attnum, &isnull);
        if (isnull)
            continue;
...
int json_query_next_batch(JsonQueryState *state, TupleTableSlot **slots, int max_slots)
{
    int count = 0;
...
void json_query_rescan(JsonQueryState *state)
{
    if (state->use_index) {
...
void json_query_end(JsonQueryState *state)
{
    if (state == NULL)
        return;
...
bool json_query_eval(JsonQueryExpr *expr, Jsonb *value, bool *is_null)
{
    ListCell *lc;
    ...
    *is_null = false;
    ...
    case JSON_EXPR_OPERATOR: {
        ...
        case JSON_OP_CONTAINS: {
            ...
            return DatumGetBool(DirectFunctionCall2(jsonb_contains, JsonbPGetDatum(value),
        ...
        case JSON_OP_EXISTS: {
            ...
            return DatumGetBool(
                DirectFunctionCall2(jsonb_exists, JsonbPGetDatum(value), PointerGetDatum(key)));
        }

        case JSON_OP_IS_NULL: {
            if (expr->left != NULL && expr->left->type == JSON_EXPR_PATH) {
                JsonPathType type = json_index_get_path_type(value, expr->left->path);
        ...
        if (expr->left != NULL) {
            if (expr->left->type == JSON_EXPR_PATH) {
                ...
                PG_TRY();
                {
                    left_val = DirectFunctionCall2(jsonb_object_field_text, JsonbPGetDatum(value),
                                                   path_datum);
    ...
    case JSON_EXPR_AND: {
        ...
        foreach (lc, expr->children) {
            bool child_null;
            bool child_result = json_query_eval(lfirst(lc), value, &child_null);
            if (child_null) {
                *is_null = true;
                return false;
            }
    ...
    case JSON_EXPR_NOT: {
        ...
        bool child_null;
        bool child_result = json_query_eval(linitial(expr->children), value, &child_null);
        if (child_null) {
            *is_null = true;
            return false;
        }
...
Datum json_query_eval_datum(JsonQueryExpr *expr, Jsonb *value, Oid *result_type, bool *is_null)
{
    if (expr == NULL) {
        *is_null = true;
    ...
    case JSON_EXPR_PATH: {
        ...
        {
            result =
                DirectFunctionCall2(jsonb_object_field_text, JsonbPGetDatum(value), path_datum);
...
void json_query_eval_batch(JsonQueryExpr *expr, Jsonb **values, int count, bool *results)
{
...
BatchParseContext *json_query_batch_init(int batch_size)
{
    ...
    ctx->parse_context =
        AllocSetContextCreate(JsonQueryContext, "BatchParse", ALLOCSET_DEFAULT_SIZES);
    ctx->batch_size = batch_size;
    ctx->cache_size = JSON_QUERY_PATH_CACHE_SIZE;
    ctx->path_cache =
        MemoryContextAllocZero(ctx->parse_context, ctx->cache_size * sizeof(struct PathCache));
    ctx->cache_count = 0;
    ctx->values_parsed = 0;
    ctx->cache_hits = 0;
    ctx->cache_misses = 0;
...
void json_query_batch_parse(BatchParseContext *ctx, Datum *json_datums, int count,
                            const char **paths, int path_count, Datum **results, bool **nulls)
{
    ...
    for (int p = 0; p < path_count; p++) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            {
                results[p][i] =
                    DirectFunctionCall2(jsonb_object_field_text, JsonbPGetDatum(jb), path_datum);
...
void json_query_batch_extract(BatchParseContext *ctx, Jsonb **values, int count, const char *path,
                              Oid target_type, Datum *results, bool *nulls)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        {
            results[i] =
                DirectFunctionCall2(jsonb_object_field_text, JsonbPGetDatum(values[i]), path_datum);
...
void json_query_batch_free(BatchParseContext *ctx)
{
    if (ctx == NULL)
        return;
...
void json_query_cache_path(BatchParseContext *ctx, const char *path)
{
    if (ctx->cache_count >= ctx->cache_size)
        return;

    ctx->path_cache[ctx->cache_count].path = MemoryContextStrdup(ctx->parse_context, path);
...
void json_query_estimate_cost(JsonQueryExpr *expr, Oid table_oid, int16 column_attnum,
...
void json_query_estimate_index_cost(JsonQueryExpr *expr, Oid index_oid, double *startup_cost,
                                    double *total_cost)
{
    ...
    *total_cost = random_page_cost * 10 + cpu_index_tuple_cost * 100 * index_selectivity;
}
...
char *json_query_expr_to_string(JsonQueryExpr *expr)
{
    StringInfoData buf;
    ListCell *lc;
    ...
    case JSON_EXPR_CONST:
        if (expr->const_isnull)
            ...
            appendStringInfo(&buf, "'%s'", TextDatumGetCString(expr->const_value));
    ...
    case JSON_EXPR_AND:
        ...
        foreach (lc, expr->children) {
            ...
            appendStringInfoString(&buf, json_query_expr_to_string(lfirst(lc)));
        }
        appendStringInfoString(&buf, ")");
        break;
    ...
    case JSON_EXPR_NOT:
        appendStringInfo(&buf, "NOT %s", json_query_expr_to_string(linitial(expr->children)));
        break;
...
char *json_query_plan_to_string(JsonQueryPlan *plan)
{
    StringInfoData buf;
    ...
    appendStringInfo(&buf, "  Use Index: %s (OID: %u)\n",
                     plan->selected_index != InvalidOid ? "yes" : "no", plan->selected_index);
...
char *json_query_simplify_path(const char *path)
{
    return json_index_normalize_path(path);
}

List *json_query_merge_paths(List *path_exprs)
{
...
List *json_query_reorder_paths(List *path_exprs, JsonPathStatsCollection *stats)
{
...
Datum orochi_json_query(PG_FUNCTION_ARGS)
{
...

    {
        ...
        TupleTableSlot *slot = MakeSingleTupleTableSlot(funcctx->tuple_desc, &TTSOpsVirtual);
...
Datum orochi_json_extract_batch(PG_FUNCTION_ARGS)
{
    ...
    deconstruct_array(json_array, JSONBOID, -1, false, TYPALIGN_INT, &json_datums, &json_nulls,
                      &count);
...
Datum orochi_json_query_explain(PG_FUNCTION_ARGS)
{
    Oid table_oid = PG_GETARG_OID(0);
    text *column_name_text = PG_GETARG_TEXT_PP(1);
    text *query_text = PG_GETARG_TEXT_PP(2);
...
Datum orochi_json_query_stats(PG_FUNCTION_ARGS)
{
    Oid table_oid = PG_GETARG_OID(0);
    text *column_name_text = PG_GETARG_TEXT_PP(1);
    char *column_name;
