...
static void analyze_jsonb_value(Jsonb *jb, const char *current_path,
                                JsonPathStatsCollection *stats,
                                int depth);
static JsonIndexType recommend_index_type(JsonPathStats *path_stats);
static char *build_index_create_sql(Oid table_oid, int16 column_attnum,
...
static void
ensure_json_index_context(void)
{
    if (JsonIndexContext == NULL)
    {
        JsonIndexContext = AllocSetContextCreate(TopMemoryContext,
                                                 "JsonIndexContext",
...
char **
json_index_parse_path(const char *path, int *depth)
{
    char       *path_copy;
    char       *token;
    char       *saveptr;
...
char *
json_index_normalize_path(const char *path)
{
    StringInfoData buf;
    int         depth;
    char      **components;
...
bool
json_index_is_valid_path(const char *path)
{
    int         depth;
    char      **components;
...
JsonPathType
json_index_get_path_type(Jsonb *jb, const char *path)
{
    JsonbValue *v;
    Datum       path_datum;
    Datum       result_datum;
    ...
    PG_TRY();
    {
        result_datum = DirectFunctionCall2(jsonb_path_query_first,
                                           JsonbPGetDatum(jb),
                                           path_datum);
...
const char *
json_index_type_name(JsonIndexType type)
{
    switch (type)
    {
        case JSON_INDEX_GIN:
...
JsonIndexType
json_index_parse_type(const char *name)
{
...
static void
analyze_jsonb_value(Jsonb *jb, const char *current_path,
                    JsonPathStatsCollection *stats, int depth)
{
    JsonbIterator *it;
        ...
        switch (token)
        {
            ...
            case WJB_ELEM:
            {
                ...
                if (path_stats != NULL)
                {
                        ...
                        case jbvString:
                            ...
                            path_stats->avg_value_size =
                                (path_stats->avg_value_size * (path_stats->access_count - 1) +
                                 v.val.string.len) / path_stats->access_count;
                            break;
                        case jbvNumeric:
                    ...
                    else if (path_stats->value_type != current_type &&
                             current_type != JSON_PATH_NULL)
...
void
json_index_record_access(Oid table_oid, int16 column_attnum, const char *path)
{
...
JsonPathStatsCollection *
json_index_get_all_stats(Oid table_oid, int16 column_attnum)
{
    /* In production, this would read from catalog table */
    /* For now, perform fresh analysis */
    return json_index_analyze_paths(table_oid, column_attnum,
                                    JSON_INDEX_SAMPLE_SIZE);
}

void
json_index_reset_stats(Oid table_oid, int16 column_attnum)
{
    ...
    SPI_execute_with_args(
        "DELETE FROM orochi.orochi_json_path_stats "
...
void
json_index_free_stats(JsonPathStatsCollection *stats)
{
    if (stats == NULL)
        return;
...
static JsonIndexType
recommend_index_type(JsonPathStats *path_stats)
{
    if (path_stats == NULL)
        return JSON_INDEX_GIN;
    ...
    /* Arrays/Objects -> GIN */
    if (path_stats->value_type == JSON_PATH_ARRAY ||
        path_stats->value_type == JSON_PATH_OBJECT)
        return JSON_INDEX_GIN;
...
static char *
build_index_create_sql(Oid table_oid, int16 column_attnum,
                       const char *path, JsonIndexType type)
{
    StringInfoData buf;
    char       *table_name;
    ...
    switch (type)
    {
        case JSON_INDEX_GIN:
            appendStringInfo(&buf,
                "CREATE INDEX idx_%s_%s_gin ON %s.%s USING gin ((%s -> '%s'))",
                table_name, safe_path, schema_name, table_name,
                column_name, path);
            break;

        case JSON_INDEX_BTREE:
            appendStringInfo(&buf,
                "CREATE INDEX idx_%s_%s_btree ON %s.%s ((%s ->> '%s'))",
                table_name, safe_path, schema_name, table_name,
                column_name, path);
            break;
        ...
        case JSON_INDEX_HASH:
            appendStringInfo(&buf,
                "CREATE INDEX idx_%s_%s_hash ON %s.%s USING hash ((%s ->> '%s'))",
                table_name, safe_path, schema_name, table_name,
                column_name, path);
            break;
        ...
        case JSON_INDEX_EXPRESSION:
            appendStringInfo(&buf,
                "CREATE INDEX idx_%s_%s_expr ON %s.%s ((%s #>> '{%s}'))",
                table_name, safe_path, schema_name, table_name,
                column_name, path);
            break;
...
List *
json_index_get_recommendations(Oid table_oid, int16 column_attnum,
                               int max_recommendations)
{
    ...
    for (int i = 0; i < stats->path_count && i < max_recommendations; i++)
    {
        ...
        rec->create_statement = build_index_create_sql(table_oid, column_attnum,
                                                       ps->path, rec->index_type);
        rec->priority = i + 1;
...
int
json_index_apply_recommendations(Oid table_oid, int16 column_attnum,
                                 int max_indexes)
{
    ...
    recommendations = json_index_get_recommendations(table_oid, column_attnum,
                                                     max_indexes);
...
double
json_index_estimate_benefit(Oid table_oid, int16 column_attnum,
                            const char *path, JsonIndexType type)
{
    JsonPathStats *stats;
    double benefit = 0.0;
...
void
json_index_free_recommendations(List *recommendations)
{
    ListCell   *lc;
...
Oid
json_index_create(Oid table_oid, int16 column_attnum,
                  const char **paths, int path_count,
                  JsonIndexType index_type)
{
    return json_index_create_with_options(table_oid, column_attnum,
                                          paths, path_count,
                                          index_type, false, NULL);
}

Oid
json_index_create_with_options(Oid table_oid, int16 column_attnum,
                               const char **paths, int path_count,
                               JsonIndexType index_type,
                               bool is_unique, const char *predicate)
{
    ...
    appendStringInfo(&sql, "CREATE %sINDEX idx_%s_%s_json ON %s.%s ",
                     is_unique ? "UNIQUE " : "",
    ...
    switch (index_type)
    {
        case JSON_INDEX_GIN:
            if (path_count == 1)
            {
                appendStringInfo(&sql, "USING gin ((%s -> '%s'))",
                                 column_name, paths[0]);
            }
            else
            {
                appendStringInfo(&sql, "USING gin (%s jsonb_path_ops)",
                                 column_name);
            }
            break;
        ...
        case JSON_INDEX_HASH:
            ...
            appendStringInfo(&sql, "USING hash ((%s ->> '%s'))",
                             column_name, paths[0]);
            break;
    ...
    if (ret != SPI_OK_UTILITY)
    {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("failed to create JSON index"),
...
json_index_drop(Oid index_oid)
{
    ...
    if (ret != SPI_OK_UTILITY)
    {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("failed to drop JSON index")));
    }
...
bool
json_index_path_is_indexed(Oid table_oid, int16 column_attnum, const char *path)
{
    List       *indexes;
    ListCell   *lc;
    bool        found = false;
...
List *
json_index_get_indexes(Oid table_oid, int16 column_attnum)
{
    List       *result = NIL;
    Relation    rel;
    List       *index_oids;
...
JsonIndexInfo *
json_index_get_info(Oid index_oid)
{
    JsonIndexInfo *info;
    Relation    index_rel;
    char       *index_name;
...
void
json_index_free_info(JsonIndexInfo *info)
{
    if (info == NULL)
        return;
...
Oid
json_index_create_gin(Oid table_oid, int16 column_attnum,
                      const char *operator_class)
{
    ...
    if (operator_class && *operator_class)
    {
        appendStringInfo(&sql,
            ...
            column_name, operator_class);
    }
    else
    {
        appendStringInfo(&sql,
            "CREATE INDEX idx_%s_%s_gin ON %s.%s USING gin (%s)",
    ...
    if (ret != SPI_OK_UTILITY)
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("failed to create GIN index")));
...
Oid
json_index_create_gin_path_ops(Oid table_oid, int16 column_attnum)
{
    return json_index_create_gin(table_oid, column_attnum, "jsonb_path_ops");
}

bool
json_index_gin_can_accelerate(Oid index_oid, const char *path,
                              int operator_type)
{
    Relation    index_rel;
...
Oid
json_index_create_expression(Oid table_oid, int16 column_attnum,
                             const char *path, Oid cast_type)
{
    ...
    initStringInfo(&sql);
    appendStringInfo(&sql,
        "CREATE INDEX idx_%s_%s_expr ON %s.%s (((%s ->> '%s')::%s))",
        table_name, safe_path, schema_name, table_name,
        column_name, path, type_name);
    ...
    if (ret != SPI_OK_UTILITY)
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("failed to create expression index")));
...
Oid
json_index_create_composite(Oid table_oid, int16 column_attnum,
                            ...
                            int path_count)
{
    ...
    if (path_count <= 0)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("at least one path required")));
    ...
    initStringInfo(&sql);
    appendStringInfo(&sql,
        "CREATE INDEX idx_%s_%s_composite ON %s.%s (",
        table_name, column_name, schema_name, table_name);
    ...
    if (ret != SPI_OK_UTILITY)
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("failed to create composite index")));
...
Datum
orochi_create_json_index(PG_FUNCTION_ARGS)
{
    ...
    /* Create the index */
    index_oid = json_index_create(table_oid, column_attnum,
                                  paths, path_count, JSON_INDEX_GIN);
...
Datum
orochi_analyze_json_paths(PG_FUNCTION_ARGS)
{
    Oid         table_oid = PG_GETARG_OID(0);
    text       *column_name_text = PG_GETARG_TEXT_PP(1);
    char       *column_name;
...
Datum
orochi_json_index_recommendations(PG_FUNCTION_ARGS)
{
    Oid         table_oid = PG_GETARG_OID(0);
    text       *column_name_text = PG_GETARG_TEXT_PP(1);
...
Datum
orochi_json_path_is_indexed(PG_FUNCTION_ARGS)
{
    Oid         table_oid = PG_GETARG_OID(0);
    text       *column_name_text = PG_GETARG_TEXT_PP(1);
...
Datum
orochi_json_path_stats(PG_FUNCTION_ARGS)
{
    Oid         table_oid = PG_GETARG_OID(0);
    text       *column_name_text = PG_GETARG_TEXT_PP(1);
    text       *path_text = PG_GETARG_TEXT_PP(2);
