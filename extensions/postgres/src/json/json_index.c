...
static void ensure_json_index_context(void) {
  if (JsonIndexContext == NULL) {
    JsonIndexContext = AllocSetContextCreate(
        TopMemoryContext, "JsonIndexContext", ALLOCSET_DEFAULT_SIZES);
  }
}
...
static void analyze_jsonb_value(Jsonb *jb, const char *current_path,
  ...
  while ((token = JsonbIteratorNext(&it, &v, false)) != WJB_DONE) {
    ...
    case WJB_ELEM: {
      ...
      if (path_stats == NULL &&
          stats->path_count < JSON_INDEX_MAX_TRACKED_PATHS) {
...
void json_index_record_access(Oid table_oid, int16 column_attnum,
                              const char *path) {
...
JsonPathStats *json_index_get_path_stats(Oid table_oid, int16 column_attnum,
                                         const char *path) {
...
JsonPathStatsCollection *json_index_get_all_stats(Oid table_oid,
                                                  int16 column_attnum) {
...
static char *build_index_create_sql(Oid table_oid, int16 column_attnum,
  ...
  case JSON_INDEX_GIN:
    appendStringInfo(
        &buf, "CREATE INDEX idx_%s_%s_gin ON %s.%s USING gin ((%s -> '%s'))",
        table_name, safe_path, schema_name, table_name, column_name, path);
    break;

  case JSON_INDEX_BTREE:
    appendStringInfo(
        &buf, "CREATE INDEX idx_%s_%s_btree ON %s.%s ((%s ->> '%s'))",
        table_name, safe_path, schema_name, table_name, column_name, path);
    break;

  case JSON_INDEX_HASH:
    appendStringInfo(
        &buf, "CREATE INDEX idx_%s_%s_hash ON %s.%s USING hash ((%s ->> '%s'))",
        table_name, safe_path, schema_name, table_name, column_name, path);
    break;

  case JSON_INDEX_EXPRESSION:
    appendStringInfo(
        &buf, "CREATE INDEX idx_%s_%s_expr ON %s.%s ((%s #>> '{%s}'))",
        table_name, safe_path, schema_name, table_name, column_name, path);
    break;
...
List *json_index_get_recommendations(Oid table_oid, int16 column_attnum,
...
int json_index_apply_recommendations(Oid table_oid, int16 column_attnum,
                                     int max_indexes) {
  ...
  recommendations =
      json_index_get_recommendations(table_oid, column_attnum, max_indexes);
...
double json_index_estimate_benefit(Oid table_oid, int16 column_attnum,
                                   const char *path, JsonIndexType type) {
  JsonPathStats *stats;
  double benefit = 0.0;
...
Oid json_index_create(Oid table_oid, int16 column_attnum, const char **paths,
                      int path_count, JsonIndexType index_type) {
  return json_index_create_with_options(table_oid, column_attnum, paths,
                                        path_count, index_type, false, NULL);
}

Oid json_index_create_with_options(Oid table_oid, int16 column_attnum,
                                   const char **paths, int path_count,
                                   JsonIndexType index_type, bool is_unique,
  ...
  appendStringInfo(&sql, "CREATE %sINDEX idx_%s_%s_json ON %s.%s ",
                   is_unique ? "UNIQUE " : "", table_name, column_name,
...
bool json_index_path_is_indexed(Oid table_oid, int16 column_attnum,
                                const char *path) {
  List *indexes;
  ListCell *lc;
  bool found = false;
...
Oid json_index_create_gin(Oid table_oid, int16 column_attnum,
  ...
  if (operator_class && *operator_class) {
    ...
    appendStringInfo(&sql, "CREATE INDEX idx_%s_%s_gin ON %s.%s USING gin (%s)",
                     table_name, column_name, schema_name, table_name,
                     column_name);
  }
...
bool json_index_gin_can_accelerate(Oid index_oid, const char *path,
                                   int operator_type) {
  Relation index_rel;
...
Oid json_index_create_expression(Oid table_oid, int16 column_attnum,
  ...
  appendStringInfo(&sql,
                   ...
                   table_name, safe_path, schema_name, table_name, column_name,
                   path, type_name);
...
Oid json_index_create_composite(Oid table_oid, int16 column_attnum,
  ...
  initStringInfo(&sql);
  appendStringInfo(&sql, "CREATE INDEX idx_%s_%s_composite ON %s.%s (",
                   table_name, column_name, schema_name, table_name);
  ...
  for (int i = 0; i < path_count; i++) {
    ...
    appendStringInfo(&sql, "((%s ->> '%s')::%s)", column_name, paths[i],
                     type_name);
  }
...
Datum orochi_create_json_index(PG_FUNCTION_ARGS) {
  ...
  /* Create the index */
  index_oid = json_index_create(table_oid, column_attnum, paths, path_count,
                                JSON_INDEX_GIN);
...
Datum orochi_analyze_json_paths(PG_FUNCTION_ARGS) {
  ...
  if (call_cntr < max_calls) {
    ...
    values[6] =
        CStringGetTextDatum(json_index_type_name(ps->recommended_index));
...
Datum orochi_json_index_recommendations(PG_FUNCTION_ARGS) {
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    recommendations =
        json_index_get_recommendations(table_oid, column_attnum, max_recs);
