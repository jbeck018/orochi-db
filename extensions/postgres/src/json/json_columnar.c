...
static Datum extract_jsonb_path_value(Jsonb *jb, const char *path,
                                      Oid target_type, bool *isnull);
...
static void ensure_columnar_context(void) {
  if (JsonColumnarContext == NULL) {
    JsonColumnarContext = AllocSetContextCreate(
        TopMemoryContext, "JsonColumnarContext", ALLOCSET_DEFAULT_SIZES);
  }
}
...
static Datum extract_jsonb_path_value(Jsonb *jb, const char *path,
  ...
  PG_TRY();
  {
    ...
    case JSONBOID:
      ...
      result_jb = DatumGetJsonbP(DirectFunctionCall2(
          jsonb_object_field, JsonbPGetDatum(jb), path_datum));
      *isnull = false;
...
static void store_column_metadata(JsonExtractedColumn *col) {
  ...
  SPI_execute_with_args(
      ...
      "  last_refreshed = NOW()",
      13,
      (Oid[]){INT4OID, OIDOID, INT2OID, TEXTOID, OIDOID, INT4OID, INT4OID,
              INT8OID, INT8OID, INT8OID, INT8OID, FLOAT8OID, TIMESTAMPTZOID},
      (Datum[]){
          Int32GetDatum(col->column_id), ObjectIdGetDatum(col->table_oid),
...
static JsonExtractedColumn *load_column_metadata(int32 column_id) {
  ...
  col =
      MemoryContextAllocZero(JsonColumnarContext, sizeof(JsonExtractedColumn));

  col->column_id = DatumGetInt32(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
  col->table_oid = DatumGetObjectId(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 2, &isnull));
  col->source_attnum = DatumGetInt16(
  ...
  col->target_type = DatumGetObjectId(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 5, &isnull));
  col->storage_type = DatumGetInt32(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 6, &isnull));
  col->state = DatumGetInt32(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 7, &isnull));
  col->row_count = DatumGetInt64(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 8, &isnull));
  col->null_count = DatumGetInt64(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 9, &isnull));
  col->distinct_count = DatumGetInt64(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 10, &isnull));
  col->storage_size = DatumGetInt64(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 11, &isnull));
  col->compression_ratio = DatumGetFloat8(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 12, &isnull));
...
List *json_columnar_extract_columns(Oid table_oid, int16 source_attnum,
  ...
  /* Extract data */
  {
    ...
    while (table_scan_getnextslot(scan, ForwardScanDirection, slot)) {
      ...
      foreach (lc, result) {
        ...
        value = extract_jsonb_path_value(jb, col->path, col->target_type,
                                         &value_isnull);
...
List *json_columnar_auto_extract(Oid table_oid, int16 source_attnum,
                                 int max_columns) {
  ...
  for (int i = 0; i < stats->path_count - 1; i++) {
    for (int j = i + 1; j < stats->path_count; j++) {
      double score_i =
          stats->paths[i].access_count * (1.0 - stats->paths[i].selectivity);
      double score_j =
          stats->paths[j].access_count * (1.0 - stats->paths[j].selectivity);
      if (score_j > score_i) {
  ...
  result = json_columnar_extract_columns(table_oid, source_attnum, paths,
                                         path_count);
...
void json_columnar_drop_column(int32 column_id) {
  ...
  SPI_execute_with_args(
      "DELETE FROM orochi.orochi_json_columns WHERE column_id = $1", 1,
      (Oid[]){INT4OID}, (Datum[]){Int32GetDatum(column_id)}, NULL, false, 0);
...
List *json_columnar_get_columns(Oid table_oid, int16 source_attnum) {
  ...
  if (ret == SPI_OK_SELECT) {
    for (i = 0; i < SPI_processed; i++) {
      ...
      column_id = DatumGetInt32(SPI_getbinval(
...
JsonColumnarWriteState *json_columnar_begin_write(Oid table_oid,
                                                  int16 source_attnum) {
  ...
  state->write_context = AllocSetContextCreate(
      JsonColumnarContext, "JsonColumnarWrite", ALLOCSET_DEFAULT_SIZES);
...
void json_columnar_write_value(JsonColumnarWriteState *state, Jsonb *value,
                               int64 row_id) {
  ListCell *lc;
  int col_idx = 0;
  ...
  foreach (lc, state->extracted_columns) {
    ...
    extracted =
        extract_jsonb_path_value(value, col->path, col->target_type, &isnull);
...
json_columnar_begin_read(Oid table_oid, int16 source_attnum, List *column_ids) {
  ...
  state->read_context = AllocSetContextCreate(
      JsonColumnarContext, "JsonColumnarRead", ALLOCSET_DEFAULT_SIZES);
...
bool json_columnar_read_value(JsonColumnarReadState *state, int32 column_id,
                              int64 row_id, Datum *value, bool *isnull) {
  int ret;
  StringInfoData sql;
  int col_idx = -1;
  ListCell *lc;
...
int json_columnar_read_batch(JsonColumnarReadState *state, int32 column_id,
                             int64 start_row, int max_rows, Datum *values,
                             bool *nulls) {
  int ret;
  StringInfoData sql;
  int col_idx = -1;
  ...
  appendStringInfo(&sql,
                   ...
                   col_idx, state->table_oid, state->source_attnum, start_row,
                   max_rows);
  ...
  for (uint64 i = 0; i < SPI_processed; i++) {
    values[i] = SPI_getbinval(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, 1,
                              &nulls[i]);
    rows_read++;
  }
...
JsonColumnarStorageType json_columnar_select_storage(JsonExtractedColumn *col,
                                                     Datum *sample_values,
                                                     int sample_count) {
...
void json_columnar_decompress(const char *compressed, int64 compressed_size,
                              JsonColumnarStorageType storage_type, int count,
  ...
  case JSON_COLUMNAR_DICTIONARY:
    ...
    actual_size = decompress_lz4(compressed, compressed_size, decompressed,
                                 expected_size);
    if (actual_size > 0)
  ...
  case JSON_COLUMNAR_DELTA:
    ...
    decompressed = palloc(expected_size);
    actual_size = decompress_delta(compressed, compressed_size, decompressed,
                                   expected_size);
    if (actual_size > 0)
      memcpy(values, decompressed, Min(actual_size, expected_size));
    pfree(decompressed);
  ...
  case JSON_COLUMNAR_RLE:
    /* Run-length decoding */
    decompressed = palloc(expected_size);
    actual_size = decompress_rle(compressed, compressed_size, decompressed,
                                 expected_size);
    if (actual_size > 0)
      memcpy(values, decompressed, Min(actual_size, expected_size));
    pfree(decompressed);
...
void json_columnar_build_dictionary(Datum *values, int count,
  ...
  dict_hash = hash_create("DictHash", count / 10 + 1, &hash_ctl,
...
JsonColumnarStats *json_columnar_get_stats(Oid table_oid, int16 source_attnum) {
  ...
  stats =
      MemoryContextAllocZero(JsonColumnarContext, sizeof(JsonColumnarStats));
  ...
  if (stats->original_size > 0)
    stats->space_savings =
        1.0 - (double)stats->columnar_size / stats->original_size;
...
Datum orochi_extract_columns(PG_FUNCTION_ARGS) {
  ...
  deconstruct_array(paths_array, TEXTOID, -1, false, TYPALIGN_INT, &path_datums,
                    &path_nulls, &path_count);
  ...
  result = json_columnar_extract_columns(table_oid, column_attnum, paths,
                                         path_count);
...
void json_columnar_enable_hybrid(Oid table_oid, int16 source_attnum,
                                 const char **columnar_paths, int path_count) {
  /* Extract the specified paths to columnar storage */
  json_columnar_extract_columns(table_oid, source_attnum, columnar_paths,
                                path_count);
...
JsonHybridConfig *json_columnar_get_hybrid_config(Oid table_oid,
                                                  int16 source_attnum) {
  ...
  config =
      MemoryContextAllocZero(JsonColumnarContext, sizeof(JsonHybridConfig));
  ...
  foreach (lc, columns) {
    JsonExtractedColumn *col = lfirst(lc);
    config->columnar_paths =
        lappend(config->columnar_paths, pstrdup(col->path));
  }
...
void json_columnar_compact(Oid table_oid, int16 source_attnum) {
  ...
  appendStringInfo(&sql,
                   ...
                   JSON_COLUMNAR_CHUNK_SIZE, table_oid, source_attnum,
                   table_oid, source_attnum);
...
void json_columnar_vacuum(Oid table_oid, int16 source_attnum) {
  ...
  appendStringInfo(&sql,
                   ...
                   ")",
                   table_oid, source_attnum, table_oid, source_attnum,
  ...
  resetStringInfo(&sql);
  appendStringInfo(&sql, "VACUUM orochi.orochi_json_data_%u_%d", table_oid,
                   source_attnum);
