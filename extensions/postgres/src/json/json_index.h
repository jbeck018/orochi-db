 ...
 */
extern Oid json_index_create(Oid table_oid, int16 column_attnum, const char **paths, int path_count,
                             JsonIndexType index_type);

/*
 * Create a JSON path index with options
 */
extern Oid json_index_create_with_options(Oid table_oid, int16 column_attnum, const char **paths,
 ...
 */
extern bool json_index_path_is_indexed(Oid table_oid, int16 column_attnum, const char *path);
 ...
 */
extern JsonPathStatsCollection *json_index_analyze_paths(Oid table_oid, int16 column_attnum,
                                                         int sample_size);

/*
 * Record path access for statistics
 */
extern void json_index_record_access(Oid table_oid, int16 column_attnum, const char *path);

/*
 * Get path statistics
 */
extern JsonPathStats *json_index_get_path_stats(Oid table_oid, int16 column_attnum,
                                                const char *path);

/*
 * Get all path statistics for a column
 */
extern JsonPathStatsCollection *json_index_get_all_stats(Oid table_oid, int16 column_attnum);
 ...
 */
extern int json_index_apply_recommendations(Oid table_oid, int16 column_attnum, int max_indexes);

/*
 * Get estimated benefit of indexing a path
 */
extern double json_index_estimate_benefit(Oid table_oid, int16 column_attnum, const char *path,
                                          JsonIndexType type);
 ...
 */
extern Oid json_index_create_gin(Oid table_oid, int16 column_attnum, const char *operator_class);
 ...
 */
extern bool json_index_gin_can_accelerate(Oid index_oid, const char *path, int operator_type);
 ...
 */
extern Oid json_index_create_expression(Oid table_oid, int16 column_attnum, const char *path,
                                        Oid cast_type);

/*
 * Create composite expression index on multiple paths
 */
extern Oid json_index_create_composite(Oid table_oid, int16 column_attnum, const char **paths,
