...
S3Client *s3_source_init(S3SourceConfig *config) {
  ...
  elog(LOG, "S3 source initialized: bucket=%s, prefix=%s, region=%s",
       config->bucket, config->prefix ? config->prefix : "(none)",
       s3_config.region);
 ...
 */
List *s3_source_list_new_files(S3Client *client, const char *prefix,
  ...
  /* Build full URL */
  snprintf(url, sizeof(url), "%s://%s%s?%s", client->use_ssl ? "https" : "http",
  ...
  while (p < end) {
    ...
    if (key_start && key_end && key_start < contents_end) {
      ...
      if (lm_start && lm_end && lm_start < contents_end) {
        ...
        if (sscanf(lm_start, "%d-%d-%dT%d:%d:%d", &tm_time.tm_year,
...
int s3_process_files(Pipeline *pipeline, S3Client *client, int max_files) {
  ...
  new_files =
      s3_source_list_new_files(client, config->prefix, config->file_pattern, 0);
  ...
  foreach (lc, new_files) {
    ...
    if (config->max_file_size > 0 && file->size > config->max_file_size) {
      elog(WARNING, "Skipping file %s: size %ld exceeds limit %ld", file->key,
