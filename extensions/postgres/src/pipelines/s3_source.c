 ...
 */
S3Client *
s3_source_init(S3SourceConfig *config)
{
    S3Client *client;
    ...
    else
    {
        ...
        char endpoint[256];
        snprintf(endpoint, sizeof(endpoint), "s3.%s.amazonaws.com",
                 s3_config.region);
    ...
    elog(LOG, "S3 source initialized: bucket=%s, prefix=%s, region=%s",
         config->bucket,
 ...
 */
List *
s3_source_list_new_files(S3Client *client, const char *prefix,
                         const char *pattern, TimestampTz since)
{
    List *result = NIL;
    CURL *curl;
    CURLcode res;
    ...
    {
        elog(WARNING, "S3 list request failed: HTTP %ld, CURL error: %s",
 ...
 */
static List *
parse_s3_list_response(const char *xml_response, int64 size)
{
    List *files = NIL;
    ...
    while (p < end)
    {
        ...
        if (key_start && key_end && key_start < contents_end)
        {
            ...
            if (lm_start && lm_end && lm_start < contents_end)
            {
                ...
                if (sscanf(lm_start, "%d-%d-%dT%d:%d:%d",
 ...
 */
char *
s3_source_fetch_file(S3Client *client, const char *key, int64 *size)
{
    ...
    {
        elog(WARNING, "Failed to fetch S3 file %s: %s",
             key, result->error_message ? result->error_message : "unknown error");
    }
 ...
 */
bool
s3_source_mark_processed(int64 pipeline_id, const char *key)
{
 ...
 */
List *
s3_source_get_processed_files(int64 pipeline_id)
{
    List *files = NIL;
    StringInfoData query;
    int ret;
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0)
    {
        ...
        {
            char *key = SPI_getvalue(SPI_tuptable->vals[i],
 ...
 */
bool
s3_source_is_processed(int64 pipeline_id, const char *key)
{
    StringInfoData query;
    int ret;
 ...
 */
int
s3_process_files(Pipeline *pipeline, S3Client *client, int max_files)
{
    S3SourceConfig *config;
    List *new_files;
    ListCell *lc;
    ...
    new_files = s3_source_list_new_files(client, config->prefix,
                                         config->file_pattern, 0);
    ...
    foreach(lc, new_files)
    {
        ...
        {
            elog(WARNING, "Skipping file %s: size %ld exceeds limit %ld",
                 file->key, file->size, config->max_file_size);
            continue;
        }
        ...
        PG_TRY();
        {
            ...
            if (records != NIL)
            {
                ...
                elog(DEBUG1, "Processed S3 file %s: %d records, %ld bytes",
                     file->key, list_length(records), size);
    ...
    elog(LOG, "S3 source processed %d files (%ld bytes) for pipeline %ld",
         processed, total_bytes, pipeline->pipeline_id);
 ...
 */
static bool
matches_pattern(const char *filename, const char *pattern)
{
 ...
 */
static char *
extract_filename(const char *key)
{
 ...
 */
void
s3_source_close(S3Client *client)
{
    if (client)
    {
