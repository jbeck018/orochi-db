...
extern bool orochi_update_pipeline_config(int64 pipeline_id,
                                          const char *options_json);
...
extern int kafka_source_poll(void *consumer, char **messages, int max_messages,
                             int timeout_ms);
...
extern List *s3_source_list_new_files(struct S3Client *client,
                                      ...
                                      TimestampTz since);
extern char *s3_source_fetch_file(struct S3Client *client, const char *key,
                                  int64 *size);
...
extern List *parse_csv_records(const char *data, int64 size, char delimiter,
                               bool has_header);
...
extern int64 pipeline_insert_batch(Pipeline *pipeline, List *records,
                                   bool use_copy);
extern bool pipeline_apply_transforms(Pipeline *pipeline, void *record,
