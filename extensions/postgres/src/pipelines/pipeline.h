...
/* Pipeline lifecycle */
extern int64 orochi_create_pipeline(const char *name, PipelineSource source_type,
...
extern List *s3_source_list_new_files(struct S3Client *client, const char *prefix,
                                      const char *pattern, TimestampTz since);
extern char *s3_source_fetch_file(struct S3Client *client, const char *key, int64 *size);
...
extern List *parse_csv_records(const char *data, int64 size, char delimiter, bool has_header);
...
extern int64 pipeline_insert_batch(Pipeline *pipeline, List *records, bool use_copy);
extern bool pipeline_apply_transforms(Pipeline *pipeline, void *record, Datum *values, bool *nulls);
