...
static void update_pipeline_stats(Pipeline *pipeline, int64 processed,
                                  int64 failed);
...
Size orochi_pipeline_shmem_size(void) { return sizeof(PipelineSharedState); }

void orochi_pipeline_shmem_init(void) {
  bool found;

  pipeline_shared_state = (PipelineSharedState *)ShmemInitStruct(
      "Orochi Pipeline State", sizeof(PipelineSharedState), &found);
 ...
 */
int64 orochi_create_pipeline(const char *name, PipelineSource source_type,
                             Oid target_table_oid, PipelineFormat format,
  ...
  appendStringInfo(&query,
                   ...
                   "(name, source_type, target_table_oid, format, options, "
                   "state, created_at) "
                   "VALUES (%s, %d, %u, %d, %s, %d, NOW()) "
                   "RETURNING pipeline_id",
                   quote_literal_cstr(name), (int)source_type, target_table_oid,
                   (int)format,
  ...
  if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0) {
    bool isnull;
    pipeline_id = DatumGetInt64(SPI_getbinval(
 ...
 */
bool orochi_start_pipeline(int64 pipeline_id) {
  Pipeline *pipeline;
 ...
 */
bool orochi_pause_pipeline(int64 pipeline_id) {
  StringInfoData query;
  int ret;
  int worker_pid;
  ...
  worker_pid = DatumGetInt32(
      SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, NULL));
  pfree(query.data);
 ...
 */
bool orochi_resume_pipeline(int64 pipeline_id) {
  PipelineState current_state;

  current_state = orochi_get_pipeline_state(pipeline_id);
  if (current_state != PIPELINE_STATE_PAUSED) {
    elog(NOTICE, "pipeline %ld is not paused (state: %s)", pipeline_id,
         pipeline_state_name(current_state));
    return false;
  }
 ...
 */
bool orochi_stop_pipeline(int64 pipeline_id) {
  StringInfoData query;
  int ret;
  int worker_pid = 0;
 ...
 */
bool orochi_drop_pipeline(int64 pipeline_id, bool if_exists) {
  StringInfoData query;
  int ret;
  PipelineState state;
  ...
  if (state == PIPELINE_STATE_CREATED && !if_exists) {
    ...
    initStringInfo(&query);
    appendStringInfo(
        &query, "SELECT 1 FROM orochi.orochi_pipelines WHERE pipeline_id = %ld",
        pipeline_id);
    ret = SPI_execute(query.data, true, 1);
    pfree(query.data);
    SPI_finish();
  ...
  initStringInfo(&query);
  appendStringInfo(
      &query, "DELETE FROM orochi.orochi_pipelines WHERE pipeline_id = %ld",
      pipeline_id);
  SPI_execute(query.data, false, 0);
  pfree(query.data);
 ...
 */
Pipeline *orochi_get_pipeline(int64 pipeline_id) {
 ...
 */
PipelineState orochi_get_pipeline_state(int64 pipeline_id) {
  StringInfoData query;
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    bool isnull;
    state = (PipelineState)DatumGetInt32(SPI_getbinval(
        SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
  }
 ...
 */
PipelineStats *orochi_get_pipeline_stats(int64 pipeline_id) {
  PipelineStats *stats;
  StringInfoData query;
  int ret;
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    stats->last_message_time =
        DatumGetTimestampTz(SPI_getbinval(tuple, tupdesc, 6, &isnull));
    if (!isnull)
    ...
    stats->throughput_per_sec =
        DatumGetFloat8(SPI_getbinval(tuple, tupdesc, 10, &isnull));
  }
 ...
 *    List all pipelines
 */
List *orochi_list_pipelines(void) {
  List *result = NIL;
  StringInfoData query;
  int ret;
  int i;
 ...
 */
void pipeline_worker_main(Datum main_arg) {
  ...
  while (!got_sigterm) {
    ...
    PG_CATCH();
    {
      ...
      /* Safely get error data - use TopMemoryContext if pipeline_context is
       * NULL */
      ...
      if (pipeline->stats &&
        ...
        elog(LOG, "Pipeline %ld exceeded error threshold, stopping",
             pipeline_id);
        got_sigterm = true;
      }
...
static Pipeline *load_pipeline_from_catalog(int64 pipeline_id) {
  ...
  pipeline->pipeline_context = AllocSetContextCreate(
      TopMemoryContext, "Pipeline Context", ALLOCSET_DEFAULT_SIZES);
  ...
  if (ret == SPI_OK_SELECT && SPI_processed > 0) {
    ...
    pipeline->source_type = (PipelineSource)DatumGetInt32(
        SPI_getbinval(tuple, tupdesc, 3, &isnull));
    pipeline->target_table_oid =
        DatumGetObjectId(SPI_getbinval(tuple, tupdesc, 4, &isnull));
    pipeline->format = (PipelineFormat)DatumGetInt32(
        SPI_getbinval(tuple, tupdesc, 5, &isnull));

    options_json = SPI_getvalue(tuple, tupdesc, 6);

    pipeline->state =
        (PipelineState)DatumGetInt32(SPI_getbinval(tuple, tupdesc, 7, &isnull));
    pipeline->created_at =
    ...
    pipeline->error_threshold =
        DatumGetInt32(SPI_getbinval(tuple, tupdesc, 14, &isnull));
...
static void process_pipeline_batch(Pipeline *pipeline) {
  ...
  case PIPELINE_SOURCE_S3:
    ...
    elog(DEBUG1, "Processing S3 files for pipeline %ld", pipeline->pipeline_id);
    break;
...
static void update_pipeline_stats(Pipeline *pipeline, int64 processed,
  ...
  appendStringInfo(
      ...
      "messages_received = orochi.orochi_pipeline_stats.messages_received + "
      "EXCLUDED.messages_received, "
      "messages_processed = orochi.orochi_pipeline_stats.messages_processed + "
      "EXCLUDED.messages_processed, "
      "messages_failed = orochi.orochi_pipeline_stats.messages_failed + "
      "EXCLUDED.messages_failed, "
      "rows_inserted = orochi.orochi_pipeline_stats.rows_inserted + "
  ...
  PG_TRY();
  {
    ...
    while ((r = JsonbIteratorNext(&it, &v, false)) != WJB_DONE) {
      if (r == WJB_KEY) {
        ...
        if (strcmp(key, "bootstrap_servers") == 0 ||
            strcmp(key, "brokers") == 0) {
          if (v.type == jbvString)
        ...
        } else if (strcmp(key, "consumer_group") == 0 ||
                   strcmp(key, "group_id") == 0) {
          if (v.type == jbvString)
          ...
          if (v.type == jbvNumeric)
            config->partition = DatumGetInt32(DirectFunctionCall1(
                numeric_int4, NumericGetDatum(v.val.numeric)));
        } else if (strcmp(key, "start_offset") == 0) {
          if (v.type == jbvNumeric)
            config->start_offset = DatumGetInt64(DirectFunctionCall1(
                numeric_int8, NumericGetDatum(v.val.numeric)));
          ...
          if (v.type == jbvNumeric)
            config->fetch_min_bytes = DatumGetInt32(DirectFunctionCall1(
                numeric_int4, NumericGetDatum(v.val.numeric)));
        } else if (strcmp(key, "fetch_max_wait_ms") == 0) {
          if (v.type == jbvNumeric)
            config->fetch_max_wait_ms = DatumGetInt32(DirectFunctionCall1(
                numeric_int4, NumericGetDatum(v.val.numeric)));
        } else if (strcmp(key, "session_timeout_ms") == 0) {
          if (v.type == jbvNumeric)
            config->session_timeout_ms = DatumGetInt32(DirectFunctionCall1(
                numeric_int4, NumericGetDatum(v.val.numeric)));
        } else if (strcmp(key, "heartbeat_interval_ms") == 0) {
          if (v.type == jbvNumeric)
            config->heartbeat_interval_ms = DatumGetInt32(DirectFunctionCall1(
                numeric_int4, NumericGetDatum(v.val.numeric)));
          ...
          if (v.type == jbvNumeric)
            config->auto_commit_interval = DatumGetInt32(DirectFunctionCall1(
                numeric_int4, NumericGetDatum(v.val.numeric)));
        }
...
const char *pipeline_source_name(PipelineSource source) {
  switch (source) {
  case PIPELINE_SOURCE_KAFKA:
    return "kafka";
  case PIPELINE_SOURCE_S3:
  ...
  case PIPELINE_SOURCE_FILESYSTEM:
    return "filesystem";
  case PIPELINE_SOURCE_HTTP:
    return "http";
  case PIPELINE_SOURCE_KINESIS:
    return "kinesis";
  default:
    return "unknown";
  }
...
PipelineSource pipeline_parse_source(const char *name) {
  if (pg_strcasecmp(name, "kafka") == 0)
    return PIPELINE_SOURCE_KAFKA;
  if (pg_strcasecmp(name, "s3") == 0)
    return PIPELINE_SOURCE_S3;
  if (pg_strcasecmp(name, "filesystem") == 0)
    return PIPELINE_SOURCE_FILESYSTEM;
  if (pg_strcasecmp(name, "http") == 0)
    return PIPELINE_SOURCE_HTTP;
  if (pg_strcasecmp(name, "kinesis") == 0)
    return PIPELINE_SOURCE_KINESIS;
...
const char *pipeline_state_name(PipelineState state) {
  switch (state) {
...
PipelineState pipeline_parse_state(const char *name) {
  if (pg_strcasecmp(name, "created") == 0)
    return PIPELINE_STATE_CREATED;
  if (pg_strcasecmp(name, "running") == 0)
    return PIPELINE_STATE_RUNNING;
  if (pg_strcasecmp(name, "paused") == 0)
    return PIPELINE_STATE_PAUSED;
  if (pg_strcasecmp(name, "stopped") == 0)
    return PIPELINE_STATE_STOPPED;
  if (pg_strcasecmp(name, "error") == 0)
    return PIPELINE_STATE_ERROR;
  if (pg_strcasecmp(name, "draining") == 0)
    return PIPELINE_STATE_DRAINING;

  return PIPELINE_STATE_CREATED;
}

const char *pipeline_format_name(PipelineFormat format) {
  switch (format) {
  case PIPELINE_FORMAT_JSON:
    return "json";
  case PIPELINE_FORMAT_CSV:
    return "csv";
  case PIPELINE_FORMAT_AVRO:
    return "avro";
  case PIPELINE_FORMAT_PARQUET:
    return "parquet";
...
PipelineFormat pipeline_parse_format(const char *name) {
  if (pg_strcasecmp(name, "json") == 0)
    return PIPELINE_FORMAT_JSON;
  if (pg_strcasecmp(name, "csv") == 0)
    return PIPELINE_FORMAT_CSV;
  if (pg_strcasecmp(name, "avro") == 0)
    return PIPELINE_FORMAT_AVRO;
  if (pg_strcasecmp(name, "parquet") == 0)
    return PIPELINE_FORMAT_PARQUET;
  if (pg_strcasecmp(name, "line") == 0)
    return PIPELINE_FORMAT_LINE;
...
int64 pipeline_insert_batch(Pipeline *pipeline, List *records, bool use_copy) {
  ...
  foreach (lc, records) {
    ...
    PG_TRY();
    {
      ...
      ret = SPI_execute_with_args(query.data, 1, argtypes, argvals, NULL, false,
                                  0);
...
Datum orochi_create_pipeline_sql(PG_FUNCTION_ARGS) {
...
Datum orochi_start_pipeline_sql(PG_FUNCTION_ARGS) {
  int64 pipeline_id = PG_GETARG_INT64(0);
  bool result = orochi_start_pipeline(pipeline_id);
  PG_RETURN_BOOL(result);
}

Datum orochi_pause_pipeline_sql(PG_FUNCTION_ARGS) {
  int64 pipeline_id = PG_GETARG_INT64(0);
  bool result = orochi_pause_pipeline(pipeline_id);
  PG_RETURN_BOOL(result);
}

Datum orochi_resume_pipeline_sql(PG_FUNCTION_ARGS) {
  int64 pipeline_id = PG_GETARG_INT64(0);
  bool result = orochi_resume_pipeline(pipeline_id);
  PG_RETURN_BOOL(result);
}

Datum orochi_stop_pipeline_sql(PG_FUNCTION_ARGS) {
  int64 pipeline_id = PG_GETARG_INT64(0);
  bool result = orochi_stop_pipeline(pipeline_id);
  PG_RETURN_BOOL(result);
}

Datum orochi_drop_pipeline_sql(PG_FUNCTION_ARGS) {
  int64 pipeline_id = PG_GETARG_INT64(0);
