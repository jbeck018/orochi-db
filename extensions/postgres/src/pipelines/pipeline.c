...
Size orochi_pipeline_shmem_size(void)
{
 ...
 */
int64 orochi_create_pipeline(const char *name, PipelineSource source_type, Oid target_table_oid,
                             PipelineFormat format, const char *options_json)
{
    Pipeline *pipeline;
    ...
    if (strlen(name) > PIPELINE_MAX_NAME_LENGTH)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("pipeline name too long (max %d characters)", PIPELINE_MAX_NAME_LENGTH)));

    if (!OidIsValid(target_table_oid))
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("invalid target table")));
    ...
    if (ret == SPI_OK_INSERT_RETURNING && SPI_processed > 0) {
        ...
        ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR), errmsg("failed to create pipeline")));
    }
...
bool orochi_start_pipeline(int64 pipeline_id)
{
    ...
    if (WaitForBackgroundWorkerStartup(handle, &pid) != BGWH_STARTED) {
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR), errmsg("pipeline worker failed to start")));
    }
...
bool orochi_pause_pipeline(int64 pipeline_id)
{
    ...
    worker_pid =
        DatumGetInt32(SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, NULL));
    pfree(query.data);
...
bool orochi_drop_pipeline(int64 pipeline_id, bool if_exists)
{
    ...
    if (state == PIPELINE_STATE_CREATED && !if_exists) {
        ...
        SPI_connect();
        initStringInfo(&query);
        appendStringInfo(&query, "SELECT 1 FROM orochi.orochi_pipelines WHERE pipeline_id = %ld",
                         pipeline_id);
        ret = SPI_execute(query.data, true, 1);
        pfree(query.data);
        SPI_finish();
    ...
    /* Delete stats */
    initStringInfo(&query);
    appendStringInfo(&query, "DELETE FROM orochi.orochi_pipeline_stats WHERE pipeline_id = %ld",
                     pipeline_id);
    SPI_execute(query.data, false, 0);
    pfree(query.data);
    ...
    initStringInfo(&query);
    appendStringInfo(&query, "DELETE FROM orochi.orochi_pipelines WHERE pipeline_id = %ld",
                     pipeline_id);
    SPI_execute(query.data, false, 0);
    pfree(query.data);
...
PipelineState orochi_get_pipeline_state(int64 pipeline_id)
{
    ...
    initStringInfo(&query);
    appendStringInfo(&query, "SELECT state FROM orochi.orochi_pipelines WHERE pipeline_id = %ld",
                     pipeline_id);

    ret = SPI_execute(query.data, true, 1);
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        bool isnull;
        state = (PipelineState)DatumGetInt32(
...
void pipeline_worker_main(Datum main_arg)
{
    ...
    while (!got_sigterm) {
        ...
        PG_CATCH();
        {
             ...
             * NULL */
            ecxt = MemoryContextSwitchTo(pipeline->pipeline_context ? pipeline->pipeline_context
                                                                    : TopMemoryContext);
            edata = CopyErrorData();
            FlushErrorState();
            ...
            if (pipeline->stats && pipeline->stats->messages_failed > pipeline->error_threshold) {
                StringInfoData query;
                initStringInfo(&query);
                appendStringInfo(&query,
...
static Pipeline *load_pipeline_from_catalog(int64 pipeline_id)
{
    ...
    pipeline->pipeline_context =
        AllocSetContextCreate(TopMemoryContext, "Pipeline Context", ALLOCSET_DEFAULT_SIZES);
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        ...
        pipeline->source_type =
            (PipelineSource)DatumGetInt32(SPI_getbinval(tuple, tupdesc, 3, &isnull));
...
static void process_pipeline_batch(Pipeline *pipeline)
{
    ...
    case PIPELINE_SOURCE_KAFKA:
        ...
        elog(DEBUG1, "Processing Kafka batch for pipeline %ld", pipeline->pipeline_id);
        break;
    ...
    case PIPELINE_SOURCE_FILESYSTEM:
        elog(DEBUG1, "Processing filesystem for pipeline %ld", pipeline->pipeline_id);
        break;

    default:
        elog(WARNING, "Unsupported source type for pipeline %ld", pipeline->pipeline_id);
        break;
    }
}
        ...
        while ((r = JsonbIteratorNext(&it, &v, false)) != WJB_DONE) {
            if (r == WJB_KEY) {
                ...
                if (strcmp(key, "bootstrap_servers") == 0 || strcmp(key, "brokers") == 0) {
                    ...
                    if (v.type == jbvNumeric)
                        config->start_offset = DatumGetInt64(
                    ...
                    if (v.type == jbvNumeric)
                        config->fetch_min_bytes = DatumGetInt32(
                            DirectFunctionCall1(numeric_int4, NumericGetDatum(v.val.numeric)));
                } else if (strcmp(key, "fetch_max_wait_ms") == 0) {
                    if (v.type == jbvNumeric)
                        config->fetch_max_wait_ms = DatumGetInt32(
                            DirectFunctionCall1(numeric_int4, NumericGetDatum(v.val.numeric)));
                } else if (strcmp(key, "session_timeout_ms") == 0) {
                    if (v.type == jbvNumeric)
                        config->session_timeout_ms = DatumGetInt32(
                            DirectFunctionCall1(numeric_int4, NumericGetDatum(v.val.numeric)));
                } else if (strcmp(key, "heartbeat_interval_ms") == 0) {
                    if (v.type == jbvNumeric)
                        config->heartbeat_interval_ms = DatumGetInt32(
                            DirectFunctionCall1(numeric_int4, NumericGetDatum(v.val.numeric)));
                    ...
                    if (v.type == jbvNumeric)
                        config->auto_commit_interval = DatumGetInt32(
                            DirectFunctionCall1(numeric_int4, NumericGetDatum(v.val.numeric)));
                }
        ...
        while ((r = JsonbIteratorNext(&it, &v, false)) != WJB_DONE) {
            if (r == WJB_KEY) {
                ...
                if (strcmp(key, "bucket") == 0) {
                    ...
                    if (v.type == jbvNumeric)
                        config->poll_interval_sec = DatumGetInt32(
                            DirectFunctionCall1(numeric_int4, NumericGetDatum(v.val.numeric)));
...
Datum orochi_create_pipeline_sql(PG_FUNCTION_ARGS)
{
    ...
    int64 pipeline_id =
        orochi_create_pipeline(name, source_type, target_table, format_type, options);
