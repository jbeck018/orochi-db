...
TimestampTz orochi_time_bucket_offset(Interval *bucket_width, TimestampTz ts, Interval *offset)
{
...
TimestampTz orochi_time_bucket_origin(Interval *bucket_width, TimestampTz ts, TimestampTz origin)
{
...
void orochi_create_hypertable(Oid table_oid, const char *time_column, Interval *chunk_interval,
                              bool if_not_exists)
{
    Relation rel;
    TupleDesc tupdesc;
    ...
    if (orochi_is_hypertable(table_oid)) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_DUPLICATE_OBJECT), errmsg("table is already a hypertable")));
    }
    ...
    for (i = 0; i < tupdesc->natts; i++) {
        ...
        if (!attr->attisdropped && strcmp(NameStr(attr->attname), time_column) == 0) {
            column_found = true;
    ...
    if (column_type != TIMESTAMPOID && column_type != TIMESTAMPTZOID && column_type != DATEOID &&
        column_type != INT8OID && column_type != INT4OID) {
    ...
    orochi_register_dimension(table_oid, time_column, 0 /* time dimension */, 1, interval_usec);

    elog(NOTICE, "Created hypertable \"%s.%s\" with chunk interval of %ld hours", schema_name,
         table_name, interval_usec / USECS_PER_HOUR);
}
                                         ...
                                         int num_partitions)
{
    ...
    elog(NOTICE, "Added space dimension on column \"%s\" with %d partitions", space_column,
         num_partitions);
}
 ...
 */
void orochi_register_dimension(Oid hypertable_oid, const char *column_name, int dimension_type,
                               int num_partitions, int64 interval_length)
{
...
void orochi_add_dimension(Oid hypertable_oid, const char *column_name, int num_partitions,
                          Interval *interval)
{
    ...
    orochi_register_dimension(hypertable_oid, column_name, dimension_type, num_partitions,
                              interval_usec);

    elog(NOTICE, "Added dimension on column \"%s\" with %d partitions", column_name,
         num_partitions);
}
 ...
 */
static void calculate_chunk_range(Oid hypertable_oid, TimestampTz ts, TimestampTz *range_start,
...
List *orochi_get_chunks_in_time_range(Oid hypertable_oid, TimestampTz start, TimestampTz end)
{
    List *catalog_chunks;
    List *result = NIL;
    ListCell *lc;
...
void orochi_compress_chunk(int64 chunk_id)
{
    ...
    initStringInfo(&query);
    appendStringInfo(&query, "SELECT pg_total_relation_size('orochi.%s'::regclass)", chunk_name);
    ...
    initStringInfo(&query);
    appendStringInfo(&query, "CREATE TABLE IF NOT EXISTS orochi.compressed_chunk_data ("
...
void orochi_decompress_chunk(int64 chunk_id)
{
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        ...
        initStringInfo(&query);
        appendStringInfo(&query, "DELETE FROM orochi.compressed_chunk_data WHERE chunk_id = %ld",
                         chunk_id);
        SPI_execute(query.data, false, 0);
        pfree(query.data);
    }
 ...
 */
static int store_policy(Oid hypertable_oid, int policy_type, Interval *trigger_interval)
{
    StringInfoData query;
    ...
    snprintf(interval_str, sizeof(interval_str), "'%d days %d hours %d mins'::interval",
             trigger_interval->day, (int)(trigger_interval->time / USECS_PER_HOUR),
 ...
 */
void orochi_create_continuous_aggregate(const char *view_name, const char *query,
                                        Interval *refresh_interval, bool with_data)
{
     ...
     */
    {
        ...
        appendStringInfo(&source_query, "SELECT c.oid FROM pg_class c "
        ...
        if (ret == SPI_OK_SELECT && SPI_processed > 0) {
            bool isnull;
            source_table_oid = DatumGetObjectId(
    ...
    appendStringInfo(&mat_table_sql, "CREATE TABLE IF NOT EXISTS orochi.%s AS %s WITH NO DATA",
    ...
    /* Get materialization table OID */
    {
        ...
        appendStringInfo(&oid_query, "SELECT 'orochi.%s'::regclass::oid", mat_table_name);

        ret = SPI_execute(oid_query.data, true, 1);
        if (ret == SPI_OK_SELECT && SPI_processed > 0) {
            bool isnull;
            mat_table_oid = DatumGetObjectId(
                SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
        } else {
    ...
    /* Get view OID */
    {
        StringInfoData oid_query;
        initStringInfo(&oid_query);
        appendStringInfo(&oid_query, "SELECT '%s.%s'::regclass::oid", view_schema, view_table);

        ret = SPI_execute(oid_query.data, true, 1);
        if (ret == SPI_OK_SELECT && SPI_processed > 0) {
            bool isnull;
            view_oid = DatumGetObjectId(
                SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
        } else {
            view_oid = InvalidOid;
        }
    ...
    if (OidIsValid(view_oid) && OidIsValid(source_table_oid)) {
        orochi_catalog_register_continuous_agg(view_oid, source_table_oid, mat_table_oid, query);
    }
    ...
    if (with_data && OidIsValid(mat_table_oid)) {
        ...
        appendStringInfo(&initial_refresh_sql, "INSERT INTO orochi.%s %s", mat_table_name, query);
 ...
 */
void orochi_refresh_continuous_aggregate(Oid view_oid, TimestampTz start, TimestampTz end)
{
    ...
    if (OidIsValid(agg_info->materialization_table_oid)) {
        ...
        if (ret == SPI_OK_SELECT && SPI_processed > 0) {
            ...
            appendStringInfo(&delete_sql, "DELETE FROM orochi.%s WHERE TRUE", mat_table_name);
    ...
    elog(NOTICE, "Refreshed continuous aggregate %u: deleted %ld rows, inserted %ld rows", view_oid,
 ...
 */
void orochi_log_invalidation(Oid hypertable_oid, TimestampTz start, TimestampTz end)
{
...
void orochi_run_maintenance(void)
{
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        for (i = 0; i < SPI_processed; i++) {
            ...
            elog(DEBUG1, "Processing policy %d (type=%d) for %s", policy_id, policy_type,
                 table_name);
            ...
            case POLICY_TYPE_TIERING:
                /* Apply tiering - move cold chunks to S3/cold storage */
                {
                    ...
                    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
                        ...
                        for (j = 0; j < SPI_processed; j++) {
                            bool chunk_isnull;
                            int64 tier_chunk_id = DatumGetInt64(SPI_getbinval(
                                SPI_tuptable->vals[j], SPI_tuptable->tupdesc, 1, &chunk_isnull));
                            int64 chunk_size = DatumGetInt64(SPI_getbinval(
                                SPI_tuptable->vals[j], SPI_tuptable->tupdesc, 4, &chunk_isnull));
                            bool is_compressed = DatumGetBool(SPI_getbinval(
                                SPI_tuptable->vals[j], SPI_tuptable->tupdesc, 5, &chunk_isnull));
     ...
     */
    initStringInfo(&query);
    appendStringInfoString(&query, "SELECT ca.view_oid, ca.source_table_oid "
    ...
    if (ret == SPI_OK_SELECT && SPI_processed > 0) {
        elog(LOG, "Found %u continuous aggregates needing refresh", (unsigned int)SPI_processed);
