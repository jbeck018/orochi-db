...
static size_t orochi_curl_write_cb_mp(void *contents, size_t size, size_t nmemb,
                                      void *userp) {
 ...
 */
static size_t orochi_curl_header_cb_mp(char *buffer, size_t size, size_t nitems,
                                       void *userp) {
  ...
  if (realsize > prefix_len &&
      strncasecmp(buffer, etag_prefix, prefix_len) == 0) {
...
char *s3_multipart_upload_start(S3Client *client, const char *key) {
  ...
  payload_hash =
      generate_aws_signature_v4(client, "POST", uri, query_string, "", 0,
  ...
  if (res == CURLE_OK && http_code >= 200 && http_code < 300) {
    ...
    if (upload_id) {
      ...
      elog(WARNING, "Failed to parse UploadId from response: %s",
           response_buf.data);
    }
  } else {
 ...
 */
char *s3_multipart_upload_part_ex(S3Client *client, const char *key,
  ...
  /* Generate AWS Signature V4 */
  payload_hash =
      generate_aws_signature_v4(client, "PUT", uri, query_string, data, size,
                                date_stamp, amz_date, &authorization);
 ...
 */
bool s3_multipart_upload_part(S3Client *client, const char *key,
                              const char *upload_id, int part_number,
                              const char *data, int64 size) {
  char *etag = s3_multipart_upload_part_ex(client, key, upload_id, part_number,
                                           data, size);
  if (etag != NULL) {
    pfree(etag);
    return true;
...
bool s3_multipart_upload_complete_with_parts(S3Client *client, const char *key,
  ...
  elog(DEBUG1, "CompleteMultipartUpload XML (%d parts): %s", num_parts,
       complete_xml.data);
  ...
  payload_hash = generate_aws_signature_v4(
      client, "POST", uri, query_string, complete_xml.data, complete_xml.len,
      date_stamp, amz_date, &authorization);
  ...
  snprintf(header_buf, sizeof(header_buf), "Content-Length: %d",
           complete_xml.len);
  headers = curl_slist_append(headers, header_buf);
 ...
 */
bool s3_multipart_upload_complete(S3Client *client, const char *key,
                                  const char *upload_id) {
 ...
 */
void s3_multipart_upload_abort(S3Client *client, const char *key,
                               const char *upload_id) {
  CURL *curl;
  CURLcode res;
  struct curl_slist *headers = NULL;
  ...
  /* Generate AWS Signature V4 */
  payload_hash =
      generate_aws_signature_v4(client, "DELETE", uri, query_string, "", 0,
                                date_stamp, amz_date, &authorization);
 ...
 */
S3UploadResult *orochi_upload_chunk_to_s3(S3Client *client, const char *key,
                                          const char *data, int64 size) {
  ...
  if (size < MULTIPART_THRESHOLD) {
    ...
    if (max_parts > 10000) {
      ...
      result->error_message =
          pstrdup("File too large: would exceed 10000 parts limit");
      return result;
    }
    ...
    if (success) {
      ...
      if (s3_multipart_upload_complete_with_parts(client, key, upload_id, parts,
        ...
        if (part_count > 0 && parts[0].etag != NULL) {
          snprintf(final_etag, 128, "%s-%d", parts[part_count - 1].etag,
                   part_count);
        } else {
        ...
        elog(LOG, "Successfully uploaded %ld bytes in %d parts to %s/%s", size,
             part_count, client->bucket, key);
      } else {
        result->success = false;
