...
#define POSTGRES_EPOCH_JDATE                                                   \
  2451545 /* Julian date of Postgres epoch (2000-01-01) */

/* SHA256 hash of empty string for AWS signature */
#define EMPTY_SHA256_HASH                                                      \
...
static size_t orochi_curl_write_cb(void *contents, size_t size, size_t nmemb,
                                   void *userp) {
...
char *generate_aws_signature_v4(S3Client *client, const char *method,
  ...
  unsigned char date_key[32], date_region_key[32], date_region_service_key[32],
      signing_key[32];
  ...
  appendStringInfo(&canonical_request,
                   ...
                   "%s",                                    /* Payload hash */
                   method, canonical_uri, query_string ? query_string : "",
                   client->endpoint, payload_hash_hex, amz_date,
                   payload_hash_hex);
  ...
  sha256_hash(canonical_request.data, canonical_request.len,
              canonical_request_hash);
  bytes_to_hex(canonical_request_hash, 32, canonical_request_hash_hex);
  pfree(canonical_request.data);

  /* Build scope */
  snprintf(scope, sizeof(scope), "%s/%s/s3/aws4_request", date_stamp,
           client->region);
  ...
  hmac_sha256(date_key, 32, (unsigned char *)client->region,
              strlen(client->region), date_region_key);
  hmac_sha256(date_region_key, 32, (unsigned char *)"s3", 2,
              date_region_service_key);
  hmac_sha256(date_region_service_key, 32, (unsigned char *)"aws4_request", 12,
              signing_key);

  /* Calculate signature */
  hmac_sha256(signing_key, 32, (unsigned char *)string_to_sign.data,
              string_to_sign.len, signature_bytes);
...
S3Client *s3_client_create(OrochiS3Config *config) {
  ...
  client->region =
      config->region ? pstrdup(config->region) : pstrdup("us-east-1");
  client->use_ssl = config->use_ssl;
  client->timeout_ms =
      config->connection_timeout > 0 ? config->connection_timeout : 30000;
...
bool s3_client_test_connection(S3Client *client) {
  ...
  snprintf(url, sizeof(url), "%s://%s/%s", client->use_ssl ? "https" : "http",
           client->endpoint, client->bucket);
  ...
  generate_aws_signature_v4(client, "HEAD", client->bucket, "", NULL, 0,
...
S3UploadResult *s3_upload(S3Client *client, const char *key, const char *data,
  ...
  /* Generate AWS Signature V4 */
  payload_hash =
      generate_aws_signature_v4(client, "PUT", uri, NULL, data, size,
                                date_stamp, amz_date, &authorization);
  ...
  if (res != CURLE_OK) {
    ...
    if (http_code >= 200 && http_code < 300) {
      ...
      elog(LOG, "S3 Upload successful: %s/%s (%ld bytes)", client->bucket, key,
           size);
    } else {
      result->success = false;
      result->error_message = pstrdup(response_buf.data);
      elog(WARNING, "S3 Upload failed: HTTP %ld - %s", http_code,
           response_buf.data);
    }
  }
...
S3DownloadResult *s3_download(S3Client *client, const char *key) {
  ...
  payload_hash = generate_aws_signature_v4(
      client, "GET", uri, NULL, "", 0, date_stamp, amz_date, &authorization);
  ...
  if (res != CURLE_OK) {
    ...
    if (http_code >= 200 && http_code < 300) {
      ...
      elog(LOG, "S3 Download successful: %s/%s (%zu bytes)", client->bucket,
           key, response_buf.size);
    } else {
      result->success = false;
      result->error_message = pstrdup(response_buf.data);
      elog(WARNING, "S3 Download failed: HTTP %ld - %s", http_code,
           response_buf.data);
      pfree(response_buf.data);
    }
  }
...
bool s3_delete(S3Client *client, const char *key) {
  ...
  /* Generate AWS Signature V4 */
  payload_hash = generate_aws_signature_v4(
      client, "DELETE", uri, NULL, "", 0, date_stamp, amz_date, &authorization);
  ...
  if (res == CURLE_OK && http_code >= 200 && http_code < 300) {
    ...
    elog(WARNING, "S3 Delete failed: %s/%s - HTTP %ld", client->bucket, key,
         http_code);
  }
...
bool s3_object_exists(S3Client *client, const char *key) {
  ...
  /* Generate AWS Signature V4 */
  payload_hash = generate_aws_signature_v4(
      client, "HEAD", uri, NULL, "", 0, date_stamp, amz_date, &authorization);
...
static size_t orochi_curl_header_cb(char *buffer, size_t size, size_t nitems,
                                    void *userdata) {
...
S3Object *s3_head_object(S3Client *client, const char *key) {
  ...
  if (res == CURLE_OK && http_code == 200) {
    ...
    obj->content_type = header_data.content_type
                            ? header_data.content_type
                            : pstrdup("application/octet-stream");
    obj->storage_class = header_data.storage_class ? header_data.storage_class
                                                   : pstrdup("STANDARD");
...
List *s3_list_objects(S3Client *client, const char *prefix) {
  ...
  while (is_truncated) {
    ...
    if (client->use_ssl)
      snprintf(url, sizeof(url), "https://%s%s?%s", client->endpoint, uri,
               query_string);
    else
      snprintf(url, sizeof(url), "http://%s%s?%s", client->endpoint, uri,
               query_string);

    /* Generate AWS Signature V4 */
    payload_hash =
        generate_aws_signature_v4(client, "GET", uri, query_string, "", 0,
                                  date_stamp, amz_date, &authorization);
    ...
    snprintf(header_buf, sizeof(header_buf), "Authorization: %s",
             authorization);
    headers = curl_slist_append(headers, header_buf);
    ...
    /* Parse XML response */
    {
      ...
      if (is_truncated) {
        ...
        if (continuation_token)
          pfree(continuation_token);
        continuation_token =
            extract_xml_text("NextContinuationToken", &token_pos);
      ...
      while (contents_start != NULL) {
        ...
        /* Extract object metadata */
        {
          ...
          if (key_str) {
            ...
            obj->storage_class =
                storage_str ? storage_str : pstrdup("STANDARD");
...
void orochi_set_tiering_policy(Oid table_oid, Interval *hot_to_warm,
...
void orochi_move_to_tier(int64 chunk_id, OrochiStorageTier tier) {
  ...
  if (current_tier == tier) {
    elog(NOTICE, "chunk %ld is already in %s tier", chunk_id,
  ...
  if (tier < current_tier) {
    ...
    elog(NOTICE, "Recalling chunk %ld from %s to %s", chunk_id,
         orochi_tier_name(current_tier), orochi_tier_name(tier));
  } else {
    /* Moving to colder tier */
    elog(NOTICE, "Moving chunk %ld from %s to %s", chunk_id,
         orochi_tier_name(current_tier), orochi_tier_name(tier));
  }

  if (!do_tier_transition(chunk_id, tier))
    ereport(ERROR, (errcode(ERRCODE_IO_ERROR),
                    errmsg("failed to move chunk %ld to %s tier", chunk_id,
                           orochi_tier_name(tier))));
}
...
static bool do_tier_transition(int64 chunk_id, OrochiStorageTier to_tier) {
  ...
  case OROCHI_TIER_COLD: {
    ...
    /* Read chunk data from PostgreSQL relation */
    {
      ...
      spi_ret =
          SPI_execute(psprintf("SELECT a.attnum, a.atttypid, t.typname "
                               "FROM pg_attribute a "
      ...
      appendBinaryStringInfo(&copy_data, "PGCOPY\n\377\r\n\0", 11);
      {
        ...
        appendBinaryStringInfo(&copy_data, (char *)&header_ext_len,
                               sizeof(int32));
      }

      /* Fetch all rows from chunk table */
      spi_ret =
          SPI_execute(psprintf("SELECT * FROM %s", chunk_table_name), true, 0);
      ...
      if (spi_ret == SPI_OK_SELECT) {
        ...
        for (i = 0; i < SPI_processed; i++) {
          ...
          /* Write field count */
          appendBinaryStringInfo(&copy_data, (char *)&field_count,
                                 sizeof(int16));
          ...
          for (j = 0; j < tupdesc->natts; j++) {
            ...
            if (isnull) {
              ...
              appendBinaryStringInfo(&copy_data, (char *)&data_len,
                                     sizeof(int32));
      ...
      if (copy_data.len > 0) {
        compressed_data = palloc(
            copy_data.len + 1024); /* Extra space for compression overhead */
        compressed_size = columnar_compress_buffer(
            copy_data.data, copy_data.len, compressed_data,
            copy_data.len + 1024, compression,
            COLUMNAR_COMPRESSION_LEVEL_DEFAULT);

        if (compressed_size <= 0) {
          elog(WARNING,
               "Compression failed for chunk %ld, using uncompressed data",
               chunk_id);
      ...
      appendBinaryStringInfo(&chunk_buffer, (char *)&chunk_info->row_count,
                             sizeof(int64));
      appendBinaryStringInfo(&chunk_buffer, (char *)&column_count,
                             sizeof(int32));
      appendBinaryStringInfo(&chunk_buffer, (char *)&compression,
                             sizeof(uint32));
      appendBinaryStringInfo(&chunk_buffer, (char *)&copy_data.len,
                             sizeof(int64));
      appendBinaryStringInfo(&chunk_buffer, (char *)&compressed_size,
                             sizeof(int64));
      ...
      pfree(chunk_buffer.data);
      pfree(copy_data.data);
      if (compression != OROCHI_COMPRESS_NONE &&
          compressed_data != copy_data.data)
        pfree(compressed_data);
    }

    if (!result->success) {
      elog(WARNING, "Failed to upload chunk %ld to S3: %s", chunk_id,
...
void orochi_recall_from_cold(int64 chunk_id) {
  ...
  PG_TRY();
  {
    ...
    if (success) {
      ...
      elog(LOG,
           "Successfully recalled chunk %ld from cold storage to WARM tier",
           chunk_id);
    } else {
      ereport(ERROR, (errcode(ERRCODE_IO_ERROR),
...
static bool orochi_restore_chunk_from_s3(int64 chunk_id, S3Client *client,
  ...
  if (download_result->size < 64) {
    elog(WARNING,
         "Downloaded chunk %ld is too small: %ld bytes (expected >= 64)",
  ...
  {
    elog(WARNING,
         "Invalid chunk magic for chunk %ld: 0x%08X (expected 0x4F524348)",
         chunk_id, magic);
    pfree(download_result->data);
    pfree(download_result);
    return false;
  ...
  if (header_chunk_id != chunk_id) {
    elog(WARNING, "Chunk ID mismatch: expected %ld, got %ld", chunk_id,
         header_chunk_id);
    pfree(download_result->data);
    pfree(download_result);
    return false;
  ...
  elog(DEBUG1,
       ...
       chunk_id, row_count, column_count, compression_type, uncompressed_size,
  ...
  /* Verify checksum */
  {
    ...
    for (i = 0; i < compressed_size; i++) {
      calculated_checksum =
          ((calculated_checksum << 5) + calculated_checksum) + cdata[i];
    }

    if (calculated_checksum != checksum) {
      elog(WARNING,
           "Checksum mismatch for chunk %ld: got 0x%08X, expected 0x%08X",
  ...
  if (compression_type != OROCHI_COMPRESS_NONE) {
    ...
    if (decompressed_size != uncompressed_size) {
      elog(WARNING,
           "Decompressed size mismatch for chunk %ld: got %ld, expected %ld",
  ...
  PG_TRY();
  {
    ...
    appendStringInfo(&create_table_query, "DROP TABLE IF EXISTS %s CASCADE",
                     chunk_table_name);
     ...
     */
    {
      ...
      data_size = compression_type == OROCHI_COMPRESS_NONE ? compressed_size
                                                           : uncompressed_size;
      ...
      appendStringInfo(&copy_cmd, "COPY %s FROM '%s' WITH (FORMAT binary)",
                       chunk_table_name, temp_file_path);

      elog(DEBUG1, "Restoring chunk %ld: %s (%ld bytes)", chunk_id,
           copy_cmd.data, data_size);
    ...
    initStringInfo(&create_table_query);
    appendStringInfo(&create_table_query, "ALTER TABLE %s SET LOGGED",
                     chunk_table_name);
    ...
    success = true;
    elog(LOG, "Successfully restored chunk %ld with %ld rows from S3", chunk_id,
         row_count);
...
int orochi_apply_tiering_for_table(Oid table_oid) {
  ...
  foreach (lc, chunks) {
    ...
    if (age_usec > cold_threshold_usec &&
        chunk->storage_tier < OROCHI_TIER_COLD)
      target_tier = OROCHI_TIER_COLD;
    else if (age_usec > warm_threshold_usec &&
             chunk->storage_tier < OROCHI_TIER_WARM)
      target_tier = OROCHI_TIER_WARM;
    else if (age_usec > hot_threshold_usec &&
             chunk->storage_tier < OROCHI_TIER_WARM)
      target_tier = OROCHI_TIER_WARM;
...
Datum orochi_remove_node_sql(PG_FUNCTION_ARGS) {
  ...
  SPI_connect();
  {
    ...
    initStringInfo(&query);
    appendStringInfo(
        &query, "SELECT shard_id FROM orochi.orochi_shards WHERE node_id = %d",
        node_id);
  ...
  if (shards_to_move > 0) {
    ...
    if (list_length(active_nodes) == 0) {
      ereport(ERROR,
              (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
               errmsg("cannot remove node %d: no other active nodes available",
                      node_id),
    ...
    elog(NOTICE, "Migrated %d shards from node %d to other nodes", shards_moved,
         node_id);
  }
...
Datum orochi_drain_node_sql(PG_FUNCTION_ARGS) {
  ...
  if (num_nodes == 0) {
    ereport(ERROR,
            (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
             errmsg("cannot drain node %d: no other active nodes available",
                    node_id)));
  }
  ...
  elog(NOTICE, "Drained node %d: migrated %d shards to %d nodes", node_id,
       shards_migrated, num_nodes);
...
Datum orochi_rebalance_shards_sql(PG_FUNCTION_ARGS) {
  ...
  char *strategy =
      strategy_text ? text_to_cstring(strategy_text) : "by_shard_count";
  List *active_nodes;
  int num_nodes;
  int total_shards = 0;
  int target_per_node;
  ...
  if (OidIsValid(table_oid)) {
    ...
    if (SPI_execute(query.data, true, 1) == SPI_OK_SELECT &&
        SPI_processed > 0) {
      bool isnull;
      total_shards = DatumGetInt64(SPI_getbinval(
          SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
    }
    pfree(query.data);
  } else {
    if (SPI_execute("SELECT COUNT(*) FROM orochi.orochi_shards", true, 1) ==
            SPI_OK_SELECT &&
        SPI_processed > 0) {
      bool isnull;
      total_shards = DatumGetInt64(SPI_getbinval(
  ...
  if (strcmp(strategy, "by_shard_count") == 0) {
    ...
    foreach (lc, active_nodes) {
      ...
      if (SPI_execute(query.data, true, 1) == SPI_OK_SELECT &&
          SPI_processed > 0) {
        bool isnull;
        node_shard_count = DatumGetInt64(SPI_getbinval(
            SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
      }
      pfree(query.data);
    ...
    foreach (lc, overloaded) {
      ...
      if (SPI_execute(query.data, true, 0) == SPI_OK_SELECT &&
        ...
        for (i = 0; i < SPI_processed && list_length(underloaded) > 0; i++) {
          bool isnull;
          int64 shard_id = DatumGetInt64(SPI_getbinval(
    ...
    elog(NOTICE,
         "Size-based rebalancing: moving large shards from heavy nodes");
