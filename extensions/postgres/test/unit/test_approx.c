...
static TestHLL *test_hll_create(uint8_t precision)
{
    ...
    if (precision > HLL_PRECISION_MAX)
...
static double test_tdigest_quantile(TestTDigest *td, double q)
{
    ...
    for (int i = 0; i < td->num_centroids; i++) {
        ...
        if (target_weight < next_weight) {
            ...
            double next_mean =
                (i == td->num_centroids - 1) ? td->max_value : td->centroids[i + 1].mean;
...
static void test_hll_merge(void)
{
    ...
    for (int i = 0; i < num_regs; i++) {
        merged->registers[i] =
            (hll1->registers[i] > hll2->registers[i]) ? hll1->registers[i] : hll2->registers[i];
    }
...
static void test_reservoir_sampling(void)
{
    ...
    for (int i = 0; i < reservoir_size; i++) {
        if (reservoir[i] < stream_size / 3)
            early++;
        else if (reservoir[i] < 2 * stream_size / 3)
            middle++;
        else
            late++;
    }

    bool balanced = (early >= 20 && early <= 50) && (middle >= 20 && middle <= 50) &&
                    (late >= 20 && late <= 50);
